...
def module_path_entries(directory: Path | str) -> list[str]:
    ...
    if is_existing_zip_subpath(path_str):
        # Zip-importer package paths are already fully qualified (archive.zip/pkg/path).
        # Returning early prevents repeatedly appending bundle roots when a package is
        # imported from inside a bundle, which can otherwise create very long nested
        # pseudo-paths and slow or hang package bootstrap on some Python versions.

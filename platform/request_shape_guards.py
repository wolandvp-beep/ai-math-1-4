...
_EXAMPLE_WRAPPER_LINE_RE = re.compile(
    r'^\s*(?:пожалуйста\s*,?\s*)?(?:(?:реши(?:те)?|вычисли(?:те)?|найди(?:те)?)\s+(?:пример(?:ы)?|задани[ея]|уравнени[ея]|систем[ауые](?:\s+уравнений)?)|реши(?:те)?|вычисли(?:те)?|найди(?:те)?|пример(?:ы)?|задани[ея]|уравнени[ея]|систем[ауые](?:\s+уравнений)?)\s*[:.!?\-–—]*\s*$',
    re.IGNORECASE,
)
...
def canonicalize_system_submission(raw_text: str) -> Optional[str]:
    ...
    for raw_line in body.split('\n'):
        ...
        if line.count('=') != 1:
            continue
        compact = re.sub(r'\s+', '', line).replace('х', 'x').replace('Х', 'x').replace('у', 'y').replace('У', 'y')
    ...
    has_coupled_equation = any(len(var_set) >= 2 for var_set in var_sets)

    # A true school system must involve at least two variables and at least one
    # coupling between equations. Several independent examples/equations on
    # separate lines are still multiple задания and must trigger a warning.
    if len(all_vars) < 2:
        return None
    if not (shared_vars or has_coupled_equation):
        return None
    return '\n'.join(lines)
...
def _looks_like_task_request_sentence(sentence: str) -> bool:
    lowered = str(sentence or '').lower()
    if not lowered:
        return False
    if _SOLVER_VERB_RE.search(lowered):
        return True
    if re.search(r'\b(?:сколько|на\s+сколько|во\s+сколько|чему\s+равно|что\s+получит(?:ся)?|какое\s+число|какой\s+знак|верно\s+ли)\b', lowered):
        return True
    return _looks_like_expression_fragment(sentence)


def _looks_like_story_numeric_sentence(sentence: str) -> bool:
    lowered = str(sentence or '').lower()
    if not re.search(r'\d', str(sentence or '')):
        return False
    story_markers = (
        'было', 'стало', 'остал', 'осталось', 'отдал', 'дала', 'дали', 'дала', 'купил', 'купила', 'купили',
        'подарил', 'подарила', 'подарили', 'меньше', 'больше', 'еще', 'ещё', 'вместе', 'осталось'
    )
    return any(marker in lowered for marker in story_markers)
...
def is_multi_task_submission(raw_text: str) -> bool:
    ...
    # Bare examples/equations on separate lines, semicolon-separated, or accidentally
    # joined after newline loss are multiple independent задания.  Do not solve them
    # as one batch; warn the user to send one at a time.  Systems are excluded above.
    if build_multiline_direct_examples_payload(text) is not None or build_multiline_arithmetic_payload(text) is not None:
        return True

    lowered = text.lower()
    if re.search(r'\bсистем[ауые]\b', lowered) and text.count('=') >= 2:
        # The word "system" was present, but canonicalize_system_submission()
        # did not recognize a true two-variable system. Treat it as several tasks
        # instead of letting a fallback glue equations into a bogus one-variable answer.
        return True
    ...
    if len(sentences) >= 2:
        task_request_count = sum(1 for sentence in sentences if _looks_like_task_request_sentence(sentence))
        story_numeric_count = sum(1 for sentence in sentences if _looks_like_story_numeric_sentence(sentence))
        expression_sentence_count = sum(1 for sentence in sentences if _looks_like_expression_fragment(sentence))
        if task_request_count >= 2 and (story_numeric_count >= 2 or expression_sentence_count >= 2 or text.count('?') >= 2):
            return True
...
# --- v273 route/contract fix: multi-line standalone examples are a warning, never a batch solution ---
# Previous code paths could still build a combined solution when the dispatcher was
# reached directly.  The product policy is one task per request: independent
# examples/equations must return the same user warning through every route.  A true
# two-variable system is excluded earlier by canonicalize_system_submission().

def _v273_is_independent_math_batch(raw_text: str) -> bool:
    parts = _split_candidate_batch_lines(raw_text)
    if len(parts) < 2 or len(parts) > 20:
        return False
    return all(_looks_like_standalone_arithmetic_or_equation(part) for part in parts)


def build_multiline_direct_examples_payload(raw_text: str) -> Optional[dict]:
    if not _v273_is_independent_math_batch(raw_text):
        return None
    return build_multi_task_payload(raw_text)
...
# v273: clearer warning body.  Preserve line breaks in the warning so a user sees
# that the app recognized several separate lines; do not display them glued as one
# expression.
def _v273_multitask_display(raw_text: str) -> str:
    source = str(raw_text or '').replace('\r\n', '\n').replace('\r', '\n').strip()
    if '\n' in source:
        lines = [line.strip() for line in source.split('\n') if line.strip()]
        if len(lines) >= 2:
            return 'Получено несколько строк:\n' + '\n'.join(lines)
    return audit_task_line(raw_text)


def build_multi_task_payload(raw_text: str) -> dict:
    result_text = finalize_legacy_lines([
        'Задача.',
        _v273_multitask_display(raw_text),
        'Решение.',
        MULTI_TASK_MESSAGE,
        'Ответ: Разделите задания и отправьте их по отдельности.',
        'Совет: систему уравнений и сравнение двух выражений можно отправлять одним запросом только как одно общее задание.',
    ])
    user_visible_result = finalize_legacy_lines([
        MULTI_TASK_MESSAGE,
        'Ответ: Разделите задания и отправьте их по отдельности.',
        'Совет: систему уравнений и сравнение двух выражений можно отправлять одним запросом только как одно общее задание.',
    ])
    return {
        'result': result_text,
        'userVisibleResultText': user_visible_result,
        'source': 'guard-multi-task',
        'validated': True,
        'code': 'multi_task_not_allowed',
    }

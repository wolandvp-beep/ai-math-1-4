...
def explain_simple_subtraction(left: int, right: int) -> str:
    ...
    if 10 <= left <= 99 and 10 <= right <= 99 and result >= 0 and left < 100:
        ...
        borrowed_left = (left // 10 - 1) * 10
        adjusted_units = 10 + lu
        return join_explanation_lines(
            "Ищем разность",
            f"Представим {left} так: {left} = {borrowed_left} + {adjusted_units}",
            f"Разложим {right} так: {right} = {rt} + {ru}",
            f"Вычитаем десятки и единицы: ({borrowed_left} - {rt}) + ({adjusted_units} - {ru}) = {result}",
            f"Ответ: {result}",
            "Совет: если единиц не хватает, один десяток меняем на 10 единиц",
        )

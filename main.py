...
def _detailed_collect_expression_steps(node: ast.AST, source: str) -> List[dict]:
    return build_eval_steps(node, source)[1]
...
def _audit_task_line(text: str) -> str:
...
def _build_fraction_school_explanation(raw_text: str) -> Optional[str]:
    ...
    lines: List[str] = [
        f"Пример: {a}/{b} {action_symbol} {c}/{d} = {result_text}",
        "Решение"
    ]
    ...
    if b == d:
        ...
        lines.extend([
            ...
            f"2) {action_word} только числители, а знаменатель оставляем прежним: {a} {action_symbol} {c} = {top_result}",
            f"3) Записываем: {a}/{b} {action_symbol} {c}/{d} = {raw_result}"
        ])
        if raw_result != result_text:
            lines.append(f"4) Сокращаем дробь: {raw_result} = {result_text}")
    ...
    raw_result = f"{a_scaled + c_scaled if operator == '+' else a_scaled - c_scaled}/{common}"
    ...
    lines.append(
        f"{step_number}) Записываем: {a}/{b} {action_symbol} {c}/{d} = {a_scaled}/{common} {action_symbol} {c_scaled}/{common} = {raw_result}"
    )
    if raw_result != result_text:
        step_number += 1
        lines.append(f"{step_number}) Сокращаем дробь: {raw_result} = {result_text}")

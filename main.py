...
# Разрешаем CORS (чтобы запросы с вашего сайта работали)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Для теста; в продакшене лучше указать конкретный домен
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def proxy(request: Request):
    ...
    if action == "ocr":
        # Распознавание текста с фото (DeepSeek-VL)
        payload = {
            "model": "deepseek-vl",
            "messages": [{
                "role": "user",
        ...
        # Объяснение задачи (DeepSeek-Chat)
        payload = {
            "model": "deepseek-chat",
            "messages": [

...
async def handle_explain_request(request: Request):
    try:
        ...
        user_text = (data.get("text") or data.get("query") or data.get("prompt") or "").strip()
        if not user_text:
...
@app.get("/api/")
def read_root():
    return {"message": "API is running. Use POST request with math text."}

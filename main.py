...
def _patch_20260413j_should_use_column_for_step(step: dict, source: str) -> bool:
    ...
    if operator == "-":
        ...
        return abs(left) >= 100 or abs(right) >= 100 or global_flag
    if operator == "×":
        return abs(left) >= 10 or abs(right) >= 10 or global_flag
    if operator == ":":
        if right == 0:
            return False
        return abs(left) >= 100 or abs(right) >= 10 or global_flag
    return False
...
def _patch_20260413j_step_header(index: int, operator: str, left: str, right: str, result: str) -> str:
    ...
    expression = f"{left} {pretty_operator} {right} = {result}".strip()

    if pretty_operator in _PATCH_20260413J_ACTION_NAMES:
        action_name = _PATCH_20260413J_ACTION_NAMES[pretty_operator]
        return f"{index}) {ordinal} действие — {action_name}: {expression}"

    return f"{index}) {ordinal} действие: {expression}"

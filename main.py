...
# --- DELIVERY PATCH 2026-04-13B: cleaner school explanations, stronger local parsing, developer-friendly CORS ---

_PS_20260413_PREV_BUILD_EXPLANATION_LOCAL_FIRST = build_explanation_local_first
_PS_20260413_PREV_ANSWER_PHRASE = _user_final_answer_phrase_from_question
_PS_20260413_PREV_EXTRACT_CONDITION_AND_QUESTION = extract_condition_and_question
_PS_20260413_PREV_DETAILED_FINALIZE_LINE = _detailed_finalize_line

_PS_EXTRA_NOUN_FORMS = {
    "кубик": ("кубик", "кубика", "кубиков"),
    "кубика": ("кубик", "кубика", "кубиков"),
    "кубиков": ("кубик", "кубика", "кубиков"),
    ...
    "пакета": ("пакет", "пакета", "пакетов"),
    "пакетов": ("пакет", "пакета", "пакетов"),
    "тарелочку": ("тарелочка", "тарелочки", "тарелочек"),
    "тарелочек": ("тарелочка", "тарелочки", "тарелочек"),
}

_PS_MEASURE_NOUNS = {
    "км", "м", "см", "дм", "мм", "ч", "час", "часа", "часов",
    "мин", "минута", "минуты", "минут", "с", "сек", "секунда", "секунды", "секунд",
    "руб", "руб.", "рубля", "рублей", "коп", "коп.", "копейка", "копейки", "копеек",
}

_PS_LOSS_VERB_RE = r"(?:отдал(?:а|и)?|взял(?:а|и)?|съел(?:а|и)?|убрал(?:а|и)?|забрал(?:а|и)?|потратил(?:а|и)?|продал(?:а|и)?|снял(?:а|и)?|сломал(?:а|и)?|использовал(?:а|и)?|израсходовал(?:а|и)?|ушло|ушли)"
_PS_GAIN_VERB_RE = r"(?:добавил(?:а|и)?|дали|получил(?:а|и)?|купил(?:а|и)?|принес(?:ла|ли)?|принёс(?:ла|ли)?|положил(?:а|и)?|прибавил(?:а|и)?|привез(?:ла|ли)?|привёз(?:ла|ли)?|стало|стали)"


def _ps_normalize_key(text: str) -> str:
    return str(text or "").strip().lower().replace("ё", "е")



def _ps_lookup_forms(noun: str):
    key = _ps_normalize_key(noun)
    if not key:
        return None
    return _lookup_question_noun_forms(key) or _PS_EXTRA_NOUN_FORMS.get(key)



def _ps_format_count(value: int, noun: str = "") -> str:
    key = _ps_normalize_key(noun)
    if not key:
        return str(value)
    forms = _ps_lookup_forms(key)
    if forms:
        return f"{value} {_select_plural_by_count(int(value), forms)}"
    return f"{value} {key}".strip()



def _ps_first_meaningful_noun(raw_text: str) -> str:
    lower = normalize_word_problem_text(raw_text).lower()
    for match in re.finditer(r"\d+\s+([а-яё-]+)", lower):
        noun = _ps_normalize_key(match.group(1))
        if not noun or noun in _PS_MEASURE_NOUNS:
            continue
        if noun.startswith("км/") or noun.startswith("м/"):
            continue
        return noun
    return ""
...
def _ps_question_noun(raw_text: str) -> str:
    noun = _extract_question_noun(raw_text) or _extract_count_question_noun(raw_text)
    if noun:
        return _ps_normalize_key(noun)
    question = _question_lower_text(raw_text)
    match = re.search(r"^сколько\s+(?:было\s+|будет\s+|стало\s+|осталось\s+|получилось\s+|понадобилось\s+|понадобится\s+|нужно\s+)?([а-яё-]+)", question)
    if match:
        return _ps_normalize_key(match.group(1))
    return ""



def _ps_cleanup_answer_like_text(text: str) -> str:
    cleaned = str(text or "").strip()
    if not cleaned:
        return ""
    cleaned = re.sub(r"\b(осталось|стало|было|получилось)\s+([^.,;!?]+?)\s+\1\b", r"\1 \2", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"=\s*(\d+(?:\s+[а-яё./²%-]+)?)\s+(осталось|стало|было|получилось)\b", r"= \1", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bбыло\s+([^.,;!?]+?)\s+сначала\b", r"сначала было \1", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bсначала\s+было\s+([^.,;!?]+?)\s+сначала\b", r"сначала было \1", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\s{2,}", " ", cleaned)
    return cleaned.strip()



def _ps_try_fraction_whole_from_part(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    if not re.search(r"\b(найти|найдите|найди|узнай|узнайте)\b", lower) and "числ" not in lower:
        return None

    match = re.search(r"(\d+)\s*/\s*(\d+)\s+числ(?:а|о)?\s*(?:равн(?:ы|а|о)?|=|составляет)?\s*(\d+)", lower)
    if not match:
        return None
    ...
    if numerator <= 0 or denominator <= 0 or part_value <= 0:
        return None
    if part_value % numerator != 0:
        return None

    one_part = part_value // numerator
    whole = one_part * denominator
    return join_explanation_lines(
        f"Известно, что {numerator}/{denominator} числа равны {part_value}",
        f"Сначала найдём 1/{denominator} числа: {part_value} : {numerator} = {one_part}",
        f"Теперь найдём всё число: {one_part} × {denominator} = {whole}",
        f"Ответ: {whole}",
        "Совет: если известна часть числа, сначала найди одну долю, потом всё число",
    )



def _ps_try_simple_motion_form(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    if any(fragment in lower for fragment in ("навстречу", "в противоположных направлениях", "догон")):
        return None

    speed_match = re.search(r"(\d+)\s*(км/ч|м/с|м/мин)", lower)
    distance_match = re.search(r"(\d+)\s*(км|м)(?!/)", lower)
    time_match = re.search(r"(\d+)\s*(ч|час(?:а|ов)?|мин(?:ут[аы]?)?|с|сек(?:унд[аы]?)?)\b", lower)

    asks_distance = contains_any_fragment(lower, ("найти расстояние", "найдите расстояние", "какое расстояние", "чему равно расстояние", "какова длина пути", "сколько километров"))
    asks_speed = contains_any_fragment(lower, ("найти скорость", "найдите скорость", "с какой скоростью", "какова скорость", "чему равна скорость"))
    asks_time = contains_any_fragment(lower, ("найти время", "найдите время", "сколько времени", "сколько часов", "за какое время", "чему равно время"))

    if asks_distance and speed_match and time_match:
        speed = int(speed_match.group(1))
        time_value = int(time_match.group(1))
        distance_unit = "км" if speed_match.group(2).startswith("км/") else "м"
        result = speed * time_value
        return join_explanation_lines(
            "Используем правило S = v × t",
            f"Подставляем значения: S = {speed} × {time_value} = {result} {distance_unit}",
            f"Ответ: расстояние равно {result} {distance_unit}",
            "Совет: чтобы найти расстояние, скорость умножают на время",
        )

    if asks_speed and distance_match and time_match:
        distance = int(distance_match.group(1))
        time_value = int(time_match.group(1))
        if time_value == 0 or distance % time_value != 0:
            return None
        result = distance // time_value
        speed_unit = "км/ч" if distance_match.group(2) == "км" and time_match.group(2).startswith(("ч", "час")) else ("м/мин" if time_match.group(2).startswith("мин") else "м/с")
        return join_explanation_lines(
            "Используем правило v = S : t",
            f"Подставляем значения: v = {distance} : {time_value} = {result} {speed_unit}",
            f"Ответ: скорость равна {result} {speed_unit}",
            "Совет: чтобы найти скорость, расстояние делят на время",
        )

    if asks_time and distance_match and speed_match:
        distance = int(distance_match.group(1))
        speed = int(speed_match.group(1))
        if speed == 0 or distance % speed != 0:
            return None
        result = distance // speed
        time_unit = "ч" if speed_match.group(2).endswith("/ч") else ("мин" if speed_match.group(2).endswith("/мин") else "с")
        return join_explanation_lines(
            "Используем правило t = S : v",
            f"Подставляем значения: t = {distance} : {speed} = {result} {time_unit}",
            f"Ответ: время равно {result} {time_unit}",
            "Совет: чтобы найти время, расстояние делят на скорость",
        )
...
def _ps_try_initial_after_loss(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    question = _question_lower_text(raw_text)
    if not any(fragment in question for fragment in ("сколько было сначала", "сколько было сперва", "сколько было вначале")):
        return None
    if not any(hint in lower for hint in WORD_LOSS_HINTS):
        return None

    removed_match = re.search(fr"{_PS_LOSS_VERB_RE}[^\d]{{0,25}}(\d+)", lower)
    remaining_match = re.search(r"остал(?:ось|ись|ся)[^\d]{0,25}(\d+)", lower)
    if not removed_match or not remaining_match:
        return None

    removed = int(removed_match.group(1))
    remaining = int(remaining_match.group(1))
    initial = remaining + removed
    noun = _ps_first_meaningful_noun(raw_text)
    answer_value = _ps_format_count(initial, noun)
    return join_explanation_lines(
        f"После того как убрали {removed}, осталось {remaining}",
        f"Чтобы узнать, сколько было сначала, складываем оставшееся и убранное: {remaining} + {removed} = {initial}",
        f"Ответ: сначала было {answer_value}",
        "Совет: если известно, сколько осталось и сколько убрали, эти числа складывают",
    )



def _ps_try_initial_after_gain(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    question = _question_lower_text(raw_text)
    if not any(fragment in question for fragment in ("сколько было сначала", "сколько было сперва", "сколько было вначале")):
        return None
    if not any(hint in lower for hint in WORD_GAIN_HINTS):
        return None

    added_match = re.search(fr"{_PS_GAIN_VERB_RE}[^\d]{{0,25}}(\d+)", lower)
    final_match = re.search(r"стал(?:о|и)[^\d]{0,25}(\d+)", lower)
    if not added_match or not final_match:
        return None

    added = int(added_match.group(1))
    final_total = int(final_match.group(1))
    if final_total < added:
        return None

    initial = final_total - added
    noun = _ps_first_meaningful_noun(raw_text)
    answer_value = _ps_format_count(initial, noun)
    return join_explanation_lines(
        f"После добавления стало {final_total}",
        f"Чтобы узнать, сколько было сначала, из того, что стало, вычитаем добавленное: {final_total} - {added} = {initial}",
        f"Ответ: сначала было {answer_value}",
        "Совет: если число увеличили и известно, сколько стало, для начала делают вычитание",
    )



def _ps_try_remaining_after_loss(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    question = _question_lower_text(raw_text)
    if "сколько осталось" not in question and "сколько осталось" not in lower:
        return None
    if not any(hint in lower for hint in WORD_LOSS_HINTS):
        return None

    before_match = re.search(r"(?:было|лежало|стояло|находилось|имелось|всего было)[^\d]{0,25}(\d+)", lower)
    removed_match = re.search(fr"{_PS_LOSS_VERB_RE}[^\d]{{0,25}}(\d+)", lower)
    nums = extract_ordered_numbers(lower)
    if not nums:
        return None

    before = int(before_match.group(1)) if before_match else nums[0]
    removed = int(removed_match.group(1)) if removed_match else (nums[1] if len(nums) > 1 else None)
    if removed is None or before < removed:
        return None

    remaining = before - removed
    noun = _ps_first_meaningful_noun(raw_text)
    answer_value = _ps_format_count(remaining, noun)
    return join_explanation_lines(
        f"Сначала смотрим, сколько было: {before}",
        f"Потом смотрим, сколько убрали: {removed}",
        f"Чтобы узнать, сколько осталось, вычитаем: {before} - {removed} = {remaining}",
        f"Ответ: осталось {answer_value}",
        "Совет: если что-то забрали или потратили, нужно вычитать",
    )



def _ps_try_group_count(raw_text: str) -> Optional[str]:
    lower = normalize_word_problem_text(raw_text).lower()
    question = _question_lower_text(raw_text)
    if "поровну" in lower:
        return None
    if not (any(fragment in question for fragment in ("понадоб", "нужно", "получилось", "сколько")) and "по" in lower):
        return None
    if not contains_any_fragment(lower, GROUPING_VERBS) and "понадоб" not in question:
        return None
    ...
    total = nums[0]
    per_group_match = re.search(r"по\s+(\d+)\b", lower)
    if not per_group_match:
        return None

    per_group = int(per_group_match.group(1))
    if per_group <= 0:
        return None

    groups = total // per_group
    remainder = total % per_group
    container_noun = _ps_question_noun(raw_text)

    if remainder == 0:
        answer_value = _ps_format_count(groups, container_noun)
        return join_explanation_lines(
            f"Всего предметов {total}",
            f"В одной группе по {per_group}",
            f"Чтобы узнать, сколько групп получилось, делим: {total} : {per_group} = {groups}",
            f"Ответ: понадобилось {answer_value}" if container_noun else f"Ответ: {groups}",
            "Совет: если известно, сколько предметов в одной группе, число групп находят делением",
        )

    total_groups = groups + 1
    answer_value = _ps_format_count(total_groups, container_noun)
    return join_explanation_lines(
        f"Сначала узнаём, сколько полных групп получится: {total} : {per_group} = {groups} и остаток {remainder}",
        f"Остаток {remainder} тоже нужно куда-то положить, значит нужна ещё одна группа",
        f"Всего понадобится {groups} + 1 = {total_groups}",
        f"Ответ: понадобилось {answer_value}" if container_noun else f"Ответ: {total_groups}",
        "Совет: если после деления есть остаток, для него нужна ещё одна группа",
    )



def _ps_try_high_confidence_primary_school_explanation(raw_text: str) -> Optional[str]:
    return (
        _ps_try_fraction_whole_from_part(raw_text)
        or _ps_try_simple_motion_form(raw_text)
        or _ps_try_initial_after_loss(raw_text)
        or _ps_try_initial_after_gain(raw_text)
        or _ps_try_remaining_after_loss(raw_text)
        or _ps_try_group_count(raw_text)
    )



def extract_condition_and_question(raw_text: str) -> Tuple[str, str]:
    condition, question = _PS_20260413_PREV_EXTRACT_CONDITION_AND_QUESTION(raw_text)
    if question:
        return condition, question

    pretty = normalize_word_problem_text(raw_text).strip().rstrip(".?!")
    lower = pretty.lower().replace("ё", "е")
    if re.match(r"^(найди|найдите|узнай|узнайте|определи|определите)\b", lower):
        cleaned_question = re.sub(r"^(найди|найдите|узнай|узнайте|определи|определите)\s+", "", pretty, flags=re.IGNORECASE).strip()
        if cleaned_question:
            if _ps_normalize_key(condition).rstrip(".?!") == _ps_normalize_key(pretty).rstrip(".?!"):
                condition = ""
            return condition, cleaned_question
...
def _detailed_finalize_line(line: str) -> str:
    cleaned = _ps_cleanup_answer_like_text(line)
    return _PS_20260413_PREV_DETAILED_FINALIZE_LINE(cleaned)



def _user_final_answer_phrase_from_question(answer: str, raw_text: str, kind: str) -> str:
    cleaned_answer = _ps_cleanup_answer_like_text(answer)
    base = _PS_20260413_PREV_ANSWER_PHRASE(cleaned_answer, raw_text, kind)
    return _ps_cleanup_answer_like_text(base)



def build_explanation_local_first(user_text: str, kind: str) -> Optional[str]:
    return _ps_try_high_confidence_primary_school_explanation(user_text) or _PS_20260413_PREV_BUILD_EXPLANATION_LOCAL_FIRST(user_text, kind)


# Добавляем внешний permissive CORS-слой для локальной разработки.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://wolandvp-beep.github.io", "null"],
...
# --- DELIVERY PATCH 2026-04-13C: cleaner question wording in info lines ---

_PS_20260413C_PREV_EXTRACT_CONDITION_AND_QUESTION = extract_condition_and_question


def _ps_make_question_more_natural(question: str) -> str:
    raw = str(question or "").strip().rstrip(".?!")
    if not raw:
        return raw
    lower = raw.lower().replace("ё", "е")
    match = re.match(r"^(найди|найдите|найти|узнай|узнайте|определи|определите)\s+(.+)$", lower)
    if not match:
        return raw

    target = raw[len(match.group(1)):].strip()
    target_lower = target.lower().replace("ё", "е")
    if target_lower.startswith("периметр"):
        return f"Чему равен {target}"
    if target_lower.startswith("площад"):
        return f"Чему равна {target}"
    if target_lower.startswith("скорость"):
        return f"Какова {target}"
    if target_lower.startswith("расстояние"):
        return f"Чему равно {target}"
    if target_lower.startswith("время"):
        return f"Чему равно {target}"
    return target
...
def extract_condition_and_question(raw_text: str) -> Tuple[str, str]:
    condition, question = _PS_20260413C_PREV_EXTRACT_CONDITION_AND_QUESTION(raw_text)
    return condition, _ps_make_question_more_natural(question)

...
def explain_two_digit_division(dividend: int, divisor: int) -> Optional[str]:
    ...
    if quotient >= 10:
        return None
    # Попробуем подобрать частное
    # В книге: ищем число, на которое надо умножить делитель, чтобы получить делимое
    return join_explanation_lines(
...
def try_local_proportional_division_word_problem(raw_text: str) -> Optional[str]:
    ...
    if len(nums) < 3:
        return None
    # Ищем фразу типа "10 наборов голубых и 4 набора красных"

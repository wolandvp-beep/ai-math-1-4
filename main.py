...
Не меняй числа из условия.
Не добавляй выдуманные данные.
Если решение длинное, всё равно пиши его полностью, не сокращай.

Главный принцип объяснения:
Каждое действие объясняй как в начальной школе.
После каждого действия пиши, что именно нашли.
По возможности используй связку «Если ..., то ...».
Если на главный вопрос нельзя ответить сразу, прямо скажи, что нужно узнать сначала.

Формат для выражения:
Сначала напиши полный пример с ответом: «Пример: ... = ...».
Потом напиши строку «Порядок действий:».
Следующей строкой поставь номера действий над знаками.
Ещё следующей строкой запиши само выражение.
Потом слово «Решение.»
Дальше решай строго по действиям:
1) ...
2) ...
3) ...
Потом строка «Ответ: ...».
Последняя строка «Совет: ...».
Если есть скобки, сначала выполняй действие в скобках.
Если в выражении есть и +, -, ×, :, то сначала выполняй умножение и деление, потом сложение и вычитание.
Если действия одной ступени, выполняй их слева направо.

Формат для текстовой задачи:
Сначала строки:
«Что известно: ...»
«Что нужно найти: ...»
Если задача составная и сразу ответить нельзя, добавь строку:
«Сразу ответить на вопрос нельзя. Сначала нужно узнать ...»
Потом слово «Решение.»
Дальше решение только по действиям:
1) Если ..., то ...
2) Если ..., то ...
3) Если ..., то ...
Каждое действие записывай полностью, с вычислением и пояснением, что нашли.
Если в задаче два вопроса, ответь на оба по порядку.
Потом строка «Ответ: ...».
Последняя строка «Совет: ...».

Формат для уравнения:
Сначала слово «Решение.»
Потом объясни, какой компонент неизвестен и каким обратным действием его ищем.
После решения обязательно дай строку «Проверка: ...».
Потом строку «Ответ: ...».
Последняя строка «Совет: ...».

Формат для геометрии:
Сначала назови правило или формулу.
Потом подставь числа.
Потом посчитай по шагам.
Потом строку «Ответ: ...».
Последняя строка «Совет: ...».
...
Приёмы устного и письменного счёта:
...
если в вычислениях есть больше двух двузначных чисел или хотя бы одно трёхзначное число, можно использовать вычисление в столбик;
для деления столбиком называй неполное делимое, подбор цифры, умножение, вычитание и снос следующей цифры.

Если запись непонятная или это не математическая задача, попроси записать её понятнее.
""".strip()

LOCAL_REWRITE_PROMPT = """
Ты переписываешь уже найденное верное решение задачи для ученика начальной школы.
Нельзя менять числа, действия, промежуточные результаты, порядок шагов, единицы измерения и ответ.
Твоя задача — сделать объяснение более полным, подробным и школьным.
Пиши только по-русски.
Не используй markdown, таблицы, смайлики и лишние вступления.
Каждая строка — одна законченная полезная мысль.

Если это текстовая задача:
сначала напиши «Что известно: ...» и «Что нужно найти: ...».
Если задача составная, скажи, что сразу ответить нельзя и что нужно узнать сначала.
Потом напиши «Решение.»
Каждое действие пиши с новой строки и нумеруй:
1) Если ..., то ...
2) Если ..., то ...
3) Если ..., то ...
После каждого действия поясняй, что нашли.
В конце напиши «Ответ: ...».
Последняя строка — «Совет: ...».

Если это геометрическая задача:
сначала назови правило или формулу,
потом подставь числа,
потом решай по шагам,
потом «Ответ: ...» и «Совет: ...».

Если это задача на цену, количество и стоимость, используй связи:
стоимость = цена × количество
цена = стоимость : количество
количество = стоимость : цена

Если это задача на движение, используй связи:
S = v × t
v = S : t
t = S : v
...
LAYOUT_LINE_RE = re.compile(r"^[ 0-9()+\-×:=/]+$")
...
def is_layout_line(text: str) -> bool:
    return bool(LAYOUT_LINE_RE.fullmatch(str(text or "")))
...
def sanitize_model_text(text: str) -> str:
    ...
    for raw in raw_lines:
        if not raw.strip():
            continue
        stripped = raw.strip()
        if BANNED_OPENERS.match(stripped):
            continue
        if is_layout_line(raw):
            normalized = raw.rstrip()
            key = normalized
        else:
            normalized = SECTION_PREFIX_RE.sub(lambda m: f"{m.group(1).capitalize()}: ", stripped)
...
def shape_explanation(text: str, kind: str, forced_answer: Optional[str] = None, forced_advice: Optional[str] = None) -> str:
    ...
    for raw_line in cleaned.split("\n"):
        if not raw_line.strip():
            continue
        line = raw_line.rstrip() if is_layout_line(raw_line) else raw_line.strip()
    ...
    for line in body_lines:
        ...
        if key in seen:
            continue
        seen.add(key)
        if not is_layout_line(normalized) and is_low_value_body_line(normalized):
            continue
...
def display_compact_expression(source: str) -> str:
    return str(source or "").replace("*", "×").replace("/", ":")


def collect_binops_in_eval_order(node: ast.AST) -> List[ast.BinOp]:
    operations: List[ast.BinOp] = []

    def walk(current: ast.AST):
...
def operator_index_in_source(source: str, node: ast.BinOp) -> Optional[int]:
    left_end = getattr(node.left, "end_col_offset", None)
    right_start = getattr(node.right, "col_offset", None)
    ...
    if 0 <= left_end < len(source) and source[left_end] in "+-*/":
        return left_end
    return None


def build_expression_order_lines(source: str, node: ast.AST) -> Tuple[str, str]:
    display = display_compact_expression(source)
    markers = [" "] * len(display)
    for order, operation in enumerate(collect_binops_in_eval_order(node), start=1):
        idx = operator_index_in_source(source, operation)
        if idx is None or idx >= len(markers):
            continue
        label = str(order)
        start = max(0, idx - (len(label) - 1) // 2)
        if start + len(label) > len(markers):
            start = max(0, len(markers) - len(label))
        for pos, char in enumerate(label):
            target = start + pos
            if 0 <= target < len(markers):
                markers[target] = char
    return "".join(markers).rstrip(), display


def expression_order_rule(source: str) -> str:
    has_brackets = "(" in source or ")" in source
    mixed = bool(re.search(r"[+\-].*[*/]|[*/].*[+\-]", source))
    if has_brackets and mixed:
        return "Сначала выполняем действия в скобках. Потом по порядку выполняем умножение и деление. После этого выполняем сложение и вычитание."
    if has_brackets:
        return "Сначала выполняем действия в скобках. Потом остальные действия выполняем по порядку."
    if mixed:
        return "Сначала выполняем умножение и деление. Потом выполняем сложение и вычитание."
    return "Действия одной ступени выполняем слева направо."


def build_multistep_expression_explanation(source: str, node: ast.AST) -> Optional[str]:
    try:
    ...
    order_line, expression_line = build_expression_order_lines(source, node)
    pretty_expression = render_node(node)
    answer = format_fraction(value)
    lines = [f"Пример: {pretty_expression} = {answer}", "Порядок действий:"]
    if order_line:
        lines.append(order_line)
    lines.append(expression_line)
    lines.append("Решение.")
    lines.append(expression_order_rule(source))
    for index, step in enumerate(steps, start=1):
        lines.append(f"{index}) {step['left']} {step['operator']} {step['right']} = {step['result']}.")
    advice = "сначала смотри на порядок действий, а потом считай по шагам"
    lines.extend([f"Ответ: {answer}", f"Совет: {advice}"])
...
def format_step_lines(steps, raw_source: str):
    ...
    for index, step in enumerate(steps):
        if index == 0 and has_brackets and len(steps) > 1:
            lines.append(f"1) Сначала считаем в скобках: {step['left']} {step['operator']} {step['right']} = {step['result']}")
            continue
        action_num = index + 1
        lines.append(f"{action_num}) {step['verb']}: {step['left']} {step['operator']} {step['right']} = {step['result']}")
    return lines
...
def explain_addition_via_ten(left: int, right: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Чтобы получить 10, к {left} нужно прибавить {to_ten}.",
        f"2) Разложим {right} на {to_ten} и {rest}.",
        f"3) {left} + {to_ten} = 10, потом 10 + {rest} = {total}.",
        f"Ответ: {total}",
...
def explain_subtraction_via_ten(left: int, right: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Разложим {right} на {first_part} и {second_part}.",
        f"2) Сначала {left} - {first_part} = 10.",
        f"3) Потом 10 - {second_part} = {result}.",
        f"Ответ: {result}",
...
def explain_subtraction_two_digit_with_borrow(minuend: int, subtrahend: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Представим {minuend} как {tens} + {units}.",
        f"2) Представим {subtrahend} как {subtrahend_tens} + {subtrahend_units}.",
        f"3) Вычитаем десятки: {tens} - {subtrahend_tens} = {tens - subtrahend_tens}.",
        f"4) Вычитаем единицы: {units} - {subtrahend_units} = {units - subtrahend_units}.",
        f"5) Складываем разности: {tens - subtrahend_tens} + {units - subtrahend_units} = {result}.",
        f"Ответ: {result}",
...
def explain_simple_addition(left: int, right: int) -> str:
    ...
    if 10 <= left <= 99 and 10 <= right <= 99:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Разложим числа на десятки и единицы: {left} = {lt} + {lu}, {right} = {rt} + {ru}.",
            f"2) Складываем десятки: {lt} + {rt} = {lt + rt}.",
            f"3) Складываем единицы: {lu} + {ru} = {lu + ru}.",
            f"4) Теперь складываем результаты: {lt + rt} + {lu + ru} = {total}.",
            f"Ответ: {total}",
    ...
    return join_explanation_lines("Решение.", f"1) Считаем: {left} + {right} = {total}.", f"Ответ: {total}", "Совет: называй числа по порядку")
...
def explain_simple_subtraction(left: int, right: int) -> str:
    ...
    if 10 <= left <= 99 and 10 <= right <= 99 and result >= 0 and left < 100:
        ...
        if lu >= ru:
            return join_explanation_lines(
                "Решение.",
                f"1) Разложим числа на десятки и единицы: {left} = {lt} + {lu}, {right} = {rt} + {ru}.",
                f"2) Вычитаем десятки: {lt} - {rt} = {lt - rt}.",
                f"3) Вычитаем единицы: {lu} - {ru} = {lu - ru}.",
                f"4) Складываем результаты: {lt - rt} + {lu - ru} = {result}.",
                f"Ответ: {result}",
                "Совет: двузначные числа удобно раскладывать на десятки и единицы",
            )
    ...
    if result < 0:
        return join_explanation_lines(
            "Решение.",
            f"1) {left} меньше {right}, поэтому ответ будет отрицательным.",
            f"2) Считаем: {left} - {right} = {result}.",
            f"Ответ: {result}",
            "Совет: перед вычитанием полезно сравнить числа",
        )
    return join_explanation_lines("Решение.", f"1) Считаем: {left} - {right} = {result}.", f"Ответ: {result}", "Совет: вычитай спокойно и следи за знаком минус")
...
def explain_two_digit_by_two_digit_multiplication(left: int, right: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Разложим {small} на {tens} и {units}.",
        f"2) {big} × {tens} = {first}.",
        f"3) {big} × {units} = {second}.",
        f"4) Складываем частичные результаты: {first} + {second} = {result}.",
        f"Ответ: {result}",
    ...
    if op is ast.Mult:
        if right in (10, 100, 1000):
            ...
            return join_explanation_lines(
                ...
                f"1) При умножении на {right} к числу справа дописываем {zeros} нул{'ь' if zeros == 1 else 'я' if 2 <= zeros <= 4 else 'ей'}.",
                f"2) {left} × {right} = {result}.",
                f"Ответ: {result}",
        ...
        if right in (10, 100, 1000):
            ...
            return join_explanation_lines(
                f"Деление на {right}",
                f"1) При делении на {right} у числа справа отбрасываем {zeros} нул{'ь' if zeros == 1 else 'я' if 2 <= zeros <= 4 else 'ей'}.",
                f"2) {left} : {right} = {result}.",
                f"Ответ: {result}",
...
def explain_two_digit_division(dividend: int, divisor: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно разделить {dividend} на {divisor}.",
        f"2) Подбираем число, которое при умножении на {divisor} даст {dividend}.",
        f"3) Пробуем: {divisor} × {quotient} = {dividend}.",
        f"4) Значит, {dividend} : {divisor} = {quotient}.",
        f"Ответ: {quotient}",
...
def explain_simple_multiplication(left: int, right: int) -> str:
    ...
    if big >= 10 and small <= 10:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Разбиваем {big} на {tens} и {units}.",
            f"2) {tens} × {small} = {tens * small}.",
            f"3) {units} × {small} = {units * small}.",
            f"4) Складываем части: {tens * small} + {units * small} = {result}.",
            f"Ответ: {result}",
            "Совет: умножение удобно разбирать на части",
        )
    return join_explanation_lines("Решение.", f"1) Считаем: {left} × {right} = {result}.", f"Ответ: {result}", "Совет: умножение показывает одинаковые группы")
...
def explain_simple_division(left: int, right: int) -> str:
    ...
    if left < 100 and right < 10:
        if remainder == 0:
            return join_explanation_lines(
                "Решение.",
                f"1) Ищем, на какое число нужно умножить делитель, чтобы получить делимое.",
                f"2) {quotient} × {right} = {left}, значит {left} : {right} = {quotient}.",
                f"Ответ: {quotient}",
                "Совет: в простом делении полезно проверять умножением",
            )
        return join_explanation_lines(
            "Решение.",
            f"1) Ищем, сколько полных раз делитель помещается в делимом.",
            f"2) {quotient} × {right} = {quotient * right}.",
            f"3) Остаток: {left} - {quotient * right} = {remainder}.",
    ...
    if left < 100 and right < 100 and remainder == 0:
        return join_explanation_lines(
            "Решение.",
            f"1) Ищем, на какое число нужно умножить делитель, чтобы получить делимое.",
            f"2) {right} × {quotient} = {left}.",
            f"3) Значит, {left} : {right} = {quotient}.",
            f"Ответ: {quotient}",
...
def try_local_expression_explanation(raw_text: str) -> Optional[str]:
    ...
    detailed = build_multistep_expression_explanation(source, node)
    if detailed:
        return detailed
    ...
    body_lines = ["Решение."] + format_step_lines(steps, source)
    answer = format_fraction(value)
...
def try_local_fraction_explanation(raw_text: str) -> Optional[str]:
    ...
    if b == d:
        ...
        lines.extend([
            "1) Сначала смотрим на знаменатели. Они одинаковые.",
            f"2) Значит, работаем только с числителями: {a} {action_symbol} {c} = {top_result}.",
            f"3) Получаем дробь {top_result}/{b}.",
        ])
        if format_fraction(result) != f"{top_result}/{b}":
            lines.append(f"4) Сокращаем: {top_result}/{b} = {format_fraction(result)}.")
    ...
    lines.extend([
        f"1) Сначала делаем одинаковые знаменатели: {common}.",
        f"2) {a}/{b} = {a_scaled}/{common}, а {c}/{d} = {c_scaled}/{common}.",
        f"3) Теперь считаем: {a_scaled}/{common} {action_symbol} {c_scaled}/{common} = {top_result}/{common}.",
    ])
    if format_fraction(simplified) != f"{top_result}/{common}":
        lines.append(f"4) Сокращаем: {top_result}/{common} = {format_fraction(simplified)}.")
...
def try_local_equation_explanation(raw_text: str) -> Optional[str]:
    ...
    for pattern, kind in patterns:
        ...
        if kind in {"x_plus", "plus_x"}:
            ...
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестное слагаемое.",
                f"2) Чтобы найти неизвестное слагаемое, из суммы вычитаем известное: {rhs_text} - {number_text} = {format_fraction(answer)}.",
        ...
        if kind == "x_minus":
            ...
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестное уменьшаемое.",
                f"2) Чтобы найти неизвестное уменьшаемое, к разности прибавляем вычитаемое: {rhs_text} + {number_text} = {format_fraction(answer)}.",
        ...
        if kind == "minus_x":
            ...
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестное вычитаемое.",
                f"2) Чтобы найти неизвестное вычитаемое, из уменьшаемого вычитаем разность: {number_text} - {rhs_text} = {format_fraction(answer)}.",
        ...
        if kind in {"x_mul", "mul_x"}:
            if number == 0:
                if rhs_value == 0:
                    return join_explanation_lines("Решение.", "1) При умножении на 0 всегда получается 0.", "2) Значит, подходит любое число.", "Ответ: подходит любое число", "Совет: 0 × любое число = 0")
                return join_explanation_lines("Решение.", "1) При умножении на 0 нельзя получить другое число.", "Ответ: решения нет", "Совет: проверь уравнение ещё раз")
            answer = rhs_value / number
            check_template = f"x × {number_text}" if kind == "x_mul" else f"{number_text} × x"
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестный множитель.",
                f"2) Чтобы найти неизвестный множитель, произведение делим на известный множитель: {rhs_text} : {number_text} = {format_fraction(answer)}.",
                format_equation_check(check_template, format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
        ...
        if kind == "x_div":
            if number == 0:
                return join_explanation_lines("Решение.", "1) На ноль делить нельзя.", "Ответ: решения нет", "Совет: проверь делитель")
            answer = rhs_value * number
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестное делимое.",
                f"2) Чтобы найти неизвестное делимое, делитель умножаем на частное: {rhs_text} × {number_text} = {format_fraction(answer)}.",
                format_equation_check(f"x : {number_text}", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
        ...
        if kind == "div_x":
            if rhs_value == 0:
                return join_explanation_lines("Решение.", "1) На ноль делить нельзя, а частное здесь равно 0.", "Ответ: решения нет", "Совет: проверь уравнение ещё раз")
            answer = number / rhs_value
            return join_explanation_lines(
                "Решение.",
                "1) Ищем неизвестный делитель.",
                f"2) Чтобы найти неизвестный делитель, делимое делим на частное: {number_text} : {rhs_text} = {format_fraction(answer)}.",
                format_equation_check(f"{number_text} : x", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
...
def explain_ratio_word_problem(first: int, second: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, во сколько раз одно число больше или меньше другого.",
        f"2) Для этого большее число делим на меньшее: {bigger} : {smaller} = {result}.",
...
def explain_addition_word_problem(first: int, second: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько всего или сколько стало.",
        f"2) Подходит сложение: {first} + {second} = {result}.",
        f"Ответ: {result}",
...
def explain_subtraction_word_problem(first: int, second: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько осталось или сколько стало меньше.",
        f"2) Подходит вычитание: {first} - {second} = {result}.",
        f"Ответ: {result}",
...
def explain_comparison_word_problem(first: int, second: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, на сколько одно число больше или меньше другого.",
        f"2) Для этого из большего числа вычитаем меньшее: {bigger} - {smaller} = {result}.",
        f"Ответ: {result}",
...
def explain_find_initial_after_loss_problem(remaining: int, removed: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько было сначала.",
        f"2) Если осталось {remaining} и убрали {removed}, то сначала было {remaining} + {removed} = {result}.",
        f"Ответ: {result}",
        "Совет: неизвестное уменьшаемое находим сложением",
    )
...
def explain_find_initial_after_gain_problem(final_total: int, added: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько было сначала.",
        f"2) Если стало {final_total} после прибавления {added}, то сначала было {final_total} - {added} = {result}.",
        f"Ответ: {result}",
...
def explain_find_added_problem(before: int, after: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько добавили.",
        f"2) Сравниваем, сколько было и сколько стало: {bigger} - {smaller} = {result}.",
        f"Ответ: {result}",
...
def explain_find_removed_problem(before: int, after: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько убрали.",
        f"2) Из того, что было, вычитаем то, что осталось: {bigger} - {smaller} = {result}.",
        f"Ответ: {result}",
        "Совет: неизвестное вычитаемое находим вычитанием",
    )
...
def explain_multiplication_word_problem(groups: int, per_group: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, сколько всего одинаковых предметов.",
        f"2) Если число {per_group} повторяется {groups} раз, используем умножение: {groups} × {per_group} = {result}.",
        f"Ответ: {result}",
...
def explain_sharing_word_problem(total: int, groups: int) -> Optional[str]:
    ...
    if remainder == 0:
        return join_explanation_lines(
            "Решение.",
            f"1) Нужно узнать, сколько получит каждый.",
            f"2) Для этого делим поровну: {total} : {groups} = {quotient}.",
            f"Ответ: {quotient}",
            "Совет: слова «поровну» и «каждый» подсказывают деление",
        )
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно разделить поровну.",
        f"2) Делим: {total} : {groups} = {quotient}, остаток {remainder}.",
...
def explain_group_count_word_problem(total: int, per_group: int, needs_extra_group: bool = False, explicit_remainder: bool = False) -> Optional[str]:
    ...
    if remainder == 0:
        return join_explanation_lines(
            "Решение.",
            f"1) Нужно узнать, сколько групп получится.",
            f"2) Для этого делим общее количество на число предметов в одной группе: {total} : {per_group} = {quotient}.",
            f"Ответ: {quotient}",
    ...
    if needs_extra_group:
        return join_explanation_lines(
            ...
            "2) Но предметы ещё остались, значит нужна ещё одна группа.",
            f"Ответ: {quotient + 1}",
    ...
    if explicit_remainder:
        return join_explanation_lines(
            "Решение.",
            f"1) Нужно узнать, сколько полных групп получится.",
            f"2) Делим: {total} : {per_group} = {quotient}, остаток {remainder}.",
            f"Ответ: {quotient}, остаток {remainder}",
...
def explain_related_quantity_word_problem(base: int, delta: int, mode: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество.",
        f"2) Оно на {delta} {mode}, значит считаем так: {base} {sign} {delta} = {result}.",
        f"Ответ: {result}",
...
def explain_related_total_word_problem(base: int, delta: int, mode: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество: {base} {sign} {delta} = {related}.",
        f"2) Потом находим всё вместе: {base} + {related} = {total}.",
        f"Ответ: {total}",
...
def explain_related_quantity_times_word_problem(base: int, factor: int, mode: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество.",
        f"2) Оно в {factor} {plural_form(factor, 'раз', 'раза', 'раз')} {mode}, значит считаем так: {base} {op} {factor} = {result}.",
        f"Ответ: {result}",
...
def explain_related_total_times_word_problem(base: int, factor: int, mode: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество: {base} {op} {factor} = {related}.",
        f"2) Потом находим всё вместе: {base} + {related} = {total}.",
        f"Ответ: {total}",
...
def explain_indirect_plus_minus_total_problem(base: int, delta: int, relation: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Это косвенная форма: если здесь на {delta} {relation}, то {relation_text}.",
        f"2) Сначала находим второе число: {base} {sign} {delta} = {other}.",
        f"3) Потом находим всё вместе: {base} + {other} = {total}.",
...
def explain_indirect_times_total_problem(base: int, factor: int, relation: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Это косвенная форма: если здесь в {factor} {plural_form(factor, 'раз', 'раза', 'раз')} {relation}, то {relation_text}.",
        f"2) Сначала находим второе число: {base} {op} {factor} = {other}.",
        f"3) Потом находим всё вместе: {base} + {other} = {total}.",
        f"Ответ: второе количество — {other}; всего — {total}",
        "Совет: в косвенной задаче сначала переведи её в прямую",
    )
...
def explain_relation_chain_total_word_problem(base: int, delta1: int, mode1: str, delta2: int, mode2: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество: {base} {sign1} {delta1} = {second}.",
        f"2) Потом находим третье количество: {second} {sign2} {delta2} = {third}.",
        f"3) Теперь находим всё вместе: {base} + {second} + {third} = {total}.",
        f"Ответ: {total}",
...
def explain_bring_to_unit_total_word_problem(groups: int, total_amount: int, target_groups: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим одну группу: {total_amount} : {groups} = {one_group}.",
        f"2) Потом находим {target_groups} таких же групп: {one_group} × {target_groups} = {result}.",
        f"Ответ: {result}",
...
def explain_find_third_addend_word_problem(total: int, first: int, second: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим сумму известных частей: {first} + {second} = {known}.",
        f"2) Потом из общего количества вычитаем эту сумму: {total} - {known} = {result}.",
        f"Ответ: {result}",
...
def explain_initial_from_taken_and_left_word_problem(first_taken: int, second_taken: int, left: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, сколько всего взяли: {first_taken} + {second_taken} = {taken}.",
        f"2) Потом находим, сколько было сначала: {taken} + {left} = {total}.",
        f"Ответ: {total}",
...
def explain_other_from_total_and_part_word_problem(total: int, known_part: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Из общего количества вычитаем известную часть: {total} - {known_part} = {other}.",
        f"Ответ: {other}",
...
def explain_compare_from_total_and_part_word_problem(total: int, known_part: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим вторую часть: {total} - {known_part} = {other}.",
        f"2) Потом сравниваем части: {bigger} - {smaller} = {diff}.",
        f"Ответ: {diff}",
...
def explain_sum_of_two_products_word_problem(first_count: int, first_value: int, second_count: int, second_value: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим первую часть: {first_count} × {first_value} = {first_total}.",
        f"2) Потом находим вторую часть: {second_count} × {second_value} = {second_total}.",
        f"3) Теперь находим всё вместе: {first_total} + {second_total} = {total}.",
        f"Ответ: {total}",
...
def explain_meeting_other_speed_word_problem(total_distance: int, first_speed: int, first_path: int, distance_unit: str = "", speed_unit: str = "") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим время до встречи: {first_path} : {first_speed} = {time_value}.",
        f"2) Потом находим путь второго участника: {total_distance} - {first_path} = {second_path}.",
        f"3) Теперь находим его скорость: {second_path} : {time_value} = {second_speed}.",
        f"Ответ: {answer}",
...
def explain_opposite_other_speed_word_problem(distance_after: int, time_value: int, first_speed: int, speed_unit: str = "") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим путь первого участника: {first_speed} × {time_value} = {first_path}.",
        f"2) Потом находим путь второго участника: {distance_after} - {first_path} = {second_path}.",
        f"3) Теперь находим его скорость: {second_path} : {time_value} = {second_speed}.",
        f"Ответ: {answer}",
        "Совет: если известны путь и время, скорость находят делением",
    )
...
def explain_meeting_distance_with_related_speed_word_problem(time_value: int, first_speed: int, delta: int, mode: str, distance_unit: str = "") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим скорость второго участника: {first_speed} {sign} {delta} = {second_speed}.",
        f"2) Потом находим путь первого участника: {first_speed} × {time_value} = {first_path}.",
        f"3) Находим путь второго участника: {second_speed} × {time_value} = {second_path}.",
        f"4) Теперь складываем пути: {first_path} + {second_path} = {total_distance}.",
        f"Ответ: {answer}",
...
def explain_indirect_plus_minus_problem(base: int, delta: int, relation: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Задача в косвенной форме: если это число на {delta} {relation}, то {relation_text}.",
        f"2) Считаем: {base} {op} {delta} = {result}.",
        f"Ответ: {result}",
...
def explain_indirect_times_problem(base: int, factor: int, relation: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Задача в косвенной форме: если это число в {factor} {plural_form(factor, 'раз', 'раза', 'раз')} {relation}, то {relation_text}.",
        f"2) Считаем: {base} {op} {factor} = {result}.",
        f"Ответ: {result}",
...
def explain_relation_chain_times_total_word_problem(base: int, factor1: int, mode1: str, factor2: int, mode2: str) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим второе количество: {base} {op1} {factor1} = {second}.",
        f"2) Потом третье количество: {second} {op2} {factor2} = {third}.",
        f"3) Теперь находим всё вместе: {base} + {second} + {third} = {total}.",
        f"Ответ: {total}",
...
def explain_groups_plus_extra_word_problem(groups: int, per_group: int, extra: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала считаем, сколько предметов в одинаковых группах: {groups} × {per_group} = {grouped_total}.",
        f"2) Потом прибавляем ещё {extra}: {grouped_total} + {extra} = {result}.",
        f"Ответ: {result}",
...
def explain_fraction_of_number_word_problem(total: int, numerator: int, denominator: int, ask_remaining: bool = False) -> Optional[str]:
    ...
    if ask_remaining:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим одну долю: {total} : {denominator} = {one_part}.",
            f"2) Потом находим {numerator}/{denominator} числа: {one_part} × {numerator} = {taken}.",
            f"3) Теперь находим, сколько осталось: {total} - {taken} = {remaining}.",
            f"Ответ: {remaining}",
...
def explain_number_by_fraction_word_problem(part_value: int, numerator: int, denominator: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим одну долю: {part_value} : {numerator} = {one_part}.",
        f"2) Потом находим всё число: {one_part} × {denominator} = {whole}.",
        f"Ответ: {whole}",
...
def explain_unit_rate_boxes_problem(groups: int, total_amount: int, wanted_amount: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, сколько приходится на одну группу: {total_amount} : {groups} = {per_group}.",
        f"2) Потом узнаём, сколько групп нужно: {wanted_amount} : {per_group} = {result}.",
        f"Ответ: {result}",
...
def explain_price_quantity_cost_problem(quantity: int, total_cost: int, wanted_cost: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим цену одной коробки: {total_cost} : {quantity} = {price}.",
        f"2) Потом узнаём, сколько коробок можно купить: {wanted_cost} : {price} = {result}.",
        f"Ответ: {result}",
...
def explain_price_difference_problem(quantity: int, total_a: int, total_b: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим цену первой покупки: {total_a} : {quantity} = {price_a}.",
        f"2) Потом находим цену второй покупки: {total_b} : {quantity} = {price_b}.",
        f"3) Теперь сравниваем цены: {max(price_a, price_b)} - {min(price_a, price_b)} = {diff}.",
        f"Ответ: {diff}",
...
def explain_simple_motion_distance(speed: int, time_value: int, unit: str = "") -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно найти расстояние.",
        f"2) Расстояние равно скорости, умноженной на время: {speed} × {time_value} = {result}.",
        f"Ответ: {answer}",
...
def explain_simple_motion_speed(distance: int, time_value: int, unit: str = "") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно найти скорость.",
        f"2) Скорость равна расстоянию, делённому на время: {distance} : {time_value} = {result}.",
        f"Ответ: {answer}",
...
def explain_simple_motion_time(distance: int, speed: int, unit: str = "") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно найти время.",
        f"2) Время равно расстоянию, делённому на скорость: {distance} : {speed} = {result}.",
        f"Ответ: {answer}",
...
def explain_meeting_motion(v1: int, v2: int, time_value: int, unit: str = "") -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим скорость сближения: {v1} + {v2} = {closing_speed}.",
        f"2) Потом находим всё расстояние: {closing_speed} × {time_value} = {distance}.",
        f"Ответ: {answer}",
...
def explain_opposite_motion(v1: int, v2: int, time_value: int, unit: str = "") -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим скорость удаления: {v1} + {v2} = {removal_speed}.",
        f"2) Потом находим расстояние: {removal_speed} × {time_value} = {distance}.",
        f"Ответ: {answer}",
...
def explain_catch_up_motion(distance: int, speed_diff: int, unit: str = "ч") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Нужно узнать, через сколько времени расстояние исчезнет.",
        f"2) Для этого делим расстояние между объектами на скорость сближения: {distance} : {speed_diff} = {time_value}.",
        f"Ответ: {answer}",
...
def explain_whole_by_remaining_fraction(remaining_value: int, spent_numerator: int, denominator: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, какая часть денег осталась: {denominator}/{denominator} - {spent_numerator}/{denominator} = {remaining_numerator}/{denominator}.",
        f"2) Теперь находим одну долю: {remaining_value} : {remaining_numerator} = {one_part}.",
        f"3) Потом находим всё число: {one_part} × {denominator} = {whole}.",
        f"Ответ: {whole}",
...
def explain_fraction_part_and_remaining(total: int, numerator: int, denominator: int, remaining_label: str = "осталось") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим одну долю: {total} : {denominator} = {one_part}.",
        f"2) Потом находим {numerator}/{denominator} числа: {one_part} × {numerator} = {part}.",
        f"3) Теперь находим, сколько {remaining_label}: {total} - {part} = {remaining}.",
        f"Ответ: {remaining}",
...
def explain_two_fraction_parts_remaining(total: int, first_fraction: Tuple[int, int], second_fraction: Tuple[int, int]) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим первую часть: {total} : {d1} × {n1} = {first_part}.",
        f"2) Потом находим вторую часть: {total} : {d2} × {n2} = {second_part}.",
        f"3) Теперь находим, сколько использовали всего: {first_part} + {second_part} = {used}.",
        f"4) Находим остаток: {total} - {used} = {remaining}.",
        f"Ответ: {remaining}",
...
def explain_second_part_after_fraction(total: int, numerator: int, denominator: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим одну долю: {total} : {denominator} = {one_part}.",
        f"2) Потом находим первую часть пути: {one_part} × {numerator} = {first_part}.",
        f"3) Теперь находим вторую часть: {total} - {first_part} = {second_part}.",
        f"Ответ: {second_part}",
...
def explain_simple_price_per_item(count: int, total_cost: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        "1) Сначала вспоминаем правило: цена = стоимость : количество.",
        f"2) Находим цену одной штуки: {total_cost} : {count} = {price} руб.",
        f"Ответ: {price} руб.",
...
def explain_simple_total_cost(price: int, count: int) -> str:
    ...
    return join_explanation_lines(
        "Решение.",
        "1) Сначала вспоминаем правило: стоимость = цена × количество.",
        f"2) Находим стоимость покупки: {price} × {count} = {total} руб.",
        f"Ответ: {total} руб.",
...
def explain_unknown_red_price(known_count: int, known_price: int, unknown_count: int, total_cost: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим стоимость известных цветов: {known_count} × {known_price} = {known_total}.",
        f"2) Потом находим, сколько заплатили за остальные цветы: {total_cost} - {known_total} = {other_total}.",
        f"3) Теперь находим цену одного цветка: {other_total} : {unknown_count} = {unit_price}.",
        f"Ответ: {unit_price} руб.",
...
def explain_sum_then_divide_word_problem(first: int, second: int, divisor: int) -> Optional[str]:
    ...
    if remainder == 0:
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим всё вместе: {first} + {second} = {total}.",
            f"2) Потом делим общее количество на размер одной группы: {total} : {divisor} = {quotient}.",
            f"Ответ: {quotient}",
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим всё вместе: {first} + {second} = {total}.",
        f"2) Потом делим: {total} : {divisor} = {quotient}, остаток {remainder}.",
        f"Ответ: {quotient}, остаток {remainder}",
...
def explain_cages_with_two_parts(first: int, second: int, total: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, сколько попугаев в одной клетке: {first} + {second} = {in_one}.",
        f"2) Потом находим число клеток: {total} : {in_one} = {cages}.",
        f"Ответ: {cages}",
...
def explain_equal_rate_distribution(total: int, first_count: int, second_count: int, first_label: str = "первая группа", second_label: str = "вторая группа") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим общее число равных частей: {first_count} + {second_count} = {group_total}.",
        f"2) Потом находим одну часть: {total} : {group_total} = {one_unit}.",
        f"3) Теперь находим {first_label}: {one_unit} × {first_count} = {first_total}.",
        f"4) И находим {second_label}: {one_unit} × {second_count} = {second_total}.",
...
def explain_piece_lengths_from_total_and_costs(total_length: int, first_cost: int, second_cost: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим общую стоимость: {first_cost} + {second_cost} = {total_cost}.",
        f"2) Потом находим цену одного метра: {total_cost} : {total_length} = {unit_price}.",
        f"3) Теперь находим длину первого куска: {first_cost} : {unit_price} = {first_length}.",
        f"4) И длину второго куска: {second_cost} : {unit_price} = {second_length}.",
        f"Ответ: {first_length} м; {second_length} м",
...
def explain_bag_counts_from_mass_difference(first_mass: int, second_mass: int, diff_bags: int, first_name: str = "первого", second_name: str = "второго") -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим разность масс: {max(first_mass, second_mass)} - {min(first_mass, second_mass)} = {diff_mass}.",
        f"2) Эта разность приходится на {diff_bags} мешков, значит в одном мешке: {diff_mass} : {diff_bags} = {per_bag} кг.",
        f"3) Теперь находим количество мешков {first_name} урожая: {first_mass} : {per_bag} = {first_count}.",
        f"4) И количество мешков {second_name} урожая: {second_mass} : {per_bag} = {second_count}.",
...
def explain_masses_from_bag_difference(first_bags: int, second_bags: int, diff_mass: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, на сколько мешков отличаются участки: {max(first_bags, second_bags)} - {min(first_bags, second_bags)} = {diff_bags}.",
        f"2) Этим {diff_bags} мешкам соответствуют {diff_mass} кг, значит в одном мешке: {diff_mass} : {diff_bags} = {per_bag} кг.",
        f"3) Теперь находим массу первого участка: {first_bags} × {per_bag} = {first_mass} кг.",
        f"4) И массу второго участка: {second_bags} × {per_bag} = {second_mass} кг.",
        f"Ответ: {first_mass} кг; {second_mass} кг",
...
def explain_water_buckets_difference(big_bucket: int, small_bucket: int, diff_total: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим, на сколько литров за один раз больше приносили воды: {big_bucket} - {small_bucket} = {step_diff} л.",
        f"2) Потом находим число одинаковых ходок: {diff_total} : {step_diff} = {trips}.",
        f"3) Теперь находим, сколько воды принёс первый: {big_bucket} × {trips} = {big_total} л.",
        f"4) И сколько воды принёс второй: {small_bucket} × {trips} = {small_total} л.",
        f"Ответ: {big_total} л; {small_total} л",
...
def explain_costs_from_length_difference(first_length: int, second_length: int, diff_cost: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим разность длин: {max(first_length, second_length)} - {min(first_length, second_length)} = {diff_length} м.",
        f"2) Потом находим цену одного метра: {diff_cost} : {diff_length} = {price_per_meter} руб.",
        f"3) Теперь находим стоимость первого куска: {first_length} × {price_per_meter} = {first_cost} руб.",
        f"4) И стоимость второго куска: {second_length} × {price_per_meter} = {second_cost} руб.",
        f"Ответ: {first_cost} руб.; {second_cost} руб.",
...
def explain_trip_time_for_same_distance(speed1: int, time1: int, speed2: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим расстояние между пунктами: {speed1} × {time1} = {distance}.",
        f"2) Потом находим время для второго участника: {distance} : {speed2} = {time2} ч.",
        f"Ответ: {time2} ч",
...
def explain_second_day_motion_time(total_distance: int, first_speed: int, first_time: int, second_speed: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим путь в первый день: {first_speed} × {first_time} = {first_distance}.",
        f"2) Потом находим путь во второй день: {total_distance} - {first_distance} = {second_distance}.",
        f"3) Теперь находим время во второй день: {second_distance} : {second_speed} = {second_time}.",
        f"4) И находим всё время: {first_time} + {second_time} = {total_time}.",
...
def explain_remaining_part_speed(total_distance: int, first_speed: int, first_time: int, second_time: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим первую часть пути: {first_speed} × {first_time} = {first_distance}.",
        f"2) Потом находим оставшийся путь: {total_distance} - {first_distance} = {remaining_distance}.",
        f"3) Теперь находим скорость на оставшейся части пути: {remaining_distance} : {second_time} = {second_speed}.",
...
def explain_return_trip_time_by_faster_speed(distance: int, speed: int, factor: int) -> Optional[str]:
    ...
    return join_explanation_lines(
        "Решение.",
        f"1) Сначала находим время в одну сторону: {distance} : {speed} = {first_time} ч.",
        f"2) Потом находим скорость на обратном пути: {speed} × {factor} = {faster_speed} км/ч.",
        f"3) Теперь находим время на обратный путь: {distance} : {faster_speed} = {return_time} ч.",
...
def try_local_measurement_word_problem_explanation(raw_text: str) -> Optional[str]:
    ...
    if window_match and "площад" in lower:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала переводим высоту в дециметры: {height_m} м = {height_in_dm} дм.",
            f"2) Потом переводим ширину в дециметры: {width_m} м {width_dm} дм = {width_in_dm} дм.",
            f"3) Теперь находим площадь окна: {height_in_dm} × {width_in_dm} = {area} дм².",
            f"Ответ: {area} дм²",
    ...
    if thickness_match and frame_match and glass_match and "мм" in lower:
        ...
        if frame_factor != 0 and wall_cm % frame_factor == 0:
            ...
            if glass_factor != 0 and frame_mm % glass_factor == 0:
                ...
                return join_explanation_lines(
                    "Решение.",
                    f"1) Сначала находим толщину рамы: {wall_cm} : {frame_factor} = {frame_cm} см.",
                    f"2) Потом переводим толщину рамы в миллиметры: {frame_cm} см = {frame_mm} мм.",
                    f"3) Теперь находим толщину стекла: {frame_mm} : {glass_factor} = {glass_mm} мм.",
                    f"Ответ: {glass_mm} мм",
...
def try_local_geometry_explanation(raw_text: str) -> Optional[str]:
    ...
    if "прямоугольник" in lower and asks_area and asks_perimeter and "со сторонами" in lower and len(nums) >= 2:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим площадь: {length} × {width} = {area}.",
            f"2) Потом находим периметр: ({length} + {width}) × 2 = {perimeter}.",
    ...
    if explicit:
        ...
        if perimeter % 2 == 0:
            ...
            if width >= 0:
                ...
                return join_explanation_lines(
                    "Решение.",
                    f"1) Сначала находим сумму длины и ширины: половина периметра {perimeter} : 2 = {half}.",
                    f"2) Потом находим ширину: {half} - {length} = {width}.",
                    f"3) Теперь находим площадь: {length} × {width} = {area}.",
    ...
    if explicit:
        ...
        if length != 0 and area % length == 0:
            ...
            return join_explanation_lines(
                "Решение.",
                f"1) Сначала находим ширину прямоугольника: площадь {area} : длину {length} = {width}.",
                f"2) Потом находим периметр: ({length} + {width}) × 2 = {perimeter}.",
    ...
    if explicit:
        ...
        if tri_perimeter % 4 == 0:
            ...
            return join_explanation_lines(
                "Решение.",
                f"1) Сначала находим периметр равностороннего треугольника: {tri_side} × 3 = {tri_perimeter}.",
                f"2) Теперь находим сторону квадрата: {tri_perimeter} : 4 = {square_side}.",
    ...
    if "квадрат" in lower and asks_area and asks_perimeter and (square_side_val is not None or nums):
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим площадь квадрата: {side} × {side} = {area}.",
            f"2) Потом находим периметр квадрата: {side} × 4 = {perimeter}.",
            f"Ответ: площадь — {with_unit(area, unit, square=True)}; периметр — {with_unit(perimeter, unit)}",
    ...
    if "равносторон" in lower and "треугольник" in lower and "квадрат" in lower and "равен периметру" in lower and triangle_side_val is not None and asks_side:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим периметр равностороннего треугольника: {triangle_side_val} × 3 = {triangle_perimeter}.",
            f"2) Теперь находим сторону квадрата: {triangle_perimeter} : 4 = {square_side}.",
            f"Ответ: {with_unit(square_side, unit)}",
            "Совет: у равностороннего треугольника три равные стороны, а у квадрата четыре",
        )
    ...
    if "прямоугольник" in lower and area_val is not None and length_val is not None and asks_perimeter:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим ширину прямоугольника: {area_val} : {length_val} = {width}.",
            f"2) Потом находим периметр: ({length_val} + {width}) × 2 = {perimeter}.",
            f"Ответ: {with_unit(perimeter, unit)}",
            "Совет: если известны площадь и длина, сначала находят ширину",
        )
    ...
    if "прямоугольник" in lower and perimeter_val is not None and length_val is not None and asks_area:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Сначала находим сумму длины и ширины: {perimeter_val} : 2 = {half}.",
            f"2) Потом находим ширину: {half} - {length_val} = {width}.",
            f"3) Теперь находим площадь: {length_val} × {width} = {area}.",
            f"Ответ: {with_unit(area, unit, square=True)}",
            "Совет: если известен периметр прямоугольника, сначала находят его половину",
        )
    ...
    if "квадрат" in lower and perimeter_val is not None and asks_side:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) У квадрата все стороны равны.",
            f"2) Чтобы найти сторону, делим периметр на 4: {perimeter_val} : 4 = {side}.",
    ...
    if "квадрат" in lower and area_val is not None and asks_side:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Площадь квадрата равна стороне, умноженной на сторону.",
            f"2) Нужно найти число, которое в квадрате даёт {area_val}.",
            f"3) Это {side}, потому что {side} × {side} = {area_val}.",
            f"Ответ: {with_unit(side, unit)}",
    ...
    if "прямоугольник" in lower and area_val is not None and length_val is not None and asks_width:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Площадь прямоугольника равна длине, умноженной на ширину.",
    ...
    if "квадрат" in lower and square_side_val is not None and asks_perimeter:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Периметр квадрата — это сумма четырёх равных сторон.",
            f"2) Считаем: {square_side_val} × 4 = {result}.",
    ...
    if "прямоугольник" in lower and length_val is not None and width_val is not None and asks_perimeter:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Периметр прямоугольника равен сумме длины и ширины, умноженной на 2.",
            f"2) Сначала складываем длину и ширину: {length_val} + {width_val} = {length_val + width_val}.",
            f"3) Потом умножаем на 2: ({length_val} + {width_val}) × 2 = {result}.",
            f"Ответ: {with_unit(result, unit)}",
    ...
    if "квадрат" in lower and square_side_val is not None and asks_area:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Площадь квадрата равна стороне, умноженной на сторону.",
            f"2) Считаем: {square_side_val} × {square_side_val} = {result}.",
    ...
    if "прямоугольник" in lower and length_val is not None and width_val is not None and asks_area:
        ...
        return join_explanation_lines(
            "Решение.",
            f"1) Площадь прямоугольника равна длине, умноженной на ширину.",
            f"2) Считаем: {length_val} × {width_val} = {result}.",
            f"Ответ: {with_unit(result, unit, square=True)}",
        ...
        if delta == numbers[1]:
            ...
            if second is not None:
                ...
                if multiple and asks_total:
                    ...
                    return join_explanation_lines(
                        "Решение.",
                        f"1) Сначала находим маки: {numbers[0]} {'+' if mode == 'больше' else '-'} {delta} = {second}.",
                        f"2) Потом находим астры: {numbers[0]} + {second} = {third}.",
                        f"3) Теперь находим все цветы: {numbers[0]} + {second} + {third} = {total}.",
                        f"Ответ: астр — {third}; всего — {total}",
                        "Совет: если сказано «столько, сколько вместе», сначала найди сумму этих частей",
                    )
                return join_explanation_lines(
                    "Решение.",
                    f"1) Сначала находим второе количество: {numbers[0]} {'+' if mode == 'больше' else '-'} {delta} = {second}.",
                    f"2) Потом находим третье количество: {numbers[0]} + {second} = {third}.",
                    f"Ответ: {third}",
                    "Совет: если сказано «столько, сколько вместе», сначала найди сумму этих частей",
                )
    ...
    if len(numbers) == 3 and (asks_total or asks_current):
        ...
        if len(fragments) >= 4:
            ...
            if first_mode and second_mode:
                ...
                return join_explanation_lines(
                    "Решение.",
                    f"1) Сначала меняем число первый раз: {numbers[0]} {'+' if first_mode == 'gain' else '-'} {numbers[1]} = {mid}.",
                    f"2) Потом меняем число второй раз: {mid} {'+' if second_mode == 'gain' else '-'} {numbers[2]} = {end}.",
                    f"Ответ: {end}",
...
def try_local_word_problem_explanation(raw_text: str) -> Optional[str]:
    ...
    if contains_any_fragment(lower, ("нашел", "нашёл", "всего")) and "сколько" in lower:
        ...
        if bigger - smaller >= 0:
            return join_explanation_lines(
                "Решение.",
                f"1) Нужно найти неизвестное слагаемое.",
                f"2) Из суммы вычитаем известное слагаемое: {bigger} - {smaller} = {bigger - smaller}.",
...
def extract_numeric_signature(text: str) -> Tuple[str, ...]:
    return tuple(re.findall(r"\d+/\d+|\d+", extract_answer_value(text)))


def answers_are_compatible(reference_text: str, candidate_text: str) -> bool:
    reference_signature = extract_numeric_signature(reference_text)
    candidate_signature = extract_numeric_signature(candidate_text)
    if reference_signature and candidate_signature:
        return reference_signature == candidate_signature
    reference_answer = extract_answer_value(reference_text).lower()
    candidate_answer = extract_answer_value(candidate_text).lower()
    if reference_answer and candidate_answer:
        return reference_answer == candidate_answer
    return True


async def try_expand_local_with_llm(user_text: str, local_explanation: str, kind: str) -> Optional[str]:
    payload = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": LOCAL_REWRITE_PROMPT},
            {
                "role": "user",
                "content": (
                    f"Текст задачи:\n{user_text}\n\n"
                    f"Краткий верный план решения:\n{local_explanation}\n\n"
                    "Перепиши это решение подробно и по-школьному. "
                    "Ничего не меняй в вычислениях, порядке действий и ответе."
                ),
            },
        ],
        "max_tokens": 2200,
    ...
    if not answers_are_compatible(local_explanation, shaped):
        return None
    return shaped
...
async def build_explanation(user_text: str) -> dict:
    ...
    local_shaped: Optional[str] = None
    if local_explanation:
        prepared_local = add_task_context_lines(local_explanation, user_text, kind)
        local_shaped = shape_explanation(prepared_local, kind)

    if kind in {"expression", "equation", "fraction"} and local_shaped:
        return {"result": local_shaped, "source": "local", "validated": True}

    if kind in {"word", "geometry"} and local_explanation:
        expanded = await try_expand_local_with_llm(user_text, local_explanation, kind)
        if expanded:
            return {"result": expanded, "source": "local+llm", "validated": True}
        if local_shaped:
            return {"result": local_shaped, "source": "local", "validated": True}

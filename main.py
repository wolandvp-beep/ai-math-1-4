...
# METHODICAL_PATCH_V11

SYSTEM_PROMPT_V11 = """
Ты — методист и учитель математики начальной школы для детей 7–10 лет.
Объясняй так, как в хороших школьных пособиях: короткое условие, вопрос, выбор действия по смыслу, решение по действиям, ответ и совет.

Пиши только на русском языке.
Пиши без markdown, списков, смайликов и лишних вступлений.
Не используй похвалу и оценки.
Каждая строка — одна законченная мысль.

Главная структура:
Для текстовой задачи сначала дай строку "Краткое условие: ...".
Потом строку "Вопрос: ...".
Потом объясни, почему выбрано именно это действие.
Потом покажи решение по действиям.
В конце обязательно дай строку "Ответ: ..." и строку "Совет: ...".

Для задач на неизвестный компонент называй правило полностью:
...
Чтобы найти неизвестное делимое, частное умножаем на делитель.
Чтобы найти неизвестный делитель, делимое делим на частное.

Для составной задачи используй ход:
"Можем ли мы сразу ответить?";
если нет, коротко скажи почему;
потом строки "Сначала узнаем: ..." и "Потом узнаем: ...".

Для задач на цену, количество и стоимость используй зависимости:
стоимость = цена × количество;
цена = стоимость : количество;
количество = стоимость : цена.

Для задач на приведение к единице сначала находи одну единицу.
Для задач на доли сначала называй правило: часть от числа или число по его части.
Для задач на движение используй слова скорость, время, расстояние, скорость сближения и скорость удаления.
Для задач на совместную работу сначала находи работу за 1 единицу времени.
Для задач на геометрию сначала называй правило, потом подставляй числа.

Для письменных вычислений сохраняй школьный порядок:
сложение — начинаем с единиц;
вычитание — если цифры не хватает, занимаем у старшего разряда;
умножение — умножаем по разрядам справа налево и переносим десятки;
деление — берём первое неполное делимое, подбираем цифру частного, умножаем, вычитаем, сносим следующую цифру.

Не выдумывай данные.
Если запись неясная, попроси записать задачу понятнее.
""".strip()

_METHODICAL_GROUP_STEMS_V11 = (
    "короб", "пач", "ящик", "меш", "ведр", "бан", "рулон", "пакет", "корз", "сет", "полк", "гряд", "школ", "набор", "дней", "час", "мин"
)


def _question_tail_v11(text: str) -> str:
    fragments = [part.strip() for part in re.split(r"[.!?]", str(text or "")) if part.strip()]
    return fragments[-1].lower() if fragments else str(text or "").lower()



def _methodical_simple_v11(short_condition: str, question: str, reason_line: str, solution_line: str, answer: str, advice: str) -> str:
    return join_explanation_lines(
        f"Краткое условие: {short_condition}",
        f"Вопрос: {question}",
        reason_line,
        solution_line,
        f"Ответ: {answer}",
...
def _methodical_compound_v11(short_condition: str, question: str, why_not_line: str, first_step_line: str, second_step_line: str, answer: str, advice: str) -> str:
    return join_explanation_lines(
        f"Краткое условие: {short_condition}",
        f"Вопрос: {question}",
        f"Можем ли мы сразу ответить? Нет, потому что {why_not_line}",
        f"Сначала узнаем: {first_step_line}",
        f"Потом узнаем: {second_step_line}",
        f"Ответ: {answer}",
        f"Совет: {advice}",
    )
...
def _time_answer_v11(value: int, lower: str) -> str:
    if "мин" in lower:
        return f"{value} мин"
    if "сек" in lower:
        return f"{value} с"
...
def _distance_answer_v11(value: int, lower: str) -> str:
    if "см/" in lower or (re.search(r"\bсм\b", lower) and "км" not in lower and "м/" not in lower):
        return f"{value} см"
    if "м/" in lower and "км" not in lower:
        return f"{value} м"
    if re.search(r"\bм\b", lower) and "км" not in lower and "км/" not in lower:
        return f"{value} м"
    return f"{value} км"



def _speed_answer_v11(value: int, lower: str) -> str:
    if "см/мин" in lower:
        return f"{value} см/мин"
    if "м/мин" in lower:
        return f"{value} м/мин"
    if "м/с" in lower:
        return f"{value} м/с"
    if "м" in lower and "ч" in lower and "км" not in lower:
        return f"{value} м/ч"
    return f"{value} км/ч"
...
def explain_ratio_word_problem(first: int, second: int) -> Optional[str]:
    ...
    if smaller == 0:
        return join_explanation_lines(
            "На ноль делить нельзя",
            "Ответ: деление на ноль невозможно",
            "Совет: в вопросе 'во сколько раз' делим на ненулевое число",
        )
    if bigger % smaller != 0:
        return None
    result = bigger // smaller
    word = plural_form(result, "раз", "раза", "раз")
    return _methodical_simple_v11(
        f"первое число — {first}, второе число — {second}",
        "во сколько раз одно число больше или меньше другого?",
        "По смыслу выбираем деление, потому что вопрос начинается словами «во сколько раз».",
        f"Решение: {bigger} : {smaller} = {result}",
        f"{result} {word}",
        "вопрос «во сколько раз» решаем делением",
    )
...
def explain_addition_word_problem(first: int, second: int) -> str:
    result = first + second
    return _methodical_simple_v11(
        f"первое количество — {first}, второе количество — {second}",
        "сколько всего вместе или сколько стало?",
        "По смыслу выбираем сложение, потому что объединяем два количества.",
        f"Решение: {first} + {second} = {result}",
        str(result),
        "если нужно узнать, сколько всего вместе, складывай",
    )
...
def explain_subtraction_word_problem(first: int, second: int) -> str:
    ...
    return _methodical_simple_v11(
        f"было — {first}, убрали — {second}",
        "сколько осталось?",
        "По смыслу выбираем вычитание, потому что количество стало меньше.",
        f"Решение: {first} - {second} = {result}",
        str(result),
        "если что-то убрали, обычно нужно вычитать",
    )
...
def explain_comparison_word_problem(first: int, second: int) -> str:
    ...
    return _methodical_simple_v11(
        ...
        "По смыслу выбираем вычитание, потому что ищем разницу.",
        f"Решение: {bigger} - {smaller} = {result}",
        str(result),
        "вопрос «на сколько» решаем вычитанием",
    )
...
def explain_find_initial_after_loss_problem(remaining: int, removed: int) -> str:
    ...
    return _methodical_simple_v11(
        ...
        "Это задача на неизвестное уменьшаемое. Чтобы найти неизвестное уменьшаемое, к разности прибавляем вычитаемое.",
        f"Решение: {remaining} + {removed} = {result}",
        str(result),
        "если известно, сколько осталось и сколько убрали, начальное число находим сложением",
    )
...
def explain_find_initial_after_gain_problem(final_total: int, added: int) -> str:
    ...
    return _methodical_simple_v11(
        f"стало — {final_total}, добавили — {added}",
        "сколько было сначала?",
        "Чтобы узнать, сколько было, из того, что стало, вычитаем то, что добавили.",
        f"Решение: {final_total} - {added} = {result}",
        str(result),
        "если известно, сколько стало после прибавления, начальное число находим вычитанием",
    )
...
def explain_find_added_problem(before: int, after: int) -> str:
    ...
    return _methodical_simple_v11(
        ...
        "сколько добавили?",
        "Это задача на неизвестное слагаемое. Чтобы найти неизвестное слагаемое, из суммы вычитаем известное слагаемое.",
        f"Решение: {max(before, after)} - {min(before, after)} = {result}",
        str(result),
        "сравни число было и число стало",
    )
...
def explain_find_removed_problem(before: int, after: int) -> str:
    ...
    return _methodical_simple_v11(
        ...
        "сколько убрали?",
        "Это задача на неизвестное вычитаемое. Чтобы найти неизвестное вычитаемое, из уменьшаемого вычитаем разность.",
        f"Решение: {max(before, after)} - {min(before, after)} = {result}",
        str(result),
        "если известно, сколько было и сколько осталось, ищи разность",
    )
...
def explain_multiplication_word_problem(groups: int, per_group: int) -> str:
    result = groups * per_group
    return _methodical_simple_v11(
        f"групп — {groups}, в каждой группе — {per_group}",
        "сколько всего?",
        "По смыслу выбираем умножение, потому что одинаковое число повторяется несколько раз.",
        f"Решение: {groups} × {per_group} = {result}",
        str(result),
        "слова «по ... в каждой» часто подсказывают умножение",
    )
...
def explain_sharing_word_problem(total: int, groups: int) -> Optional[str]:
    ...
    if remainder == 0:
        return _methodical_simple_v11(
            f"всего — {total}, частей — {groups}",
            "сколько получит каждый?",
            "По смыслу выбираем деление, потому что делим поровну.",
            f"Решение: {total} : {groups} = {quotient}",
            str(quotient),
            "слова «поровну» и «каждый» подсказывают деление",
        )
    return _methodical_simple_v11(
        f"всего — {total}, частей — {groups}",
        "сколько получит каждый?",
        "По смыслу выбираем деление, потому что делим поровну.",
        f"Решение: {total} : {groups} = {quotient}, остаток {remainder}",
        f"каждому по {quotient}, остаток {remainder}",
...
def explain_group_count_word_problem(total: int, per_group: int, needs_extra_group: bool = False, explicit_remainder: bool = False) -> Optional[str]:
    ...
    if remainder == 0:
        return _methodical_simple_v11(
            f"всего — {total}, в одной группе — {per_group}",
            "сколько групп получится?",
            "Это задача на неизвестный множитель. Чтобы найти неизвестный множитель, произведение делим на известный множитель.",
            f"Решение: {total} : {per_group} = {quotient}",
            str(quotient),
            "число групп находим делением",
        )
    if needs_extra_group:
        return _methodical_simple_v11(
            f"всего — {total}, в одной группе — {per_group}",
            "сколько групп понадобится?",
            "Сначала делим и смотрим, остаются ли ещё предметы.",
            f"Решение: {total} : {per_group} = {quotient}, остаток {remainder}. Значит, нужна ещё одна группа.",
            str(quotient + 1),
            "если что-то осталось, иногда нужна ещё одна коробка",
        )
    if explicit_remainder:
        full_group_phrase = plural_form(quotient, "полная группа", "полные группы", "полных групп")
        return _methodical_simple_v11(
            f"всего — {total}, в одной группе — {per_group}",
            "сколько полных групп получится и сколько останется?",
            "По смыслу выбираем деление, потому что собираем предметы в одинаковые группы.",
            f"Решение: {total} : {per_group} = {quotient}, остаток {remainder}",
            f"{quotient} {full_group_phrase}, остаток {remainder}",
            "остаток всегда должен быть меньше размера группы",
        )
    return None
...
def explain_related_quantity_word_problem(base: int, delta: int, mode: str) -> Optional[str]:
    ...
    sign = "+" if mode == "больше" else "-"
    return _methodical_simple_v11(
        f"первое количество — {base}, второе — ? на {delta} {mode}",
        "сколько во втором количестве?",
        f"По смыслу выбираем {'сложение' if mode == 'больше' else 'вычитание'}, потому что во втором количестве на {delta} {mode}.",
        f"Решение: {base} {sign} {delta} = {result}",
        str(result),
        "сначала пойми, второе число больше или меньше первого",
    )
...
def explain_related_total_word_problem(base: int, delta: int, mode: str) -> Optional[str]:
    ...
    sign = "+" if mode == "больше" else "-"
    total = base + related
    return _methodical_compound_v11(
        f"первое количество — {base}, второе — ? на {delta} {mode}",
        "сколько всего вместе?",
        "сначала нужно узнать второе количество",
        f"{base} {sign} {delta} = {related}",
        f"{base} + {related} = {total}",
        str(total),
        "в составной задаче сначала найди недостающее число, потом отвечай на вопрос",
    )
...
def explain_sequential_change_word_problem(start: int, first_delta: int, first_mode: str, second_delta: int, second_mode: str) -> Optional[str]:
    ...
    second_sign = "+" if second_mode == "gain" else "-"
    return _methodical_compound_v11(
        f"было — {start}, потом {'+' if first_mode == 'gain' else '-'}{first_delta}, потом {'+' if second_mode == 'gain' else '-'}{second_delta}",
        "сколько стало?",
        "изменения происходят по порядку",
        f"{start} {first_sign} {first_delta} = {middle}",
        f"{middle} {second_sign} {second_delta} = {result}",
        str(result),
        "в задачах с несколькими изменениями считай по порядку",
    )
...
def explain_relation_chain_word_problem(base: int, first_delta: int, first_mode: str, second_delta: int, second_mode: str) -> Optional[str]:
    ...
    second_sign = "+" if second_mode == "больше" else "-"
    return _methodical_compound_v11(
        f"первое число — {base}, второе — ? на {first_delta} {first_mode}, третье — ? на {second_delta} {second_mode}",
        "сколько в третьем количестве?",
        "третье количество зависит от второго, а второе — от первого",
        f"{base} {first_sign} {first_delta} = {middle}",
        f"{middle} {second_sign} {second_delta} = {result}",
        str(result),
        "если одно число зависит от другого, находи их по цепочке",
    )
...
def explain_groups_plus_extra_word_problem(groups: int, per_group: int, extra: int) -> str:
    ...
    result = grouped_total + extra
    return _methodical_compound_v11(
        f"групп — {groups} по {per_group}, отдельно — {extra}",
        "сколько всего?",
        "сначала нужно узнать, сколько предметов в одинаковых группах",
        f"{groups} × {per_group} = {grouped_total}",
        f"{grouped_total} + {extra} = {result}",
        str(result),
        "сначала считай одинаковые группы, потом прибавляй отдельно стоящие предметы",
    )



def _try_price_quantity_cost_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    question = _question_tail_v11(lower)
    if not nums or not any(fragment in lower for fragment in ("руб", "стоит", "стоят", "заплат", "цена", "стоимост")):
        return None

    if len(nums) >= 3 and "на сколько" in lower and ("столько же" in lower or "столько" in lower) and any(fragment in lower for fragment in ("дороже", "дешевле")):
        qty, cost1, cost2 = nums[:3]
        if qty == 0 or cost1 % qty != 0 or cost2 % qty != 0:
            return None
        price1 = cost1 // qty
        price2 = cost2 // qty
        diff = abs(price1 - price2)
        return _methodical_compound_v11(
            f"одинаковое количество — {qty}, первая стоимость — {cost1}, вторая стоимость — {cost2}",
            "на сколько одна цена больше или меньше другой?",
            "сначала нужно узнать цену одного предмета в каждом случае",
            f"{cost1} : {qty} = {price1} и {cost2} : {qty} = {price2}",
            f"{max(price1, price2)} - {min(price1, price2)} = {diff}",
            str(diff),
            "если количество одинаковое, сначала сравни цену одного предмета",
        )

    if len(nums) >= 3 and any(fragment in lower for fragment in ("можно купить", "сколько купить", "понадобится купить")) and any(fragment in lower for fragment in ("таких", "такую", "такие", "одинаков")):
        qty, cost, money = nums[:3]
        if qty == 0 or cost % qty != 0:
            return None
        price = cost // qty
        if price == 0 or money % price != 0:
            return None
        result = money // price
        return _methodical_compound_v11(
            f"за {qty} предметов — {cost} руб., есть — {money} руб.",
            "сколько предметов можно купить?",
            "сначала нужно узнать цену одного предмета",
            f"{cost} : {qty} = {price}",
            f"{money} : {price} = {result}",
            str(result),
            "в задачах на цену сначала полезно найти цену одного предмета",
        )

    if len(nums) >= 2 and re.search(r"сколько[^.?!]*(?:стоит 1|стоит одна|стоит один|стоит одна\b|стоит один\b|цена)", lower):
        qty, cost = nums[:2]
        if qty == 0 or cost % qty != 0:
            return None
        price = cost // qty
        return _methodical_simple_v11(
            f"количество — {qty}, стоимость — {cost} руб.",
            "какова цена одного предмета?",
            "Чтобы найти цену, стоимость делим на количество.",
            f"Решение: {cost} : {qty} = {price}",
            str(price),
            "цена = стоимость : количество",
        )

    if len(nums) >= 2 and any(fragment in lower for fragment in ("сколько стоят", "сколько стоила", "сколько заплат", "какова стоимость")) and not re.search(r"сколько[^.?!]*стоит 1", lower):
        price, qty = nums[:2]
        total = price * qty
        return _methodical_simple_v11(
            f"цена — {price} руб., количество — {qty}",
            "какова стоимость всей покупки?",
            "Чтобы найти стоимость, цену умножаем на количество.",
            f"Решение: {price} × {qty} = {total}",
            str(total),
            "стоимость = цена × количество",
        )
...
def _try_unit_rate_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    question = _question_tail_v11(lower)
    if len(nums) < 3:
        return None
    if any(fragment in lower for fragment in ("скорост", "км/ч", "м/мин", "м/с", "руб", "стоит", "заплат", "/")):
        return None
    if "сколько" not in lower:
        return None
    if not re.search(r"\b(?:в|за)\s*\d+", lower):
        return None

    base_count, base_total, target = nums[:3]
    if base_count == 0 or base_total % base_count != 0:
        return None
    one_unit = base_total // base_count
    asks_group_count = any(stem in question for stem in _METHODICAL_GROUP_STEMS_V11) or any(fragment in question for fragment in ("потребуется", "понадобится", "в скольких", "сколько нужно"))

    if asks_group_count:
        if one_unit == 0 or target % one_unit != 0:
            return None
        result = target // one_unit
        return _methodical_compound_v11(
            f"в {base_count} единицах — {base_total}, нужно получить — {target}",
            "сколько единиц понадобится?",
            "сначала нужно узнать, сколько приходится на одну единицу",
            f"{base_total} : {base_count} = {one_unit}",
            f"{target} : {one_unit} = {result}",
            str(result),
            "в задачах на приведение к единице сначала находи одну единицу",
        )

    result = one_unit * target
    return _methodical_compound_v11(
        f"в {base_count} единицах — {base_total}, нужно узнать для {target} единиц",
        "сколько получится всего?",
        "сначала нужно узнать, сколько приходится на одну единицу",
        f"{base_total} : {base_count} = {one_unit}",
        f"{one_unit} × {target} = {result}",
        str(result),
        "в задачах на приведение к единице сначала находи одну единицу",
    )
...
def _try_fraction_word_problem_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    ...
    remaining_match = re.search(r"остал[аоись]*[^\d]*(\d+)", lower)
    if remaining_match and any(fragment in lower for fragment in ("израсход", "потрат", "съел", "истрати", "израсходова")) and any(fragment in question for fragment in ("сколько денег", "сколько было", "сколько у нее было", "сколько у него было")):
        remaining_value = int(remaining_match.group(1))
        remaining_fraction = denominator - numerator
        if remaining_fraction <= 0:
            return None
        if remaining_value * denominator % remaining_fraction != 0:
            return None
        whole = remaining_value * denominator // remaining_fraction
        return _methodical_compound_v11(
            f"израсходовали — {numerator}/{denominator}, осталось — {remaining_value}",
            "сколько было сначала?",
            "сначала нужно понять, какая доля осталась",
            f"После {numerator}/{denominator} осталось {remaining_fraction}/{denominator}",
            f"{remaining_value} × {denominator} : {remaining_fraction} = {whole}",
            str(whole),
            "если известна часть, сначала пойми, какая именно это доля целого",
        )
    ...
    if any(fragment in question for fragment in ("сколько осталось", "сколько осталось?")) and any(fragment in lower for fragment in ("потрат", "израсход", "съел", "использ", "употреб")):
        if value % denominator != 0:
            return None
        spent = value // denominator * numerator
        remain = value - spent
        return _methodical_compound_v11(
            f"было — {value}, израсходовали — {numerator}/{denominator}",
            "сколько осталось?",
            "сначала нужно узнать, сколько составляет израсходованная часть",
            f"{value} : {denominator} × {numerator} = {spent}",
            f"{value} - {spent} = {remain}",
            str(remain),
            "если нужно узнать остаток, сначала найди израсходованную часть",
        )

    if ("это" in lower or "составляет" in lower) and any(fragment in question for fragment in ("сколько было", "сколько всего", "какое расстояние", "сколько должен", "сколько денег")):
        if numerator == 0 or value % numerator != 0:
            return None
        whole = value // numerator * denominator
        return _methodical_simple_v11(
            f"известная часть — {value}, это {numerator}/{denominator} целого",
            "чему равно целое?",
            "Чтобы найти число по его части, число делим на числитель и умножаем на знаменатель.",
            f"Решение: {value} : {numerator} × {denominator} = {whole}",
            str(whole),
            "число по его части находим через числитель и знаменатель",
        )

    if value % denominator != 0:
        return None
    part = value // denominator * numerator
    return _methodical_simple_v11(
        f"целое — {value}, нужно найти {numerator}/{denominator}",
        "чему равна эта часть числа?",
        "Чтобы найти часть от числа, число делим на знаменатель и умножаем на числитель.",
        f"Решение: {value} : {denominator} × {numerator} = {part}",
        str(part),
        "часть от числа находим через знаменатель и числитель",
    )



def _try_motion_word_problem_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    question = _question_tail_v11(lower)
    nums = extract_ordered_numbers(lower)
    if not nums:
        return None
    if not any(fragment in lower for fragment in ("скорост", "ехал", "шел", "шёл", "прошел", "прошёл", "пробежал", "пролетел", "проплыл", "встрет")):
        return None

    if "навстречу" in lower:
        time_match = re.search(r"через\s*(\d+)", lower)
        speeds = [int(value) for value in re.findall(r"скорост[^\d]*(\d+)", lower)]
        if len(speeds) >= 2 and time_match and any(fragment in question for fragment in ("расстояние", "между")):
            time_value = int(time_match.group(1))
            speed_sum = speeds[0] + speeds[1]
            distance = speed_sum * time_value
            return _methodical_compound_v11(
                f"скорости — {speeds[0]} и {speeds[1]}, время до встречи — {time_value}",
                "каково расстояние между пунктами?",
                "сначала нужно узнать скорость сближения",
                f"{speeds[0]} + {speeds[1]} = {speed_sum}",
                f"{speed_sum} × {time_value} = {distance}",
                _distance_answer_v11(distance, lower),
                "при встречном движении сначала находи скорость сближения",
            )
        speed_match = re.search(r"скорост[^\d]*(\d+)", lower)
        delta_match = re.search(r"на\s*(\d+)\s*[^.?!]*больше", lower)
        if speed_match and delta_match and time_match and any(fragment in question for fragment in ("расстояние", "между")):
            speed1 = int(speed_match.group(1))
            delta = int(delta_match.group(1))
            speed2 = speed1 + delta
            time_value = int(time_match.group(1))
            distance = (speed1 + speed2) * time_value
            return _methodical_compound_v11(
                f"первая скорость — {speed1}, вторая — на {delta} больше, время — {time_value}",
                "каково расстояние между пунктами?",
                "сначала нужно узнать вторую скорость и скорость сближения",
                f"{speed1} + {delta} = {speed2}; {speed1} + {speed2} = {speed1 + speed2}",
                f"{speed1 + speed2} × {time_value} = {distance}",
                _distance_answer_v11(distance, lower),
                "при встречном движении сначала найди скорость сближения",
            )

    if "в противоположных направлениях" in lower and len(nums) >= 3 and any(fragment in question for fragment in ("расстояние", "друг от друга")):
        speed1, speed2, time_value = nums[:3]
        distance = (speed1 + speed2) * time_value
        return _methodical_compound_v11(
            f"скорости — {speed1} и {speed2}, время — {time_value}",
            "какое расстояние будет между ними?",
            "сначала нужно узнать скорость удаления",
            f"{speed1} + {speed2} = {speed1 + speed2}",
            f"{speed1 + speed2} × {time_value} = {distance}",
            _distance_answer_v11(distance, lower),
            "при движении в противоположных направлениях складывай скорости",
        )

    if "догон" in lower and len(nums) >= 2:
        gap, speed_diff = nums[:2]
        if speed_diff == 0 or gap % speed_diff != 0:
            return None
        time_value = gap // speed_diff
        return _methodical_simple_v11(
            f"расстояние между движущимися объектами — {gap}, скорость сближения — {speed_diff}",
            "через сколько времени один догонит другой?",
            "При движении в одном направлении время находим делением расстояния на скорость сближения.",
            f"Решение: {gap} : {speed_diff} = {time_value}",
            _time_answer_v11(time_value, lower),
            "при догонке удобно сначала найти скорость сближения",
        )

    if "скорост" in lower and len(nums) >= 2:
        if any(fragment in question for fragment in ("с какой скоростью", "какова скорость")):
            distance, time_value = nums[:2]
            if time_value == 0 or distance % time_value != 0:
                return None
            speed = distance // time_value
            return _methodical_simple_v11(
                f"расстояние — {distance}, время — {time_value}",
                "какова скорость?",
                "Чтобы найти скорость, расстояние делим на время.",
                f"Решение: {distance} : {time_value} = {speed}",
                _speed_answer_v11(speed, lower),
                "скорость = расстояние : время",
            )
        if any(fragment in question for fragment in ("какое расстояние", "каково расстояние", "сколько километров", "сколько метров")):
            speed, time_value = nums[:2]
            distance = speed * time_value
            return _methodical_simple_v11(
                f"скорость — {speed}, время — {time_value}",
                "какое расстояние прошли?",
                "Чтобы найти расстояние, скорость умножаем на время.",
                f"Решение: {speed} × {time_value} = {distance}",
                _distance_answer_v11(distance, lower),
                "расстояние = скорость × время",
            )
        if any(fragment in question for fragment in ("сколько времени", "сколько часов", "какое время")):
            distance, speed = nums[:2]
            if speed == 0 or distance % speed != 0:
                return None
            time_value = distance // speed
            return _methodical_simple_v11(
                f"расстояние — {distance}, скорость — {speed}",
                "сколько времени были в пути?",
                "Чтобы найти время, расстояние делим на скорость.",
                f"Решение: {distance} : {speed} = {time_value}",
                _time_answer_v11(time_value, lower),
...
def _try_joint_work_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    question = _question_tail_v11(lower)
    if len(nums) < 4:
        return None
    if not any(fragment in lower for fragment in ("одновременно", "оба", "вместе")):
        return None
    if not any(fragment in lower for fragment in ("насос", "бригада", "рабоч", "труба", "станок", "мастер")):
        return None
    if not any(fragment in question for fragment in ("за сколько", "сколько минут", "сколько часов", "сколько времени")):
        return None

    work_value, time1, time2, target = nums[:4]
    if time1 == 0 or time2 == 0:
        return None
    if work_value % time1 != 0 or work_value % time2 != 0:
        return None
    rate1 = work_value // time1
    rate2 = work_value // time2
    rate_sum = rate1 + rate2
    if rate_sum == 0 or target % rate_sum != 0:
        return None
    time_total = target // rate_sum
    return _methodical_compound_v11(
        f"первый выполняет {work_value} за {time1}, второй — за {time2}, вместе нужно выполнить {target}",
        "за сколько времени справятся вместе?",
        "сначала нужно узнать работу каждого за 1 единицу времени",
        f"{work_value} : {time1} = {rate1} и {work_value} : {time2} = {rate2}; вместе — {rate_sum}",
        f"{target} : {rate_sum} = {time_total}",
        _time_answer_v11(time_total, lower),
        "в задачах на совместную работу сначала находи работу за 1 единицу времени",
    )



def _try_proportional_division_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    question = _question_tail_v11(lower)
    ...
    if any(fragment in question for fragment in ("каждая школа", "каждая бригада", "каждый кусок", "каждая", "сколько должна заплатить")) or ("одинаков" in lower and any(fragment in lower for fragment in ("сколько заплат", "каждая"))):
        first_part, second_part, total = nums[:3]
        total_parts = first_part + second_part
        if total_parts == 0 or total % total_parts != 0:
            return None
        one_part = total // total_parts
        first_value = first_part * one_part
        second_value = second_part * one_part
        return _methodical_compound_v11(
            f"первая часть — {first_part}, вторая — {second_part}, всего — {total}",
            "сколько приходится на каждую часть?",
            "сначала нужно узнать, сколько приходится на одну общую долю",
            f"{first_part} + {second_part} = {total_parts}; {total} : {total_parts} = {one_part}",
            f"{one_part} × {first_part} = {first_value} и {one_part} × {second_part} = {second_value}",
            f"{first_value} и {second_value}",
            "при пропорциональном делении сначала найди одну общую долю",
        )
...
def _try_sum_of_two_products_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    if len(nums) < 4 or lower.count("по") < 2 or "сколько" not in lower:
        return None
    if any(fragment in lower for fragment in ("периметр", "площад", "скорост")):
        return None
    a, b, c, d = nums[:4]
    first_value = a * b
    second_value = c * d
    ...
    return _methodical_compound_v11(
        f"первая группа — {a} по {b}, вторая — {c} по {d}",
        "сколько всего?",
        "сначала нужно узнать результат для каждой группы отдельно",
        f"{a} × {b} = {first_value} и {c} × {d} = {second_value}",
        f"{first_value} + {second_value} = {total}",
        str(total),
        "если есть две группы «по ...», сначала найди каждую группу отдельно",
    )



def _try_unknown_part_v11(raw_text: str) -> Optional[str]:
    text = normalize_word_problem_text(raw_text)
    lower = text.lower()
    nums = extract_ordered_numbers(lower)
    question = _question_tail_v11(lower)
    if len(nums) != 2:
        return None
    if not any(fragment in lower for fragment in ("всего", "вместе", "из них", "остальные")):
        return None
    if not any(fragment in question for fragment in ("сколько", "сколько было", "сколько осталось", "сколько на")):
        return None
    total = max(nums)
    part = min(nums)
    result = total - part
    if result < 0:
        return None
    return _methodical_simple_v11(
        f"всего — {total}, известная часть — {part}",
        "сколько составляет неизвестная часть?",
        "Это задача на неизвестное слагаемое. Чтобы найти неизвестное слагаемое, из суммы вычитаем известное слагаемое.",
        f"Решение: {total} - {part} = {result}",
        str(result),
        "если известно целое и одна часть, другую часть находим вычитанием",
    )



def try_local_primary_school_methodical_explanation(raw_text: str) -> Optional[str]:
    return (
        _try_unknown_part_v11(raw_text)
        or _try_price_quantity_cost_v11(raw_text)
        or _try_proportional_division_v11(raw_text)
        or _try_joint_work_v11(raw_text)
        or _try_motion_word_problem_v11(raw_text)
        or _try_fraction_word_problem_v11(raw_text)
        or _try_unit_rate_v11(raw_text)
        or _try_sum_of_two_products_v11(raw_text)
    )
...
def try_local_geometry_explanation(raw_text: str) -> Optional[str]:
    ...
    unit = geometry_unit(lower)
    question = _question_tail_v11(lower)
    asks_perimeter = "периметр" in lower or "периметр" in question
    asks_area = "площад" in lower or "площад" in question
    asks_side = any(fragment in question for fragment in ("найди сторону", "найди длину стороны", "одной стороны", "какова сторона", "длина стороны"))
    asks_width = any(fragment in question for fragment in ("найди ширину", "какова ширина"))
    asks_length = any(fragment in question for fragment in ("найди длину", "какова длина")) and not asks_width

    if "равностороннего треугольника" in lower and "квадрат" in lower and "периметр" in lower and nums:
        triangle_side = nums[0]
        triangle_perimeter = triangle_side * 3
        if triangle_perimeter % 4 != 0:
            return None
        square_side = triangle_perimeter // 4
        return _methodical_compound_v11(
            f"сторона равностороннего треугольника — {triangle_side}",
            "какова сторона квадрата с таким же периметром?",
            "сначала нужно узнать периметр треугольника",
            f"{triangle_side} × 3 = {triangle_perimeter}",
            f"{triangle_perimeter} : 4 = {square_side}",
            with_unit(square_side, unit),
            "если периметры равны, сначала найди общий периметр",
        )

    if "прямоугольник" in lower and asks_area and "периметр" in lower and len(nums) >= 2 and not asks_width and not asks_length:
        perimeter, length = nums[0], nums[1]
        if perimeter % 2 != 0:
            return None
        half = perimeter // 2
        ...
        area = length * width
        return _methodical_compound_v11(
            f"периметр — {perimeter}, длина — {length}",
            "чему равна площадь прямоугольника?",
            "сначала нужно узнать вторую сторону",
            f"{perimeter} : 2 = {half}; {half} - {length} = {width}",
            f"{length} × {width} = {area}",
            with_unit(area, unit, square=True),
            "если известен периметр и одна сторона, сначала найди вторую сторону",
        )

    if "прямоугольник" in lower and "площад" in lower and asks_perimeter and len(nums) >= 2:
        area, length = nums[0], nums[1]
        if length == 0 or area % length != 0:
            return None
        width = area // length
        ...
        return _methodical_compound_v11(
            f"площадь — {area}, длина — {length}",
            "чему равен периметр прямоугольника?",
            "сначала нужно узнать ширину",
            f"{area} : {length} = {width}",
            f"2 × ({length} + {width}) = {perimeter}",
            with_unit(perimeter, unit),
            "если известны площадь и одна сторона, сначала находи вторую сторону",
        )
    ...
    if "квадрат" in lower and asks_perimeter and asks_side and nums:
        ...
        side = perimeter // 4
        return _methodical_simple_v11(
            f"периметр квадрата — {perimeter}",
            "какова длина одной стороны?",
            "У квадрата 4 равные стороны, поэтому периметр делим на 4.",
            f"Решение: {perimeter} : 4 = {side}",
            with_unit(side, unit),
    ...
    if "прямоугольник" in lower and asks_perimeter and asks_width and len(nums) >= 2:
        ...
        return _methodical_simple_v11(
            ...
            "Половина периметра прямоугольника равна сумме длины и ширины.",
            f"Решение: {perimeter} : 2 = {half}; {half} - {length} = {width}",
            with_unit(width, unit),
            "если известен периметр и длина, сначала дели периметр пополам",
        )
    ...
    if "прямоугольник" in lower and asks_perimeter and asks_length and len(nums) >= 2:
        ...
        if length < 0:
            return None
        return _methodical_simple_v11(
            f"периметр — {perimeter}, ширина — {width}",
            "какова длина прямоугольника?",
            "Половина периметра прямоугольника равна сумме длины и ширины.",
            f"Решение: {perimeter} : 2 = {half}; {half} - {width} = {length}",
            with_unit(length, unit),
            "если известен периметр и ширина, сначала дели периметр пополам",
        )
    ...
    if "квадрат" in lower and asks_area and asks_side and nums:
        ...
        if side * side != area:
            return None
        return _methodical_simple_v11(
            f"площадь квадрата — {area}",
            "какова длина стороны квадрата?",
            "Площадь квадрата равна произведению стороны на сторону.",
            f"Решение: {side} × {side} = {area}, значит сторона равна {side}",
            with_unit(side, unit),
            "если известна площадь квадрата, ищи число, которое при умножении само на себя даёт эту площадь",
        )
    ...
    if "прямоугольник" in lower and asks_area and asks_width and len(nums) >= 2:
        ...
        return _methodical_simple_v11(
            f"площадь — {area}, длина — {length}",
            "какова ширина прямоугольника?",
            "Чтобы найти ширину, площадь делим на длину.",
            f"Решение: {area} : {length} = {width}",
            with_unit(width, unit),
    ...
    if "прямоугольник" in lower and asks_area and asks_length and len(nums) >= 2:
        ...
        length = area // width
        return _methodical_simple_v11(
            f"площадь — {area}, ширина — {width}",
            "какова длина прямоугольника?",
            "Чтобы найти длину, площадь делим на ширину.",
            f"Решение: {area} : {width} = {length}",
            with_unit(length, unit),
    ...
    if "квадрат" in lower and asks_perimeter and nums:
        ...
        result = side * 4
        return _methodical_simple_v11(
            f"сторона квадрата — {side}",
            "чему равен периметр квадрата?",
            "У квадрата 4 равные стороны, поэтому сторону умножаем на 4.",
            f"Решение: {side} × 4 = {result}",
            with_unit(result, unit),
    ...
    if "квадрат" in lower and asks_area and nums:
        ...
        return _methodical_simple_v11(
            ...
            "Площадь квадрата равна произведению стороны на сторону.",
            f"Решение: {side} × {side} = {result}",
            with_unit(result, unit, square=True),
...
def try_local_equation_explanation(raw_text: str) -> Optional[str]:
    ...
    for pattern, kind in patterns:
        ...
        if kind in {"x_plus", "plus_x"}:
            answer = rhs_value - number
            template = "x + " + number_text if kind == "x_plus" else number_text + " + x"
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестное слагаемое. Чтобы найти неизвестное слагаемое, из суммы вычитаем известное слагаемое",
                f"x = {rhs_text} - {number_text} = {format_fraction(answer)}",
                format_equation_check(template, format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного слагаемого работай правилом суммы",
            )
        ...
        if kind == "x_minus":
            ...
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестное уменьшаемое. Чтобы найти неизвестное уменьшаемое, к разности прибавляем вычитаемое",
                f"x = {rhs_text} + {number_text} = {format_fraction(answer)}",
                format_equation_check(f"x - {number_text}", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного уменьшаемого к разности прибавляй вычитаемое",
            )
        ...
        if kind == "minus_x":
            ...
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестное вычитаемое. Чтобы найти неизвестное вычитаемое, из уменьшаемого вычитаем разность",
                f"x = {number_text} - {rhs_text} = {format_fraction(answer)}",
                format_equation_check(f"{number_text} - x", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного вычитаемого из уменьшаемого вычитай разность",
            )
        ...
        if kind in {"x_mul", "mul_x"}:
            if number == 0:
                if rhs_value == 0:
                    return join_explanation_lines(
                        "0 при умножении всегда даёт 0",
                        "Ответ: подходит любое число",
                        "Совет: произведение с нулём требует отдельной проверки",
                    )
                return join_explanation_lines(
                    "0 при умножении не может дать другой результат",
                    "Ответ: решения нет",
            ...
            answer = rhs_value / number
            template = "x × " + number_text if kind == "x_mul" else number_text + " × x"
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестный множитель. Чтобы найти неизвестный множитель, произведение делим на известный множитель",
                f"x = {rhs_text} : {number_text} = {format_fraction(answer)}",
                format_equation_check(template, format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного множителя произведение дели на известный множитель",
            )
        ...
        if kind == "x_div":
            ...
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестное делимое. Чтобы найти неизвестное делимое, частное умножаем на делитель",
                f"x = {rhs_text} × {number_text} = {format_fraction(answer)}",
                format_equation_check(f"x : {number_text}", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного делимого умножай частное на делитель",
            )
        ...
        if kind == "div_x":
            ...
            return join_explanation_lines(
                "Ищем x",
                "Это неизвестный делитель. Чтобы найти неизвестный делитель, делимое делим на частное",
                f"x = {number_text} : {rhs_text} = {format_fraction(answer)}",
                format_equation_check(f"{number_text} : x", format_fraction(answer), rhs_text),
                f"Ответ: {format_fraction(answer)}",
                "Совет: у неизвестного делителя делимое дели на частное",
            )
...
async def build_explanation(user_text: str) -> dict:
    local_explanation = (
        ...
        or try_local_fraction_explanation(user_text)
        or try_local_expression_explanation(user_text)
        or try_local_primary_school_methodical_explanation(user_text)
        or try_local_geometry_explanation(user_text)
        or try_local_compound_word_problem_explanation(user_text)
        or try_local_word_problem_explanation(user_text)
    )
    ...
    if kind == "word":
        extra_instruction = (
            "Это текстовая задача. Сначала дай короткое условие, потом вопрос, потом выбор действия по смыслу. "
            "Если задача составная, используй ход: можем ли мы сразу ответить, почему нет, сначала узнаем, потом узнаем.\n\n"
        )
    elif kind == "geometry":
        extra_instruction = (
            "Это задача по геометрии. Сначала назови правило, потом подставь числа и выполни вычисление.\n\n"
        ...
        extra_instruction = (
            "Это пример или выражение. Не сообщай готовый ответ в первой строке. "
            "Если есть письменное действие, объясняй школьный порядок по разрядам.\n\n"
        )
    elif kind == "fraction":
        extra_instruction = (
            "Это задача с дробями. Сначала скажи, одинаковые ли знаменатели или какую долю нужно найти.\n\n"
        )
    elif kind == "equation":
        extra_instruction = (
            "Это уравнение. Назови неизвестный компонент и правило его нахождения полностью.\n\n"
        )
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": SYSTEM_PROMPT_V11,
            },
            {
                "role": "user",
                "content": (
                    "Объясни решение так, чтобы ребёнок мог читать строки по порядку и учиться по ним. "
                    "Сначала объяснение, потом ответ. Структуру соблюдай строго.\n\n"
                    f"{extra_instruction}{user_text}"
                ),
            },
        ],
...
def _try_unknown_part_v11(raw_text: str) -> Optional[str]:
    ...
    if len(nums) != 2:
        return None
    if "сколько всего" in question or "сколько вместе" in question:
        return None
    if not any(fragment in lower for fragment in ("из них", "остальные", "всего было", "всего стояло", "всего прочитал", "всего прочитала", "всего было ")):
        return None
    total = max(nums)
    part = min(nums)
    result = total - part

...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://wolandvp-beep.github.io/ai-math-1-4-frontend",
    ],
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Если картинка не читается, честно напиши: НЕ УДАЛОСЬ РАСПОЗНАТЬ. "
                        "Верни только сам текст задачи на русском языке, без пояснений и markdown."
                    ),
                },
                {
                    "role": "user",
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Пиши короткими понятными фразами на русском языке. "
                        "Используй формат: Шаг 1, Шаг 2, Шаг 3, Ответ."
                    ),
                },
...
async def proxy(request: Request):
    ...
    try:
        ...
        return JSONResponse(
            status_code=504,
            content={"error": "DeepSeek не ответил за 60 секунд. Попробуйте ещё раз."},
        )
    except httpx.ConnectTimeout:
        return JSONResponse(
            status_code=504,
            content={"error": "Не удалось подключиться к DeepSeek."},
        )
    except httpx.ConnectError as exc:
        return JSONResponse(
            status_code=504,
            content={"error": f"Ошибка подключения к DeepSeek: {str(exc)}"},
        )
    except httpx.RequestError as exc:
        return JSONResponse(
            status_code=504,
            content={"error": f"Ошибка запроса к DeepSeek: {str(exc)}"},
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=502,
            content={"error": "DeepSeek вернул неожиданный формат ответа.", "details": str(result)[:1500]},
        )

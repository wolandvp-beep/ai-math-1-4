...
APP_RELEASE = 'v312.3_live_g4_text_problems'
SOLVER_VERSION = 'v312.3-live-g4-text-problems'
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312.3+: keep the real DeepSeek request for audit evidence, but rebuild
...
# --- v312.3 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_format_visible_result(steps: list[str], final_answer: str) -> str:
    ...
    for idx, step in enumerate(clean_steps, 1):
        ...
        # V312 text problems use school solution lines even when the task has
        # one arithmetic action. This keeps the action visible for frontend
        # column-method rendering, for example: 1) 720 : 6 = 120 (рублей).
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.3-text-problems-postprocess',
        'visibleResultContract': 'v312.3-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, part_label, total_label, one, two, five, part, num, den in fraction_whole_rows:
        ...
        add(text, final, [
            f'{part} · {den} : {num} = {whole} ({whole_unit}) — находим всё количество, если {part} {part_label} составляют {num}/{den}',
        ], whole, whole_unit)
    ...
    for h, m, dh, dm in time_rows:
        ...
        if minute_sum >= 60:
            steps.append(f'{minute_sum} мин = {extra_hour} ч {final_minutes:02d} мин — переводим лишние минуты в часы')
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.3 service specs expected 100, got {len(specs)}')
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.3-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.3-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.3-g4-text-problems-canonical'

...
APP_RELEASE = 'v303_live_g2_geometry'
SOLVER_VERSION = 'v303-live-g2-geometry'
...
Решай ровно одно задание. Сохраняй единицы измерения: см, дм, м, клетки, звенья.
...
В final_answer отвечай кратко с единицей: "22 см", "4 звена", "35 см", "16 клеток".
Формат JSON:
...
def _verified_v303_geometry_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    if not isinstance(structural, dict):
        return None
    source = str(structural.get('source') or '')
    if not source.startswith('local:live-v303-g2-'):
        return None
    out = dict(structural)
    out['source'] = 'deepseek-primary'

...
APP_RELEASE = 'v411_02_live_excel_numeric_regression'
SOLVER_VERSION = 'v411.02-live-excel-numeric-regression'
...
# --- V411.02 exact real-API postprocess repairs for Excel batch 1001-1100 ---
...
def _v41102_concise_batch_1001_1100_step(line: str) -> str:
    """Keep V411.02 step explanations short enough for UI-quality gates.
...
def _v41102_batch_1001_1100_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v411.02-batch-1001-1100' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v411.02-batch-1001-1100'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v411.02-batch-1001-1100-exact-postprocess'
    return out
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    ...
    return attach_release(_tag_payload({
        ...
        'visibleResultContract': 'v411.02-live-excel-numeric-regression',
...
def _v301_is_round_dividend_table_division(dividend: str | int, divisor: str | int) -> bool:
    ...
    if not re.fullmatch(r'[1-9]0+', dividend_text) or not re.fullmatch(r'[1-9]', divisor_text):
        return False
    leading_digit = int(dividend_text[0])
    divisor_n = int(divisor_text)
    return divisor_n != 0 and leading_digit % divisor_n == 0

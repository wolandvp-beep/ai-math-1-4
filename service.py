...
APP_RELEASE = 'v311_live_g4_arithmetic_actions'
SOLVER_VERSION = 'v311-live-g4-arithmetic-actions'
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V309.05: keep the real DeepSeek request for audit evidence, but never let
    # short/random model wording leak into the deterministic grade-3 information
    # section. The browser-visible payload is rebuilt from the structural parser
    # immediately after the external call completes.
...
def _v311_try_expression(original_text: str) -> dict | None:
    ...
    if ans is None:
        return None
    pretty = expr.replace('*', '·')
    return _v311_payload(original_text, source='local:live-v311-g4-expression', steps=[f'{pretty} = {ans}'], final_answer=str(ans), answer_number=str(ans))
...
def _v311_try_equation(original_text: str) -> dict | None:
    ...
    if pat:
        a, b = map(int, pat.groups()); value = b - a; step = f'x = {b} - {a} = {value}'
    pat = pat or re.match(r'^(\d+)\s*\+\s*x\s*=\s*(\d+)$', eq)
    if pat and value is None:
        a, b = map(int, pat.groups()); value = b - a; step = f'x = {b} - {a} = {value}'
    pat = re.match(r'^x\s*-\s*(\d+)\s*=\s*(\d+)$', eq)
    if pat:
        a, b = map(int, pat.groups()); value = b + a; step = f'x = {b} + {a} = {value}'
    ...
    if pat:
        a, b = map(int, pat.groups()); value = a - b; step = f'x = {a} - {b} = {value}'
    pat = re.match(r'^x\s*·\s*(\d+)\s*=\s*(\d+)$', eq)
    if pat:
        a, b = map(int, pat.groups())
        if a != 0 and b % a == 0:
            value = b // a; step = f'x = {b} : {a} = {value}'
    pat = pat or re.match(r'^(\d+)\s*·\s*x\s*=\s*(\d+)$', eq)
    if pat and value is None:
        a, b = map(int, pat.groups())
        if a != 0 and b % a == 0:
    ...
    if pat:
        a, b = map(int, pat.groups())
        if b != 0 and a % b == 0:
            value = a // b; step = f'x = {a} : {b} = {value}'
    if value is None:
        return None
    final = f'x = {value}'
    return _v311_payload(original_text, source='local:live-v311-g4-equation', steps=[step], final_answer=final, answer_number=str(value))
...
def canonicalize_v311_arithmetic_actions_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['userVisibleResultText'] = str(structural.get('result') or '')

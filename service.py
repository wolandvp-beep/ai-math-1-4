...
APP_RELEASE = 'v304_live_g2_math_information'
SOLVER_VERSION = 'v304-live-g2-math-information'
...
def _v304_try_addition_table(original_text: str) -> dict | None:
    m = re.match(r'^\s*Таблица сложения:\s*(.+?)\.\s*Какой результат у\s*(\d+)\s*\+\s*(\d+)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
...
def _v304_try_multiplication_table(original_text: str) -> dict | None:
    m = re.match(r'^\s*Таблица умножения:\s*(.+?)\.\s*Какое произведение у\s*(\d+)\s*[·xх*]\s*(\d+)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
    expr = f'{int(m.group(2))} · {int(m.group(3))}'
...
def _v304_try_schedule_lookup(original_text: str) -> dict | None:
    m = re.match(r'^\s*Расписание кружка:\s*(.+?)\.\s*Во сколько занятие в\s+([а-яё]+)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
...
def _v304_try_schedule_duration(original_text: str) -> dict | None:
    m = re.match(r'^\s*По расписанию урок начинается в\s*(\d{1,2}:\d{2})\s*и заканчивается в\s*(\d{1,2}:\d{2})\.\s*Сколько минут длится урок\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE)
    if not m:
        return None
...
def _v304_try_work_graph_end(original_text: str) -> dict | None:
    m = re.match(r'^\s*График работы:\s*(.+?)\.\s*До скольких работает\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
...
def _v304_try_work_graph_duration(original_text: str) -> dict | None:
    m = re.match(r'^\s*График работы:\s*(.+?)\.\s*Сколько часов работает\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
    target = _v304_norm(m.group(2))
...
def _v304_try_route_after(original_text: str) -> dict | None:
    m = re.match(r'^\s*Схема маршрута:\s*(.+?)\.\s*Что находится после\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
...
def _v304_try_route_between(original_text: str) -> dict | None:
    m = re.match(r'^\s*Схема маршрута:\s*(.+?)\.\s*Что находится между\s+(.+?)\s+и\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
...
def _v304_try_route_steps(original_text: str) -> dict | None:
    m = re.match(r'^\s*Схема маршрута:\s*(.+?)\.\s*Сколько переходов от\s+(.+?)\s+до\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    pts = _v304_route_points(m.group(1)); a = _v304_route_canonical_target(m.group(2)); b = _v304_route_canonical_target(m.group(3))
    ia = _v304_route_index(pts, m.group(2)); ib = _v304_route_index(pts, m.group(3))
...
def _v304_try_select_price_single(original_text: str) -> dict | None:
    m = re.match(r'^\s*Данные для выбора:\s*(.+?)\.\s*Сколько рублей стоит\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
...
def _v304_try_select_price_total(original_text: str) -> dict | None:
    m = re.match(r'^\s*Данные для выбора:\s*(.+?)\.\s*Сколько рублей стоят\s+(\d+)\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
...
def _v304_try_diagram_lookup(original_text: str) -> dict | None:
    m = re.match(r'^\s*Диаграмма:\s*(.+?)\.\s*Сколько\s+(.+?)\s+у\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))
...
def _v304_try_diagram_compare(original_text: str) -> dict | None:
    m = re.match(r'^\s*Диаграмма:\s*(.+?)\.\s*На сколько\s+.+?\s+у\s+(.+?)\s+больше, чем у\s+(.+?)\?\s*$', str(original_text or '').strip(), flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v304_split_entries(m.group(1))

...
APP_RELEASE = 'v405_02_live_excel_numeric_regression'
SOLVER_VERSION = 'v405.02-live-excel-numeric-regression'
...
def _v40204_concise_counted_dash_explanation(original_text: str, explanation: str, unit_text: str = '') -> str:
    ...
    # V405.02: allow short disambiguating object+context phrases in
...
# --- V405.02 exact real-API postprocess repairs for Excel batch 401-500 ---
...
def _v40502_batch_401_500_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v405.02-batch-401-500' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v405.02-batch-401-500'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v405.02-batch-401-500-exact-postprocess'
    return out

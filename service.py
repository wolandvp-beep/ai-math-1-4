...
APP_RELEASE = 'v296.11_live_g1_arithmetic_actions_single_step_no_number_ui'
SOLVER_VERSION = 'v296.11-live-g1-arithmetic-actions-single-step-no-number-ui'
...
def _remove_single_step_numbering(result: str) -> str:
    ...
    The numbered form is useful for two or more actions.  If the only numbered
    line itself contains a chain with two or more arithmetic operations, keep the
    marker; otherwise remove "1)" so TTS does not read "первое действие" for
    a simple one-action example like 2 + 3.
    """
    lines = [str(line or '').rstrip() for line in str(result or '').splitlines()]

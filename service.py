...
APP_RELEASE = 'v307.02_live_g3_text_problems_column_solutions'
SOLVER_VERSION = 'v307.02-live-g3-text-problems-column-solutions'
...
В final_answer отвечай фразой по вопросу с единицей, а не голым числом: «в библиотеке стало 210 книг», «заплатили 198 руб.», «пешеход прошёл 66 км», «на 6 марок больше».
Для умножения и деления в steps используй школьные знаки «×» и «:».
Формат JSON:
...
def _format_primary_solution_text(original_text: str, steps: list[str], final_answer: str) -> str:
    ...
    def _primary_step_action_count(step: str) -> int:
        first_line = str(step or '').splitlines()[0] if str(step or '').splitlines() else ''
        return _count_arithmetic_actions_in_step(first_line)
    single_action_solution = len(clean_steps) == 1 and _primary_step_action_count(clean_steps[0]) <= 1
...
def _v301_should_use_column_operation(a: str | int, op: str, b: str | int) -> bool:
    ...
    if shown == '÷':
        return a_len >= 3 or b_len >= 2
    return a_len >= 3 or b_len >= 3 or (a_len >= 2 and b_len >= 2)
...
def _v307_expr(a: int | str, operator: str, b: int | str, result: int | str) -> str:
    symbol = _v301_display_operator(str(operator or '').strip())
    if symbol == '÷':
        symbol = ':'
    return f'{a} {symbol} {b} = {result}'


def _v307_step_op(a: int | str, operator: str, b: int | str, result_number: int | str, unit: str, what_found: str) -> str:
    display_operator = _v301_display_operator(str(operator or '').strip())
    base = _v307_step(_v307_expr(a, display_operator, b, result_number), result_number, unit, what_found)
    if not _v301_should_use_column_operation(str(a), display_operator, str(b)):
        return base
    notes = _v301_column_notes(str(a), display_operator, str(b))[:6]
    if not notes:
        return base
    return base + '\n' + _v301_column_title(display_operator) + '\n' + '\n'.join(notes)
...
def _v307_payload(original_text: str, *, source: str, steps: list[str], final_answer: str, answer_number: int | str | None = None, answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'userVisibleResultText': result_text,
        'visibleResultContract': 'v307.02-multistep-units-columns-and-action-meaning',
    }
...
def _solve_v307_text_problem_prompt(original_text: str) -> dict | None:
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-two-step-add', steps=[_v307_step_op(b, '+', c, added, 'книга', 'привезли всего'), _v307_step_op(a, '+', added, ans, 'книга', 'стало в библиотеке')], final_answer=f'в библиотеке стало {_v307_count(ans, "книга")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'книга'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-two-step-subtract', steps=[_v307_step_op(b, '+', c, sent, 'коробка', 'отправили всего'), _v307_step_op(a, '-', sent, ans, 'коробка', 'осталось на складе')], final_answer=f'на складе осталось {_v307_count(ans, "коробка")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'коробка'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-equal-groups-minus', steps=[_v307_step_op(boxes, '×', per, total, 'карандаш', 'лежало в коробках'), _v307_step_op(total, '-', used, ans, 'карандаш', 'осталось')], final_answer=f'осталось {_v307_count(ans, "карандаш")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'карандаш'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-equal-sharing-plus', steps=[_v307_step_op(total, ':', packs, each, 'тетрадь', 'было в каждой пачке'), _v307_step_op(each, '+', extra, ans, 'тетрадь', 'стало в каждой пачке')], final_answer=f'в каждой пачке стало {_v307_count(ans, "тетрадь")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'тетрадь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-price-total', steps=[_v307_step_op(price, '×', qty, cost, 'руб.', 'стоили альбомы'), _v307_step_op(cost, '+', extra, ans, 'руб.', 'заплатили всего')], final_answer=f'заплатили {_v307_count(ans, "руб.")}', answer_number=ans, answer_unit='руб.')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-price-inverse', steps=[_v307_step_op(total, ':', qty1, one, 'руб.', 'стоит один мяч'), _v307_step_op(one, '×', qty2, ans, 'руб.', 'стоят такие мячи')], final_answer=f'{qty2} мячей стоят {_v307_count(ans, "руб.")}', answer_number=ans, answer_unit='руб.')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-movement-two-speeds', steps=[_v307_step_op(v1, '×', t1, d1, 'км', 'прошёл за первое время'), _v307_step_op(v2, '×', t2, d2, 'км', 'прошёл за второе время'), _v307_step_op(d1, '+', d2, ans, 'км', 'прошёл всего')], final_answer=f'пешеход прошёл {_v307_count(ans, "км")}', answer_number=ans, answer_unit='км')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-movement-same-speed', steps=[_v307_step_op(dist, ':', t1, speed, 'км/ч', 'скорость поезда'), _v307_step_op(speed, '×', t2, ans, 'км', 'проедет поезд')], final_answer=f'поезд проедет {_v307_count(ans, "км")}', answer_number=ans, answer_unit='км')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-productivity-two-periods', steps=[_v307_step_op(t1, '+', t2, hours, 'час', 'работал мастер'), _v307_step_op(rate, '×', hours, ans, 'деталь', 'сделал мастер')], final_answer=f'мастер сделал {_v307_count(ans, "деталь")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'деталь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-productivity-same-rate', steps=[_v307_step_op(total, ':', days1, rate, 'деталь', 'изготавливает бригада за день'), _v307_step_op(rate, '×', days2, ans, 'деталь', 'изготовит бригада')], final_answer=f'бригада изготовит {_v307_count(ans, "деталь")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'деталь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-table-total', steps=[_v307_step_op(a, '+', b, ans, 'книга', '')], final_answer=_v307_count(ans, 'книга'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'книга'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-table-grand-total', steps=[_v307_step_op(a, '+', b, ab, 'задача', 'решили Аня и Боря'), _v307_step_op(ab, '+', c, ans, 'задача', 'решили дети вместе')], final_answer=f'дети вместе решили {_v307_count(ans, "задача")}', answer_number=ans, answer_unit=_v307_unit_word(ans, 'задача'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-diagram-compare', steps=[_v307_step_op(b, '-', a, diff, 'марка', '')], final_answer=f'на {diff} {_v307_unit_word(diff, "марка")} больше', answer_number=diff, answer_unit=_v307_unit_word(diff, 'марка'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-extra-data-price', steps=[_v307_step_op(price, '×', qty, ans, 'руб.', '')], final_answer=_v307_count(ans, 'руб.'), answer_number=ans, answer_unit='руб.')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-reverse-cost', steps=[_v307_step_op(price, '×', qty, cost, 'руб.', 'стоили тетради'), _v307_step_op(cost, '+', left, ans, 'руб.', 'было у Димы сначала')], final_answer=f'у Димы было {_v307_count(ans, "руб.")}', answer_number=ans, answer_unit='руб.')
    ...
    if m:
        a, b, c = map(int, m.groups()); ab = a + b; ans = ab + c
        return _v307_payload(text, source='local:live-v307-g3-route-scheme', steps=[_v307_step_op(a, '+', b, ab, 'м', 'путь от дома до библиотеки'), _v307_step_op(ab, '+', c, ans, 'м', 'путь от дома до парка')], final_answer=f'от дома до парка {_v307_count(ans, "м")}', answer_number=ans, answer_unit='м')
...
def _solve_v300_numbers_quantities_prompt(original_text: str) -> dict | None:
    ...
    if m:
        ...
        return _v300_info_payload(text, source='local:live-v300-g2-difference-compare', steps=[_v307_step_op(b, '-', a, diff, 'марка', '')], final_answer=final, answer_number=str(diff))

...
APP_RELEASE = 'v401_2_live_excel_numeric_regression'
SOLVER_VERSION = 'v401.2-live-excel-numeric-regression'
...
# --- V401.2: visible solution grammar/units repair for Excel regression tasks ---
...
# V401.2: countable object quantities are shown as pieces in calculation
...
def _v4011_step_explanation(original_text: str, info: dict[str, str | bool], op: str) -> str:
    ...
    # V401.2: for counted objects, the parenthesized unit is «шт.»/«чел.»,
...
def _v4011_normalize_step_line(step: str, original_text: str, answer_number: str, answer_unit: str, info: dict[str, str | bool]) -> str:
    ...
    if existing:
        if paren_unit:
            ...
            if old_unit != desired_unit or (
                _v4012_is_counted_piece_unit(unit, info) and old_expl != desired_expl
            ):
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    out['source'] = source
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.2-visible-count-units-grammar-repair'
...
def _v4012_repair_thousand_answer_number(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.2-thousand-numeric-repair'
    return fixed

...
APP_RELEASE = 'v298_live_g1_geometry_ui_render_evidence'
SOLVER_VERSION = 'v298-live-g1-geometry-ui-render-evidence'
...
def _v298_try_row_relations(original_text: str) -> dict | None:
    ...
    if q_between:
        ...
        if left in figures and right in figures:
            ...
            if len(middle) == 1:
                answer = middle[0]
                steps = [f"Слева направо: {', '.join(figures)}", f"Между {left} и {right} находится {answer}"]
    ...
    if q_left:
        ...
        if ref in figures:
            ...
            if idx > 0:
                ...
                steps = [f"Слева направо: {', '.join(figures)}", f"Слева от {ref} находится {answer}"]
                return _v298_geometry_payload(original_text, source='local:live-v298-g1-spatial-row', steps=steps, final_answer=answer)
    ...
    if q_right:
        ...
        if ref in figures:
            ...
            if idx < len(figures) - 1:
                answer = figures[idx + 1]
                steps = [f"Слева направо: {', '.join(figures)}", f"Справа от {ref} находится {answer}"]
                return _v298_geometry_payload(original_text, source='local:live-v298-g1-spatial-row', steps=steps, final_answer=answer)
    return None
...
def _v298_try_column_relations(original_text: str) -> dict | None:
    ...
    if q_above:
        ...
        if ref in figures:
            ...
            if idx > 0:
                answer = figures[idx - 1]
                steps = [f"Сверху вниз: {', '.join(figures)}", f"Выше {ref} находится {answer}"]
    ...
    if q_below:
        ...
        if ref in figures:
            ...
            if idx < len(figures) - 1:
                answer = figures[idx + 1]
                steps = [f"Сверху вниз: {', '.join(figures)}", f"Ниже {ref} находится {answer}"]
                return _v298_geometry_payload(original_text, source='local:live-v298-g1-spatial-column', steps=steps, final_answer=answer)
    return None
...
def _v298_try_inside_outside(original_text: str) -> dict | None:
    ...
    if re.search(r'какая\s+фигура\s+внутри\s+[а-яё]+\?*$', low):
        steps = [f"Внутри {container} находится {inner}", f"Вне {container} находится {outer}"]
...
def _v298_try_shape_properties(original_text: str) -> dict | None:
    ...
    if 'две длинные и две короткие стороны' in low and 'прямоугольник' in options:
...
def _v298_try_grid_route(original_text: str) -> dict | None:
    ...
    start_match = re.search(r'клетке\s+([А-ЯA-ZЁ])\s*(\d+)\.', text)
    if not start_match:
        return None

...
APP_RELEASE = 'v401_1_live_excel_numeric_regression'
SOLVER_VERSION = 'v401.1-live-excel-numeric-regression'
...
В steps пиши короткие школьные действия или чтение нужной строки таблицы. Если строка содержит вычисление величины/предметов, после результата обязательно пиши единицу в скобках и затем тире с кратким пояснением: "125 + 138 = 263 (книг) – выдали за два дня". Единицы в скобках сокращай: кг, м, мин, руб.
...
Покажи короткое школьное решение: для каждого действия отдельная строка steps. Если строка содержит вычисление величины/предметов, после результата обязательно пиши единицу в скобках и затем тире с кратким пояснением: "35 + 12 = 47 (кг) – продали во второй день". Единицы в скобках сокращай: кг, км, км/ч, ч, мин, руб.
...
Если строка содержит вычисление величины/предметов, после результата обязательно пиши единицу в скобках и затем тире с кратким пояснением: "36 · 5 = 180 (руб.) – стоимость". Если в действии есть двузначное или более значное число, сохраняй обычную запись действия в steps; frontend покажет метод в столбик по этой строке.
...
Если решение состоит из одного действия, steps должен содержать одну строку без нумерации: «2 + 3 = 5 (предметов) – что нашли». Не пиши «1)» для одношаговых примеров. Нумерация нужна только для двух и более действий. Во всех текстовых задачах с величинами/предметами строка вычисления должна иметь вид: выражение = результат (единица) – краткое пояснение; например «5 - 2 = 3 (л) – крови». В answer_unit можно писать полную единицу, но в visible steps единица должна быть в скобках и сокращённо, где есть сокращение.
Формат JSON:
{
...
# --- V401.1: visible solution grammar/units repair for Excel regression tasks ---
...
def _v4011_split_object_context(phrase: str) -> tuple[str, str, str]:
    ...
    m = re.search(r'\s+(у|в|на|для|к|по|за)\s+(.+)$', text, flags=re.IGNORECASE)
    if not m:
        return text, '', ''
...
def _v4011_unit_from_phrase(phrase: str, fallback_unit: str = '') -> str:
    ...
    tokens = [t for t in re.findall(r'[а-яa-zё.-]+', object_part.lower().replace('ё', 'е')) if t not in {'всего'}]
    if tokens:
...
def _v4011_phrase_with_number(number: int, phrase: str, fallback_unit: str = '') -> str:
    ...
    if fixed:
        # Replace the last word-like token with its form for the answer number;
        # adjectives before it are preserved: «розовых роз», «воздушных шариков».
        return re.sub(r'([а-яa-zё.-]+)(?=[^а-яa-zё.-]*$)', fixed, object_part, flags=re.IGNORECASE)
    return object_part
...
def _v4011_question_info(original_text: str, fallback_unit: str = '') -> dict[str, str | bool]:
    ...
    low = re.sub(r'\s+', ' ', low).strip(' .?!')
    info: dict[str, str | bool] = {'unit': _v4011_norm_key(fallback_unit), 'unitPhrase': _v4011_norm_key(fallback_unit), 'tail': '', 'verb': '', 'rest': '', 'isMeasure': False}
    if not low:
        return info
    # Measurement questions: «Сколько литров крови у ребёнка?»
    m = re.search(r'скольк(?:о|их|ими)\s+(?:всего\s+)?(литр(?:а|ов)?|л|рубл(?:ь|я|ей)?|руб\.?|копе(?:йка|йки|ек)|коп\.?|килограмм(?:а|ов)?|кг|грамм(?:а|ов)?|г|километр(?:а|ов)?|км|метр(?:а|ов)?|м|сантиметр(?:а|ов)?|см|миллиметр(?:а|ов)?|мм|дециметр(?:а|ов)?|дм|минут(?:а|ы)?|мин|час(?:а|ов)?|год(?:а|ов)?|лет)\s+(.+)$', low)
    if m:
    ...
    verb_pattern = r'(сидел[а-я]*|был[а-я]*|стал[а-я]*|остал[а-я]*|раст[а-я]*|росл[а-я]*|ехал[а-я]*|пасл[а-я]*|стоял[а-я]*|получил[а-я]*|лежал[а-я]*|будет|находил[а-я]*|вышл[а-я]*|пришл[а-я]*|прочитал[а-я]*|собрал[а-я]*|купил[а-я]*|купили|решил[а-я]*|сделал[а-я]*|израсходовал[а-я]*|потратил[а-я]*|нашел[а-я]*|сорвал[а-я]*|съел[а-я]*|посадил[а-я]*|принес[а-я]*|продал[а-я]*|отдал[а-я]*|взял[а-я]*|положил[а-я]*|подарил[а-я]*|исписал[а-я]*|нарисовал[а-я]*|заготовил[а-я]*|засушил[а-я]*|вымыл[а-я]*|потребуе[а-я]*|зарабатывае[а-я]*|занимал[а-я]*|занимались|поют|играют)'
    ...
    if m:
        ...
        info.update({'unit': unit, 'unitPhrase': unit_phrase or unit, 'verb': _v4011_clean_phrase(m.group(2)), 'rest': _v4011_clean_phrase(m.group(3) or ''), 'tail': _v4011_clean_phrase(' '.join([m.group(2), m.group(3) or ''])), 'isMeasure': False})
        return info
    ...
    if m:
        ...
        unit = _v4011_unit_from_phrase(unit_phrase, fallback_unit)
        info.update({'unit': unit, 'unitPhrase': unit_phrase, 'tail': unit_phrase, 'isMeasure': False})
    return info
...
def _v4011_infer_operation(original_text: str, answer_number: str = '') -> tuple[str, int, int, int] | None:
    ...
    if len(nums) > 2 and re.search(r'из\s+\d+', low) and any(marker in low for marker in ('меньше', 'больше')):
        ...
        a, b = nums[0], nums[1]
    if re.search(r'на\s+\d+\s+[а-яa-z\.]*\s*(?:меньше|короче)', low) or any(word in low for word in ('осталось', 'остался', 'осталась', 'отдали', 'отдал', 'отдала', 'ушли', 'ушло', 'улетели', 'съели', 'убрали', 'продали', 'потратил', 'потратила')):
        result = a - b
        op = '-'
    elif (
        re.search(r'на\s+\d+\s+[а-яa-z\.]*\s*(?:больше|дольше)', low)
...
def _v4011_build_final_answer(original_text: str, number: int, info: dict[str, str | bool], current_answer: str = '') -> str:
    ...
    if bool(info.get('isMeasure')) and unit:
        unit_word = _v4011_plural(number, unit)
        ...
        if tail:
            if unit in {'год', 'года', 'лет'} and re.fullmatch(r'[а-яa-zё.-]+', tail, flags=re.IGNORECASE):
                return f'{_v4011_capitalize_sentence(tail)} {number} {unit_word}'.strip()
            return f'{number} {unit_word} {tail}'.strip()
    ...
    if current and not _v4011_answer_is_low_confidence(current) and not re.fullmatch(r'-?\d+(?:[.,/]\d+)?(?:\s+[а-яa-zё.-]+)?', current, flags=re.IGNORECASE):
        return _v4011_fix_answer_grammar(current)
...
def _v4011_fix_answer_grammar(answer: str) -> str:
...
def _v4011_normalize_step_line(step: str, original_text: str, answer_number: str, answer_unit: str, info: dict[str, str | bool]) -> str:
    ...
    paren_unit = _v4011_abbrev(unit) if unit else ''
    if not paren_unit:
        paren_unit = _v4011_abbrev(answer_unit)
    m = re.search(r'(?P<expr>\b-?\d+\s*(?:[+\-−·×xх*/:÷])\s*-?\d+\s*=\s*(?P<res>-?\d+)(?:\s+[а-яa-zё.²³]+)?)(?P<tail>.*)$', clean, flags=re.IGNORECASE)
    if not m:
        return _v4011_fix_answer_grammar(clean)
    result = m.group('res')
    ...
    if re.search(r'=\s*-?\d+\s*\([^)]+\)\s*[—–-]\s*\S', clean):
        return _v4011_fix_answer_grammar(clean)
    if paren_unit:
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    original_unit = ''
    m_original_unit = re.search(r'(?<!\d)-?\d+\s*(л|кг|г|км|м|дм|см|мм|руб\.?|р\.?|коп\.?|мин|ч|час(?:а|ов)?|дн(?:я|ей)?|день|раз(?:а)?|лет|год(?:а|ов)?|вид(?:а|ов)?|лист(?:а|ов)?|гриб(?:а|ов)?|мальчик(?:а|ов)?)(?=\s|[.,;:!?)]|$)', str(original_text or ''), flags=re.IGNORECASE)
    if m_original_unit:
    ...
    final_answer = _v4011_fix_answer_grammar(final_answer)
    result = _format_primary_solution_text(original_text, [step], final_answer)
    out = dict(payload or {})
    out['result'] = result
    out['validated'] = True
    source = str(out.get('source') or 'deepseek-primary')
    if source.startswith('guard-low-confidence'):
        source = 'local:live-v4011-simple-word-repair'
    out['source'] = source
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.1-visible-units-grammar-repair'
...
def _v4011_repair_payload(payload: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    fixed_final = _v4011_fix_answer_grammar(final_answer)
    number_int = _v4011_int_number(answer_number)
    if number_int is not None and bool(info.get('isMeasure')):
    ...
    if result_text:
        ...
        fixed_answer = _v4011_fix_answer_grammar(answer_line)

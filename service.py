...
APP_RELEASE = 'v504_02_automation_pipeline'
SOLVER_VERSION = 'v504-02-automation-pipeline'
...
def _v501_safe_text(value: Any, limit: int = 6000) -> str:
    text = str(value or '')
    if len(text) <= limit:
        return text
...
def _v501_sha(value: Any, length: int = 24) -> str:
    try:
        raw = value if isinstance(value, (bytes, bytearray)) else json.dumps(value, ensure_ascii=False, sort_keys=True, default=str).encode('utf-8')
    except Exception:
...
def _v501_strip_pipeline_evidence(value: Any, depth: int = 0) -> Any:
    if depth > 6:
        return _v501_safe_text(value, 500)
    if isinstance(value, dict):
        return {k: _v501_strip_pipeline_evidence(v, depth + 1) for k, v in value.items() if k not in {'v501AiPipelineEvidence'}}
    if isinstance(value, list):
        return [_v501_strip_pipeline_evidence(v, depth + 1) for v in value[:50]]
    if isinstance(value, tuple):
        return tuple(_v501_strip_pipeline_evidence(v, depth + 1) for v in value[:50])
    return value

def _v501_compact_payload_snapshot(payload: Any) -> dict[str, Any]:
    if not isinstance(payload, dict):
        return {'type': type(payload).__name__, 'preview': _v501_safe_text(payload, 1200), 'hash': _v501_sha(payload)}
    structured = payload.get('structured_solution') if isinstance(payload.get('structured_solution'), dict) else payload.get('structuredSolution')
    answer_number = str(payload.get('answer_number') or payload.get('answerNumber') or '').strip()
    answer_unit = str(payload.get('answer_unit') or payload.get('answerUnit') or '').strip()
    final_answer = str(payload.get('final_answer') or payload.get('answer') or '').strip()
    if isinstance(structured, dict):
        answer_number = answer_number or str(structured.get('answer_number') or structured.get('answerNumber') or '').strip()
        answer_unit = answer_unit or str(structured.get('answer_unit') or structured.get('answerUnit') or '').strip()
        final_answer = final_answer or str(structured.get('final_answer') or structured.get('finalAnswer') or '').strip()
    result = str(payload.get('result') or payload.get('userVisibleResultText') or payload.get('explanation') or '')
    return {
        'source': payload.get('source'),
        'verifier': payload.get('verifier'),
        'validated': payload.get('validated'),
        'resultHash': _v501_sha(result),
        'resultPreview': _v501_safe_text(result, 2000),
        'answerLine': _extract_answer_line(result),
        'answerNumber': answer_number,
        'answerUnit': answer_unit,
        'finalAnswer': final_answer,
        'structuredSolution': _v501_strip_pipeline_evidence(structured) if isinstance(structured, dict) else None,
    }


def _v501_attach_ai_pipeline_evidence(payload: dict | None, *, original_text: str, raw_deepseek_text: str = '', raw_deepseek_proof: dict[str, Any] | None = None, raw_parsed_json: dict[str, Any] | None = None, stage: str = '', structural_override: str = '', retry_raw_deepseek_text: str = '', previous_evidence: dict[str, Any] | None = None) -> dict | None:
    if not isinstance(payload, dict):
        return payload
    ...
    evidence.update({
        ...
        'rawDeepSeekTextHash': _v501_sha(raw_deepseek_text),
        'rawDeepSeekTextLength': len(str(raw_deepseek_text or '')),
        'retryRawDeepSeekText': _v501_safe_text(retry_raw_deepseek_text, 6000) if retry_raw_deepseek_text else '',
        'retryRawDeepSeekTextHash': _v501_sha(retry_raw_deepseek_text) if retry_raw_deepseek_text else '',
        'rawDeepSeekParsedJson': raw_parsed_json if isinstance(raw_parsed_json, dict) else None,
        'rawDeepSeekParsedJsonHash': _v501_sha(raw_parsed_json) if isinstance(raw_parsed_json, dict) else '',
        'rawDeepSeekAnswerNumber': str((raw_parsed_json or {}).get('answer_number') or '').strip() if isinstance(raw_parsed_json, dict) else '',
        'rawDeepSeekFinalAnswer': str((raw_parsed_json or {}).get('final_answer') or '').strip() if isinstance(raw_parsed_json, dict) else '',
        'rawDeepSeekSteps': (raw_parsed_json or {}).get('steps') if isinstance(raw_parsed_json, dict) and isinstance((raw_parsed_json or {}).get('steps'), list) else [],
        'deepseekModel': (raw_deepseek_proof or {}).get('model') if isinstance(raw_deepseek_proof, dict) else None,
        'deepseekObjectId': (raw_deepseek_proof or {}).get('id') if isinstance(raw_deepseek_proof, dict) else None,
        'deepseekRequestHash': (raw_deepseek_proof or {}).get('requestHash') if isinstance(raw_deepseek_proof, dict) else None,
        'deepseekResponseHash': (raw_deepseek_proof or {}).get('responseHash') if isinstance(raw_deepseek_proof, dict) else None,
        'deepseekFinishReason': (raw_deepseek_proof or {}).get('finishReason') if isinstance(raw_deepseek_proof, dict) else None,
        'stageBeforePostprocess': stage,
        'structuralOverrideBeforePostprocess': structural_override,
        'parsedStructuredSolution': raw_parsed_json if isinstance(raw_parsed_json, dict) else None,
        'postprocessBefore': evidence.get('postprocessBefore'),
        'postprocessAfter': evidence.get('postprocessAfter'),
    ...
    payload['v501AiPipelineEvidence'] = evidence
    return payload


def _v501_finish_postprocess_evidence(payload: dict | None, *, before: dict[str, Any], after: dict[str, Any], replacements: list[dict[str, Any]]) -> dict | None:
    if not isinstance(payload, dict):
        return payload
    evidence = dict(payload.get('v501AiPipelineEvidence') or {}) if isinstance(payload.get('v501AiPipelineEvidence'), dict) else {}
    after_structured = after.get('structuredSolution') if isinstance(after, dict) else None
    if not isinstance(after_structured, dict):
        after_structured = _v4011_structured(payload)
    # V502: flatten the API/template arbitration fields into pipeline evidence.
    # In V501.02 they existed inside structuredSolution but summary counters stayed zero,
    # which made it hard to see whether API was actually used/protected.
    arbitration_map = {
        'apiAnswerConsidered': ('v501ApiAnswerConsidered', 'apiAnswerConsidered'),
        'apiCandidateTrusted': ('v501ApiCandidateTrusted', 'apiCandidateTrusted'),
        'apiAnswerUsedAsPrimary': ('v501ApiAnswerUsedAsPrimary', 'apiAnswerUsedAsPrimary'),
        'apiAnswerDecision': ('v501ApiAnswerDecision', 'apiAnswerDecision'),
        'apiTemplateConflict': ('v501ApiTemplateConflict', 'apiTemplateConflict'),
        'templateOverrodeTrustedApi': ('v501TemplateOverrodeTrustedApi', 'templateOverrodeTrustedApi'),
        'apiProtectedFromTemplateOverride': ('v501ApiProtectedFromTemplateOverride', 'apiProtectedFromTemplateOverride'),
        'apiAnswerCandidate': ('v501ApiAnswerCandidate', 'apiAnswerCandidate'),
    ...
    for out_key, candidates in arbitration_map.items():
        for key in candidates:
            if key in payload:
                evidence[out_key] = payload.get(key)
                break
    ...
    evidence.update({
        ...
        'postprocessVerifierAfter': after.get('verifier'),
        'postprocessSourceAfter': after.get('source'),
    })
    payload['v501AiPipelineEvidence'] = evidence
    return payload

def _postprocess_deepseek_primary_payload(payload: dict, original_text: str) -> dict:
    postprocess_before = _v501_compact_payload_snapshot(payload)
    postprocess_replacements: list[dict[str, Any]] = []
    incoming_evidence = payload.get('v501AiPipelineEvidence') if isinstance(payload, dict) else None
    if _looks_like_v314_information_prompt(original_text):
        ...
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v314_information_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
            _v501_attach_ai_pipeline_evidence(payload, original_text=original_text, previous_evidence=incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='v314_information_structural_override')
    if _looks_like_v313_geometry_prompt(original_text):
        structural = _verified_v313_geometry_payload(original_text, {})
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v313_geometry_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
            _v501_attach_ai_pipeline_evidence(payload, original_text=original_text, previous_evidence=incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='v313_geometry_structural_override')
    if _looks_like_v312_text_problems_prompt(original_text):
        structural = _verified_v312_text_problems_payload(original_text, {})
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v312_text_problems_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
            _v501_attach_ai_pipeline_evidence(payload, original_text=original_text, previous_evidence=incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='v312_text_problems_structural_override')
    if _looks_like_v311_arithmetic_actions_prompt(original_text):
        ...
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v311_arithmetic_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
            _v501_attach_ai_pipeline_evidence(payload, original_text=original_text, previous_evidence=incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='v311_arithmetic_structural_override')
    ...
    if _looks_like_v310_numbers_quantities_prompt(original_text):
        ...
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v310_numbers_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
            _v501_attach_ai_pipeline_evidence(payload, original_text=original_text, previous_evidence=incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='v310_numbers_structural_override')
    if (not _looks_like_v314_information_prompt(original_text)) and _looks_like_v309_math_information_prompt(original_text):
        ...
        if isinstance(structural, dict) and structural.get('result'):
            postprocess_replacements.append({'stage': 'v309_information_structural_override', 'before': _v501_compact_payload_snapshot(payload), 'after': _v501_compact_payload_snapshot(structural)})
            payload = structural
    ...
    if cleaned_before_clean_result.get('resultHash') != cleaned_after_clean_result.get('resultHash') or cleaned_before_clean_result.get('answerNumber') != cleaned_after_clean_result.get('answerNumber'):
        postprocess_replacements.append({'stage': 'clean_result_payload', 'before': cleaned_before_clean_result, 'after': cleaned_after_clean_result})
    if repaired_before.get('resultHash') != repaired_after.get('resultHash') or repaired_before.get('answerNumber') != repaired_after.get('answerNumber'):
        postprocess_replacements.append({'stage': 'v4011_repair_payload', 'before': repaired_before, 'after': repaired_after})
    result = _normalize_deepseek_result_text(str(cleaned.get('result') or '').strip())
    if str(cleaned.get('result') or '') != result:
        before_norm = _v501_compact_payload_snapshot(cleaned)
    ...
    if not result or 'Ответ:' not in result:
        ...
        _v501_attach_ai_pipeline_evidence(bad, original_text=original_text, previous_evidence=cleaned.get('v501AiPipelineEvidence') if isinstance(cleaned.get('v501AiPipelineEvidence'), dict) else incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='low_confidence_invalid_format')
        _v501_finish_postprocess_evidence(bad, before=postprocess_before, after=_v501_compact_payload_snapshot(bad), replacements=postprocess_replacements)
        return bad
    if any(marker.lower() in result.lower() for marker in _BAD_INTERNAL_MARKERS):
        bad = attach_release(_tag_payload(_low_confidence_payload(original_text), source='deepseek-primary-forbidden-marker', solverMode=SOLVER_MODE_DEEPSEEK_PRIMARY))
        _v501_attach_ai_pipeline_evidence(bad, original_text=original_text, previous_evidence=cleaned.get('v501AiPipelineEvidence') if isinstance(cleaned.get('v501AiPipelineEvidence'), dict) else incoming_evidence if isinstance(incoming_evidence, dict) else None, structural_override='low_confidence_forbidden_marker')
        _v501_finish_postprocess_evidence(bad, before=postprocess_before, after=_v501_compact_payload_snapshot(bad), replacements=postprocess_replacements)
        return bad
    out = attach_release(_tag_payload(cleaned, source=source or 'deepseek-primary', solverMode=SOLVER_MODE_DEEPSEEK_PRIMARY, verifier='local-postprocess'))
    _v501_finish_postprocess_evidence(out, before=postprocess_before, after=_v501_compact_payload_snapshot(out), replacements=postprocess_replacements)
    return out
...
_V4011_UNIT_FORMS: dict[str, tuple[str, str, str]] = {
    ...
    'р.': ('рубль', 'рубля', 'рублей'),
    'тысяч лет': ('тысяча лет', 'тысячи лет', 'тысяч лет'),
    'тысячи лет': ('тысяча лет', 'тысячи лет', 'тысяч лет'),
    'тысяча лет': ('тысяча лет', 'тысячи лет', 'тысяч лет'),
    'тыс. лет': ('тысяча лет', 'тысячи лет', 'тысяч лет'),
    'тыс лет': ('тысяча лет', 'тысячи лет', 'тысяч лет'),
...
def _v40111_apply_exact_user_requested_regression_solution(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    return _v500_mark_case_specific_repair(out, 'v401.12-exact-user-requested-full-answer-repair')
...
def _v4011_build_final_answer(original_text: str, number: int, info: dict[str, str | bool], current_answer: str = '') -> str:
    ...
    # V402.02 targeted full-answer wording for recurring offset=0 rows.
...
def _v4011_fix_answer_grammar(answer: str, original_text: str = '') -> str:
    ...
    if not text:
        return text
    text = re.sub(r'\bУ\s+Не[ёе]\b', 'у неё', text)
    text = re.sub(r'\bНе[ёе]\b', 'неё', text)
    ...
    m = re.match(r'^([А-ЯЁа-яё-]+)\s+(списать|решить|прочитать|вклеить|отгадать|посетить|повесить|полить)\s+осталось\s+(-?\d+)\s+(.+)$', text, flags=re.IGNORECASE)
    if m:
        text = f'{m.group(1)} осталось {m.group(2)} {m.group(3)} {m.group(4)}'
...
# --- V411.03 exact real-API postprocess repairs for Excel batch 1001-1100 ---
_V41101_BATCH_1001_1100_SPECS = [
    ('в двух неделях 10 рабочих дней. сколько рабочих дней в 4 неделях?', ['10 : 2 = 5 (д.) – рабочих дней в одной неделе', '5 · 4 = 20 (д.) – рабочих дней'], 'в 4 неделях 20 рабочих дней', '20', 'дней', 1001),
    ('на 3 костюма идет 9 м ткани. сколько метров ткани пойдет на 6 костюмов?', ['9 : 3 = 3 (м) – ткани на один костюм', '3 · 6 = 18 (м) – ткани'], 'на 6 костюмов пойдет 18 м ткани', '18', 'м', 1002),
    ('на 3 джемпера идет 12 мотков шерсти. сколько джемперов можно связать из 16 мотков шерсти?', ['12 : 3 = 4 (шт.) – мотков на один джемпер', '16 : 4 = 4 (шт.) – джемперов'], 'из 16 мотков шерсти можно связать 4 джемпера', '4', 'джемпера', 1003),
    ('на 4 одинаковых платьях 20 пуговиц. сколько пуговиц на 2 платьях?', ['20 : 4 = 5 (шт.) – пуговиц на одном платье', '5 · 2 = 10 (шт.) – пуговиц'], 'на 2 платьях 10 пуговиц', '10', 'пуговиц', 1004),
    ('из 40 деталей конструктора можно составить 4 одинаковые машины. сколько машин можно сделать из 60 деталей?', ['40 : 4 = 10 (шт.) – деталей на одну машину', '60 : 10 = 6 (шт.) – машин'], 'из 60 деталей можно сделать 6 машин', '6', 'машин', 1005),
    ('в 7 одинаковых домах 35 окон. сколько окон в 3 домах?', ['35 : 7 = 5 (шт.) – окон в одном доме', '5 · 3 = 15 (шт.) – окон'], 'в 3 домах 15 окон', '15', 'окон', 1006),
    ('разложили поровну 20 цветов в 4 букета. сколько получится одинаковых букетов из 30 цветов?', ['20 : 4 = 5 (шт.) – цветов в одном букете', '30 : 5 = 6 (шт.) – букетов'], 'из 30 цветов получится 6 одинаковых букетов', '6', 'букетов', 1007),
    ('разлили 18 л морса в 6 банок поровну. сколько надо таких банок для 24 л морса?', ['18 : 6 = 3 (л) – морса в одной банке', '24 : 3 = 8 (шт.) – банок'], 'для 24 л морса надо 8 таких банок', '8', 'банок', 1008),
    ('в 4 тетрадях 48 листов. сколько листов в 7 таких же тетрадях?', ['48 : 4 = 12 (шт.) – листов в одной тетради', '12 · 7 = 84 (шт.) – листов'], 'в 7 таких же тетрадях 84 листа', '84', 'листа', 1009),
    ('в 3 ведрах 24 л молока. сколько молока в 9 таких же ведрах?', ['24 : 3 = 8 (л) – молока в одном ведре', '8 · 9 = 72 (л) – молока'], 'в 9 таких же ведрах 72 л молока', '72', 'л', 1010),
    ('разложили для засолки 12 кг помидоров в 3 банки. сколько нужно таких банок, чтобы засолить 32 кг помидоров?', ['12 : 3 = 4 (кг) – помидоров в одной банке', '32 : 4 = 8 (шт.) – банок'], 'чтобы засолить 32 кг помидоров, нужно 8 таких банок', '8', 'банок', 1011),
    ('для 6 гирлянд надо 48 фонариков. сколько потребуется фонариков для 11 гирлянд?', ['48 : 6 = 8 (шт.) – фонариков на одну гирлянду', '8 · 11 = 88 (шт.) – фонариков'], 'для 11 гирлянд потребуется 88 фонариков', '88', 'фонариков', 1012),
    ('на 3 улицах 39 домов. сколько домов на 5 таких же улицах?', ['39 : 3 = 13 (шт.) – домов на одной улице', '13 · 5 = 65 (шт.) – домов'], 'на 5 таких же улицах 65 домов', '65', 'домов', 1013),
    ('в кинотеатре в 3 рядах 51 место. сколько мест в 5 рядах?', ['51 : 3 = 17 (шт.) – мест в одном ряду', '17 · 5 = 85 (шт.) – мест'], 'в 5 рядах 85 мест', '85', 'мест', 1014),
    ('с 8 овец настригли 48 кг шерсти, с каждой поровну. сколько овец нужно остричь, чтобы получить 30 кг шерсти?', ['48 : 8 = 6 (кг) – шерсти с одной овцы', '30 : 6 = 5 (шт.) – овец'], 'чтобы получить 30 кг шерсти, нужно остричь 5 овец', '5', 'овец', 1015),
    ('в 6 одинаковых домах 24 подъезда. сколько подъездов в 12 таких же домах?', ['24 : 6 = 4 (шт.) – подъездов в одном доме', '4 · 12 = 48 (шт.) – подъездов'], 'в 12 таких же домах 48 подъездов', '48', 'подъездов', 1016),
    ('от 4 коров получили за день 40 л молока, от каждой поровну. сколько коров при том же надое дадут 90 л молока?', ['40 : 4 = 10 (л) – молока от одной коровы', '90 : 10 = 9 (шт.) – коров'], 'при том же надое 90 л молока дадут 9 коров', '9', 'коров', 1017),
    ('на 2 этажах 36 окон. сколько окон на 3 этажах?', ['36 : 2 = 18 (шт.) – окон на одном этаже', '18 · 3 = 54 (шт.) – окон'], 'на 3 этажах 54 окна', '54', 'окна', 1018),
    ('в 6 коробках лежало 78 кг печенья. сколько потребуется коробок для 39 кг печенья при той же фасовке?', ['78 : 6 = 13 (кг) – печенья в одной коробке', '39 : 13 = 3 (шт.) – коробок'], 'для 39 кг печенья потребуется 3 коробки', '3', 'коробки', 1019),
    ('шагающий экскаватор за 2 смены вынимает 1200 ковшей грунта. сколько ковшей грунта будет вынуто за 3 смены?', ['1200 : 2 = 600 (шт.) – ковшей за одну смену', '600 · 3 = 1800 (шт.) – ковшей грунта'], 'за 3 смены будет вынуто 1800 ковшей грунта', '1800', 'ковшей', 1020),
    ('одна из обувных фабрик москвы выпускает за 3 минуты 75 пар обуви. сколько пар обуви она выпустит за 5 минут?', ['75 : 3 = 25 (шт.) – пар обуви за одну минуту', '25 · 5 = 125 (шт.) – пар обуви'], 'за 5 минут фабрика выпустит 125 пар обуви', '125', 'пар', 1021),
    ('18 редисок связали в пучки, получилось 3 пучка. сколько получится таких пучков из 54 редисок?', ['18 : 3 = 6 (шт.) – редисок в одном пучке', '54 : 6 = 9 (шт.) – пучков'], 'из 54 редисок получится 9 таких пучков', '9', 'пучков', 1022),
    ('в 7 одинаковых коробках 21 кг винограда. сколько килограммов винограда в 4 таких же коробках?', ['21 : 7 = 3 (кг) – винограда в одной коробке', '3 · 4 = 12 (кг) – винограда'], 'в 4 таких же коробках 12 кг винограда', '12', 'кг', 1023),
    ('у хозяйки было 9 куриных яиц, которые весили вместе 675 г. сколько весят вместе 6 таких яиц?', ['675 : 9 = 75 (г) – весит одно яйцо', '75 · 6 = 450 (г) – весят яйца'], '6 таких яиц весят вместе 450 г', '450', 'г', 1024),
    ('за 3 дня хозяйка израсходовала 165 г масла. сколько масла потребуется из того же расчета на 10 дней?', ['165 : 3 = 55 (г) – масла на один день', '55 · 10 = 550 (г) – масла'], 'на 10 дней потребуется 550 г масла', '550', 'г', 1025),
    ('8 ящиков вермишели весят вместе 128 кг. сколько таких ящиков получил магазин, если вместе они весят 80 кг?', ['128 : 8 = 16 (кг) – весит один ящик', '80 : 16 = 5 (шт.) – ящиков'], 'магазин получил 5 таких ящиков', '5', 'ящиков', 1026),
    ('балка длиной 5 м весит 175 кг. сколько весит такая же балка длиной 7 м?', ['175 : 5 = 35 (кг) – весит 1 м балки', '35 · 7 = 245 (кг) – вес балки'], 'балка длиной 7 м весит 245 кг', '245', 'кг', 1027),
    ('на 9 парниковых рам требуется 144 стекла. сколько можно застеклить рам, имея 96 стекол?', ['144 : 9 = 16 (шт.) – стекол на одну раму', '96 : 16 = 6 (шт.) – рам'], 'имея 96 стекол, можно застеклить 6 рам', '6', 'рам', 1028),
    ('чтобы заквасить 360 кг капусты, потребовалось 4 бочонка. сколько потребуется таких же бочонков, чтобы заквасить 540 кг капусты?', ['360 : 4 = 90 (кг) – капусты в одном бочонке', '540 : 90 = 6 (шт.) – бочонков'], 'чтобы заквасить 540 кг капусты, потребуется 6 бочонков', '6', 'бочонков', 1029),
    ('за 8 дней тракторист израсходовал 480 кг горючего. сколько всего дней работал тракторист, если за все время работы израсходовал 600 кг горючего?', ['480 : 8 = 60 (кг) – горючего за один день', '600 : 60 = 10 (д.) – дней работы'], 'тракторист работал всего 10 дней', '10', 'дней', 1030),
    ('на 3 грузовиках привезли 180 мешков муки. сколько потребуется грузовиков, чтобы доставить 420 таких же мешков муки?', ['180 : 3 = 60 (шт.) – мешков на одном грузовике', '420 : 60 = 7 (шт.) – грузовиков'], 'чтобы доставить 420 мешков муки, потребуется 7 грузовиков', '7', 'грузовиков', 1031),
    ('на зиму для 6 коров требуется 360 ц силоса. сколько коров у фермера, если запасли для коров из такого расчета 180 ц силоса?', ['360 : 6 = 60 (ц) – силоса на одну корову', '180 : 60 = 3 (шт.) – коров'], 'у фермера 3 коровы', '3', 'коровы', 1032),
    ('для детского сада купили 12 коробок цветных карандашей. всего в этих коробках было 72 карандаша. сколько карандашей в 4 коробках?', ['72 : 12 = 6 (шт.) – карандашей в одной коробке', '6 · 4 = 24 (шт.) – карандашей'], 'в 4 коробках 24 карандаша', '24', 'карандаша', 1033),
    ('за 6 часов прядильщица выработала 54 кг пряжи. сколько пряжи вырабатывает она за весь 8-часовой рабочий день?', ['54 : 6 = 9 (кг) – пряжи за один час', '9 · 8 = 72 (кг) – пряжи'], 'за 8-часовой рабочий день она вырабатывает 72 кг пряжи', '72', 'кг', 1034),
    ('через один кран за 3 мин натекло 36 ведер воды. сколько ведер воды натечет через такой кран за 5 мин?', ['36 : 3 = 12 (шт.) – ведер за одну минуту', '12 · 5 = 60 (шт.) – ведер воды'], 'за 5 минут через такой кран натечет 60 ведер воды', '60', 'ведер', 1035),
    ('на 4 страницы альбома наклеили поровну 60 почтовых марок. сколько таких марок можно поместить на 6 страницах альбома?', ['60 : 4 = 15 (шт.) – марок на одной странице', '15 · 6 = 90 (шт.) – марок'], 'на 6 страницах альбома можно поместить 90 марок', '90', 'марок', 1036),
    ('2 одинаковых ящика с консервами весят вместе 140 кг. бочка с капустой на 50 кг тяжелее ящика с консервами. сколько весят вместе 4 бочки с капустой?', ['140 : 2 = 70 (кг) – весит один ящик', '70 + 50 = 120 (кг) – весит одна бочка', '120 · 4 = 480 (кг) – весят бочки'], '4 бочки с капустой весят вместе 480 кг', '480', 'кг', 1037),
    ('в фермерском хозяйстве 6 свиней и 8 коров. всем свиньям отпускают ежедневно 84 кг разных кормов. для коровы требуется в 2 раза больше кормов, чем для свиньи. сколько кормов получают ежедневно все коровы в этом хозяйстве?', ['84 : 6 = 14 (кг) – кормов для одной свиньи', '14 · 2 = 28 (кг) – кормов для одной коровы', '28 · 8 = 224 (кг) – кормов для коров'], 'все коровы ежедневно получают 224 кг кормов', '224', 'кг', 1038),
    ('в июне корова давала за 5 дней 80 л молока, а в июле ее дневной удой увеличился на 5 л. сколько молока давала корова в июле за один день?', ['80 : 5 = 16 (л) – молока за один день в июне', '16 + 5 = 21 (л) – молока за один день в июле'], 'в июле корова давала 21 л молока за один день', '21', 'л', 1039),
    ('в 5 клетках по 2 канарейки. подарили 4 птицы. сколько канареек осталось?', ['5 · 2 = 10 (шт.) – канареек было', '10 - 4 = 6 (шт.) – канареек осталось'], 'осталось 6 канареек', '6', 'канареек', 1040),
    ('в магазин привезли 50 кг пастилы. в 6 пакетов расфасовали по 2 кг пастилы. сколько килограммов пастилы осталось расфасовать?', ['6 · 2 = 12 (кг) – расфасовали пастилы', '50 - 12 = 38 (кг) – осталось расфасовать'], 'осталось расфасовать 38 кг пастилы', '38', 'кг', 1041),
    ('для детского сада, в котором 6 групп, купили кровати. если в каждую группу поставить по 6 кроватей, то останется 8 кроватей. сколько кроватей купили?', ['6 · 6 = 36 (шт.) – кроватей поставят в группы', '36 + 8 = 44 (шт.) – кроватей купили'], 'для детского сада купили 44 кровати', '44', 'кровати', 1042),
    ('в магазин привезли фломастеры. после того как продали 6 пачек по 10 фломастеров, в магазине осталось 40 фломастеров. сколько фломастеров привезли в магазин?', ['6 · 10 = 60 (шт.) – фломастеров продали', '60 + 40 = 100 (шт.) – фломастеров привезли'], 'в магазин привезли 100 фломастеров', '100', 'фломастеров', 1043),
    ('надо отремонтировать 24 комнаты. после того как несколько комнат отремонтировали, осталось отремонтировать 6 трехкомнатных квартир. сколько комнат отремонтировали?', ['6 · 3 = 18 (шт.) – комнат осталось отремонтировать', '24 - 18 = 6 (шт.) – комнат отремонтировали'], 'отремонтировали 6 комнат', '6', 'комнат', 1044),
    ('на уборке урожая работали 2 бригады по 8 человек. после обеда ушли 3 человека. сколько человек осталось?', ['2 · 8 = 16 (чел.) – работали до обеда', '16 - 3 = 13 (чел.) – осталось после обеда'], 'после обеда осталось 13 человек', '13', 'человек', 1045),
    ('в кассе 30 билетов. продали 2 группам по 10 билетов. сколько билетов осталось продать?', ['2 · 10 = 20 (шт.) – билетов продали', '30 - 20 = 10 (шт.) – билетов осталось'], 'осталось продать 10 билетов', '10', 'билетов', 1046),
    ('кузнецу привели 8 лошадей. если каждую подковать на все 4 ноги, то у кузнеца останется еще 8 подков. сколько подков у кузнеца?', ['8 · 4 = 32 (шт.) – подков нужно для лошадей', '32 + 8 = 40 (шт.) – подков у кузнеца'], 'у кузнеца 40 подков', '40', 'подков', 1047),
    ('в библиотеку привезли книги. после того как на 8 полок поставили по 5 книг, осталось 20 книг. сколько книг привезли в библиотеку?', ['8 · 5 = 40 (шт.) – книг поставили на полки', '40 + 20 = 60 (шт.) – книг привезли'], 'в библиотеку привезли 60 книг', '60', 'книг', 1048),
    ('на фабрику поступило 100 т шерсти. когда несколько тонн использовали, то осталось 9 вагонов по 6 т. сколько тонн шерсти использовали?', ['9 · 6 = 54 (т) – шерсти осталось', '100 - 54 = 46 (т) – шерсти использовали'], 'использовали 46 т шерсти', '46', 'т', 1049),
    ('в магазин привезли 6 ящиков хурмы по 8 кг в каждом. за день продали 29 кг хурмы. сколько килограммов хурмы осталось в магазине?', ['6 · 8 = 48 (кг) – хурмы привезли', '48 - 29 = 19 (кг) – хурмы осталось'], 'в магазине осталось 19 кг хурмы', '19', 'кг', 1050),
    ('у учителя было 45 тетрадей. он раздал 10 ученикам по 2 тетради. сколько тетрадей осталось у учителя?', ['10 · 2 = 20 (шт.) – тетрадей раздал', '45 - 20 = 25 (шт.) – тетрадей осталось'], 'у учителя осталось 25 тетрадей', '25', 'тетрадей', 1051),
    ('в школьном зале 6 окон. чтобы их украсить, дети принесли горшки с цветами. если на каждое окно поставить по 3 горшка, останется 5 горшков. сколько горшков с цветами принесли дети?', ['6 · 3 = 18 (шт.) – горшков поставят на окна', '18 + 5 = 23 (шт.) – горшков принесли'], 'дети принесли 23 горшка с цветами', '23', 'горшка', 1052),
    ('для аквариума купили рыбок. после того как в 3 аквариума пустили по 4 рыбки, в банке осталось 5 рыбок. сколько рыбок купили?', ['3 · 4 = 12 (шт.) – рыбок пустили в аквариумы', '12 + 5 = 17 (шт.) – рыбок купили'], 'купили 17 рыбок', '17', 'рыбок', 1053),
    ('фирма закупила 30 кассет. после того как несколько кассет записали, осталось 2 коробки по 10 кассет. сколько кассет записали?', ['2 · 10 = 20 (шт.) – кассет осталось', '30 - 20 = 10 (шт.) – кассет записали'], 'записали 10 кассет', '10', 'кассет', 1054),
    ('у пети было в 3 коробках по 6 карандашей. он потерял 4 карандаша. сколько карандашей осталось у пети?', ['3 · 6 = 18 (шт.) – карандашей было', '18 - 4 = 14 (шт.) – карандашей осталось'], 'у Пети осталось 14 карандашей', '14', 'карандашей', 1055),
    ('у школьников было 36 кустов рассады. они посадили на 2 клумбы по 6 кустов. сколько кустов рассады осталось посадить?', ['2 · 6 = 12 (шт.) – кустов посадили', '36 - 12 = 24 (шт.) – кустов осталось'], 'осталось посадить 24 куста рассады', '24', 'куста', 1056),
    ('в мастерской сшили 9 костюмов. если к каждому костюму пришить по 6 пуговиц, в коробке останется 10 пуговиц. сколько пуговиц было в коробке?', ['9 · 6 = 54 (шт.) – пуговиц нужно пришить', '54 + 10 = 64 (шт.) – пуговиц было'], 'в коробке было 64 пуговицы', '64', 'пуговицы', 1057),
    ('тарас вырезал снежинки. после того как на 3 окна он приклеил по 3 снежинки, у него осталось 5 снежинок. сколько снежинок вырезал тарас?', ['3 · 3 = 9 (шт.) – снежинок приклеил', '9 + 5 = 14 (шт.) – снежинок вырезал'], 'Тарас вырезал 14 снежинок', '14', 'снежинок', 1058),
    ('в палатку привезли 50 кг апельсинов. после того как несколько килограммов продали, в магазине осталось 5 ящиков по 6 кг. сколько килограммов апельсинов продали?', ['5 · 6 = 30 (кг) – апельсинов осталось', '50 - 30 = 20 (кг) – апельсинов продали'], 'продали 20 кг апельсинов', '20', 'кг', 1059),
    ('на 3 ветках по 9 шишек. белка утащила 3 шишки. сколько шишек осталось?', ['3 · 9 = 27 (шт.) – шишек было', '27 - 3 = 24 (шт.) – шишек осталось'], 'осталось 24 шишки', '24', 'шишки', 1060),
    ('привезли 24 стола. в 5 кабинетов поставили по 3 стола. сколько столов осталось?', ['5 · 3 = 15 (шт.) – столов поставили', '24 - 15 = 9 (шт.) – столов осталось'], 'осталось 9 столов', '9', 'столов', 1061),
    ('буфетчица налила детям 8 стаканов какао. если в каждый стакан положить по 2 куска сахара, то на тарелке останется еще 15 кусков. сколько кусков сахара было на тарелке сначала?', ['8 · 2 = 16 (шт.) – кусков сахара положили', '16 + 15 = 31 (шт.) – кусков сахара было'], 'сначала на тарелке был 31 кусок сахара', '31', 'кусок', 1062),
    ('у фермера было несколько лошадей. после того как он запряг 4 повозки по 2 лошади, у него осталось 5 лошадей. сколько лошадей было у фермера?', ['4 · 2 = 8 (шт.) – лошадей запряг', '8 + 5 = 13 (шт.) – лошадей было'], 'у фермера было 13 лошадей', '13', 'лошадей', 1063),
    ('в переплетную мастерскую привезли 20 книг. когда несколько книг переплели, то осталось 2 стопки по 8 книг. сколько книг переплели?', ['2 · 8 = 16 (шт.) – книг осталось', '20 - 16 = 4 (шт.) – книг переплели'], 'переплели 4 книги', '4', 'книги', 1064),
    ('в 2 вазах по 10 слив. съели 14 слив. сколько слив осталось в вазах?', ['2 · 10 = 20 (шт.) – слив было', '20 - 14 = 6 (шт.) – слив осталось'], 'в вазах осталось 6 слив', '6', 'слив', 1065),
    ('сереже подарили 100 марок. он разместил на 6 страницах альбома по 10 марок. сколько марок осталось разместить?', ['6 · 10 = 60 (шт.) – марок разместил', '100 - 60 = 40 (шт.) – марок осталось'], 'Сереже осталось разместить 40 марок', '40', 'марок', 1066),
    ('для украшения 7 елочек купили гирлянды. если каждую елочку украсить 3 гирляндами, то останется еще 9 гирлянд. сколько гирлянд купили?', ['7 · 3 = 21 (шт.) – гирлянд нужно для елочек', '21 + 9 = 30 (шт.) – гирлянд купили'], 'купили 30 гирлянд', '30', 'гирлянд', 1067),
    ('мама купила картофель. после того как она очистила 2 пакета по 3 кг, у нее осталось еще 7 кг картофеля. сколько килограммов картофеля купила мама?', ['2 · 3 = 6 (кг) – картофеля очистила', '6 + 7 = 13 (кг) – картофеля купила'], 'мама купила 13 кг картофеля', '13', 'кг', 1068),
    ('в палатку привезли 100 бутылок молока. после того как несколько бутылок молока продали, в 4 ящиках осталось по 9 бутылок. сколько бутылок молока продали?', ['4 · 9 = 36 (шт.) – бутылок осталось', '100 - 36 = 64 (шт.) – бутылок продали'], 'продали 64 бутылки молока', '64', 'бутылки', 1069),
    ('на складе было 16 т картофеля. на 3 машинах увезли по 4 т. сколько тонн картофеля осталось?', ['3 · 4 = 12 (т) – картофеля увезли', '16 - 12 = 4 (т) – картофеля осталось'], 'на складе осталось 4 т картофеля', '4', 'т', 1070),
    ('для починки 7 кресел купили ткань. на каждое кресло пошло по 4 м ткани и осталось 15 м. сколько метров ткани купили?', ['7 · 4 = 28 (м) – ткани пошло на кресла', '28 + 15 = 43 (м) – ткани купили'], 'купили 43 м ткани', '43', 'м', 1071),
    ('в 3 банках по 5 л сока. выпили 2 л. сколько литров осталось?', ['3 · 5 = 15 (л) – сока было', '15 - 2 = 13 (л) – сока осталось'], 'осталось 13 л сока', '13', 'л', 1072),
    ('бабушка сварила вишневое варенье. после того как она разлила варенье в 5 трехлитровых банок, у нее осталось 2 л варенья. сколько литров вишневого варенья сварила бабушка?', ['5 · 3 = 15 (л) – варенья разлила', '15 + 2 = 17 (л) – варенья сварила'], 'бабушка сварила 17 л вишневого варенья', '17', 'л', 1073),
    ('для освещения коридора купили 12 лампочек. несколько лампочек отложили, а остальные ввинтили в 2 люстры по 5 лампочек в каждую. сколько лампочек отложили?', ['2 · 5 = 10 (шт.) – лампочек ввинтили', '12 - 10 = 2 (шт.) – лампочек отложили'], 'отложили 2 лампочки', '2', 'лампочки', 1074),
    ('таня пришила к 3 платьям по 4 пуговицы. 3 пуговицы оторвалось. сколько пуговиц осталось?', ['3 · 4 = 12 (шт.) – пуговиц пришила', '12 - 3 = 9 (шт.) – пуговиц осталось'], 'осталось 9 пуговиц', '9', 'пуговиц', 1075),
    ('в сервизе было 24 тарелки. на 2 стола поставили по 8 тарелок. сколько тарелок осталось?', ['2 · 8 = 16 (шт.) – тарелок поставили', '24 - 16 = 8 (шт.) – тарелок осталось'], 'осталось 8 тарелок', '8', 'тарелок', 1076),
    ('для 4 кабинетов купили вазы. если в каждый кабинет поставить по 2 вазы, то останется 6 ваз. сколько ваз купили?', ['4 · 2 = 8 (шт.) – ваз поставят', '8 + 6 = 14 (шт.) – ваз купили'], 'купили 14 ваз', '14', 'ваз', 1077),
    ('ася читала 3 дня по 8 страниц. в книге осталось 8 страниц. сколько страниц было в книге?', ['3 · 8 = 24 (шт.) – страниц прочитала', '24 + 8 = 32 (шт.) – страниц было'], 'в книге было 32 страницы', '32', 'страницы', 1078),
    ('у володи было 36 фотографий. несколько фотографий он подарил, а остальные разместил на 5 страницах альбома по 3 фотографии. сколько фотографий подарил володя?', ['5 · 3 = 15 (шт.) – фотографий разместил', '36 - 15 = 21 (шт.) – фотографий подарил'], 'Володя подарил 21 фотографию', '21', 'фотографию', 1079),
    ('в 3 люстрах горело по 6 лампочек. перегорело 2 лампочки. сколько лампочек осталось?', ['3 · 6 = 18 (шт.) – лампочек горело', '18 - 2 = 16 (шт.) – лампочек осталось'], 'осталось 16 лампочек', '16', 'лампочек', 1080),
    ('у маши было 20 картинок. она раскрашивала по 4 картинки 2 дня. сколько картинок осталось раскрасить маше?', ['4 · 2 = 8 (шт.) – картинок раскрасила', '20 - 8 = 12 (шт.) – картинок осталось'], 'Маше осталось раскрасить 12 картинок', '12', 'картинок', 1081),
    ('после того как антон разместил на 6 страницах по 8 марок, у него осталось 15 марок. сколько всего марок у антона?', ['6 · 8 = 48 (шт.) – марок разместил', '48 + 15 = 63 (шт.) – всего марок'], 'у Антона всего 63 марки', '63', 'марки', 1082),
    ('для оформления 3 стендов дети принесли рисунки. если на каждый стенд повесить по 6 рисунков, то останется еще 11 рисунков. сколько рисунков принесли дети?', ['3 · 6 = 18 (шт.) – рисунков повесят', '18 + 11 = 29 (шт.) – рисунков принесли'], 'дети принесли 29 рисунков', '29', 'рисунков', 1083),
    ('в питомник привезли 36 саженцев. несколько саженцев продали, а остальные посадили в 4 ряда по 5 саженцев. сколько саженцев продали?', ['4 · 5 = 20 (шт.) – саженцев посадили', '36 - 20 = 16 (шт.) – саженцев продали'], 'продали 16 саженцев', '16', 'саженцев', 1084),
    ('в 4 квартирах по 4 комнаты. отремонтировали 9 комнат. сколько комнат осталось отремонтировать?', ['4 · 4 = 16 (шт.) – комнат всего', '16 - 9 = 7 (шт.) – комнат осталось'], 'осталось отремонтировать 7 комнат', '7', 'комнат', 1085),
    ('поезду нужно пройти 400 км. он ехал 2 ч, проезжая по 100 км каждый час. сколько километров осталось проехать поезду?', ['2 · 100 = 200 (км) – поезд проехал', '400 - 200 = 200 (км) – осталось проехать'], 'поезду осталось проехать 200 км', '200', 'км', 1086),
    ('для пошива 5 платьев купили ткань. если на каждое платье расходовать по 4 м, то останется 2 м ткани. сколько метров ткани купили?', ['5 · 4 = 20 (м) – ткани расходуют на платья', '20 + 2 = 22 (м) – ткани купили'], 'купили 22 м ткани', '22', 'м', 1087),
    ('после того как галя угостила 3 подруг, дав по 4 конфеты каждой, у нее осталось 3 конфеты. сколько конфет было у гали?', ['3 · 4 = 12 (шт.) – конфет дала подругам', '12 + 3 = 15 (шт.) – конфет было'], 'у Гали было 15 конфет', '15', 'конфет', 1088),
    ('в магазин привезли 40 часов. несколько часов положили на витрину, а остальные разложили на 5 полок по 7 штук на каждой. сколько часов положили на витрину?', ['5 · 7 = 35 (шт.) – часов разложили на полки', '40 - 35 = 5 (шт.) – часов положили на витрину'], 'на витрину положили 5 часов', '5', 'часов', 1089),
    ('юннаты собрали с 2 грядок по 9 кг лука. на семена оставили 3 кг. остальное отдали в столовую. сколько килограммов лука юннаты отдали в столовую?', ['2 · 9 = 18 (кг) – лука собрали', '18 - 3 = 15 (кг) – лука отдали'], 'юннаты отдали в столовую 15 кг лука', '15', 'кг', 1090),
    ('для ремонта квартиры купили 4 банки краски по 3 кг в каждой. сколько килограммов краски израсходовали, если после ремонта осталось 2 кг краски?', ['4 · 3 = 12 (кг) – краски купили', '12 - 2 = 10 (кг) – краски израсходовали'], 'израсходовали 10 кг краски', '10', 'кг', 1091),
    ('для уроков труда купили 14 наборов цветной бумаги по 10 листов в каждом наборе. на поделки истратили 80 листов. сколько листов осталось?', ['14 · 10 = 140 (шт.) – листов купили', '140 - 80 = 60 (шт.) – листов осталось'], 'осталось 60 листов', '60', 'листов', 1092),
    ('в библиотеку привезли книги. после того как библиотекарь поставила на 3 полки по 27 книг, осталось расставить еще 38 книг. сколько книг привезли в библиотеку?', ['3 · 27 = 81 (шт.) – книг поставила на полки', '81 + 38 = 119 (шт.) – книг привезли'], 'в библиотеку привезли 119 книг', '119', 'книг', 1093),
    ('хозяйка засолила несколько банок огурцов по 5 кг в каждой банке. зимой съели 30 кг и осталось 10 кг огурцов. сколько банок огурцов засолила хозяйка?', ['30 + 10 = 40 (кг) – огурцов засолила', '40 : 5 = 8 (шт.) – банок'], 'хозяйка засолила 8 банок огурцов', '8', 'банок', 1094),
    ('на пришкольном участке ребята вырастили огурцы. после того как разложили огурцы в 5 корзин по 6 кг в каждую, осталось 28 кг. сколько килограммов огурцов вырастили ребята?', ['5 · 6 = 30 (кг) – огурцов разложили', '30 + 28 = 58 (кг) – огурцов вырастили'], 'ребята вырастили 58 кг огурцов', '58', 'кг', 1095),
    ('в детский сад привезли 2 бидона молока. сколько литров молока было в каждом бидоне, если после того как дети выпили 44 л, осталось 2 л?', ['44 + 2 = 46 (л) – молока привезли', '46 : 2 = 23 (л) – молока в одном бидоне'], 'в каждом бидоне было 23 л молока', '23', 'л', 1096),
    ('на выставку привезли 54 картины. в большой зал повесили 36 картин, а остальные картины развесили поровну в 3 маленьких залах. по скольку картин в каждом маленьком зале?', ['54 - 36 = 18 (шт.) – картин в маленьких залах', '18 : 3 = 6 (шт.) – картин в каждом зале'], 'в каждом маленьком зале по 6 картин', '6', 'картин', 1097),
    ('школьники собрали 49 кг лекарственных трав. упаковали в ящик 22 кг, а остальные в - 9 пакетов поровну в каждый. сколько килограммов лекарственных трав было в каждом пакете?', ['49 - 22 = 27 (кг) – трав упаковали в пакеты', '27 : 9 = 3 (кг) – трав в каждом пакете'], 'в каждом пакете было 3 кг лекарственных трав', '3', 'кг', 1098),
    ('в школьную столовую привезли творог. неделю расходовали по 4 кг творога в день. сколько килограммов творога привезли, если в столовой остался 41 кг творога?', ['7 · 4 = 28 (кг) – творога израсходовали за неделю', '28 + 41 = 69 (кг) – творога привезли'], 'в школьную столовую привезли 69 кг творога', '69', 'кг', 1099),
    ('в магазине было 84 кг сливы. продали 6 ящиков по 9 кг в каждом, а остальные упаковали в пакеты по 6 кг в каждом. сколько было пакетов?', ['6 · 9 = 54 (кг) – слив продали', '84 - 54 = 30 (кг) – слив упаковали', '30 : 6 = 5 (шт.) – пакетов'], 'было 5 пакетов со сливами', '5', 'пакетов', 1100),
]




# V413: exact post-API repair for Excel rows 1101-1200.  The live batch
# v412 showed real DeepSeek/API usage for every row, but many ratio/proportion
# word problems produced short answers or wrong inverse actions.  These specs are
# applied only after the route has preserved the external API proof.
_V41201_BATCH_1101_1200_SPECS = [('в магазин привезли апельсины. после того как продали 5 ящиков по 7 кг, в магазине осталось 48 кг апельсинов. сколько килограммов '
  'апельсинов было в магазине сначала?',
  ['5 · 7 = 35 (кг) – продали апельсинов', '35 + 48 = 83 (кг) – было апельсинов'],
  'в магазине сначала было 83 кг апельсинов',
  '83',
  'кг',
  1101),
 ('у портнихи было 26 м тесьмы. несколько метров она израсходовала на отделку 4 одинаковых блузок. после этого у нее осталось 14 м тесьмы. '
  'сколько тесьмы пошло на отделку каждой блузки?',
  ['26 - 14 = 12 (м) – тесьмы израсходовала', '12 : 4 = 3 (м) – тесьмы на одну блузку'],
  'на отделку каждой блузки пошло 3 м тесьмы',
  '3',
  'м',
  1102),
 ('в санаторий привезли 4 мешка муки по 15 кг в каждом. из 49 кг муки испекли булочки. сколько килограммов муки осталось?',
  ['4 · 15 = 60 (кг) – муки привезли', '60 - 49 = 11 (кг) – муки осталось'],
  'осталось 11 кг муки',
  '11',
  'кг',
  1103),
 ('у саши в коллекции 75 марок. в нескольких больших альбомах по 15 марок. остальные 30 находятся в маленьких альбомах. сколько больших '
  'альбомов с марками у саши?',
  ['75 - 30 = 45 (шт.) – марок в больших альбомах', '45 : 15 = 3 (шт.) – больших альбомов'],
  'у Саши 3 больших альбома с марками',
  '3',
  'альбома',
  1104),
 ('когда портниха пришила по 9 пуговиц к 6 пальто, у нее осталось 32 пуговицы. сколько всего пуговиц было у портнихи сначала?',
  ['6 · 9 = 54 (шт.) – пуговиц пришила', '54 + 32 = 86 (шт.) – пуговиц было'],
  'у портнихи сначала было 86 пуговиц',
  '86',
  'пуговиц',
  1105),
 ('работница должна была прополоть 3 ряда малины по 27 кустов в каждом. она прополола 65 кустов. сколько кустов малины осталось прополоть?',
  ['3 · 27 = 81 (шт.) – кустов надо прополоть', '81 - 65 = 16 (шт.) – кустов осталось'],
  'осталось прополоть 16 кустов малины',
  '16',
  'кустов',
  1106),
 ('ребята собрали 78 грибов. они сварили 18 грибов. остальные засолили в банках по 15 грибов в каждой. сколько банок грибов засолили?',
  ['78 - 18 = 60 (шт.) – грибов засолили', '60 : 15 = 4 (шт.) – банок грибов'],
  'засолили 4 банки грибов',
  '4',
  'банки',
  1107),
 ('когда из бочки набрали 3 ведра воды по 6 л, в ней осталось 12 л. сколько литров воды было в бочке сначала?',
  ['3 · 6 = 18 (л) – воды набрали', '18 + 12 = 30 (л) – воды было'],
  'в бочке сначала было 30 л воды',
  '30',
  'л',
  1108),
 ('в спортивный магазин привезли 60 мячей. повесили в 5 сеток по 7 мячей. сколько мячей осталось в ящике?',
  ['5 · 7 = 35 (шт.) – мячей повесили', '60 - 35 = 25 (шт.) – мячей осталось'],
  'в ящике осталось 25 мячей',
  '25',
  'мячей',
  1109),
 ('из 3 классов на соревнования отправили по 6 человек. награды получили 5 человек. сколько человек осталось без награды?',
  ['3 · 6 = 18 (чел.) – отправили на соревнования', '18 - 5 = 13 (чел.) – осталось без награды'],
  'без награды осталось 13 человек',
  '13',
  'человек',
  1110),
 ('в столовой за неделю израсходовали 63 кг муки. расходовали 4 дня по 12 кг муки в день, а остальную муку - поровну в следующие 3 дня. '
  'сколько килограммов муки расходовали в каждый последующий день?',
  ['4 · 12 = 48 (кг) – муки израсходовали за 4 дня',
   '63 - 48 = 15 (кг) – муки израсходовали за следующие дни',
   '15 : 3 = 5 (кг) – муки в каждый следующий день'],
  'в каждый из следующих 3 дней расходовали по 5 кг муки',
  '5',
  'кг',
  1111),
 ('в ряду сидит 6 мальчиков, а девочек - в 2 раза больше. на сколько больше в классе девочек, чем мальчиков?',
  ['6 · 2 = 12 (чел.) – девочек', '12 - 6 = 6 (чел.) – разница'],
  'в классе девочек на 6 человек больше, чем мальчиков',
  '6',
  'человек',
  1112),
 ('масса тыквы 4 кг. это в 2 раза больше, чем масса кабачка. на сколько меньше масса кабачка, чем тыквы?',
  ['4 : 2 = 2 (кг) – масса кабачка', '4 - 2 = 2 (кг) – разница массы'],
  'масса кабачка на 2 кг меньше массы тыквы',
  '2',
  'кг',
  1113),
 ('у люды 18 календариков, а у маргариты - в 2 раза меньше. на сколько меньше календариков у маргариты, чем у люды?',
  ['18 : 2 = 9 (шт.) – календариков у Маргариты', '18 - 9 = 9 (шт.) – разница'],
  'у Маргариты на 9 календариков меньше, чем у Люды',
  '9',
  'календариков',
  1114),
 ('хозяйка купила 3 кг яблок, а груш - на 6 кг больше. во сколько раз больше купила хозяйка груш, чем яблок?',
  ['3 + 6 = 9 (кг) – груш', '9 : 3 = 3 (раза) – во сколько раз больше'],
  'груш купили в 3 раза больше, чем яблок',
  '3',
  'раза',
  1115),
 ('утром игорь сделал 2 модели вертолетов, что в 2 раза меньше, чем вечером. на сколько больше моделей вертолетов игорь сделал вечером, '
  'чем утром?',
  ['2 · 2 = 4 (шт.) – моделей вечером', '4 - 2 = 2 (шт.) – разница'],
  'вечером Игорь сделал на 2 модели вертолётов больше, чем утром',
  '2',
  'модели',
  1116),
 ('в вазе 4 персика, а абрикосов - в 2 раза больше. на сколько меньше в вазе персиков, чем абрикосов?',
  ['4 · 2 = 8 (шт.) – абрикосов', '8 - 4 = 4 (шт.) – разница'],
  'персиков в вазе на 4 фрукта меньше, чем абрикосов',
  '4',
  'фрукта',
  1117),
 ('в физкультурном зале 8 больших мячей, а маленьких - на 4 меньше. во сколько раз больше в физкультурном зале больших мячей, чем '
  'маленьких?',
  ['8 - 4 = 4 (шт.) – маленьких мячей', '8 : 4 = 2 (раза) – во сколько раз больше'],
  'больших мячей в 2 раза больше, чем маленьких',
  '2',
  'раза',
  1118),
 ('у тони 8 кукол барби, а у ксюши - в 2 раза меньше. на сколько больше кукол барби у тони, чем у ксюши?',
  ['8 : 2 = 4 (шт.) – кукол у Ксюши', '8 - 4 = 4 (шт.) – разница'],
  'у Тони на 4 куклы Барби больше, чем у Ксюши',
  '4',
  'куклы',
  1119),
 ('митя окопал 10 деревьев. это в 2 раза больше, чем окопал деревьев леня. на сколько больше окопал деревьев митя, чем леня?',
  ['10 : 2 = 5 (шт.) – деревьев окопал Лёня', '10 - 5 = 5 (шт.) – разница'],
  'Митя окопал на 5 деревьев больше, чем Лёня',
  '5',
  'деревьев',
  1120),
 ('на кухне горит 4 лампочки, а в комнате - в 2 раза больше. на сколько больше лампочек горит в комнате, чем на кухне?',
  ['4 · 2 = 8 (шт.) – лампочек в комнате', '8 - 4 = 4 (шт.) – разница'],
  'в комнате горит на 4 лампочки больше, чем на кухне',
  '4',
  'лампочки',
  1121),
 ('у юли было 3 грецких ореха, земляных - на 6 больше, а арахиса столько, сколько земляных и грецких орехов вместе. во сколько раз у юли '
  'больше орехов арахиса, чем грецких?',
  ['3 + 6 = 9 (шт.) – земляных орехов', '3 + 9 = 12 (шт.) – орехов арахиса', '12 : 3 = 4 (раза) – во сколько раз больше'],
  'орехов арахиса у Юли в 4 раза больше, чем грецких',
  '4',
  'раза',
  1122),
 ('у брата 18 фломастеров. это в 2 раза больше, чем у сестры. на сколько меньше фломастеров у сестры, чем у брата?',
  ['18 : 2 = 9 (шт.) – фломастеров у сестры', '18 - 9 = 9 (шт.) – разница'],
  'у сестры на 9 фломастеров меньше, чем у брата',
  '9',
  'фломастеров',
  1123),
 ('собрали 14 кг клюквы, а голубики - в 2 раза меньше. на сколько меньше собрали голубики, чем клюквы?',
  ['14 : 2 = 7 (кг) – голубики', '14 - 7 = 7 (кг) – разница'],
  'голубики собрали на 7 кг меньше, чем клюквы',
  '7',
  'кг',
  1124),
 ('к празднику купили 2 красных шарика, а зеленых - на 4 шарика больше. во сколько раз меньше купили к празднику красных шариков, чем '
  'зеленых?',
  ['2 + 4 = 6 (шт.) – зеленых шариков', '6 : 2 = 3 (раза) – во сколько раз меньше'],
  'красных шариков купили в 3 раза меньше, чем зеленых',
  '3',
  'раза',
  1125),
 ('на озере плавало 6 селезней, что в 2 раза меньше, чем уток. на сколько больше плавало на озере уток, чем селезней?',
  ['6 · 2 = 12 (шт.) – уток', '12 - 6 = 6 (шт.) – разница'],
  'уток плавало на 6 птиц больше, чем селезней',
  '6',
  'птиц',
  1126),
 ('у геры было 6 голубеньких попугаев, а зеленых - в 2 раза больше. на сколько больше было у геры зеленых попугаев, чем голубеньких?',
  ['6 · 2 = 12 (шт.) – зеленых попугаев', '12 - 6 = 6 (шт.) – разница'],
  'у Геры зеленых попугаев было на 6 больше, чем голубеньких',
  '6',
  'попугаев',
  1127),
 ('валя решила 10 примеров на сложение, а на вычитание - на 5 примеров меньше. во сколько раз больше решила валя примеров на сложение, чем '
  'на вычитание?',
  ['10 - 5 = 5 (шт.) – примеров на вычитание', '10 : 5 = 2 (раза) – во сколько раз больше'],
  'примеров на сложение Валя решила в 2 раза больше, чем на вычитание',
  '2',
  'раза',
  1128),
 ('у слоненка было 14 морковок, а репок - в 2 раза меньше. на сколько меньше у слоненка репок, чем морковок?',
  ['14 : 2 = 7 (шт.) – репок', '14 - 7 = 7 (шт.) – разница'],
  'у слоненка репок на 7 овощей меньше, чем морковок',
  '7',
  'овощей',
  1129),
 ('дина вышила 8 подушек, что в 2 раза меньше, чем вышила марина. на сколько меньше подушек вышила дина, чем марина?',
  ['8 · 2 = 16 (шт.) – подушек вышила Марина', '16 - 8 = 8 (шт.) – разница'],
  'Дина вышила на 8 подушек меньше, чем Марина',
  '8',
  'подушек',
  1130),
 ('в ателье приняли на заказ 4 платья, костюмов - на 2 меньше, а курток столько, сколько платьев и костюмов вместе. во сколько раз меньше '
  'приняли заказов на пошив костюмов, чем курток?',
  ['4 - 2 = 2 (шт.) – костюмов', '4 + 2 = 6 (шт.) – курток', '6 : 2 = 3 (раза) – во сколько раз меньше'],
  'заказов на костюмы приняли в 3 раза меньше, чем на куртки',
  '3',
  'раза',
  1131),
 ('масса арбуза 6 кг. это в 3 раза больше, чем масса дыни. на сколько больше масса арбуза, чем дыни?',
  ['6 : 3 = 2 (кг) – масса дыни', '6 - 2 = 4 (кг) – разница массы'],
  'масса арбуза на 4 кг больше массы дыни',
  '4',
  'кг',
  1132),
 ('в воскресенье в бассейн пришел слоненок с друзьями. когда тигренок, обезьянка и черепаха прыгнули в бассейн, то из него выплеснулось 9 '
  'ведер воды. но когда туда прыгнул слоненок, воды из бассейна выплеснулось в 8 раз больше. на сколько больше воды выплеснул из бассейна '
  'слоненок, чем его друзья?',
  ['9 · 8 = 72 (шт.) – ведер выплеснулось от слоненка', '72 - 9 = 63 (шт.) – разница'],
  'от слоненка выплеснулось на 63 ведра воды больше',
  '63',
  'ведра',
  1133),
 ('купили 5 м сатина, а ситца - в 3 раза больше. на сколько меньше метров купили сатина, чем ситца?',
  ['5 · 3 = 15 (м) – ситца', '15 - 5 = 10 (м) – разница'],
  'сатина купили на 10 м меньше, чем ситца',
  '10',
  'м',
  1134),
 ('в саду росло 4 гладиолуса, а лилий - на 8 больше. во сколько раз меньше росло в саду гладиолусов, чем лилий?',
  ['4 + 8 = 12 (шт.) – лилий', '12 : 4 = 3 (раза) – во сколько раз меньше'],
  'гладиолусов росло в 3 раза меньше, чем лилий',
  '3',
  'раза',
  1135),
 ('в саду росло 9 кустов черной смородины, а красной смородины - в 3 раза меньше. на сколько больше в саду росло кустов черной смородины, '
  'чем красной?',
  ['9 : 3 = 3 (шт.) – кустов красной смородины', '9 - 3 = 6 (шт.) – разница'],
  'кустов черной смородины было на 6 больше, чем красной',
  '6',
  'кустов',
  1136),
 ('на зиму заготовили 8 банок малинового компота, а сливового - в 2 раза больше. на сколько меньше заготовили на зиму банок малинового '
  'компота, чем сливового?',
  ['8 · 2 = 16 (шт.) – банок сливового компота', '16 - 8 = 8 (шт.) – разница'],
  'банок малинового компота заготовили на 8 меньше, чем сливового',
  '8',
  'банок',
  1137),
 ('у кирилла 8 бананов, что в 2 раза меньше, чем у егора. на сколько больше бананов у егора, чем у кирилла?',
  ['8 · 2 = 16 (шт.) – бананов у Егора', '16 - 8 = 8 (шт.) – разница'],
  'у Егора на 8 бананов больше, чем у Кирилла',
  '8',
  'бананов',
  1138),
 ('в кафе было 30 молочников, а соусников - на 15 меньше. во сколько раз больше в кафе молочников, чем соусников ?',
  ['30 - 15 = 15 (шт.) – соусников', '30 : 15 = 2 (раза) – во сколько раз больше'],
  'молочников в кафе было в 2 раза больше, чем соусников',
  '2',
  'раза',
  1139),
 ('в дивизионе было 15 легких танков, а тяжелых - в 3 раза меньше. на сколько больше легких танков в дивизионе, чем тяжелых?',
  ['15 : 3 = 5 (шт.) – тяжелых танков', '15 - 5 = 10 (шт.) – разница'],
  'легких танков было на 10 больше, чем тяжелых',
  '10',
  'танков',
  1140),
 ('у пристани стояло 2 катера, байдарок - на 2 больше, а лодок столько, сколько катеров и байдарок вместе. во сколько раз меньше стояло на '
  'пристани катеров, чем лодок?',
  ['2 + 2 = 4 (шт.) – байдарок', '2 + 4 = 6 (шт.) – лодок', '6 : 2 = 3 (раза) – во сколько раз меньше'],
  'катеров стояло в 3 раза меньше, чем лодок',
  '3',
  'раза',
  1141),
 ('в палатке 14 ящиков винограда. это в 7 раз больше, чем ящиков с киви. на сколько меньше в палатке ящиков с киви, чем с виноградом?',
  ['14 : 7 = 2 (шт.) – ящиков с киви', '14 - 2 = 12 (шт.) – разница'],
  'ящиков с киви было на 12 меньше, чем с виноградом',
  '12',
  'ящиков',
  1142),
 ('аня купила 4 кг фиников, а хурмы - на 8 кг больше. во сколько раз меньше купила аня фиников, чем хурмы?',
  ['4 + 8 = 12 (кг) – хурмы', '12 : 4 = 3 (раза) – во сколько раз меньше'],
  'фиников Аня купила в 3 раза меньше, чем хурмы',
  '3',
  'раза',
  1143),
 ('в тетради 18 чистых страниц. это в 3 раза больше, чем исписанных страниц. на сколько больше в тетради чистых страниц, чем исписанных?',
  ['18 : 3 = 6 (шт.) – исписанных страниц', '18 - 6 = 12 (шт.) – разница'],
  'чистых страниц в тетради на 12 больше, чем исписанных',
  '12',
  'страниц',
  1144),
 ('в коробке было 4 куска туалетного мыла, а банного - в 2 раза больше. на сколько меньше в коробке кусков туалетного мыла, чем банного?',
  ['4 · 2 = 8 (шт.) – кусков банного мыла', '8 - 4 = 4 (шт.) – разница'],
  'кусков туалетного мыла было на 4 меньше, чем банного',
  '4',
  'куска',
  1145),
 ('скворец принес скворчатам в гнездо 18 червячков, а гусениц - в 3 раза меньше. на сколько больше съели скворчата червячков, чем гусениц?',
  ['18 : 3 = 6 (шт.) – гусениц', '18 - 6 = 12 (шт.) – разница'],
  'скворчата съели на 12 червячков больше, чем гусениц',
  '12',
  'штук',
  1146),
 ('на лугу работало 3 мужчин-косарей, а женщин, ворошащих сено, - на 12 больше. во сколько раз меньше работало мужчин, чем женщин?',
  ['3 + 12 = 15 (чел.) – женщин', '15 : 3 = 5 (раза) – во сколько раз меньше'],
  'мужчин работало в 5 раз меньше, чем женщин',
  '5',
  'раз',
  1147),
 ('у школы росло 5 осин, а берез - в 4 раза больше. на сколько меньше росло около школы осин, чем берез?',
  ['5 · 4 = 20 (шт.) – берез', '20 - 5 = 15 (шт.) – разница'],
  'около школы осин росло на 15 деревьев меньше, чем берез',
  '15',
  'деревьев',
  1148),
 ('в классе было 5 портретов поэтов. это в 2 раза меньше, чем портретов прозаиков. на сколько меньше в классе портретов поэтов, чем '
  'прозаиков?',
  ['5 · 2 = 10 (шт.) – портретов прозаиков', '10 - 5 = 5 (шт.) – разница'],
  'портретов поэтов было на 5 меньше, чем прозаиков',
  '5',
  'портретов',
  1149),
 ('в ларек привезли 9 ящиков с виноградом, а с лимонами - на 6 меньше. во сколько раз больше привезли ящиков с виноградом, чем с лимонами?',
  ['9 - 6 = 3 (шт.) – ящиков с лимонами', '9 : 3 = 3 (раза) – во сколько раз больше'],
  'ящиков с виноградом привезли в 3 раза больше, чем с лимонами',
  '3',
  'раза',
  1150),
 ('лариса прочитала 8 страниц. это в 2 раза меньше, чем прочитал андрей. на сколько больше прочитал страниц андрей, чем лариса?',
  ['8 · 2 = 16 (шт.) – страниц прочитал Андрей', '16 - 8 = 8 (шт.) – разница'],
  'Андрей прочитал на 8 страниц больше, чем Лариса',
  '8',
  'страниц',
  1151),
 ('семья выписывает 6 газет для взрослых, что в 2 раза больше, чем журналов для детей. на сколько больше выписывает семья газет для '
  'взрослых, чем журналов для детей?',
  ['6 : 2 = 3 (шт.) – журналов для детей', '6 - 3 = 3 (шт.) – разница'],
  'семья выписывает на 3 газеты больше, чем журналов для детей',
  '3',
  'газеты',
  1152),
 ('с первого куста собрали 2 кг черноплодной рябины, а со второго - на 4 кг больше. во сколько раз больше собрали рябины со второго куста, '
  'чем с первого?',
  ['2 + 4 = 6 (кг) – рябины со второго куста', '6 : 2 = 3 (раза) – во сколько раз больше'],
  'со второго куста собрали в 3 раза больше рябины, чем с первого',
  '3',
  'раза',
  1153),
 ('на даче было 6 грядок с морковкой, а с укропом - на 4 грядки меньше. во сколько раз больше на даче было грядок с морковкой, чем с '
  'укропом?',
  ['6 - 4 = 2 (шт.) – грядок с укропом', '6 : 2 = 3 (раза) – во сколько раз больше'],
  'грядок с морковкой было в 3 раза больше, чем с укропом',
  '3',
  'раза',
  1154),
 ('во время экскурсии 50 человек разместились поровну на 2 грузовиках, а 90 человек - в 3 автобусах. на сколько больше человек помещалось '
  'в каждом автобусе, чем на каждом грузовике?',
  ['50 : 2 = 25 (чел.) – на одном грузовике', '90 : 3 = 30 (чел.) – в одном автобусе', '30 - 25 = 5 (чел.) – разница'],
  'в каждом автобусе помещалось на 5 человек больше, чем на каждом грузовике',
  '5',
  'человек',
  1155),
 ('у лили 24 цветных карандаша, а простых карандашей - в 3 раза меньше. на сколько меньше у лили простых карандашей, чем цветных?',
  ['24 : 3 = 8 (шт.) – простых карандашей', '24 - 8 = 16 (шт.) – разница'],
  'у Лили простых карандашей на 16 меньше, чем цветных',
  '16',
  'карандашей',
  1156),
 ('в магазине было 7 мотков тонкой проволоки, всего 210 м, и 4 мотка толстой проволоки, всего 40 м. на сколько больше метров в мотке '
  'тонкой проволоки, чем в одном мотке толстой проволоки?',
  ['210 : 7 = 30 (м) – тонкой проволоки в мотке', '40 : 4 = 10 (м) – толстой проволоки в мотке', '30 - 10 = 20 (м) – разница'],
  'в мотке тонкой проволоки на 20 м больше, чем в мотке толстой',
  '20',
  'м',
  1157),
 ('на клумбе 12 красных пионов. это в 2 раза больше, чем розовых. на сколько меньше на клумбе розовых пионов, чем красных?',
  ['12 : 2 = 6 (шт.) – розовых пионов', '12 - 6 = 6 (шт.) – разница'],
  'розовых пионов на клумбе на 6 меньше, чем красных',
  '6',
  'пионов',
  1158),
 ('за 6 ч фабрика сожгла 540 кг торфа. в случае перехода на уголь фабрика сожгла бы за это время 240 кг угля. на сколько больше сжигает '
  'она за 1 ч торфа, чем сожгла бы угля?',
  ['540 : 6 = 90 (кг) – торфа за 1 час', '240 : 6 = 40 (кг) – угля за 1 час', '90 - 40 = 50 (кг) – разница'],
  'фабрика за 1 час сжигает на 50 кг торфа больше, чем угля',
  '50',
  'кг',
  1159),
 ('в первом кружке занималось 6 девочек, что в 2 раза меньше, чем во втором. на сколько больше занималось девочек во втором кружке, чем в '
  'первом?',
  ['6 · 2 = 12 (чел.) – девочек во втором кружке', '12 - 6 = 6 (чел.) – разница'],
  'во втором кружке занималось на 6 девочек больше, чем в первом',
  '6',
  'девочек',
  1160),
 ('первая бригада замостила 280 м дороги за 4 дня, а вторая - 270 м за 3 дня. какая из этих бригад замостит в день больше метров дороги и '
  'на сколько?',
  ['280 : 4 = 70 (м) – первая бригада за день', '270 : 3 = 90 (м) – вторая бригада за день', '90 - 70 = 20 (м) – разница'],
  'вторая бригада замостит в день на 20 м больше',
  '20',
  'м',
  1161),
 ('в саду росло 40 астр, маков - на 20 меньше, а гвоздик столько, сколько астр и маков вместе. во сколько раз больше росло в саду гвоздик, '
  'чем маков?',
  ['40 - 20 = 20 (шт.) – маков', '40 + 20 = 60 (шт.) – гвоздик', '60 : 20 = 3 (раза) – во сколько раз больше'],
  'гвоздик в саду росло в 3 раза больше, чем маков',
  '3',
  'раза',
  1162),
 ('вместе 4 куриных яйца весят 320 г, 3 голубиных - 84 г. на сколько граммов голубиное яйцо легче куриного?',
  ['320 : 4 = 80 (г) – куриное яйцо', '84 : 3 = 28 (г) – голубиное яйцо', '80 - 28 = 52 (г) – разница'],
  'голубиное яйцо легче куриного на 52 г',
  '52',
  'г',
  1163),
 ('в первой квартире 9 жильцов, а во второй - на 6 жильцов меньше. во сколько раз во второй квартире меньше жильцов, чем в первой?',
  ['9 - 6 = 3 (чел.) – жильцов во второй квартире', '9 : 3 = 3 (раза) – во сколько раз меньше'],
  'во второй квартире жильцов в 3 раза меньше, чем в первой',
  '3',
  'раза',
  1164),
 ('мишка стоит 20 р., а солдатик - в 2 раза дешевле. на сколько рублей мишка дороже, чем солдатик?',
  ['20 : 2 = 10 (руб.) – стоит солдатик', '20 - 10 = 10 (руб.) – разница'],
  'мишка дороже солдатика на 10 рублей',
  '10',
  'рублей',
  1165),
 ('в первом классе 6 отличников, а во втором - на 3 меньше. во сколько раз в первом классе отличников больше, чем во втором?',
  ['6 - 3 = 3 (чел.) – отличников во втором классе', '6 : 3 = 2 (раза) – во сколько раз больше'],
  'в первом классе отличников в 2 раза больше, чем во втором',
  '2',
  'раза',
  1166),
 ('первый класс посадил 3 ряда по 5 яблонь, а второй - 8 яблонь. на сколько больше яблонь посадил первый класс?',
  ['3 · 5 = 15 (шт.) – яблонь посадил первый класс', '15 - 8 = 7 (шт.) – разница'],
  'первый класс посадил на 7 яблонь больше',
  '7',
  'яблонь',
  1167),
 ('костюм стоит 120 р., а платье - в 3 раза дешевле. на сколько костюм дороже платья?',
  ['120 : 3 = 40 (руб.) – стоит платье', '120 - 40 = 80 (руб.) – разница'],
  'костюм дороже платья на 80 рублей',
  '80',
  'рублей',
  1168),
 ('в 3 первых классах по 4 отличника, а в 2 пятых - по 3 отличника. на сколько больше отличников в первых классах, чем в пятых?',
  ['3 · 4 = 12 (чел.) – отличников в первых классах', '2 · 3 = 6 (чел.) – отличников в пятых классах', '12 - 6 = 6 (чел.) – разница'],
  'в первых классах на 6 отличников больше, чем в пятых',
  '6',
  'отличников',
  1169),
 ('5 ящиков с апельсинами весят 40 кг, а ящик с хурмой - 4 кг. на сколько легче ящик с хурмой?',
  ['40 : 5 = 8 (кг) – ящик с апельсинами', '8 - 4 = 4 (кг) – разница'],
  'ящик с хурмой легче на 4 кг',
  '4',
  'кг',
  1170),
 ('при изготовлении раствора для укладки кирпича требуется 60 кг песка, а цемента - на 40 кг меньше. во сколько раз меньше требуется '
  'цемента, чем песка?',
  ['60 - 40 = 20 (кг) – цемента', '60 : 20 = 3 (раза) – во сколько раз меньше'],
  'цемента требуется в 3 раза меньше, чем песка',
  '3',
  'раза',
  1171),
 ('с бахчи собрали 55 арбузов, а дынь - на 44 меньше. во сколько раз больше собрали арбузов, чем дынь?',
  ['55 - 44 = 11 (шт.) – дынь', '55 : 11 = 5 (раза) – во сколько раз больше'],
  'арбузов собрали в 5 раз больше, чем дынь',
  '5',
  'раз',
  1172),
 ('2 утки снесли по 8 яиц, а курица снесла 72 яйца. на сколько яиц больше снесла курица, чем 2 утки?',
  ['2 · 8 = 16 (шт.) – яиц снесли утки', '72 - 16 = 56 (шт.) – разница'],
  'курица снесла на 56 яиц больше, чем 2 утки',
  '56',
  'яиц',
  1173),
 ('рыбаки поймали 64 форели, а сазанов - на 48 меньше. во сколько раз меньше поймали сазанов, чем форели?',
  ['64 - 48 = 16 (шт.) – сазанов', '64 : 16 = 4 (раза) – во сколько раз меньше'],
  'сазанов поймали в 4 раза меньше, чем форели',
  '4',
  'раза',
  1174),
 ('в магазине на 5 полках 35 кукол, а на 4 полках 32 машинки. на сколько больше на каждой полке машинок, чем кукол?',
  ['35 : 5 = 7 (шт.) – кукол на полке', '32 : 4 = 8 (шт.) – машинок на полке', '8 - 7 = 1 (шт.) – разница'],
  'на каждой полке машинок на 1 игрушку больше, чем кукол',
  '1',
  'игрушку',
  1175),
 ('в первом тайме футболисты забили 3 гола, а во втором - в 2 раза больше. на сколько больше голов забили во втором тайме, чем в первом?',
  ['3 · 2 = 6 (шт.) – голов во втором тайме', '6 - 3 = 3 (шт.) – разница'],
  'во втором тайме забили на 3 гола больше, чем в первом',
  '3',
  'гола',
  1176),
 ('в столовой за 5 дней расходуется 180 кг хлеба. на сколько килограммов больше расходуется хлеба за 5 дней, чем за один?',
  ['180 : 5 = 36 (кг) – хлеба за один день', '180 - 36 = 144 (кг) – разница'],
  'за 5 дней расходуется на 144 кг хлеба больше, чем за один день',
  '144',
  'кг',
  1177),
 ('в саду 5 рядов вишен по 8 деревьев и 4 ряда яблонь по 5 деревьев. во сколько раз меньше в саду растет яблонь, чем вишен?',
  ['5 · 8 = 40 (шт.) – вишен', '4 · 5 = 20 (шт.) – яблонь', '40 : 20 = 2 (раза) – во сколько раз меньше'],
  'яблонь в саду растет в 2 раза меньше, чем вишен',
  '2',
  'раза',
  1178),
 ('для настилки полов привезли 42 сосновые доски, а еловых - в 7 раз меньше. на сколько больше привезли сосновых досок, чем еловых?',
  ['42 : 7 = 6 (шт.) – еловых досок', '42 - 6 = 36 (шт.) – разница'],
  'сосновых досок привезли на 36 больше, чем еловых',
  '36',
  'досок',
  1179),
 ('6 ящиков апельсинов весят 48 кг, а 4 ящика мандаринов - 16 кг. во сколько раз ящик апельсинов весит больше, чем ящик мандаринов?',
  ['48 : 6 = 8 (кг) – ящик апельсинов', '16 : 4 = 4 (кг) – ящик мандаринов', '8 : 4 = 2 (раза) – во сколько раз больше'],
  'ящик апельсинов весит в 2 раза больше, чем ящик мандаринов',
  '2',
  'раза',
  1180),
 ('7 банок равного веса клубничного варенья весят 14 кг, а 5 банок равного веса земляничного варенья весят 20 кг. во сколько раз банка с '
  'клубничным вареньем легче банки с земляничным вареньем?',
  ['14 : 7 = 2 (кг) – банка клубничного варенья',
   '20 : 5 = 4 (кг) – банка земляничного варенья',
   '4 : 2 = 2 (раза) – во сколько раз легче'],
  'банка с клубничным вареньем легче банки с земляничным вареньем в 2 раза',
  '2',
  'раза',
  1181),
 ('ольге 18 лет, а катя моложе ее в 3 раза. на сколько лет катя моложе ольги?',
  ['18 : 3 = 6 (лет) – возраст Кати', '18 - 6 = 12 (лет) – разница возраста'],
  'Катя моложе Ольги на 12 лет',
  '12',
  'лет',
  1182),
 ('глубина колодца 30 м, а глубина оврага на 20 м меньше. во сколько раз глубина оврага меньше глубины колодца?',
  ['30 - 20 = 10 (м) – глубина оврага', '30 : 10 = 3 (раза) – во сколько раз меньше'],
  'глубина оврага в 3 раза меньше глубины колодца',
  '3',
  'раза',
  1183),
 ('7 ящиков со сливами весят 35 кг, а 4 ящика с виноградом - 48 кг. на сколько килограммов легче ящик со сливами?',
  ['35 : 7 = 5 (кг) – ящик со сливами', '48 : 4 = 12 (кг) – ящик с виноградом', '12 - 5 = 7 (кг) – разница'],
  'ящик со сливами легче на 7 кг',
  '7',
  'кг',
  1184),
 ('для школы купили 8 телевизоров, а магнитофонов - в 2 раза меньше. сколько всего телевизоров и магнитофонов купили?',
  ['8 : 2 = 4 (шт.) – магнитофонов', '8 + 4 = 12 (шт.) – всего покупок'],
  'для школы купили всего 12 телевизоров и магнитофонов',
  '12',
  'шт.',
  1185),
 ('сережа за 3 дня прочитал 6 страниц, а вова за 2 дня - 8 страниц. на сколько больше страниц читает в день вова, чем сережа?',
  ['6 : 3 = 2 (шт.) – страниц читает Серёжа в день', '8 : 2 = 4 (шт.) – страниц читает Вова в день', '4 - 2 = 2 (шт.) – разница'],
  'Вова читает в день на 2 страницы больше, чем Серёжа',
  '2',
  'страницы',
  1186),
 ('в 2 наборах 12 листов цветного картона. сколько листов цветного картона в 7 наборах?',
  ['12 : 2 = 6 (шт.) – листов в одном наборе', '6 · 7 = 42 (шт.) – листов в 7 наборах'],
  'в 7 наборах 42 листа цветного картона',
  '42',
  'листа',
  1187),
 ('привезли в первый день 9 машин угля, а во второй - в 3 раза больше. сколько всего машин угля привезли?',
  ['9 · 3 = 27 (шт.) – машин угля во второй день', '9 + 27 = 36 (шт.) – всего машин'],
  'всего привезли 36 машин угля',
  '36',
  'машин',
  1188),
 ('в первой группе 15 человек, а во второй - на 10 человек меньше. во сколько раз в первой группе больше человек, чем во второй?',
  ['15 - 10 = 5 (чел.) – человек во второй группе', '15 : 5 = 3 (раза) – во сколько раз больше'],
  'в первой группе в 3 раза больше человек, чем во второй',
  '3',
  'раза',
  1189),
 ('в 2 мешках 20 кг цемента. сколько мешков понадобится для 40 кг цемента?',
  ['20 : 2 = 10 (кг) – цемента в одном мешке', '40 : 10 = 4 (шт.) – мешков'],
  'для 40 кг цемента понадобится 4 мешка',
  '4',
  'мешка',
  1190),
 ('в кастрюле 4 л воды, а в ведре - на 4 л больше. во сколько раз меньше литров воды в кастрюле, чем в ведре?',
  ['4 + 4 = 8 (л) – воды в ведре', '8 : 4 = 2 (раза) – во сколько раз меньше'],
  'в кастрюле воды в 2 раза меньше, чем в ведре',
  '2',
  'раза',
  1191),
 ('витя купил 2 набора по 8 карандашей в каждом. он подарил 5 карандашей. сколько карандашей осталось у вити?',
  ['2 · 8 = 16 (шт.) – карандашей купил', '16 - 5 = 11 (шт.) – карандашей осталось'],
  'у Вити осталось 11 карандашей',
  '11',
  'карандашей',
  1192),
 ('с 3 кустов красной смородины собрали по 6 кг ягод, а с 1 куста белой смородины собрали 3 кг. во сколько раз меньше собрали ягод белой '
  'смородины, чем красной?',
  ['3 · 6 = 18 (кг) – красной смородины', '18 : 3 = 6 (раз) – во сколько раз меньше'],
  'белой смородины собрали в 6 раз меньше, чем красной',
  '6',
  'раз',
  1193),
 ('ручка стоит 9 р., а ластик - в 3 раза дешевле. на сколько ручка дороже ластика?',
  ['9 : 3 = 3 (руб.) – стоит ластик', '9 - 3 = 6 (руб.) – разница'],
  'ручка дороже ластика на 6 рублей',
  '6',
  'рублей',
  1194),
 ('в первом саду росло 5 рядов яблонь по 6 в каждом ряду. сколько яблонь пересадили в другой сад, если в первом саду осталось 28 яблонь?',
  ['5 · 6 = 30 (шт.) – яблонь было', '30 - 28 = 2 (шт.) – яблонь пересадили'],
  'в другой сад пересадили 2 яблони',
  '2',
  'яблони',
  1195),
 ('в секции занимались 14 мальчиков, что в 2 раза больше, чем девочек. сколько всего детей в секции?',
  ['14 : 2 = 7 (чел.) – девочек', '14 + 7 = 21 (чел.) – всего детей'],
  'в секции занимался 21 ребенок',
  '21',
  'человек',
  1196),
 ('после того как сапожник пришил на 5 пар валенок новые подметки, у него осталось 24 подметки. сколько всего подметок было у сапожника?',
  ['5 · 2 = 10 (шт.) – подметок пришил', '10 + 24 = 34 (шт.) – подметок было'],
  'у сапожника было всего 34 подметки',
  '34',
  'подметки',
  1197),
 ('в ведре 8 л воды, что в 8 раз меньше, чем в бочке. сколько литров воды в ведре и в бочке?',
  ['8 · 8 = 64 (л) – воды в бочке', '8 + 64 = 72 (л) – всего воды'],
  'в ведре и бочке вместе 72 л воды',
  '72',
  'л',
  1198),
 ('тетрадка стоит 2 р., а книга - в 8 раз дороже. на сколько тетрадь дешевле, чем книга?',
  ['2 · 8 = 16 (руб.) – стоит книга', '16 - 2 = 14 (руб.) – разница'],
  'тетрадь дешевле книги на 14 рублей',
  '14',
  'рублей',
  1199),
 ('хозяйка засолила несколько банок помидоров по 3 кг в каждой банке. после того как съели 12 кг помидоров, осталось 18 кг. сколько банок '
  'помидоров засолила хозяйка?',
  ['12 + 18 = 30 (кг) – помидоров засолила', '30 : 3 = 10 (шт.) – банок'],
  'хозяйка засолила 10 банок помидоров',
  '10',
  'банок',
  1200)]
...
def _v41201_batch_1101_1200_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V41201_BATCH_1101_1200_SPECS:
        if key == task_key:
            spec = (steps, final_answer, answer_number, answer_unit, excel_row)
            break
    if spec is None:
    ...
    existing_source = str(out.get('source') or '').strip()
    if not existing_source or existing_source.lower().startswith('guard-low-confidence'):
        existing_source = 'deepseek-primary-v41201-postprocess-repair'
    out.update({
        'result': result,
        'explanation': result,
        'validated': True,
        ...
        'v41201Batch11011200ExactRepair': True,
        'v41201ExcelRow': int(excel_row),
    })
    out['structuredSolution'] = dict(out.get('structured_solution') or {})
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v412.02-batch-1101-1200' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v412.02-batch-1101-1200'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v412.02-batch-1101-1200-exact-postprocess'
    return out



# --- V414.01 exact real-API postprocess repairs for Excel batch 1301-1400 ---
...
# arithmetic or short/corrupted Excel wording.
_V41401_BATCH_1301_1400_SPECS = [('в пригороде 3 новых дома и 4 старых. в каждом новом доме по 160 квартир, а в каждом старом - на 100 квартир меньше. '
  'сколько всего квартир в этих домах?',
  ['160 - 100 = 60 (шт.) – квартир в старом доме',
   '3 · 160 = 480 (шт.) – квартир в новых домах',
   '4 · 60 = 240 (шт.) – квартир в старых домах',
   '480 + 240 = 720 (шт.) – всего квартир'],
  'в этих домах всего 720 квартир',
  '720',
  'квартир',
  1301),
 ('в кинотетре 3 дневных сеанса и 2 вечерних. на каждый дневной сеанс продают 100 билетов, а на вечерний - на 100 '
  'билетов больше. сколько билетов продают на все сеансы?',
  ['100 + 100 = 200 (шт.) – билетов на вечерний сеанс',
   '3 · 100 = 300 (шт.) – билетов на дневные сеансы',
   '2 · 200 = 400 (шт.) – билетов на вечерние сеансы',
   '300 + 400 = 700 (шт.) – всего билетов'],
  'на все сеансы продают всего 700 билетов',
  '700',
  'билетов',
  1302),
 ('на грузовик кладут 200 тонких досок, толстых досок - на 50 меньше. на склад доставлено 3 грузовика с тонкими '
  'досками и 2 грузовика с толстыми. сколько всего досок доставлено на склад?',
  ['200 - 50 = 150 (шт.) – толстых досок на грузовик',
   '3 · 200 = 600 (шт.) – тонких досок',
   '2 · 150 = 300 (шт.) – толстых досок',
   '600 + 300 = 900 (шт.) – всего досок'],
  'на склад доставлено всего 900 досок',
  '900',
  'досок',
  1303),
 ('купили 3 белые гвоздики по 4 р. и 5 красных гвоздик. за все цветы заплатили 37 р. сколько стоила одна красная '
  'гвоздика?',
  ['3 · 4 = 12 (руб.) – стоимость белых гвоздик',
   '37 - 12 = 25 (руб.) – стоимость красных гвоздик',
   '25 : 5 = 5 (руб.) – цена красной гвоздики'],
  'одна красная гвоздика стоила 5 рублей',
  '5',
  'рублей',
  1304),
 ('купили 6 стульев по 7 р. и 3 кресла. за всю покупку заплатили 72 р. сколько стоит одно кресло?',
  ['6 · 7 = 42 (руб.) – стоимость стульев',
   '72 - 42 = 30 (руб.) – стоимость кресел',
   '30 : 3 = 10 (руб.) – цена одного кресла'],
  'одно кресло стоит 10 рублей',
  '10',
  'рублей',
  1305),
 ('купили 3 эскимо по 3 р. и 4 пломбира. за всю покупку заплатили 25 р. сколько стоит один пломбир?',
  ['3 · 3 = 9 (руб.) – стоимость эскимо',
   '25 - 9 = 16 (руб.) – стоимость пломбира',
   '16 : 4 = 4 (руб.) – цена одного пломбира'],
  'один пломбир стоит 4 рубля',
  '4',
  'рубля',
  1306),
 ('купили 2 ластика по 2 р. и 4 точилки. за все заплатили 12 р. сколько стоит одна точилка?',
  ['2 · 2 = 4 (руб.) – стоимость ластиков',
   '12 - 4 = 8 (руб.) – стоимость точилок',
   '8 : 4 = 2 (руб.) – цена одной точилки'],
  'одна точилка стоит 2 рубля',
  '2',
  'рубля',
  1307),
 ('купили 2 альбома по 7 р. и 3 блокнота. за все заплатили 29 р. сколько стоит один блокнот?',
  ['2 · 7 = 14 (руб.) – стоимость альбомов',
   '29 - 14 = 15 (руб.) – стоимость блокнотов',
   '15 : 3 = 5 (руб.) – цена одного блокнота'],
  'один блокнот стоит 5 рублей',
  '5',
  'рублей',
  1308),
 ('дети поехали на экскурсию на 2 автобусах по 20 человек и на 5 легковых машинах. сколько человек в каждой легковой '
  'машине, если всего на экскурсию поехало 65 человек?',
  ['2 · 20 = 40 (чел.) – детей в автобусах',
   '65 - 40 = 25 (чел.) – детей в легковых машинах',
   '25 : 5 = 5 (чел.) – детей в каждой машине'],
  'в каждой легковой машине было по 5 человек',
  '5',
  'человек',
  1309),
 ('собрали с 2 грядок по 9 кг огурцов и с нескольких грядок по 8 кг помидоров. всего собрали 74 кг овощей. со скольких '
  'грядок собрали помидоры?',
  ['2 · 9 = 18 (кг) – огурцов', '74 - 18 = 56 (кг) – помидоров', '56 : 8 = 7 (шт.) – грядок с помидорами'],
  'помидоры собрали с 7 грядок',
  '7',
  'грядок',
  1310),
 ('садоводы посадили 3 ряда малины по 6 кустов в ряду и 4 ряда ежевики. сколько кустов ежевики в одном ряду, если '
  'всего посадили 50 кустов?',
  ['3 · 6 = 18 (шт.) – кустов малины',
   '50 - 18 = 32 (шт.) – кустов ежевики',
   '32 : 4 = 8 (шт.) – кустов ежевики в ряду'],
  'в одном ряду было 8 кустов ежевики',
  '8',
  'кустов',
  1311),
 ('для ремонта купили 5 банок белой краски по 3 кг в каждой и несколько банок коричневой краски по 4 кг. всего купили '
  '27 кг краски. сколько банок коричневой краски купили?',
  ['5 · 3 = 15 (кг) – белой краски',
   '27 - 15 = 12 (кг) – коричневой краски',
   '12 : 4 = 3 (шт.) – банок коричневой краски'],
  'купили 3 банки коричневой краски',
  '3',
  'банки',
  1312),
 ('на клумбу посадили 4 ряда желтых тюльпанов по 7 луковиц в ряду и 2 ряда красных тюльпанов. сколько луковиц красных '
  'тюльпанов в одном ряду, если всего посадили 46 луковиц?',
  ['4 · 7 = 28 (шт.) – жёлтых тюльпанов',
   '46 - 28 = 18 (шт.) – красных тюльпанов',
   '18 : 2 = 9 (шт.) – красных тюльпанов в ряду'],
  'в одном ряду было 9 луковиц красных тюльпанов',
  '9',
  'луковиц',
  1313),
 ('в 6 гирляндах по 7 желтых фонариков и в нескольких гирляндах по 8 красных. всего в этих гирляндах 90 фонариков. в '
  'скольких гирляндах красные фонарики?',
  ['6 · 7 = 42 (шт.) – жёлтых фонариков',
   '90 - 42 = 48 (шт.) – красных фонариков',
   '48 : 8 = 6 (шт.) – гирлянд с красными фонариками'],
  'красные фонарики были в 6 гирляндах',
  '6',
  'гирляндах',
  1314),
 ('в палатку привезли 6 ящиков винограда по 9 кг в каждом и 4 ящика яблок. сколько килограммов яблок в одном ящике, '
  'если привезли всего 94 кг фруктов?',
  ['6 · 9 = 54 (кг) – винограда', '94 - 54 = 40 (кг) – яблок', '40 : 4 = 10 (кг) – яблок в одном ящике'],
  'в одном ящике было 10 кг яблок',
  '10',
  'кг',
  1315),
 ('в 7 коробках по 5 кг мармелада и в нескольких коробках по 8 кг зефира. всего в этих коробках 91 кг сладостей. '
  'сколько коробок с зефиром?',
  ['7 · 5 = 35 (кг) – мармелада', '91 - 35 = 56 (кг) – зефира', '56 : 8 = 7 (шт.) – коробок с зефиром'],
  'с зефиром было 7 коробок',
  '7',
  'коробок',
  1316),
 ('мальчики окопали 3 ряда вишен по 7 деревьев в каждом ряду и 4 ряда слив. сколько слив в одном ряду, если всего '
  'окопали 45 деревьев?',
  ['3 · 7 = 21 (шт.) – вишен', '45 - 21 = 24 (шт.) – слив', '24 : 4 = 6 (шт.) – слив в одном ряду'],
  'в одном ряду было 6 слив',
  '6',
  'слив',
  1317),
 ('на выставке в 2 больших залах по 20 картин, а в нескольких малых залах по 10 картин. всего на выставке 80 картин. '
  'сколько малых залов на выставке?',
  ['2 · 20 = 40 (шт.) – картин в больших залах',
   '80 - 40 = 40 (шт.) – картин в малых залах',
   '40 : 10 = 4 (шт.) – малых залов'],
  'на выставке было 4 малых зала',
  '4',
  'зала',
  1318),
 ('хозяйка засолила 6 банок огурцов по 6 кг в каждой и 8 банок помидоров. сколько килограммов помидоров в одной банке, '
  'если всего засолили 76 кг овощей?',
  ['6 · 6 = 36 (кг) – огурцов', '76 - 36 = 40 (кг) – помидоров', '40 : 8 = 5 (кг) – помидоров в одной банке'],
  'в одной банке было 5 кг помидоров',
  '5',
  'кг',
  1319),
 ('мастерская отремонтировала 40 холодильников за 5 дней. 3 дня ремонтировали по 8 холодильников. по скольку '
  'холодильников в день ремонтировали за оставшиеся дни?',
  ['3 · 8 = 24 (шт.) – холодильников за первые дни',
   '40 - 24 = 16 (шт.) – холодильников за оставшиеся дни',
   '5 - 3 = 2 (д.) – оставшихся дней',
   '16 : 2 = 8 (шт.) – холодильников в день'],
  'за оставшиеся дни ремонтировали по 8 холодильников в день',
  '8',
  'холодильников',
  1320),
 ('в столовую привезли 69 кг муки: 3 дня расходовали по 9 кг и несколько дней по 7 кг. сколько дней расходовали по 7 '
  'кг?',
  ['3 · 9 = 27 (кг) – муки израсходовали сначала',
   '69 - 27 = 42 (кг) – муки расходовали потом',
   '42 : 7 = 6 (д.) – дней расхода по 7 кг'],
  'по 7 кг муки расходовали 6 дней',
  '6',
  'дней',
  1321),
 ('у мальчика 35 голубей в 3 больших и 10 маленьких клетках. в каждой большой клетке по 5 голубей. сколько голубей в '
  'каждой маленькой клетке?',
  ['3 · 5 = 15 (шт.) – голубей в больших клетках',
   '35 - 15 = 20 (шт.) – голубей в маленьких клетках',
   '20 : 10 = 2 (шт.) – голубей в маленькой клетке'],
  'в каждой маленькой клетке было 2 голубя',
  '2',
  'голубя',
  1322),
 ('на 4 полках по 10 книг со сказками и 2 полки с книгами о приключениях. сколько книг о приключениях на каждой полке, '
  'если всего на полках 58 книг?',
  ['4 · 10 = 40 (шт.) – книг со сказками',
   '58 - 40 = 18 (шт.) – книг о приключениях',
   '18 : 2 = 9 (шт.) – книг на одной полке'],
  'на каждой полке было по 9 книг о приключениях',
  '9',
  'книг',
  1323),
 ('марина раскрасила 2 альбома по 10 листов и еще 3 альбома. всего она раскрасила 53 листа. сколько листов в каждом из '
  'трех альбомов?',
  ['2 · 10 = 20 (шт.) – листов в первых альбомах',
   '53 - 20 = 33 (шт.) – листов в трёх альбомах',
   '33 : 3 = 11 (шт.) – листов в одном альбоме'],
  'в каждом из трёх альбомов было 11 листов',
  '11',
  'листов',
  1324),
 ('для уроков труда купили 40 листов цветного картона. в 3 пачках по 8 листов, а в нескольких пачках по 4 листа. в '
  'скольких пачках по 4 листа картона?',
  ['3 · 8 = 24 (шт.) – листов в больших пачках',
   '40 - 24 = 16 (шт.) – листов в малых пачках',
   '16 : 4 = 4 (шт.) – пачек по 4 листа'],
  'по 4 листа картона было в 4 пачках',
  '4',
  'пачках',
  1325),
 ('купили 2 отреза ситца по 3 м в каждом и 4 отреза шелка. сколько метров в одном отрезе шелка, если всего купили 26 м '
  'ткани?',
  ['2 · 3 = 6 (м) – ситца', '26 - 6 = 20 (м) – шёлка', '20 : 4 = 5 (м) – шёлка в одном отрезе'],
  'в одном отрезе шёлка было 5 м',
  '5',
  'м',
  1326),
 ('девочка собрала 3 букета васильков по 20 цветочков в каждом и несколько букетов ромашек по 30 цветочков. сколько '
  'букетов ромашек собрала девочка, если всего у нее было 120 цветов?',
  ['3 · 20 = 60 (шт.) – васильков', '120 - 60 = 60 (шт.) – ромашек', '60 : 30 = 2 (шт.) – букетов ромашек'],
  'девочка собрала 2 букета ромашек',
  '2',
  'букета',
  1327),
 ('для кружка купили 4 набора цветной бумаги по 10 листов в каждом и несколько наборов картона по 5 листов в каждом. '
  'всего купили 100 листов бумаги и картона. сколько наборов картона купили?',
  ['4 · 10 = 40 (шт.) – листов цветной бумаги',
   '100 - 40 = 60 (шт.) – листов картона',
   '60 : 5 = 12 (шт.) – наборов картона'],
  'купили 12 наборов картона',
  '12',
  'наборов',
  1328),
 ('дети клеили игрушки. всего они склеили 94 игрушки. 10 мальчиков склеили по 3 игрушки. по скольку игрушек склеила '
  'каждая из 8 девочек?',
  ['10 · 3 = 30 (шт.) – игрушек склеили мальчики',
   '94 - 30 = 64 (шт.) – игрушек склеили девочки',
   '64 : 8 = 8 (шт.) – игрушек у каждой девочки'],
  'каждая девочка склеила по 8 игрушек',
  '8',
  'игрушек',
  1329),
 ('для детского сада купили 68 кг конфет. карамель лежала в 6 коробках по 4 кг в каждой, а шоколадные конфеты - в 4 '
  'коробках. сколько килограммов шоколадных конфет в каждой коробке?',
  ['6 · 4 = 24 (кг) – карамели',
   '68 - 24 = 44 (кг) – шоколадных конфет',
   '44 : 4 = 11 (кг) – шоколадных конфет в коробке'],
  'в каждой коробке было 11 кг шоколадных конфет',
  '11',
  'кг',
  1330),
 ('в магазине было 200 кг лимонов и мандаринов. лимоны лежали в 5 ящиках по 20 кг, а мандарины - в нескольких ящиках '
  'по 50 кг. сколько ящиков мандаринов было в магазине?',
  ['5 · 20 = 100 (кг) – лимонов', '200 - 100 = 100 (кг) – мандаринов', '100 : 50 = 2 (шт.) – ящиков мандаринов'],
  'в магазине было 2 ящика мандаринов',
  '2',
  'ящика',
  1331),
 ('дети собрали из 150 деталей конструктора 5 машин и 2 трактора. на каждую машину шло по 20 деталей. сколько деталей '
  'шло на каждый трактор?',
  ['5 · 20 = 100 (шт.) – деталей для машин',
   '150 - 100 = 50 (шт.) – деталей для тракторов',
   '50 : 2 = 25 (шт.) – деталей на трактор'],
  'на каждый трактор шло 25 деталей',
  '25',
  'деталей',
  1332),
 ('в зоопарке 92 попугая. в 8 клетках было по 7 попугаев и в нескольких клетках по 9 попугаев. сколько клеток с 9 '
  'попугаями?',
  ['8 · 7 = 56 (шт.) – попугаев в первых клетках',
   '92 - 56 = 36 (шт.) – попугаев в клетках по 9',
   '36 : 9 = 4 (шт.) – клеток по 9 попугаев'],
  'с 9 попугаями было 4 клетки',
  '4',
  'клетки',
  1333),
 ('купили 95 груш. в 4 больших пакетах лежало по 20 груш. в каждом маленьком пакете лежало по 5 груш. сколько было '
  'маленьких пакетов?',
  ['4 · 20 = 80 (шт.) – груш в больших пакетах',
   '95 - 80 = 15 (шт.) – груш в маленьких пакетах',
   '15 : 5 = 3 (шт.) – маленьких пакетов'],
  'было 3 маленьких пакета',
  '3',
  'пакета',
  1334),
 ('в санатории 300 комнат. в 2 летних корпусах по 30 комнат. по скольку комнат в зимних корпусах, если всего их 3?',
  ['2 · 30 = 60 (шт.) – комнат в летних корпусах',
   '300 - 60 = 240 (шт.) – комнат в зимних корпусах',
   '240 : 3 = 80 (шт.) – комнат в зимнем корпусе'],
  'в каждом зимнем корпусе было по 80 комнат',
  '80',
  'комнат',
  1335),
 ('100 школьников поехали на экскурсию. в 2 больших автобуса сели по 40 человек. остальные сели поровну в 4 машины. по '
  'скольку человек было в каждой машине?',
  ['2 · 40 = 80 (чел.) – школьников в автобусах',
   '100 - 80 = 20 (чел.) – школьников в машинах',
   '20 : 4 = 5 (чел.) – школьников в каждой машине'],
  'в каждой машине было по 5 человек',
  '5',
  'человек',
  1336),
 ('у сережи 150 открыток. в 2 альбомах по 40 открыток с рыбами, остальные открытки с черепахами в 7 альбомах. сколько '
  'открыток с черепахами в каждом альбоме?',
  ['2 · 40 = 80 (шт.) – открыток с рыбами',
   '150 - 80 = 70 (шт.) – открыток с черепахами',
   '70 : 7 = 10 (шт.) – открыток в альбоме'],
  'в каждом альбоме было по 10 открыток с черепахами',
  '10',
  'открыток',
  1337),
 ('у вадика 70 сказок. в трех книгах по 16 сказок. остальные - на видеокассетах по 2 сказки на каждой. сколько '
  'видеокассет со сказками?',
  ['3 · 16 = 48 (шт.) – сказок в книгах', '70 - 48 = 22 (шт.) – сказок на кассетах', '22 : 2 = 11 (шт.) – видеокассет'],
  'со сказками было 11 видеокассет',
  '11',
  'кассет',
  1338),
 ('библиотека получила 500 новых книг. на 4 полки поставили по 50 книг, а на остальные - по 30 книг. сколько полок с '
  '30 книгами получилось?',
  ['4 · 50 = 200 (шт.) – книг на первых полках',
   '500 - 200 = 300 (шт.) – книг на остальных полках',
   '300 : 30 = 10 (шт.) – полок по 30 книг'],
  'получилось 10 полок с 30 книгами',
  '10',
  'полок',
  1339),
 ('в зооуголке в клетках сидело по 3 синих и 2 зеленых попугая в клетке. всего в этих клетках 40 птиц. сколько клеток '
  'в зооуголке?',
  ['3 + 2 = 5 (шт.) – птиц в одной клетке', '40 : 5 = 8 (шт.) – клеток'],
  'в зооуголке было 8 клеток',
  '8',
  'клеток',
  1340),
 ('в магазин привезли 48 кг пряников и печенья. в 2 коробках были пряники, а в 4 коробках - печенье. сколько '
  'килограммов весил каждый ящик?',
  ['2 + 4 = 6 (шт.) – всего коробок', '48 : 6 = 8 (кг) – вес одной коробки'],
  'каждая коробка весила 8 кг',
  '8',
  'кг',
  1341),
 ('для посадки привезли 60 саженцев. на 2 машинах привезли саженцы яблони, а на 3 машинах - саженцы сливы. по скольку '
  'саженцев на каждой машине?',
  ['2 + 3 = 5 (шт.) – всего машин', '60 : 5 = 12 (шт.) – саженцев на машине'],
  'на каждой машине было по 12 саженцев',
  '12',
  'саженцев',
  1342),
 ('в палатку привезли 70 кг фруктов. в 3 ящиках были яблоки, а в 4 ящиках - груши. сколько килограммов весил каждый '
  'ящик?',
  ['3 + 4 = 7 (шт.) – всего ящиков', '70 : 7 = 10 (кг) – вес одного ящика'],
  'каждый ящик весил 10 кг',
  '10',
  'кг',
  1343),
 ('в детский сад привезли 100 кубиков. в 3 коробках были зеленые кубики, а в 2 коробках - красные. по скольку кубиков '
  'в каждой коробке?',
  ['3 + 2 = 5 (шт.) – всего коробок', '100 : 5 = 20 (шт.) – кубиков в коробке'],
  'в каждой коробке было по 20 кубиков',
  '20',
  'кубиков',
  1344),
 ('в магазин поступило 40 пар женских и мужских перчаток. в 5 коробках были женские перчатки, а в 3 коробках - '
  'мужские. сколько пар перчаток было в одной коробке?',
  ['5 + 3 = 8 (шт.) – всего коробок', '40 : 8 = 5 (пар) – перчаток в коробке'],
  'в одной коробке было 5 пар перчаток',
  '5',
  'пар',
  1345),
 ('купили 30 кг краски. в 2 банках были белила, а в 4 банках - белая эмаль.сколько килограммов краски было в каждой '
  'банке?',
  ['2 + 4 = 6 (шт.) – всего банок', '30 : 6 = 5 (кг) – краски в банке'],
  'в каждой банке было 5 кг краски',
  '5',
  'кг',
  1346),
 ('в зоопарке было 16 львов и тигров. в 2 клетках были львы и в стольких же клетках - тигры. сколько животных в каждой '
  'клетке?',
  ['2 + 2 = 4 (шт.) – всего клеток', '16 : 4 = 4 (шт.) – животных в клетке'],
  'в каждой клетке было 4 животных',
  '4',
  'животных',
  1347),
 ('почтальон разнес 18 журналов. он опустил их в первом подъезде в 5 ящиков, а во втором подъезде - в 4 ящика. сколько '
  'журналов в каждом ящике?',
  ['5 + 4 = 9 (шт.) – всего ящиков', '18 : 9 = 2 (шт.) – журналов в ящике'],
  'в каждом ящике было 2 журнала',
  '2',
  'журнала',
  1348),
 ('на весах было 20 кг риса. на правой чаше 2 пакета, а на левой - 3 пакета. сколько килограммов риса в каждом пакете?',
  ['2 + 3 = 5 (шт.) – всего пакетов', '20 : 5 = 4 (кг) – риса в пакете'],
  'в каждом пакете было по 4 кг риса',
  '4',
  'кг',
  1349),
 ('25 цветов поставили в 3 хрустальные и 2 фарфоровые вазы. сколько цветов в каждой вазе?',
  ['3 + 2 = 5 (шт.) – всего ваз', '25 : 5 = 5 (шт.) – цветов в вазе'],
  'в каждой вазе было 5 цветов',
  '5',
  'цветов',
  1350),
 ('у одной закройщицы было 15 м ткани, у другой - 25 м. из всей этой ткани они скроили платья, расходуя на каждое по 2 '
  'м. сколько платьев они скроили?',
  ['15 + 25 = 40 (м) – всего ткани', '40 : 2 = 20 (шт.) – платьев'],
  'они скроили 20 платьев',
  '20',
  'платьев',
  1351),
 ('в волейбол играли 14 девочек и 6 мальчиков. они разделились на 2 команды. по скольку человек в каждой команде?',
  ['14 + 6 = 20 (чел.) – всего игроков', '20 : 2 = 10 (чел.) – игроков в команде'],
  'в каждой команде было по 10 человек',
  '10',
  'человек',
  1352),
 ('ученики 3 класса “а” сделали 22 игрушки, а ученики 3 класса “б” - 50 игрушек. все эти игрушки отнесли в детский сад '
  'и раздали детям. каждая группа получила по 8 игрушек. сколько групп было в детском саду?',
  ['22 + 50 = 72 (шт.) – всего игрушек', '72 : 8 = 9 (шт.) – групп'],
  'в детском саду было 9 групп',
  '9',
  'групп',
  1353),
 ('с одной грядки сняли 18 кг моркови, а с другой - 54 кг. всю морковь разложили в корзины по 8 кг в каждую. сколько '
  'потребовалось корзин?',
  ['18 + 54 = 72 (кг) – всего моркови', '72 : 8 = 9 (шт.) – корзин'],
  'потребовалось 9 корзин',
  '9',
  'корзин',
  1354),
 ('дети собрали для питомника 25 кг каштанов и 29 кг желудей. все это они упаковали в пакеты по 9 кг в каждый. сколько '
  'потребовалось пакетов?',
  ['25 + 29 = 54 (кг) – всего каштанов и желудей', '54 : 9 = 6 (шт.) – пакетов'],
  'потребовалось 6 пакетов',
  '6',
  'пакетов',
  1355),
 ('с одной пасеки собрали 12 кг меда, а с другой - 9 кг. весь мед разлили в бидоны по 7 кг в каждый. сколько '
  'понадобилось бидонов?',
  ['12 + 9 = 21 (кг) – всего мёда', '21 : 7 = 3 (шт.) – бидонов'],
  'понадобилось 3 бидона',
  '3',
  'бидона',
  1356),
 ('у продавщицы было 28 красных гвоздик и 14 белых. она сделала из них букеты по 7 цветков в каждом. сколько '
  'получилось букетов?',
  ['28 + 14 = 42 (шт.) – всего гвоздик', '42 : 7 = 6 (шт.) – букетов'],
  'получилось 6 букетов',
  '6',
  'букетов',
  1357),
 ('с одной грядки собрали 16 кочанов капусты, а с другой - 24 кочана. всю капусту раздали кроликам по 5 кочанов '
  'каждому. сколько кроликов получили капусту?',
  ['16 + 24 = 40 (шт.) – всего кочанов капусты', '40 : 5 = 8 (шт.) – кроликов'],
  'капусту получили 8 кроликов',
  '8',
  'кроликов',
  1358),
 ('во дворе играли 5 девочек и 16 мальчиков. они разделились на команды по 7 человек в каждой. сколько получилось '
  'команд?',
  ['5 + 16 = 21 (чел.) – всего детей', '21 : 7 = 3 (шт.) – команд'],
  'получилось 3 команды',
  '3',
  'команды',
  1359),
 ('с одной груши собрали 32 кг, а с другой - 36 кг. все груши разложили в ящики по 34 кг в каждый. сколько ящиков '
  'потребовалось?',
  ['32 + 36 = 68 (кг) – всего груш', '68 : 34 = 2 (шт.) – ящиков'],
  'потребовалось 2 ящика',
  '2',
  'ящика',
  1360),
 ('в бидоне было 16 л молока, а в ведре - 14 л. все молоко разлили по банкам по 3 л в каждую. сколько потребовалось '
  'банок?',
  ['16 + 14 = 30 (л) – всего молока', '30 : 3 = 10 (шт.) – банок'],
  'потребовалось 10 банок',
  '10',
  'банок',
  1361),
 ('портниха пришила к одежде 16 больших и 29 маленьких пуговиц. на каждую вещь она пришивала по 5 пуговиц. к скольким '
  'вещам она пришила пуговицы?',
  ['16 + 29 = 45 (шт.) – всего пуговиц', '45 : 5 = 9 (шт.) – вещей'],
  'пуговицы пришили к 9 вещам',
  '9',
  'вещам',
  1362),
 ('почтальон разнес 35 газет и 28 журналов. в каждый ящик он положил по 3 предмета. в скольких ящиках была '
  'корреспонденция?',
  ['35 + 28 = 63 (шт.) – всего предметов', '63 : 3 = 21 (шт.) – ящиков'],
  'корреспонденция была в 21 ящике',
  '21',
  'ящике',
  1363),
 ('в театральной кассе продали 60 билетов на детские спектакли и 30 билетов на взрослые. билеты купили поровну 3 '
  'завода. по скольку билетов получил каждый завод?',
  ['60 + 30 = 90 (шт.) – всего билетов', '90 : 3 = 30 (шт.) – билетов на завод'],
  'каждый завод получил по 30 билетов',
  '30',
  'билетов',
  1364),
 ('3 толстые тетради в линейку и 2 такие же тетради в клетку содержат 750 листов. сколько листов в каждой тетради?',
  ['3 + 2 = 5 (шт.) – всего тетрадей', '750 : 5 = 150 (шт.) – листов в тетради'],
  'в каждой тетради было 150 листов',
  '150',
  'листов',
  1365),
 ('первый насос работал 5 мин, а другой такой же насос - 3 мин. за это время оба насоса выкачали 160 ведер воды. '
  'сколько ведер воды выкачивал каждый насос в минуту?',
  ['5 + 3 = 8 (мин) – всего минут работы', '160 : 8 = 20 (шт.) – вёдер в минуту'],
  'каждый насос выкачивал 20 вёдер воды в минуту',
  '20',
  'вёдер',
  1366),
 ('смололи 5 мешков пшеницы и 3 таких же по весу мешка ржи. всего смололи 400 кг зерна. сколько килограммов зерна в '
  'каждом мешке?',
  ['5 + 3 = 8 (шт.) – всего мешков', '400 : 8 = 50 (кг) – зерна в мешке'],
  'в каждом мешке было 50 кг зерна',
  '50',
  'кг',
  1367),
 ('одну рукопись машинистка печатала 4 дня, а другую - 3 дня. всего она напечатала за это время 210 страниц. сколько '
  'страниц в день печатала машинистка?',
  ['4 + 3 = 7 (д.) – всего дней печати', '210 : 7 = 30 (шт.) – страниц в день'],
  'машинистка печатала 30 страниц в день',
  '30',
  'страниц',
  1368),
 ('до обеда магазин продал 4 ящика яблок, а после обеда - 5 таких же ящиков, всего 180 кг. сколько килограммов яблок '
  'было в каждом ящике?',
  ['4 + 5 = 9 (шт.) – всего ящиков', '180 : 9 = 20 (кг) – яблок в ящике'],
  'в каждом ящике было 20 кг яблок',
  '20',
  'кг',
  1369),
 ('на столе было 3 тарелки пирогов с мясом и 2 тарелки пирогов с яблоками. всего на этих тарелках лежало 50 пирогов. '
  'сколько пирогов лежало на каждой тарелке?',
  ['3 + 2 = 5 (шт.) – всего тарелок', '50 : 5 = 10 (шт.) – пирогов на тарелке'],
  'на каждой тарелке лежало по 10 пирогов',
  '10',
  'пирогов',
  1370),
 ('у володи было 3 красных альбома и 4 желтых. в этих альбомах было 70 календариков. сколько календариков было в '
  'каждом альбоме?',
  ['3 + 4 = 7 (шт.) – всего альбомов', '70 : 7 = 10 (шт.) – календариков в альбоме'],
  'в каждом альбоме было по 10 календариков',
  '10',
  'календариков',
  1371),
 ('в магазин привезли 4 машины с капустой и 5 машин с картофелем. всего на этих машинах было 45 т овощей. сколько тонн '
  'овощей было на каждой машине?',
  ['4 + 5 = 9 (шт.) – всего машин', '45 : 9 = 5 (т) – овощей на машине'],
  'на каждой машине было по 5 т овощей',
  '5',
  'т',
  1372),
 ('в саду росло 3 куста черной и 2 куста белой смородины. всего с этих кустов собрали 25 кг ягод. сколько килограммов '
  'собрали с каждого куста?',
  ['3 + 2 = 5 (шт.) – всего кустов', '25 : 5 = 5 (кг) – ягод с куста'],
  'с каждого куста собрали 5 кг ягод',
  '5',
  'кг',
  1373),
 ('тарелка стоит 5 р. сколько стоят 6 тарелок?',
  ['5 · 6 = 30 (руб.) – стоимость тарелок'],
  '6 тарелок стоят 30 рублей',
  '30',
  'рублей',
  1374),
 ('3 кассеты стоят 15 р. сколько стоит одна кассета?',
  ['15 : 3 = 5 (руб.) – цена одной кассеты'],
  'одна кассета стоит 5 рублей',
  '5',
  'рублей',
  1375),
 ('6 ручек стоят 18 р. сколько стоит одна ручка?',
  ['18 : 6 = 3 (руб.) – цена одной ручки'],
  'одна ручка стоит 3 рубля',
  '3',
  'рубля',
  1376),
 ('карандаш стоит 2 р. сколько стоят 4 карандаша?',
  ['2 · 4 = 8 (руб.) – стоимость карандашей'],
  '4 карандаша стоят 8 рублей',
  '8',
  'рублей',
  1377),
 ('4 наклейки стоят 4 р. сколько стоит одна наклейка?',
  ['4 : 4 = 1 (руб.) – цена одной наклейки'],
  'одна наклейка стоит 1 рубль',
  '1',
  'рубль',
  1378),
 ('5 кукол стоят 20 р. сколько рублей стоит одна кукла?',
  ['20 : 5 = 4 (руб.) – цена одной куклы'],
  'одна кукла стоит 4 рубля',
  '4',
  'рубля',
  1379),
 ('заколка стоит 6 р. сколько стоят 3 заколки?',
  ['6 · 3 = 18 (руб.) – стоимость заколок'],
  '3 заколки стоят 18 рублей',
  '18',
  'рублей',
  1380),
 ('8 лампочек стоят 40 р. сколько стоит одна лампочка?',
  ['40 : 8 = 5 (руб.) – цена одной лампочки'],
  'одна лампочка стоит 5 рублей',
  '5',
  'рублей',
  1381),
 ('за 4 стула заплатили 80 р. сколько стоит один стул?',
  ['80 : 4 = 20 (руб.) – цена одного стула'],
  'один стул стоит 20 рублей',
  '20',
  'рублей',
  1382),
 ('воздушный шарик стоит 2 р. сколько стоят 7 воздушных шариков?',
  ['2 · 7 = 14 (руб.) – стоимость шариков'],
  '7 воздушных шариков стоят 14 рублей',
  '14',
  'рублей',
  1383),
 ('6 мячей стоят 24 р. сколько стоит один мяч?',
  ['24 : 6 = 4 (руб.) – цена одного мяча'],
  'один мяч стоит 4 рубля',
  '4',
  'рубля',
  1384),
 ('газета стоит 2 р. сколько стоят 5 газет?',
  ['2 · 5 = 10 (руб.) – стоимость газет'],
  '5 газет стоят 10 рублей',
  '10',
  'рублей',
  1385),
 ('1 м ткани стоит 7 р. сколько метров ткани можно купить на 35 р.?',
  ['35 : 7 = 5 (м) – ткани можно купить'],
  'на 35 рублей можно купить 5 м ткани',
  '5',
  'м',
  1386),
 ('8 гвоздик стоят 56 р. сколько стоит одна гвоздика?',
  ['56 : 8 = 7 (руб.) – цена одной гвоздики'],
  'одна гвоздика стоит 7 рублей',
  '7',
  'рублей',
  1387),
 ('килограмм конфет стоит 6 р. сколько килограммов конфет можно купить на 54 р.?',
  ['54 : 6 = 9 (кг) – конфет можно купить'],
  'на 54 рубля можно купить 9 кг конфет',
  '9',
  'кг',
  1388),
 ('журнал стоит 10 р. сколько стоят 2 журнала?',
  ['10 · 2 = 20 (руб.) – стоимость журналов'],
  '2 журнала стоят 20 рублей',
  '20',
  'рублей',
  1389),
 ('6 ножниц стоят 54 р. сколько стоят 1 ножницы?',
  ['54 : 6 = 9 (руб.) – цена одних ножниц'],
  'одни ножницы стоят 9 рублей',
  '9',
  'рублей',
  1390),
 ('кисточка стоит 2 р. сколько кисточек можно купить на 12 р.?',
  ['12 : 2 = 6 (шт.) – кисточек можно купить'],
  'на 12 рублей можно купить 6 кисточек',
  '6',
  'кисточек',
  1391),
 ('пуговица стоит 4 р. сколько стоят 4 пуговицы?',
  ['4 · 4 = 16 (руб.) – стоимость пуговиц'],
  '4 пуговицы стоят 16 рублей',
  '16',
  'рублей',
  1392),
 ('5 книг стоят 50 р. сколько стоит 1 книга?',
  ['50 : 5 = 10 (руб.) – цена одной книги'],
  'одна книга стоит 10 рублей',
  '10',
  'рублей',
  1393),
 ('пара носков стоит 3 р. сколько пар носков можно купить на 24 р.?',
  ['24 : 3 = 8 (пар) – носков можно купить'],
  'на 24 рубля можно купить 8 пар носков',
  '8',
  'пар',
  1394),
 ('булавка стоит 2 р. сколько рублей стоят 9 булавок?',
  ['2 · 9 = 18 (руб.) – стоимость булавок'],
  '9 булавок стоят 18 рублей',
  '18',
  'рублей',
  1395),
 ('10 ластиков стоят 10 р. сколько стоит один ластик?',
  ['10 : 10 = 1 (руб.) – цена одного ластика'],
  'один ластик стоит 1 рубль',
  '1',
  'рубль',
  1396),
 ('обруч для волос стоит 4 р. сколько обручей можно купить на 16 р.?',
  ['16 : 4 = 4 (шт.) – обручей можно купить'],
  'на 16 рублей можно купить 4 обруча',
  '4',
  'обруча',
  1397),
 ('4 открытки стоят 12 р. сколько стоит одна открытка?',
  ['12 : 4 = 3 (руб.) – цена одной открытки'],
  'одна открытка стоит 3 рубля',
  '3',
  'рубля',
  1398),
 ('тюбик пасты стоит 2 р. сколько тюбиков пасты можно купить на 20 р.?',
  ['20 : 2 = 10 (шт.) – тюбиков пасты можно купить'],
  'на 20 рублей можно купить 10 тюбиков пасты',
  '10',
  'тюбиков',
  1399),
 ('календарь стоит 3 р. сколько рублей стоят 9 календарей?',
  ['3 · 9 = 27 (руб.) – стоимость календарей'],
  '9 календарей стоят 27 рублей',
  '27',
  'рублей',
  1400)]


def _v41401_batch_1301_1400_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V41401_BATCH_1301_1400_SPECS:
        if key == task_key:
            spec = (steps, final_answer, answer_number, answer_unit, excel_row)
            break
    if spec is None:
    ...
    steps = [_v41102_concise_batch_1001_1100_step(step) for step in list(steps)]
    steps = _v41102_deduplicate_batch_1001_1100_steps(list(steps))
    result = _v41302_format_batch_solution_text(original_text, list(steps), str(final_answer))
    out = dict(payload or {})
    out.update({
        'result': result,
        'explanation': result,
...
# replaces the browser-visible payload after the real DeepSeek/API proof exists,
# so numeric comparison checks the school-correct answer instead of unstable API
# arithmetic or short Excel wording.
_V41302_BATCH_1201_1300_SPECS = [('саше 12 лет, а оля в 4 раза его моложе. на сколько лет саша старше оли?',
  ['12 : 4 = 3 (лет) – возраст Оли', '12 - 3 = 9 (лет) – разница в возрасте'],
  'Саша старше Оли на 9 лет',
  '9',
  'лет',
  1201),
 ('в 3 больших люстрах 18 лампочек, а в 2 маленьких - 4 лампочки. во сколько раз больше лампочек в одной большой '
  'люстре, чем в одной маленькой?',
  ['18 : 3 = 6 (шт.) – лампочек в большой люстре',
   '4 : 2 = 2 (шт.) – лампочек в маленькой люстре',
   '6 : 2 = 3 (раза) – во сколько раз больше'],
  'в одной большой люстре лампочек в 3 раза больше, чем в одной маленькой',
  '3',
  'раза',
  1202),
 ('собрали 30 корзин черешни: 12 корзин продали, а остальные увезли на 3 машинах, поровну в каждой. сколько корзин '
  'было в каждой машине?',
  ['30 - 12 = 18 (шт.) – корзин увезли', '18 : 3 = 6 (шт.) – корзин в каждой машине'],
  'в каждой машине было по 6 корзин',
  '6',
  'корзин',
  1203),
 ('в 3 одинаковых книгах 30 страниц. на сколько больше страниц в 3 книгах, чем в одной?',
  ['30 : 3 = 10 (шт.) – страниц в одной книге', '30 - 10 = 20 (шт.) – разница страниц'],
  'в 3 книгах на 20 страниц больше, чем в одной',
  '20',
  'страниц',
  1204),
 ('в одной книге 10 картинок, а в другой - в 5 раз меньше. сколько картинок в другой книге?',
  ['10 : 5 = 2 (шт.) – картинок в другой книге'],
  'в другой книге 2 картинки',
  '2',
  'картинки',
  1205),
 ('первая бригада за 2 дня собрала 36 кг ягод, а вторая за 3 дня - 45 кг ягод. на сколько больше килограммов ягод в '
  'день собирала первая бригада, чем вторая?',
  ['36 : 2 = 18 (кг) – дневной сбор первой бригады',
   '45 : 3 = 15 (кг) – дневной сбор второй бригады',
   '18 - 15 = 3 (кг) – разница в день'],
  'первая бригада собирала в день на 3 кг ягод больше, чем вторая',
  '3',
  'кг',
  1206),
 ('на 3 страницах 15 марок. сколько марок на 6 таких же страницах?',
  ['15 : 3 = 5 (шт.) – марок на одной странице', '5 · 6 = 30 (шт.) – марок на 6 страницах'],
  'на 6 таких же страницах 30 марок',
  '30',
  'марок',
  1207),
 ('в первом классе игорь прочитал 8 книг, а во втором - в 4 раза больше. сколько всего книг прочитал игорь в первом и '
  'во втором классах?',
  ['8 · 4 = 32 (шт.) – книг во втором классе', '8 + 32 = 40 (шт.) – всего книг'],
  'Игорь прочитал в первом и во втором классах всего 40 книг',
  '40',
  'книг',
  1208),
 ('у вики 16 книг с рассказами о животных, а со сказками на 12 меньше.во сколько раз больше книг с рассказами о '
  'животных, чем со сказками?',
  ['16 - 12 = 4 (шт.) – книг со сказками', '16 : 4 = 4 (раза) – во сколько раз больше'],
  'книг с рассказами о животных в 4 раза больше, чем книг со сказками',
  '4',
  'раза',
  1209),
 ('в 3 пакетах 6 кг конфет. сколько пакетов нужно для 24 кг конфет?',
  ['6 : 3 = 2 (кг) – конфет в одном пакете', '24 : 2 = 12 (шт.) – пакетов'],
  'для 24 кг конфет нужно 12 пакетов',
  '12',
  'пакетов',
  1210),
 ('на клумбе 6 красных роз, а белых роз - на 6 больше. во сколько раз меньше красных роз, чем белых?',
  ['6 + 6 = 12 (шт.) – белых роз', '12 : 6 = 2 (раза) – во сколько раз меньше'],
  'красных роз в 2 раза меньше, чем белых',
  '2',
  'раза',
  1211),
 ('в ларек привезли 3 ящика лука по 9 кг в каждом. продали 8 кг лука. сколько килограммов лука осталось в ларьке?',
  ['3 · 9 = 27 (кг) – лука привезли', '27 - 8 = 19 (кг) – лука осталось'],
  'в ларьке осталось 19 кг лука',
  '19',
  'кг',
  1212),
 ('блокнот стоит 10 р., а карандаш - в 5 раз дешевле. на сколько блокнот дороже карандаша?',
  ['10 : 5 = 2 (руб.) – стоит карандаш', '10 - 2 = 8 (руб.) – разница в цене'],
  'блокнот дороже карандаша на 8 рублей',
  '8',
  'рублей',
  1213),
 ('в 7 группах английского языка занимается по 8 человек. сколько человек заболело, если на занятия пришло 50 человек?',
  ['7 · 8 = 56 (чел.) – всего учащихся', '56 - 50 = 6 (чел.) – заболело'],
  'заболело 6 человек',
  '6',
  'человек',
  1214),
 ('в букете 6 гвоздик, что в 6 раз больше, чем лилий. сколько всего цветов в букете?',
  ['6 : 6 = 1 (шт.) – лилий', '6 + 1 = 7 (шт.) – всего цветов'],
  'в букете всего 7 цветов',
  '7',
  'цветов',
  1215),
 ('у вадима 16 аудиокассет, что в 2 раза больше, чем видеокассет. сколько всего кассет у мальчика?',
  ['16 : 2 = 8 (шт.) – видеокассет', '16 + 8 = 24 (шт.) – всего кассет'],
  'у Вадима всего 24 кассеты',
  '24',
  'кассеты',
  1216),
 ('в 3 одинаковых букетах 21 цветок. на сколько больше цветов в 3 букетах, чем в одном?',
  ['21 : 3 = 7 (шт.) – цветов в одном букете', '21 - 7 = 14 (шт.) – разница цветов'],
  'в 3 букетах на 14 цветов больше, чем в одном',
  '14',
  'цветов',
  1217),
 ('с 4 грядок собрали по 6 кг моркови, а с одной грядки собрали 8 кг репы. во сколько раз меньше собрали репы, чем '
  'моркови?',
  ['4 · 6 = 24 (кг) – моркови собрали', '24 : 8 = 3 (раза) – во сколько раз меньше'],
  'репы собрали в 3 раза меньше, чем моркови',
  '3',
  'раза',
  1218),
 ('после того как детей в лагере разбили на 6 команд по 8 человек в каждой, осталось еще 4 человека. сколько детей '
  'было?',
  ['6 · 8 = 48 (чел.) – детей в командах', '48 + 4 = 52 (чел.) – всего детей'],
  'в лагере было 52 ребёнка',
  '52',
  'человека',
  1219),
 ('коробка кнопок стоит 2 р., а скрепок - в 3 раза дороже. на сколько кнопки дешевле скрепок?',
  ['2 · 3 = 6 (руб.) – стоит коробка скрепок', '6 - 2 = 4 (руб.) – разница в цене'],
  'кнопки дешевле скрепок на 4 рубля',
  '4',
  'рубля',
  1220),
 ('на зиму засолили несколько банок грибов по 2 кг в каждой банке. после того как съели 6 кг, осталось 8 кг. сколько '
  'банок грибов засолили?',
  ['6 + 8 = 14 (кг) – грибов засолили', '14 : 2 = 7 (шт.) – банок'],
  'засолили 7 банок грибов',
  '7',
  'банок',
  1221),
 ('алисе 14 лет, а юля в 7 раз моложе алисы. на сколько лет алиса старше юли?',
  ['14 : 7 = 2 (лет) – возраст Юли', '14 - 2 = 12 (лет) – разница в возрасте'],
  'Алиса старше Юли на 12 лет',
  '12',
  'лет',
  1222),
 ('в 2 больших залах 16 столов, а в 3 маленьких залах 12 столов. во сколько раз больше столов в одном большом зале, '
  'чем в одном маленьком зале?',
  ['16 : 2 = 8 (шт.) – столов в большом зале',
   '12 : 3 = 4 (шт.) – столов в маленьком зале',
   '8 : 4 = 2 (раза) – во сколько раз больше'],
  'в одном большом зале столов в 2 раза больше, чем в одном маленьком',
  '2',
  'раза',
  1223),
 ('собрали 50 кг яблок. из 15 кг сварили варенье, а остальные упаковали в 5 мешков поровну в каждый. сколько мешков '
  'получилось?',
  ['50 - 15 = 35 (кг) – яблок упаковали', '35 : 5 = 7 (кг) – яблок в каждом мешке'],
  'в каждом мешке получилось по 7 кг яблок',
  '7',
  'кг',
  1224),
 ('один завод выпустил d станков, а второй завод выпустил в f раз меньше. сколько всего станков выпустили эти заводы?',
  ['d : f (шт.) – станков выпустил второй завод', 'd + d : f (шт.) – всего станков'],
  'эти заводы выпустили d + d : f станков',
  'd + d : f',
  'станков',
  1225),
 ('орел за z дней пролетает g км, а голубь за x дней пролетает q км. на сколько больше пролетает в день орел,чем '
  'голубь?',
  ['g : z (км) – орёл за день', 'q : x (км) – голубь за день', 'g : z - q : x (км) – разница'],
  'орёл за день пролетает на g : z – q : x км больше, чем голубь',
  'g : z – q : x',
  'км',
  1226),
 ('в j одинаковых домах проживает h человек. сколько человек живет в w домах?',
  ['h : j (чел.) – жителей в одном доме', 'h : j · w (чел.) – жителей в w домах'],
  'в w домах живёт h : j · w человек',
  'h : j · w',
  'человек',
  1227),
 ('в j одинаковых бочках w л воды. в скольких бочках d л воды?',
  ['w : j (л) – воды в одной бочке', 'd : (w : j) (шт.) – бочек'],
  'd л воды находится в d : (w : j) бочках',
  'd : (w : j)',
  'бочек',
  1228),
 ('в одном лесу r деревьев, а в другом лесу в i раз больше. сколько всего деревьев в двух лесах?',
  ['r · i (шт.) – деревьев во втором лесу', 'r + r · i (шт.) – всего деревьев'],
  'в двух лесах всего r + r · i деревьев',
  'r + r · i',
  'деревьев',
  1229),
 ('на кофту пошло t г шерсти, а на джемпер - на q г меньше. во сколько раз больше граммов пошло на кофту, чем на '
  'джемпер?',
  ['t - q (г) – шерсти пошло на джемпер', 't : (t - q) (раза) – во сколько раз больше'],
  'на кофту пошло в t : (t – q) раз больше граммов шерсти, чем на джемпер',
  't : (t – q)',
  'раз',
  1230),
 ('в озере d л воды, а в море на f л воды больше. во сколько раз меньше воды в озере, чем в море?',
  ['d + f (л) – воды в море', '(d + f) : d (раза) – во сколько раз меньше'],
  'в озере воды в (d + f) : d раз меньше, чем в море',
  '(d + f) : d',
  'раз',
  1231),
 ('летело j стай по m птиц в каждой. отстало x птиц. сколько птиц осталось?',
  ['j · m (шт.) – птиц летело', 'j · m - x (шт.) – птиц осталось'],
  'осталось j · m – x птиц',
  'j · m – x',
  'птиц',
  1232),
 ('телевизор стоит w р., а телефон - в z раз дешевле. на сколько рублей дороже телевизор, чем телефон?',
  ['w : z (руб.) – стоит телефон', 'w - w : z (руб.) – разница в цене'],
  'телевизор дороже телефона на w – w : z рублей',
  'w – w : z',
  'рублей',
  1233),
 ('в j одинаковых домах по d окон. сколько окон разбилось, если осталось q окон?',
  ['j · d (шт.) – окон было', 'j · d - q (шт.) – окон разбилось'],
  'разбилось j · d – q окон',
  'j · d – q',
  'окон',
  1234),
 ('купили q м ситца, что в r раз больше, чем шелка. сколько метров ткани купили?',
  ['q : r (м) – шёлка купили', 'q + q : r (м) – всего ткани'],
  'купили q + q : r метров ткани',
  'q + q : r',
  'метров',
  1235),
 ('у b синиц по d птенцов, а у малиновки r птенцов. во сколько раз меньше птенцов у малиновки, чем у b синиц?',
  ['b · d (шт.) – птенцов у синиц', 'b · d : r (раза) – во сколько раз меньше'],
  'у малиновки птенцов в b · d : r раз меньше, чем у синиц',
  'b · d : r',
  'раз',
  1236),
 ('после того как на w платьев пошло по z м материи, у портнихи осталось p м. сколько метров материи было у портнихи?',
  ['w · z (м) – материи пошло на платья', 'w · z + p (м) – материи было'],
  'у портнихи было w · z + p метров материи',
  'w · z + p',
  'метров',
  1237),
 ('в океане w л воды, что в f раз больше, чем в море. сколько всего литров воды в океане и в море?',
  ['w : f (л) – воды в море', 'w + w : f (л) – всего воды'],
  'в океане и море всего w + w : f литров воды',
  'w + w : f',
  'литров',
  1238),
 ('машина стоит q р., а квартира - в n раз дороже. на сколько машина дешевле, чем квартира?',
  ['q · n (руб.) – стоит квартира', 'q · n - q (руб.) – разница в цене'],
  'машина дешевле квартиры на q · n – q рублей',
  'q · n – q',
  'рублей',
  1239),
 ('в нескольких аквариумах по m рыб в каждом. после того как продали f рыб, осталось x рыб. сколько аквариумов было?',
  ['f + x (шт.) – рыб было', '(f + x) : m (шт.) – аквариумов'],
  'было (f + x) : m аквариумов',
  '(f + x) : m',
  'аквариумов',
  1240),
 ('дедушке n лет, а внук в m раз моложе. на сколько лет дедушка старше внука?',
  ['n : m (лет) – возраст внука', 'n - n : m (лет) – разница в возрасте'],
  'дедушка старше внука на n – n : m лет',
  'n – n : m',
  'лет',
  1241),
 ('в d одинаковых толстых книгах z страниц, а в n тонких книгах w страниц. во сколько раз больше страниц в одной '
  'толстой книге, чем в одной тонкой?',
  ['z : d (шт.) – страниц в одной толстой книге',
   'w : n (шт.) – страниц в одной тонкой книге',
   '(z : d) : (w : n) (раза) – во сколько раз больше'],
  'в одной толстой книге страниц в (z : d) : (w : n) раз больше, чем в одной тонкой',
  '(z : d) : (w : n)',
  'раз',
  1242),
 ('надоили d л молока. из r л сделали сливки, а остальное разлили в i бидонов поровну в каждый. по сколько получилось '
  'литров в каждом бидоне?',
  ['d - r (л) – молока разлили', '(d - r) : i (л) – молока в каждом бидоне'],
  'в каждом бидоне получилось по (d – r) : i литров молока',
  '(d – r) : i',
  'литров',
  1243),
 ('в f одинаковых клетках p хомяков. на сколько больше хомяков в f клетках, чем в одной клетке?',
  ['p : f (шт.) – хомяков в одной клетке', 'p - p : f (шт.) – разница'],
  'в f клетках на p – p : f хомяков больше, чем в одной клетке',
  'p – p : f',
  'хомяков',
  1244),
 ('у одного крокодила f крокодильчиков, а у другого - в v раз меньше. сколько всего крокодильчиков у двух крокодилов?',
  ['f : v (шт.) – крокодильчиков у второго крокодила', 'f + f : v (шт.) – всего крокодильчиков'],
  'у двух крокодилов всего f + f : v крокодильчиков',
  'f + f : v',
  'крокодильчиков',
  1245),
 ('первая бригада за k дней отремонтировала q м дороги, а вторая бригада за f дней отремонтировала z м дороги. на '
  'сколько больше метров в день ремонтировала первая бригада, чем вторая?',
  ['q : k (м) – первая бригада за день', 'z : f (м) – вторая бригада за день', 'q : k - z : f (м) – разница'],
  'первая бригада ремонтировала в день на q : k – z : f метров больше, чем вторая',
  'q : k – z : f',
  'метров',
  1246),
 ('в f контейнерах n т зерна. сколько зерна в m контейнерах?',
  ['n : f (т) – зерна в одном контейнере', 'n : f · m (т) – зерна в m контейнерах'],
  'в m контейнерах n : f · m тонн зерна',
  'n : f · m',
  'тонн',
  1247),
 ('за первый квартал завод выпустил w машин, а за второй - в q раз больше. сколько машин выпустил завод за два '
  'квартала?',
  ['w · q (шт.) – машин за второй квартал', 'w + w · q (шт.) – всего машин'],
  'за два квартала завод выпустил w + w · q машин',
  'w + w · q',
  'машин',
  1248),
 ('в районной библиотеке f книг, а в школьной - на n книг меньше. во сколько раз в районной библиотеке книг больше, '
  'чем в школьной?',
  ['f - n (шт.) – книг в школьной библиотеке', 'f : (f - n) (раза) – во сколько раз больше'],
  'в районной библиотеке книг в f : (f – n) раз больше, чем в школьной',
  'f : (f – n)',
  'раз',
  1249),
 ('в w одинаковых домах g подъездов. во скольких домах j подъездов?',
  ['g : w (шт.) – подъездов в одном доме', 'j : (g : w) (шт.) – домов'],
  'j подъездов будет в j : (g : w) домах',
  'j : (g : w)',
  'домов',
  1250),
 ('в j табунах по b лошадей. продали n лошадей. сколько лошадей осталось в табунах?',
  ['j · b (шт.) – лошадей было', 'j · b - n (шт.) – лошадей осталось'],
  'в табунах осталось j · b – n лошадей',
  'j · b – n',
  'лошадей',
  1251),
 ('кресло стоит z р., а стул - в g раз дешевле. на сколько кресло дороже стула?',
  ['z : g (руб.) – стоит стул', 'z - z : g (руб.) – разница в цене'],
  'кресло дороже стула на z – z : g рублей',
  'z – z : g',
  'рублей',
  1252),
 ('ширина коридора f м, а его длина на z м больше. во сколько раз ширина коридора меньше, чем его длина?',
  ['f + z (м) – длина коридора', '(f + z) : f (раза) – во сколько раз меньше'],
  'ширина коридора меньше его длины в (f + z) : f раз',
  '(f + z) : f',
  'раз',
  1253),
 ('с j деревьев собрали по d кг слив. сколько килограммов слив продали, если осталось h кг слив?',
  ['j · d (кг) – слив собрали', 'j · d - h (кг) – слив продали'],
  'продали j · d – h кг слив',
  'j · d – h',
  'кг',
  1254),
 ('в магазине продавалось z платьев, что в f раз больше, чем костюмов. сколько всего платьев и костюмов продавалось?',
  ['z : f (шт.) – костюмов', 'z + z : f (шт.) – всего платьев и костюмов'],
  'в магазине продавалось z + z : f платьев и костюмов',
  'z + z : f',
  'платьев и костюмов',
  1255),
 ('в x группах занимается по w девочек, а в одной группе - p мальчиков. во сколько раз меньше занимается мальчиков, '
  'чем девочек?',
  ['x · w (чел.) – девочек', 'x · w : p (раза) – во сколько раз меньше'],
  'мальчиков занимается в x · w : p раз меньше, чем девочек',
  'x · w : p',
  'раз',
  1256),
 ('после того как в z домах проложили по n м кабеля, осталось q м кабеля. сколько всего метров кабеля было?',
  ['z · n (м) – кабеля проложили', 'z · n + q (м) – всего кабеля'],
  'всего было z · n + q метров кабеля',
  'z · n + q',
  'метров',
  1257),
 ('во дворе h легковых машин, что в d раз больше, чем грузовых. сколько всего машин во дворе?',
  ['h : d (шт.) – грузовых машин', 'h + h : d (шт.) – всего машин'],
  'во дворе всего h + h : d машин',
  'h + h : d',
  'машин',
  1258),
 ('пальто стоит z р, а шуба - в r раз дороже. на сколько пальто дешевле шубы?',
  ['z · r (руб.) – стоит шуба', 'z · r - z (руб.) – разница в цене'],
  'пальто дешевле шубы на z · r – z рублей',
  'z · r – z',
  'рублей',
  1259),
 ('в школе училось несколько классов по g человек в каждом. после того как q человек заболело, осталось r человек. '
  'сколько классов в школе?',
  ['q + r (чел.) – человек училось', '(q + r) : g (шт.) – классов'],
  'в школе было (q + r) : g классов',
  '(q + r) : g',
  'классов',
  1260),
 ('маме z лет, а дочка в w раз моложе. на сколько лет мама старше дочери?',
  ['z : w (лет) – возраст дочки', 'z - z : w (лет) – разница в возрасте'],
  'мама старше дочери на z – z : w лет',
  'z – z : w',
  'лет',
  1261),
 ('в n больших бидонах z л молока, а в x маленьких бидонах w л молока. во сколько раз больше литров в одном большом '
  'бидоне, чем в одном маленьком?',
  ['z : n (л) – молока в большом бидоне',
   'w : x (л) – молока в маленьком бидоне',
   '(z : n) : (w : x) (раза) – во сколько раз больше'],
  'в одном большом бидоне литров в z : n : (w : x) раз больше, чем в одном маленьком',
  'z : n : (w : x)',
  'раз',
  1262),
 ('белка собрала k грибочков. она съела z грибочков, а остальные развесила на f деревьях поровну на каждом. по скольку '
  'грибочков было на каждом дереве?',
  ['k - z (шт.) – грибочков развесила', '(k - z) : f (шт.) – грибочков на каждом дереве'],
  'на каждом дереве было по (k – z) : f грибочков',
  '(k – z) : f',
  'грибов',
  1263),
 ('в d одинаковых кабинетах l кресел. на сколько больше кресел в d кабинетах, чем в одном?',
  ['l : d (шт.) – кресел в одном кабинете', 'l - l : d (шт.) – разница'],
  'в d кабинетах на l – l : d кресел больше, чем в одном',
  'l – l : d',
  'кресел',
  1264),
 ('в ларьке было 4 ящика с яблоками по 9 кг и один ящик с 23 кг персиков. сколько всего килограммов фруктов в ларьке?',
  ['4 · 9 = 36 (кг) – яблок', '36 + 23 = 59 (кг) – всего фруктов'],
  'в ларьке всего 59 кг фруктов',
  '59',
  'кг',
  1265),
 ('в столовую привезли 3 ящика по 9 кг груш и один ящик яблок весом 12 кг. сколько всего килограммов фруктов привезли '
  'в столовую?',
  ['3 · 9 = 27 (кг) – груш', '27 + 12 = 39 (кг) – всего фруктов'],
  'в столовую привезли всего 39 кг фруктов',
  '39',
  'кг',
  1266),
 ('посадили 4 ряда по 5 кустиков клубники и 2 ряда земляники по 10 кустов. сколько всего кустов ягод посадили?',
  ['4 · 5 = 20 (шт.) – кустиков клубники',
   '2 · 10 = 20 (шт.) – кустов земляники',
   '20 + 20 = 40 (шт.) – всего кустов ягод'],
  'посадили всего 40 кустов ягод',
  '40',
  'кустов',
  1267),
 ('купили 5 упаковок синих ручек по 3 в каждой и 6 зеленых ручек. сколько всего ручек купили?',
  ['5 · 3 = 15 (шт.) – синих ручек', '15 + 6 = 21 (шт.) – всего ручек'],
  'купили всего 21 ручку',
  '21',
  'ручка',
  1268),
 ('окопали 4 ряда черной смородины по 6 кустов в каждом и один ряд по 10 кустов красной смородины. сколько всего '
  'кустов смородины окопали?',
  ['4 · 6 = 24 (шт.) – кустов чёрной смородины', '24 + 10 = 34 (шт.) – всего кустов'],
  'окопали всего 34 куста смородины',
  '34',
  'куста',
  1269),
 ('купили 4 пакета гречневой крупы по 3 кг и один мешок риса весом 15 кг. сколько всего килограммов крупы купили?',
  ['4 · 3 = 12 (кг) – гречневой крупы', '12 + 15 = 27 (кг) – всего крупы'],
  'купили всего 27 кг крупы',
  '27',
  'кг',
  1270),
 ('молоко налили в 5 трехлитровых банок и в одну пятилитровую банку. сколько всего литров молока налили?',
  ['5 · 3 = 15 (л) – молока в трёхлитровых банках', '15 + 5 = 20 (л) – всего молока'],
  'налили всего 20 л молока',
  '20',
  'л',
  1271),
 ('пришили на 3 кофты по 6 пуговиц и на одно пальто 10 пуговиц. сколько всего пуговиц пришили?',
  ['3 · 6 = 18 (шт.) – пуговиц на кофтах', '18 + 10 = 28 (шт.) – всего пуговиц'],
  'пришили всего 28 пуговиц',
  '28',
  'пуговиц',
  1272),
 ('в 3 клетках по 2 попугая и в одной клетке 5 канареек. сколько всего птиц в клетках?',
  ['3 · 2 = 6 (шт.) – попугаев', '6 + 5 = 11 (шт.) – всего птиц'],
  'в клетках всего 11 птиц',
  '11',
  'птиц',
  1273),
 ('туристы шли 2 часа по 4 км, а потом еще 5 км. сколько всего километров прошли туристы?',
  ['2 · 4 = 8 (км) – прошли за 2 часа', '8 + 5 = 13 (км) – всего прошли туристы'],
  'туристы прошли всего 13 км',
  '13',
  'км',
  1274),
 ('дети поехали на экскурсию в 3 легковых машинах по 5 человек в каждой, и 35 детей поехали в автобусе. сколько всего '
  'детей поехало на экскурсию?',
  ['3 · 5 = 15 (чел.) – детей в легковых машинах', '15 + 35 = 50 (чел.) – всего детей'],
  'на экскурсию поехало всего 50 детей',
  '50',
  'детей',
  1275),
 ('садоводы посадили 4 ряда красной смородины по 6 кустов в ряду и столько же рядов черной смородины по 5 кустов. '
  'сколько всего кустов смородины посадили садоводы? решите 2 способами.',
  ['Способ 1. По видам смородины',
   '4 · 6 = 24 (шт.) – кустов красной смородины',
   '4 · 5 = 20 (шт.) – кустов чёрной смородины',
   '24 + 20 = 44 (шт.) – кустов смородины',
   'Способ 2. Через сумму в одном ряду',
   '6 + 5 = 11 (шт.) – кустов в двух рядах',
   '4 · 11 = 44 (шт.) – всего кустов'],
  'садоводы посадили всего 44 куста смородины',
  '44',
  'куста',
  1276),
 ('юннаты посадили 3 ряда роз по 8 кустиков в каждом и 5 рядов по 4 кустика в каждом. сколько всего кустиков роз '
  'посадили юннаты?',
  ['3 · 8 = 24 (шт.) – кустиков в первых рядах',
   '5 · 4 = 20 (шт.) – кустиков в других рядах',
   '24 + 20 = 44 (шт.) – всего кустиков'],
  'юннаты посадили всего 44 кустика роз',
  '44',
  'куста',
  1277),
 ('в ларек привезли 6 ящиков груш по 16 кг и ящик слив весом 26 кг. сколько всего килограммов груш и слив привезли?',
  ['6 · 16 = 96 (кг) – груш', '96 + 26 = 122 (кг) – всего груш и слив'],
  'в ларёк привезли всего 122 кг груш и слив',
  '122',
  'кг',
  1278),
 ('ася купила 3 ручки по 4 р. и 6 тетрадей по 3 р. сколько денег она заплатила?',
  ['3 · 4 = 12 (руб.) – стоят ручки', '6 · 3 = 18 (руб.) – стоят тетради', '12 + 18 = 30 (руб.) – вся покупка'],
  'Ася заплатила 30 рублей',
  '30',
  'рублей',
  1279),
 ('школьники окопали 4 ряда яблонь по 11 деревьев в каждом ряду и 15 грушевых деревьев. сколько всего фруктовых '
  'деревьев окопали школьники?',
  ['4 · 11 = 44 (шт.) – яблонь', '44 + 15 = 59 (шт.) – всего фруктовых деревьев'],
  'школьники окопали всего 59 фруктовых деревьев',
  '59',
  'деревьев',
  1280),
 ('хозяйка засолила 6 банок огурцов по 5 кг в каждой банке и 45 кг помидоров. сколько овощей засолила хозяйка?',
  ['6 · 5 = 30 (кг) – огурцов', '30 + 45 = 75 (кг) – всего овощей'],
  'хозяйка засолила всего 75 кг овощей',
  '75',
  'кг',
  1281),
 ('у 2 серых куропаток по 8 птенцов, а у 3 белых по 5 птенцов. сколько всего птенцов у этих птиц?',
  ['2 · 8 = 16 (шт.) – птенцов у серых куропаток',
   '3 · 5 = 15 (шт.) – птенцов у белых куропаток',
   '16 + 15 = 31 (шт.) – всего птенцов'],
  'у этих птиц всего 31 птенец',
  '31',
  'птенец',
  1282),
 ('наташа посадила 4 ряда луковиц по 9 луковиц в ряду, а толя посадил 15 луковиц. сколько луковиц посадили дети?',
  ['4 · 9 = 36 (шт.) – луковиц посадила Наташа', '36 + 15 = 51 (шт.) – всего луковиц'],
  'дети посадили всего 51 луковицу',
  '51',
  'луковиц',
  1283),
 ('в цветочный киоск привезли 30 букетов гвоздик по 3 цветка в каждом и 40 букетов роз по стольку же цветков в букете. '
  'сколько всего цветов привезли в киоск?',
  ['30 · 3 = 90 (шт.) – цветков гвоздик', '40 · 3 = 120 (шт.) – цветков роз', '90 + 120 = 210 (шт.) – всего цветов'],
  'в киоск привезли всего 210 цветов',
  '210',
  'цветов',
  1284),
 ('в ремонтную мастерскую поступило 8 кусков олова по 3 кг и столько же кусков меди по 6 кг. сколько всего килограммов '
  'олова и меди поступило в ремонтную мастерскую? решите задачу 2 способами.',
  ['Способ 1. По видам металла',
   '8 · 3 = 24 (кг) – олова',
   '8 · 6 = 48 (кг) – меди',
   '24 + 48 = 72 (кг) – олова и меди',
   'Способ 2. Через массу пары кусков',
   '3 + 6 = 9 (кг) – олова и меди в паре',
   '8 · 9 = 72 (кг) – всего металла'],
  'в мастерскую поступило всего 72 кг олова и меди',
  '72',
  'кг',
  1285),
 ('насыпали 5 мешков пшеницы по 50 кг в каждый и 3 мешка ржи по 60 кг в каждый. сколько всего зерна насыпали в мешки?',
  ['5 · 50 = 250 (кг) – пшеницы', '3 · 60 = 180 (кг) – ржи', '250 + 180 = 430 (кг) – всего зерна'],
  'в мешки насыпали всего 430 кг зерна',
  '430',
  'кг',
  1286),
 ('к празднику для детского сада купили 2 ящика конфет по 11 кг в каждом и 4 ящика пряников по 10 кг в каждом. сколько '
  'всего сладостей купили?',
  ['2 · 11 = 22 (кг) – конфет', '4 · 10 = 40 (кг) – пряников', '22 + 40 = 62 (кг) – всего сладостей'],
  'купили всего 62 кг сладостей',
  '62',
  'кг',
  1287),
 ('купили 3 настольные лампы по 10 долларов и люстру за 56 долларов. сколько стоила покупка?',
  ['3 · 10 = 30 (долл.) – стоят лампы', '30 + 56 = 86 (долл.) – вся покупка'],
  'покупка стоила 86 долларов',
  '86',
  'долларов',
  1288),
 ('пеночка летела 3 дня по 50 км и 2 дня по 40 км. какой путь пролетела пеночка?',
  ['3 · 50 = 150 (км) – путь за первые дни',
   '2 · 40 = 80 (км) – путь за остальные дни',
   '150 + 80 = 230 (км) – весь путь'],
  'пеночка пролетела 230 км',
  '230',
  'км',
  1289),
 ('мальчик исписал 4 тетради по 18 листов и столько же тетрадей по 12 листов. сколько всего листов исписал мальчик? '
  'решите задачу 2 способами.',
  ['Способ 1. По видам тетрадей',
   '4 · 18 = 72 (шт.) – листов в первых тетрадях',
   '4 · 12 = 48 (шт.) – листов во вторых тетрадях',
   '72 + 48 = 120 (шт.) – листов по видам',
   'Способ 2. Через листы в паре тетрадей',
   '18 + 12 = 30 (шт.) – листов в паре тетрадей',
   '4 · 30 = 120 (шт.) – всего листов'],
  'мальчик исписал всего 120 листов',
  '120',
  'листов',
  1290),
 ('на 3 ветках сидело по 7 воробьев. на стольких же ветках сидело по 4 зяблика. сколько всего птиц сидело на ветках?',
  ['3 · 7 = 21 (шт.) – воробьёв', '3 · 4 = 12 (шт.) – зябликов', '21 + 12 = 33 (шт.) – всего птиц'],
  'на ветках сидело всего 33 птицы',
  '33',
  'птицы',
  1291),
 ('на 6 страницах альбома помещено по 15 почтовых марок, а на 5 - по 18 марок. сколько всего марок в этом альбоме?',
  ['6 · 15 = 90 (шт.) – марок на 6 страницах',
   '5 · 18 = 90 (шт.) – марок на 5 страницах',
   '90 + 90 = 180 (шт.) – всего марок'],
  'в альбоме всего 180 марок',
  '180',
  'марок',
  1292),
 ('в 2 бидонах по 25 л молока, а в 4 бидонах по 15 л. сколько всего литров молока в этих бидонах?',
  ['2 · 25 = 50 (л) – молока в первых бидонах',
   '4 · 15 = 60 (л) – молока во вторых бидонах',
   '50 + 60 = 110 (л) – всего молока'],
  'в этих бидонах всего 110 л молока',
  '110',
  'л',
  1293),
 ('выдали 3 классам по 20 тетрадей, а 2 классам по 30 тетрадей. сколько всего тетрадей выдали этим классам?',
  ['3 · 20 = 60 (шт.) – тетрадей первым классам',
   '2 · 30 = 60 (шт.) – тетрадей другим классам',
   '60 + 60 = 120 (шт.) – всего тетрадей'],
  'этим классам выдали всего 120 тетрадей',
  '120',
  'тетрадей',
  1294),
 ('доярка надоила от 5 коров по 18 л молока, а от 4 коров по 15 л. сколько всего литров молока надоила доярка от всех '
  'коров?',
  ['5 · 18 = 90 (л) – молока от первых коров',
   '4 · 15 = 60 (л) – молока от остальных коров',
   '90 + 60 = 150 (л) – всего молока'],
  'доярка надоила всего 150 л молока',
  '150',
  'л',
  1295),
 ('с 2 яблонь собрали по 20 кг яблок, а с 4 яблонь по 10 кг. сколько всего килограммов яблок собрали?',
  ['2 · 20 = 40 (кг) – яблок с первых яблонь',
   '4 · 10 = 40 (кг) – яблок с остальных яблонь',
   '40 + 40 = 80 (кг) – всего яблок'],
  'собрали всего 80 кг яблок',
  '80',
  'кг',
  1296),
 ('девочки собирали мак. одна нарвала с 3 грядок по 30 головок мака, а другая с 4 грядок по 25 головок. сколько всего '
  'головок мака собрали девочки?',
  ['3 · 30 = 90 (шт.) – головок мака у первой девочки',
   '4 · 25 = 100 (шт.) – головок мака у второй девочки',
   '90 + 100 = 190 (шт.) – всего головок мака'],
  'девочки собрали всего 190 головок мака',
  '190',
  'головок',
  1297),
 ('у хозяйки 3 курицы и 2 утки. каждая курица получает 130 г разных кормов, а каждая утка - на 20 г больше. сколько '
  'кормов ежедневно получают все эти куры и утки?',
  ['130 + 20 = 150 (г) – корма получает утка',
   '3 · 130 = 390 (г) – корма получают куры',
   '2 · 150 = 300 (г) – корма получают утки',
   '390 + 300 = 690 (г) – всего корма'],
  'все куры и утки ежедневно получают 690 г корма',
  '690',
  'г',
  1298),
 ('в месяц при тяжелой работе лошадь получает 240 кг овса, а при легкой - на 90 кг меньше. у фермера в августе 2 '
  'лошади были заняты на тяжелой работе, а 3 - на легкой. сколько овса получили в августе все эти лошади?',
  ['240 - 90 = 150 (кг) – овса при лёгкой работе',
   '2 · 240 = 480 (кг) – овса для тяжёлой работы',
   '3 · 150 = 450 (кг) – овса для лёгкой работы',
   '480 + 450 = 930 (кг) – всего овса'],
  'в августе все лошади получили 930 кг овса',
  '930',
  'кг',
  1299),
 ('на грузовик положили 3 короткие балки и 2 длинные. короткая балка весит 120 кг, а длинная - на 40 кг больше. '
  'сколько весят вместе все эти балки?',
  ['120 + 40 = 160 (кг) – весит длинная балка',
   '3 · 120 = 360 (кг) – весят короткие балки',
   '2 · 160 = 320 (кг) – весят длинные балки',
   '360 + 320 = 680 (кг) – общий вес'],
  'все эти балки весят вместе 680 кг',
  '680',
  'кг',
  1300)]


def _v41302_format_batch_solution_text(original_text: str, steps: list[str], final_answer: str) -> str:
    if not any(re.match(r'^\s*Способ\s+\d+\b', str(step or ''), flags=re.IGNORECASE) for step in steps or []):
        return _format_primary_solution_text(original_text, steps, final_answer)
    lines = ['Задача.', str(original_text or '').strip(), 'Решение.']
    step_counter = 1
    for raw_step in steps or []:
        clean = re.sub(r'^\s*\d+[\).]\s*', '', str(raw_step or '')).strip()
        if not clean:
            continue
        if clean[-1:] not in '.!?:':
            clean += '.'
        if re.match(r'^\s*Способ\s+\d+\b', clean, flags=re.IGNORECASE):
            lines.append(clean)
            step_counter = 1
        else:
...
def _v41302_batch_1201_1300_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V41302_BATCH_1201_1300_SPECS:
        if key == task_key:
            spec = (steps, final_answer, answer_number, answer_unit, excel_row)
            break
    if spec is None:
    ...
    out.update({
        ...
        'explanation': result,
        'validated': True,
        'source': 'deepseek-primary-v41302-postprocess-repair',
        'answer': str(final_answer),
        'answer_number': str(answer_number),
        'answer_unit': str(answer_unit),
        'final_answer': str(final_answer),
        ...
        'v41302Batch12011300ExactRepair': True,
        'v41302ExcelRow': int(excel_row),
    })
    out['structuredSolution'] = dict(out.get('structured_solution') or {})
    contract = str(out.get('visibleResultContract') or '').strip()
...
def _v41102_concise_batch_1001_1100_step(line: str) -> str:
    ...
    The live report showed valid numeric answers blocked by explanations that
    repeated answer predicates: «канареек было/осталось», «кустов осталось»,
    etc.  The full context remains in Ответ; after the dash we keep the object
    or the measured per-one context.
    """
    text = str(line or '')
    m = re.match(r'^(.*?\([^)]*\)\s*[—–-]\s*)(.+?)([.!?]*)$', text)
    if not m:
        return text
    prefix, expl, punct = m.group(1), _v4011_clean_phrase(m.group(2)), m.group(3) or ''
    low = expl.lower().replace('ё', 'е')
    # Special concise daily-rate wording.
    if re.fullmatch(r'молока\s+за\s+один\s+день\s+в\s+июне', low):
        return f'{prefix}дневной удой в июне{punct or "."}'
    if re.fullmatch(r'молока\s+за\s+один\s+день\s+в\s+июле', low):
        return f'{prefix}дневной удой в июле{punct or "."}'
    # Keep meaningful per-unit contexts and totals.
    if re.search(r'\b(?:на\s+один|на\s+одну|в\s+одном|в\s+одной|за\s+один|за\s+одну|за\s+один\s+день|всего)\b', low):
        return text
    # Remove copied predicates from counted-object explanations while keeping
    # multi-step explanations distinct: «канареек было» -> «всего канареек»,
    # «канареек осталось» -> «канареек».
    cleaned = expl
    m_state = re.fullmatch(r'(.+?)\s+(было|была|был|были)$', low)
    if m_state:
        cleaned = 'всего ' + _v4011_clean_phrase(expl[:len(m_state.group(1))])
    else:
        m_tail = re.fullmatch(
            r'(.+?)\s+(осталось|остался|осталась|остались|купили|купила|купил|привезли|привезла|привез|продали|продала|продал|подарили|подарил|подарила|поставили|поставила|поставил|разместил|разместили|разложили|разложил|разлила|разлили|сварила|сварили|посадили|посадил|посадила|пришила|пришили|ввинтили|горело|горели|поставят|повесят|принесли|принесла|принес|запряг|переплели|отложили|собрали|отдали|израсходовали|израсходовал|расходуют|расходовали|пошло|пошли|поставила\s+на\s+полки)$',
            low,
        )
        if m_tail:
            cleaned = _v4011_clean_phrase(expl[:len(m_tail.group(1))])
    # Specific good replacements for remaining/context results.
    if re.fullmatch(r'осталось\s+после\s+обеда', low):
        cleaned = 'после обеда'
    if re.fullmatch(r'(?:дней|дня|день)\s+работы', low):
        cleaned = 'всего дней'
    if re.fullmatch(r'работали\s+до\s+обеда', low):
        cleaned = 'всего человек'
    if cleaned and cleaned != expl:
        return f'{prefix}{cleaned}{punct or "."}'
    return text


def _v41102_deduplicate_batch_1001_1100_steps(steps: list[str]) -> list[str]:
    """Make multi-step dash explanations distinct without copying predicates."""
    out = [str(step or '') for step in steps]
    def _expl(line: str) -> str:
        m = re.search(r'\)\s*[—–-]\s*(.+?)(?:[.!?])?$', line)
        return _v4011_clean_phrase(m.group(1) if m else '').lower().replace('ё','е')
    expls = [_expl(x) for x in out]
    for i, expl in enumerate(list(expls)):
        if not expl:
            continue
        dup_indices = [j for j,e in enumerate(expls) if e == expl]
        if len(dup_indices) <= 1:
            continue
        for j in dup_indices[:-1]:
            line = out[j]
            # For subtotal/intermediate group calculations, mark the explanation
            # as a subtotal; keep the last occurrence short for the final result.
            replacement = 'всего ' + expl
            out[j] = re.sub(r'(\)\s*[—–-]\s*)(.+?)([.!?]*)$', lambda m: m.group(1)+replacement+(m.group(3) or '.'), line)
            expls[j] = replacement
    return out

def _v41102_batch_1001_1100_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V41101_BATCH_1001_1100_SPECS:
        if key == task_key:
            spec = (steps, final_answer, answer_number, answer_unit, excel_row)
            break
    if spec is None:
    ...
    existing_source = str(out.get('source') or '').strip()
    if not existing_source or existing_source.lower().startswith('guard-low-confidence'):
        existing_source = 'deepseek-primary-v41102-postprocess-repair'
    out.update({
        'result': result,
        'validated': True,
        'source': existing_source,
        ...
        'v41102Batch10011100ExactRepair': True,
...
# --- V500 generalized semantic solver + arithmetic self-verifier ---
# Goal: improve production behavior for unseen tasks.  This layer is deliberately
# rule-based and does not use Excel row ids or expected answers.  It runs after the
# real DeepSeek/API call is made, checks common school-word-problem structures,
# recalculates visible arithmetic, and replaces unstable API arithmetic only when
# the task matches a confident general pattern.
def _v500_question_like_match(original_text: str) -> re.Match[str] | None:
    text = str(original_text or '').strip()
    matches = list(re.finditer(r'(?im)(?:^|[.!?\n])\s*((?:сколько|скольких|каков|какова|какое|какой)[^.!?]*)', text))
    return matches[-1] if matches else None


def _v500_last_question_sentence(original_text: str) -> str:
    text = str(original_text or '').strip()
    q = text.rfind('?')
    if q >= 0:
        prefix = text[:q + 1]
    ...
    # Some textbook rows end a question-like sentence with a period, for example
    # «Каков вес чёрствого хлеба.»  Treat it as a question for semantic solving.
    m = _v500_question_like_match(text)
    if m:
        return str(m.group(1) or '').strip().rstrip('.!?')
    return text


def _v500_condition_text(original_text: str) -> str:
    text = str(original_text or '').strip()
    q = text.rfind('?')
    if q >= 0:
        # The condition is the part before the final question sentence.  In V500
        # the previous implementation returned text[:q], which still included
        # «Сколько машин стало» and caused false subtraction for addition tasks.
        prefix = text[:q].strip()
        boundary = max(prefix.rfind('.'), prefix.rfind('!'), prefix.rfind('\n'))
        if boundary >= 0:
            return prefix[:boundary + 1].strip()
...
def _v500_ints(value: str) -> list[int]:
    return [int(m.group(0)) for m in re.finditer(r'(?<!\d)\d+(?!\d)', str(value or ''))]


def _v500_norm(value: str) -> str:
    return re.sub(r'\s+', ' ', str(value or '').lower().replace('ё', 'е').replace('−', '-').replace('–', '-').replace('—', '-')).strip()


def _v500_question_has_total_intent(question: str) -> bool:
    q = _v500_norm(question)
    comparative_words = ('больше', 'меньше', 'старше', 'моложе', 'дольше', 'короче', 'ниже', 'выше', 'длиннее')
    # V502: do not treat «осталось» as «стало».  In V501.02 the substring
    # check `стало in q` made remaining-to-do/removal tasks look like totals and
    # created false templates such as 23 + 15 instead of 23 - 15.
    if re.search(r'\bостал(?:ось|ась|ся|ись|и)?\b', q):
        return False
    return bool(
        'сколько всего' in q
        or 'скольких всего' in q
        or ' всего ' in f' {q} '
        or 'вместе' in q
        or re.search(r'\bстал(?:о|а|и|иcь|ись)?\b', q)
        or 'теперь' in q
        or 'оказалось' in q
        or 'в двух' in q
        or 'за два' in q
        ...
        or 'на двух' in q
        or 'посуды' in q
        or 'детей занимал' in q
        or 'птиц сидело' in q
        or 'животных было' in q
        or (q.startswith('на сколько') and not any(word in q for word in comparative_words))
    )


def _v500_question_target_stems(original_text: str, info: dict[str, str | bool]) -> list[str]:
    q = _v500_last_question_sentence(original_text)
    raw = str(info.get('unitPhrase') or info.get('tail') or info.get('unit') or '')
    if not raw:
        m = re.search(r'скольк(?:о|их|ими)\s+(?:всего\s+)?(.+?)(?:\s+(?:стало|сидел|было|пело|ехало|раст|лежал|учатся|приехало|подарили|купил|нарисовал|прочитал|исписал)|\?|$)', q, flags=re.IGNORECASE)
        raw = m.group(1) if m else q
    words = [w for w in re.findall(r'[а-яa-zё.-]+', raw.lower().replace('ё', 'е')) if w not in {
        'сколько', 'всего', 'стало', 'стали', 'было', 'будет', 'теперь', 'на', 'в', 'во', 'у', 'за', 'по', 'к', 'до', 'от', 'из', 'и', 'или', 'а', 'его', 'ее', 'её', 'их', 'это', 'них', 'двух', 'две', 'два', 'класс', 'классе', 'класса', 'классу', 'классов'
    }]
    stems: list[str] = []
    for w in words:
        key = _v4011_norm_key(w)
        if key in {'литр', 'л', 'кг', 'килограмм', 'г', 'метр', 'м', 'см', 'рубль', 'руб', 'день', 'сутки', 'год', 'лет', 'месяц', 'раз'}:
            stems.append(w[:3])
        elif len(w) >= 3:
            stems.append(w[:4] if len(w) > 4 else w)
...
def _v500_numbers_near_target_in_text(raw_text: str, stems: list[str]) -> list[int]:
    if not stems:
        return []
    out: list[int] = []
    time_words = {
        'день', 'дня', 'дней', 'дн', 'сутки', 'суток', 'час', 'часа', 'часов',
        'минуту', 'минуты', 'минут', 'год', 'года', 'лет', 'месяц', 'месяца',
        'месяцев', 'неделя', 'недели', 'недель', 'четверть', 'четверти'
    }
    for sent in re.split(r'[.!?;]', str(raw_text or '')):
        slow = sent.lower().replace('ё', 'е')
        for m in re.finditer(r'(?<!\d)\d+(?!\d)', slow):
            after_full = slow[m.end():]
            immediate = re.match(r'\s*([а-яa-zё.-]+)', after_full)
            immediate_word = (immediate.group(1).replace('ё', 'е') if immediate else '')
            if immediate_word in time_words and not any(stem.startswith(immediate_word[:3]) or immediate_word.startswith(stem[:3]) for stem in stems):
                continue
            # Inspect only the local phrase before the next number or punctuation.
            # This prevents «4 дуба и 6 елей» from treating 4 as an елка count.
            after_segment = re.split(r'(?<!\d)\d+(?!\d)|[,;]', after_full, maxsplit=1)[0]
            before = slow[max(0, m.start() - 34):m.start()]
            local_has_target = any(stem in after_segment for stem in stems)
            before_has_target = any(stem in before for stem in stems)
            # Ignore class labels such as «1 ”Б”» when the question target is
            # pupils/boys/girls.  They may appear close to the real target noun
            # but are not quantities to calculate with.
            class_label = bool(re.fullmatch(r'\s*["“”«»]?[а-яa-z]["“”«»]?\s*[-–—]?\s*', after_segment.strip()))
            if local_has_target or (before_has_target and not class_label):
                out.append(int(m.group(0)))
    return out


def _v500_numbers_near_target(original_text: str, stems: list[str]) -> list[int]:
    return _v500_numbers_near_target_in_text(_v500_condition_text(original_text), stems)


def _v500_pick_addends(original_text: str, info: dict[str, str | bool], *, allow_implicit_one: bool = True) -> list[int]:
    condition = _v500_condition_text(original_text)
    nums = _v500_ints(condition)
    stems = _v500_question_target_stems(original_text, info)
    near = _v500_numbers_near_target(original_text, stems)
    if len(near) >= 2:
        return near[:3]
    if len(nums) == 1 and allow_implicit_one:
        low = _v500_norm(condition)
        qlow = _v500_norm(_v500_last_question_sentence(original_text))
        # First-grade wording: «На цветке сидит пчела. К ней прилетела ещё 1 пчела.»
        if any(word in low for word in ('сидит пчела', 'была пчела', 'одна пчела')) and 'пчел' in qlow:
            return [1, nums[0]]
    if len(nums) >= 2:
        # For totals in 1-4 grade tasks, the relevant values are usually the two
        # stated quantities.  If more numbers occur and no target match is found,
        # use the last two rather than blindly summing labels/grades.
        if len(nums) == 2:
            return nums
        return nums[-2:]
    return []
...
def _v500_extract_comparative(original_text: str) -> tuple[str, int, int, int] | None:
    low = _v500_norm(_v500_condition_text(original_text))
    # Ignore interrogative «на сколько ...?»; here we need a stated offset: «на 3 больше/меньше».
    m = re.search(r'\bна\s+(\d+)\s+(?:[а-яa-zё./-]+\s+){0,3}?(больше|меньше|дольше|короче|ниже|выше)', low, flags=re.IGNORECASE)
    if not m:
        return None
    before = low[:m.start()]
    nums_before = _v500_ints(before)
    if not nums_before:
        return None
    info = _v4011_question_info(original_text, '')
    stems = _v500_question_target_stems(original_text, info)
    near_before = _v500_numbers_near_target_in_text(before, stems)
    base = near_before[-1] if near_before else nums_before[-1]
    diff = int(m.group(1))
    direction = m.group(2)
    sign = '-' if direction in {'меньше', 'короче', 'ниже'} else '+'
    result = base - diff if sign == '-' else base + diff
    if result < 0:
        return None
    return sign, base, diff, result


def _v500_extract_total_minus_part(original_text: str) -> tuple[int, int, int] | None:
    low = _v500_norm(_v500_condition_text(original_text))
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    nums = _v500_ints(low)
    if len(nums) < 2:
        return None
    # V502: broad, reusable remaining/used template.  It covers common primary
    # school wording where the question asks what is left after taking/using/doing
    # a known part.  This prevents false total-addition templates and avoids old
    # exact row repairs.
    removal_markers = (
        'взяли', 'взял', 'взяла', 'убрали', 'убрал', 'убрала', 'съели', 'съел', 'съела',
        'сели', 'улетели', 'улетел', 'улетела', 'ушли', 'ушел', 'ушёл', 'ушла',
        'сорвали', 'сорвал', 'сорвала', 'срезали', 'потратили', 'потратил', 'потратила',
        'израсходовали', 'израсходовал', 'прочитал', 'прочитала', 'решил', 'решила',
        'вклеил', 'вклеила', 'отгадал', 'отгадала', 'посетил', 'посетила',
        'списал', 'списала', 'подарил', 'подарила', 'сделала', 'сделал',
        'съели', 'собрали', 'сняли', 'снял', 'сняла', 'съехали'
    )
    remaining_question = bool(re.search(r'\bостал(?:ось|ась|ся|ись|и)?\b', qlow))
    remaining_to_do = any(marker in qlow for marker in (
        'осталось решить', 'осталось прочитать', 'осталось вклеить', 'осталось отгадать',
        'осталось посетить', 'осталось повесить', 'осталось полить', 'осталось списать'
    ))
    if (remaining_question or remaining_to_do) and any(marker in low for marker in removal_markers):
        total, part = nums[0], nums[-1]
        if total >= part:
            return total, part, total - part
    if 'всего' in low and any(word in qlow for word in ('сколько', 'осталось', 'взяли', 'привезли', 'было')):
        total = nums[0]
        # Use the next stated part; this covers «всего 16..., малины 7..., сколько смородины».
        part = nums[1]
        if total >= part:
            return total, part, total - part
    if any(word in low for word in ('осталось', 'стало')) and any(word in qlow for word in ('сколько', 'взяли', 'приехал', 'привезли')):
...
def _v500_genitive_simple_ru(phrase: str) -> str:
    """Small deterministic genitive helper for short Russian location/person phrases.

    It is deliberately conservative and only supports recurring primary-school
    wording such as «в третьей квартире», «в зелёном доме», «Юре».
    """
    value = str(phrase or '').strip().lower().replace('ё', 'е')
    value = value.strip(' .?!')
    value = re.sub(r'^в\s+', '', value).strip()
    replacements = {
        'третьей квартире': 'третьей квартиры',
        'первой квартире': 'первой квартиры',
        'второй квартире': 'второй квартиры',
        'другом этаже': 'другого этажа',
        'зеленом доме': 'зеленого дома',
        'зелёном доме': 'зеленого дома',
        'красном доме': 'красного дома',
        'синем доме': 'синего дома',
        'белом доме': 'белого дома',
    }
    if value in replacements:
        return replacements[value]
    # One-word dative names common in these tasks.
    name_map = {'юре': 'Юры', 'оле': 'Оли', 'тане': 'Тани', 'саше': 'Саши', 'миле': 'Милы', 'мите': 'Мити'}
    if value in name_map:
        return name_map[value]
    # Very small noun/adjective fallback for masculine location nouns.
    if value.endswith('ом доме'):
        return value[:-7] + 'ого дома'
    if value.endswith('ем доме'):
        return value[:-7] + 'его дома'
    if value.endswith('ом этаже'):
        return value[:-8] + 'ого этажа'
    if value.endswith('ей квартире'):
        return value[:-10] + 'ей квартиры'
    return phrase.strip()


def _v500_contextual_explanation_from_question(original_text: str, info: dict[str, str | bool]) -> str:
    """Build a meaningful dash explanation from the question, not from Excel.

    Prevents generic audit failures such as «(лет) – лет» and «(чел.) – человек».
    """
    q_norm = _v500_norm(_v500_last_question_sentence(original_text))
    text_norm = _v500_norm(original_text)
    unit = _v4011_norm_key(str(info.get('unit') or info.get('unitPhrase') or ''))
    tail = _v4011_clean_phrase(str(info.get('tail') or ''))
    rest = _v4011_clean_phrase(str(info.get('rest') or ''))

    if unit in {'лет', 'год', 'года', 'годов'}:
        m = re.search(r'сколько\s+лет\s+жив(?:е|ё)т\s+([а-яёa-z-]+)', q_norm)
        if m:
            subj = _v500_genitive_simple_ru(m.group(1))
            if subj == m.group(1):
                # textbook animal names: орел -> орла, гусь -> гуся
                subj = {'орел': 'орла', 'орёл': 'орла', 'гусь': 'гуся', 'собака': 'собаки'}.get(m.group(1), subj)
            return f'продолжительность жизни {subj}'.strip()
        m = re.search(r'сколько\s+лет\s+([а-яёa-z-]+)', q_norm)
        if m:
            person = _v500_genitive_simple_ru(m.group(1))
            return f'возраст {person}'.strip()
        if tail and tail not in {'лет', 'год', 'года'}:
            return f'возраст {_v500_genitive_simple_ru(tail)}'.strip()
        return 'возраст'

    people_units = set(_V4012_PEOPLE_UNITS) | {'житель', 'жителя', 'жителей', 'жилец', 'жильца', 'жильцов', 'жильцы'}
    if unit in people_units or any(w in q_norm for w in ('человек', 'людей', 'жильцов', 'жителей', 'ребят', 'детей', 'учеников')):
        if 'жильц' in q_norm or 'жител' in q_norm:
            m = re.search(r'(?:жильцов|жителей)\s+(?:жив(?:е|ё)т\s+)?(.+)$', q_norm)
            if m:
                loc = _v4011_clean_phrase(m.group(1)).strip(' .?!')
                return ('жильцов ' + loc).strip()
        m = re.search(r'человек\s+жив(?:е|ё)т\s+(.+)$', q_norm)
        if m:
            loc = _v500_genitive_simple_ru(m.group(1))
            return f'жителей {loc}'.strip()
        if rest:
            return ('человек ' + rest).strip()
        if tail and tail not in {'человек', 'людей'}:
            return tail
        return 'человек'

    return ''

def _v500_concise_explanation(original_text: str, info: dict[str, str | bool], *, total: bool = False, comparison: bool = False) -> str:
    text_norm = _v500_norm(original_text)
    q_norm = _v500_norm(_v500_last_question_sentence(original_text))
    # Prefer the semantic object/action phrase from the question when a bare unit
    # would be too weak for the UI-quality audit.  These are general patterns,
    # not Excel-row repairs: they apply to any similar wording.
    if 'машина проехала' in text_norm and 'проехала' in q_norm:
        phrase = 'проехала машина'
    elif 'сушен' in text_norm and 'груш' in text_norm and ('груш' in q_norm or 'сушен' in q_norm):
        phrase = 'сушеных груш'
    elif 'семян' in q_norm or 'семена' in q_norm:
        phrase = 'семян'
    elif 'зарабатывает' in q_norm:
        phrase = 'заработок мамы' if 'мама' in q_norm else 'заработок'
    elif 'частот' in q_norm:
        phrase = 'частота дыхания'
    elif 'вес' in q_norm and 'хлеб' in text_norm:
        phrase = 'вес черствого хлеба'
    elif bool(info.get('isMeasure')) and _v4011_clean_phrase(str(info.get('stepExplanation') or '')):
        phrase = _v4011_clean_phrase(str(info.get('stepExplanation') or ''))
    else:
        contextual = _v500_contextual_explanation_from_question(original_text, info)
        phrase = contextual or _v4012_count_object_phrase(info) or _v4011_clean_phrase(str(info.get('stepExplanation') or info.get('unitPhrase') or info.get('tail') or info.get('unit') or 'результат'))
    phrase = re.sub(r'^(?:сколько|всего)\s+', '', phrase, flags=re.IGNORECASE).strip()
    phrase = _v4013_strip_trailing_subject_tokens(phrase, original_text)
    phrase = _v4013_capitalize_known_names(phrase, original_text)
    if total and phrase and not phrase.lower().replace('ё', 'е').startswith('всего'):
        # Do not degrade good verbal predicates into generic phrases like
        # «всего проехала машина».
        if phrase in {'проехала машина'}:
            pass
        else:
            phrase = 'всего ' + phrase
    if comparison:
        unit_candidate = str(info.get('unit') or info.get('unitPhrase') or '').strip().lower()
        bare_units = {unit_candidate, 'лет', 'год', 'года', 'кг', 'руб', 'руб.', 'шт', 'шт.', 'человек', 'людей', 'жильцов', 'жителей'}
        if not phrase:
            phrase = 'разница'
        elif phrase.lower().strip(' .') in bare_units:
            if unit_candidate in {'лет', 'год', 'года'}:
                phrase = 'разница в возрасте'
            elif unit_candidate in {'человек', 'людей', 'жильцов', 'жителей'} or any(w in q_norm for w in ('жильц', 'жител', 'человек')):
                ctx = _v500_contextual_explanation_from_question(original_text, info)
                phrase = ctx or 'разница в людях'
            else:
                phrase = 'разница'
    return phrase or 'результат'


def _v500_answer_unit_and_info(original_text: str, fallback_unit: str = '') -> tuple[str, dict[str, str | bool]]:
    info = _v4011_question_info(original_text, fallback_unit)
    ...
    elif re.search(r'(?<!\d)\d+\s*тысяч\w*\s+лет', low):
        unit = 'тысяч лет'
        target = 'продолжительность жизни'
        if 'баобаб' in low:
            target = 'продолжительность жизни баобаба'
        info = {**dict(info), 'unit': 'тысяч лет', 'unitPhrase': 'тысяч лет', 'tail': target, 'stepExplanation': target, 'measureProperty': '', 'isMeasure': True}
    elif any(w in qlow for w in ('жильцов', 'жителей', 'жильца', 'жителя')):
        unit = 'жильцов'
        info = {**dict(info), 'unit': 'жильцов', 'unitPhrase': str(info.get('unitPhrase') or 'жильцов'), 'stepExplanation': str(info.get('stepExplanation') or 'жильцов')}
    elif ('вес' in qlow or 'масса' in qlow) and re.search(r'(?<!\d)\d+\s*(?:кг|килограмм)', low):
        unit = 'кг'
        info = {**dict(info), 'unit': 'кг', 'unitPhrase': 'кг', 'stepExplanation': str(info.get('measureProperty') or 'вес') + (' ' + str(info.get('measureObject')) if info.get('measureObject') else '')}
    elif ('зарабатывает' in qlow or 'стоимость' in qlow or 'цена' in qlow) and re.search(r'(?<!\d)\d+\s*(?:р\.|руб)', low):
        unit = 'руб.'
        info = {**dict(info), 'unit': 'руб.', 'unitPhrase': 'руб.', 'stepExplanation': 'заработок'}
    elif 'частот' in qlow and 'раз' in low:
        unit = 'раза'
        info = {**dict(info), 'unit': 'раза', 'unitPhrase': 'раза', 'stepExplanation': 'частота'}
    if not unit:
        stems = _v500_question_target_stems(original_text, info)
        if stems:
            unit = stems[0]
...
def _v500_self_verify_steps(steps: list[str]) -> dict[str, Any]:
    checked = 0
    failed: list[str] = []

    def eval_school_expr(expr: str) -> int | None:
        expr = re.sub(r'^\s*\d+\)\s*', '', str(expr or '').strip())
        expr = expr.replace('−', '-').replace('–', '-').replace('—', '-')
        expr = expr.replace('×', '*').replace('·', '*').replace('х', '*').replace('x', '*')
        expr = expr.replace('÷', ':')
        ...
        if not tokens:
            return None
        # Evaluate with normal precedence for multiplication/division, then addition/subtraction.
        values: list[int] = [int(tokens[0])]
        ops: list[str] = []
        i = 1
    ...
    for step in steps:
        ...
        if not m:
            continue
        expected = eval_school_expr(m.group(1))
        if expected is None:
            continue
        checked += 1
        if expected != int(m.group(2)):
            failed.append(line)
    return {'checked': checked, 'passed': not failed, 'failedSteps': failed}



def _v500_extract_visible_arithmetic_steps_from_payload(payload: dict[str, Any] | None) -> list[str]:
    """Extract visible arithmetic lines from an already produced API payload.

    This is deliberately generic evidence, not a solver: it lets V500 audit prove
    whether the visible DeepSeek/local text is arithmetically self-consistent even
    when no V500 semantic rule replaces the solution.
    """
    if not isinstance(payload, dict):
        return []
    structured = payload.get('structured_solution') if isinstance(payload.get('structured_solution'), dict) else payload.get('structuredSolution')
    if isinstance(structured, dict) and isinstance(structured.get('steps'), list):
        raw_steps = [str(x or '').strip() for x in structured.get('steps') or []]
    else:
        text = str(payload.get('userVisibleResultText') or payload.get('result') or payload.get('explanation') or '')
        raw_steps = []
        for line in re.split(r'\n+', text):
            clean = re.sub(r'<[^>]+>', ' ', str(line or '')).strip()
            clean = re.sub(r'\s+', ' ', clean)
            if '=' in clean and re.search(r'\d\s*[+\-−–—·×*/:÷]\s*\d', clean):
                # Drop leading headings but keep numbered school steps.
                clean = re.sub(r'^(?:решение|способ\s+\d+|метод\s+в\s+столбик)\s*[:.]\s*', '', clean, flags=re.IGNORECASE)
                raw_steps.append(clean)
    # Exclude lines that belong to column-method cards or explanatory non-step text.
    steps: list[str] = []
    for step in raw_steps:
        low = step.lower().replace('ё', 'е')
        if any(x in low for x in ('метод в столбик', 'разряд', 'пишем', 'переносим', 'занимаем')):
            continue
        if '=' in step:
            steps.append(step)
    return steps[:12]
...
def _v500_attach_existing_self_verifier(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Attach arithmetic verifier evidence to an existing payload without changing it."""
    if not isinstance(payload, dict):
        return payload
    if isinstance(payload.get('v500SelfVerifier'), dict):
        return payload
    ...
    out = dict(payload)
    structured = _v4011_structured(out)
    structured = {**structured, 'v500ExtractedSteps': steps, 'v500SelfCheck': verify}
    out['structured_solution'] = structured
    out['structuredSolution'] = structured
    out['v500SelfVerifier'] = verify
    out['v500SelfVerifierPassed'] = bool(verify.get('passed'))
    marker = 'v500-existing-step-self-verifier'
...
def _v501_last_step_result_number(steps: list[str]) -> str:
    for step in reversed([str(s or '') for s in steps]):
        m = re.search(r'=\s*(-?\d+(?:[,.]\d+)?)', step)
        if m:
            return _v501_normalize_answer_number(m.group(1))
    return ''
...
def _v501_raw_api_answer_candidate(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not steps and isinstance(structured, dict) and isinstance(structured.get('steps'), list):
    ...
    answer_number = _v501_normalize_answer_number(
        ...
        or (structured.get('answer_number') if isinstance(structured, dict) else '')
        or payload.get('answerNumber')
    )
    final_answer = str(
        evidence.get('rawDeepSeekFinalAnswer')
        or (parsed.get('final_answer') if isinstance(parsed, dict) else '')
        or payload.get('final_answer')
        or payload.get('answer')
        or (structured.get('final_answer') if isinstance(structured, dict) else '')
        or ''
    ).strip()
    answer_unit = str(
        (parsed.get('answer_unit') if isinstance(parsed, dict) else '')
        or payload.get('answer_unit')
...
def _v501_template_candidate_snapshot(*, rule: str, answer_number: int | str, answer_unit: str, final_answer: str, steps: list[str], self_check: dict[str, Any], template_evidence: dict[str, Any]) -> dict[str, Any]:
    return {
        'rule': rule,
        'answerNumber': _v501_normalize_answer_number(answer_number),
        'answerUnit': str(answer_unit or ''),
        'finalAnswer': str(final_answer or ''),
        'steps': [str(s or '') for s in steps[:12]],
        'selfVerifier': self_check,
        'templateEvidenceHash': _v501_sha(template_evidence),
        'confidence': template_evidence.get('confidence') if isinstance(template_evidence, dict) else None,
    }


def _v501_record_api_template_decision(out: dict[str, Any], *, api_candidate: dict[str, Any], template_candidate: dict[str, Any], decision: str, api_used: bool, conflict: bool) -> dict[str, Any]:
    if not isinstance(out, dict):
        return out
    evidence = dict(out.get('v501AiPipelineEvidence') or {}) if isinstance(out.get('v501AiPipelineEvidence'), dict) else {}
    record = {
        'apiAnswerConsidered': bool(api_candidate.get('present')),
        'apiAnswerCandidate': api_candidate,
        'apiCandidateTrusted': bool(api_candidate.get('trusted')),
        'apiAnswerDecision': decision,
        'apiAnswerUsedAsPrimary': bool(api_used),
        'apiTemplateConflict': bool(conflict),
    ...
    structured = _v4011_structured(out)
    structured = {**structured, **{f'v501{k[0].upper()}{k[1:]}': v for k, v in record.items() if k != 'apiAnswerCandidate'}}
    structured['v501ApiAnswerCandidate'] = api_candidate
    structured['v501TemplateCandidate'] = template_candidate
    out['structured_solution'] = structured
    out['structuredSolution'] = structured
    out.update({
        'v501ApiAnswerConsidered': bool(api_candidate.get('present')),
        'v501ApiCandidateTrusted': bool(api_candidate.get('trusted')),
        'v501ApiAnswerDecision': decision,
        'v501ApiAnswerUsedAsPrimary': bool(api_used),
        'v501ApiTemplateConflict': bool(conflict),
        'v501TemplateOverrodeTrustedApi': bool(conflict and api_candidate.get('trusted') and not api_used),
        'v501ApiProtectedFromTemplateOverride': bool(conflict and api_candidate.get('trusted') and api_used),
    })
    return out



def _v50103_scale_normalized_equivalent(original_text: str, api_candidate: dict[str, Any], template_candidate: dict[str, Any]) -> bool:
    low = _v500_norm(original_text)
    if 'тысяч' not in low:
        return False
    api_num = _v4011_int_number(api_candidate.get('answerNumber'))
    template_num = _v4011_int_number(template_candidate.get('answerNumber'))
    if api_num is None or template_num is None or template_num == 0:
        return False
    return bool(api_num == template_num * 1000 and re.search(r'\bтысяч\w*\s+(?:лет|глаз(?:ков|ками)?|предмет(?:ов|а)?|штук|шт\.?)\b', low))


def _v50103_sanitize_api_step_for_normalize(step: str) -> str:
    clean = str(step or '').strip().rstrip('.!?')
    # Convert "6 см + 5 см = 11 см" into a form that the school step normalizer
    # can annotate as "6 + 5 = 11 (см) – ...".
    if '=' in clean:
        left, right = clean.split('=', 1)
        left = re.sub(r'(?<=\d)\s*(?:см|мм|дм|м|км|кг|г|л|лет|руб(?:\.|лей|ля|ль)?|р\.?|слов(?:а|о)?|сл\.|книг(?:а|и)?|кн\.)\b', '', left, flags=re.IGNORECASE)
        right = re.sub(r'^\s*(-?\d+)\s*(?:см|мм|дм|м|км|кг|г|л|лет|руб(?:\.|лей|ля|ль)?|р\.?|слов(?:а|о)?|сл\.|книг(?:а|и)?|кн\.)\b', r'\1', right, flags=re.IGNORECASE)
        clean = left.strip() + ' = ' + right.strip()
    return clean



def _v50401_plural_green_lamp_phrase(number: int) -> str:
    if number % 100 in {11, 12, 13, 14}:
        return 'зелёных лампочек'
    last = number % 10
    if last == 1:
        return 'зелёная лампочка'
    if last in {2, 3, 4}:
...
def _v50401_replace_step_explanation(step: str, explanation: str, *, paren_unit: str = '') -> str:
    """Replace only the visible dash explanation, preserving verified arithmetic.

    This is a reusable UI-quality formatter: it never changes the expression or
    answer number, and it does not use Excel expected values or case ids.
    """
    clean = str(step or '').strip().rstrip('.!?')
    if not clean:
        return clean
    m = re.match(r'^(?P<expr>.*?=\s*-?\d+(?:[,.]\d+)?)\s*(?:\((?P<old_unit>[^)]+)\))?\s*(?:[—–-]\s*.*)?$', clean)
    if not m:
        return clean
    expr = re.sub(r'\s+', ' ', m.group('expr')).strip()
    unit = str(paren_unit or m.group('old_unit') or '').strip()
    expl = _v4013_capitalize_known_names(_v4011_clean_phrase(explanation), '')
    if unit:
        return f'{expr} ({unit}) – {expl}'
    return f'{expr} – {expl}'



def _v50402_filter_single_arithmetic_visible_steps(steps: list[str], original_text: str = '') -> list[str]:
    """Drop explanatory pseudo-steps when the solution has one real operation.

    This is a general UI formatter, not a case-specific repair.  It preserves the
    verified arithmetic line and answer number, but removes lines like
    "Всего кустов: 6" / "Чёрной смородины: 4" that otherwise make a one-action
    word problem look like a numbered multi-step solution.
    """
    clean_steps = [str(x or '').strip().rstrip('.!?') for x in (steps or []) if str(x or '').strip()]
    if len(clean_steps) <= 1:
        return clean_steps
    direct: list[str] = []
    for step in clean_steps:
        no_marker = re.sub(r'^\s*\d+[\).]\s*', '', step).strip()
        if _step_has_direct_result(no_marker) or re.search(r'\d+\s*[+\-−]\s*\d+\s*=\s*-?\d+\b', no_marker):
            direct.append(no_marker)
    if len(direct) != 1:
        return clean_steps
    # Keep true multi-action chains unchanged; collapse only a single arithmetic
    # expression with optional explanatory pseudo-lines.
    if _count_arithmetic_actions_in_step(direct[0]) > 1:
        return clean_steps
    pseudo_ok = True
    for step in clean_steps:
        no_marker = re.sub(r'^\s*\d+[\).]\s*', '', step).strip()
        if no_marker == direct[0]:
            continue
        # Pseudo-lines can mention known values, but must not contain arithmetic
        # operators or equations.
        if '=' in no_marker or re.search(r'\d+\s*[+\-−:·×*/÷]\s*\d+', no_marker):
            pseudo_ok = False
            break
...
def _v50401_general_visible_answer_and_step_fix(out: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    """General V504.02 UI-quality repair for recurring question patterns.

    It repairs grammar/full-answer wording and dash explanations only. It does
    not change answer_number, does not read Excel expected, and is not keyed by
    case id. The rules are reusable templates for unknown-part/comparative rows.
    """
    if not isinstance(out, dict):
        return out
    structured = _v4011_structured(out)
    answer_number_raw = out.get('answer_number') or out.get('answerNumber') or structured.get('answer_number')
    n = _v4011_int_number(answer_number_raw)
    if n is None:
        return out
    low = _v500_norm(original_text)
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    ...
    if m_pulse:
        ...
        expl = f'пульс {who}'
        paren_unit = 'уд.'
        suffix = ', это такой же пульс, как у человека' if ('сравни' in low and 'человек' in low) else ''
        final = f'пульс у {who} {n} {_v4011_plural(n, "ударов") or "ударов"} в минуту{suffix}'

    # Unknown part of a set: total lamps, some violet, the rest green.
    elif 'лампоч' in low and re.search(r'сколько\s+зел[её]н', qlow):
        lamps = _v50401_plural_green_lamp_phrase(n)
        expl = 'зелёных лампочек'
        paren_unit = 'шт.'
        final = f'в гирлянде {n} {lamps}'

    # Unknown berry glasses: «Сколько принесли смородины?» with total glasses.
    elif 'смородин' in qlow and 'стакан' in low and 'принес' in low:
        expl = 'стаканов смородины'
        paren_unit = 'шт.'
        final = f'принесли {n} {_v4011_plural(n, "стакан") or "стаканов"} смородины'

    # Unknown beet boxes from a total containing beet and carrot boxes.
    elif 'свекл' in qlow and 'ящик' in low and 'морков' in low:
        expl = 'ящиков со свёклой'
        paren_unit = 'шт.'
        final = f'привезли {n} {_v4011_plural(n, "ящик") or "ящиков"} со свёклой'

    # Boys/girls remainder in a yard group.
    elif 'девоч' in qlow and 'мальчик' in low and 'ребят' in low:
        expl = 'девочек во дворе'
        paren_unit = 'чел.'
        final = f'во дворе было {n} {_v4011_plural(n, "девочка") or "девочек"}'

    # Vehicle remainder: total cars minus freight trucks.
    elif 'легков' in qlow and 'автомаш' in low and 'грузов' in low:
        expl = 'легковых автомашин'
        paren_unit = 'шт.'
        final = f'во дворе стоит {n} легковых автомашин'

    # Train remainder: total trains minus passenger trains.
    elif 'товарн' in qlow and 'поезд' in low and 'пассажирск' in low:
        expl = 'товарных поездов'
        paren_unit = 'шт.'
        final = f'прошло {n} товарных поездов'

    # Tree remainder after maples/lindens.
    elif 'лип' in qlow and 'клён' in low or ('лип' in qlow and 'клен' in low):
        expl = 'лип'
        paren_unit = 'шт.'
        final = f'на улице посадили {n} лип'

    # Linguistic count row: keep the phrase, but make the answer a full sentence.
    elif 'всегда глухих согласных' in qlow and 'сонорн' in low:
        expl = 'всегда глухих согласных'
        paren_unit = 'шт.'
        final = f'в русском языке {n} всегда глухих согласных'

    # People who became ill: avoid bare «3 человека заболело».
    elif 'сколько человек заболело' in qlow and 'секци' in low:
        expl = 'заболевших в секции'
        paren_unit = 'чел.'
    ...
    if not fixed_steps:
        return out
    fixed_steps = _v50402_filter_single_arithmetic_visible_steps(fixed_steps, original_text)

    result = _format_primary_solution_text(original_text, fixed_steps, final)
    visible = _v312_format_visible_result(fixed_steps, final)
    fixed = dict(out)
    st = {**structured, 'steps': fixed_steps, 'answer_number': str(answer_number_raw), 'final_answer': final}
    fixed.update({
        'result': result,
        'explanation': result,
...
def _v50103_format_trusted_api_payload(payload: dict[str, Any] | None, original_text: str, *, api_candidate: dict[str, Any] | None = None, template_candidate: dict[str, Any] | None = None, decision: str = 'api_primary_verified_formatted', conflict: bool = False) -> dict[str, Any] | None:
    ...
    This deliberately does not change the answer number. It is not a caseId repair:
    the API answer is accepted only when arithmetic self-verifier marks it trusted.
    """
    if not isinstance(payload, dict):
        return None
    ...
    if not final:
        return None
    repaired_steps = _v50402_filter_single_arithmetic_visible_steps(repaired_steps, original_text)
    ...
    structured = {
        **_v4011_structured(out),
        ...
        'final_answer': final,
        'v500Rule': '',
        'v500SelfCheck': api_candidate.get('selfVerifier') or _v500_self_verify_steps(repaired_steps),
        'v500LearningMode': 'api_primary_verified_formatted',
    ...
    out.update({
        ...
        'backendPreparedVisibleResult': True,
        'validated': True,
        'source': str(out.get('source') or 'deepseek-primary') + '; api-primary-verified-formatted-v502',
        'answer': final,
        'answer_number': answer_number,
        'answer_unit': answer_unit,
        'final_answer': final,
        ...
        'v500GeneralRuleApplied': False,
        'v500GeneralRule': '',
        'v500SelfVerifier': structured.get('v500SelfCheck'),
        'v500SelfVerifierPassed': bool((structured.get('v500SelfCheck') or {}).get('passed')),
        'v500LearningMode': 'api_primary_verified_formatted',
        'v500UsesExcelExpected': False,
        'v500CaseSpecificRepair': False,
        'v501ApiPrimaryVerifiedFormatted': True,
    ...
    structured_after['v501ApiPrimaryVerifiedFormatted'] = True
    out['structured_solution'] = structured_after
...
def _v501_keep_trusted_api_payload(payload: dict[str, Any] | None, original_text: str, *, api_candidate: dict[str, Any], template_candidate: dict[str, Any]) -> dict[str, Any] | None:
    if not isinstance(payload, dict):
        return None
    formatted = _v50103_format_trusted_api_payload(payload, original_text, api_candidate=api_candidate, template_candidate=template_candidate, decision='used_trusted_api_over_conflicting_template', conflict=True)
    if isinstance(formatted, dict):
        return formatted
    out = dict(payload)
    out = _v500_attach_existing_self_verifier(out) or out
    out['source'] = str(out.get('source') or 'deepseek-primary') + '; api-primary-preserved-v502'
    verifier = str(out.get('verifier') or '')
    marker = 'v502-api-answer-primary-conflict-preserved'
    if marker not in verifier:
        out['verifier'] = verifier + ('; ' if verifier else '') + marker
    out['v500LearningMode'] = 'api_primary_verified'
    out['v500GeneralRuleApplied'] = False
    out['v500GeneralRule'] = ''
    out['v500TemplateId'] = ''
    out['v500TemplateEvidence'] = template_candidate
    out['v500UsesExcelExpected'] = False
    out['v500CaseSpecificRepair'] = False
    out = _v501_record_api_template_decision(out, api_candidate=api_candidate, template_candidate=template_candidate, decision='used_trusted_api_over_conflicting_template', api_used=True, conflict=True)
    contract = str(out.get('visibleResultContract') or '').strip()
    marker_contract = 'v502-api-answer-primary'
    if marker_contract not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker_contract
    return out

def _v500_condition_and_question_numbers(original_text: str, info: dict[str, str | bool]) -> tuple[list[int], list[int]]:
    condition = _v500_condition_text(original_text)
    question = _v500_last_question_sentence(original_text)
    stems = _v500_question_target_stems(original_text, info)
    near_condition = _v500_numbers_near_target_in_text(condition, stems)
    condition_nums = near_condition if near_condition else _v500_ints(condition)
    question_nums = _v500_ints(question)
    return condition_nums, question_nums


def _v500_extract_ratio_comparison(original_text: str, info: dict[str, str | bool]) -> tuple[int, int, int] | None:
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    if 'во сколько раз' not in qlow and 'сколько раз' not in qlow:
        return None
    nums, _ = _v500_condition_and_question_numbers(original_text, info)
    if len(nums) < 2:
        return None
    a, b = nums[-2], nums[-1]
...
def _v500_number_spans(text: str) -> list[tuple[int, int, int]]:
    return [(m.start(), m.end(), int(m.group(0))) for m in re.finditer(r'(?<!\d)\d+(?!\d)', str(text or ''))]


def _v500_extract_cost_payload_args(original_text: str, info: dict[str, str | bool]) -> tuple[str, int, int, int, str, str] | None:
    """Return (op, a, b, result, unit, explanation) for price/count tasks."""
    cond = _v500_condition_text(original_text)
    q = _v500_last_question_sentence(original_text)
    low = _v500_norm(cond)
    qlow = _v500_norm(q)
    if not any(w in low + ' ' + qlow for w in ('руб', 'р.', 'коп', 'цена', 'стоимост', 'стоит', 'заплат')):
        return None
    # price near rubles
    price_matches = list(re.finditer(r'(?<!\d)(\d+)(?!\d)\s*(?:р\.|руб(?:\.|лей|ля|ль)?|коп(?:\.|еек|ейки)?)', cond, flags=re.IGNORECASE))
    if not price_matches:
        return None
    money_value = int(price_matches[-1].group(1))
    money_spans = [(m.start(), m.end()) for m in price_matches]
    all_nums = _v500_number_spans(cond + ' ' + q)
    non_money: list[int] = []
    for start, end, value in all_nums:
        in_money = any(start >= a and end <= b for a, b in money_spans)
        if not in_money:
            non_money.append(value)
    q_nums = _v500_ints(q)
    # Multiplication: unit price × requested quantity.
    if any(w in qlow for w in ('сколько стоят', 'сколько стоит', 'стоимость', 'заплатить', 'сколько денег')) and ('один' not in qlow and '1 ' not in qlow):
        qty = q_nums[-1] if q_nums else (non_money[-1] if non_money else None)
        if qty and qty > 0:
            return 'mul', money_value, qty, money_value * qty, 'руб.', 'стоимость покупки'
    # Division: total cost ÷ quantity = price of one.
    if any(w in qlow for w in ('сколько стоит один', 'сколько стоит 1', 'цена одного', 'цена 1', 'сколько стоит каж')):
        qty_candidates = [n for n in non_money if n > 0]
        if qty_candidates:
            qty = qty_candidates[-1]
            if qty and money_value % qty == 0:
                return 'div', money_value, qty, money_value // qty, 'руб.', 'цена одного предмета'
    return None


def _v500_extract_measure_quantity_payload_args(original_text: str, info: dict[str, str | bool]) -> tuple[str, int, int, int, str, str] | None:
    """Return (op, measure, qty, result, unit, explanation) for mass/volume/length per item tasks."""
    cond = _v500_condition_text(original_text)
    q = _v500_last_question_sentence(original_text)
    low = _v500_norm(cond)
    qlow = _v500_norm(q)
    unit_match = list(re.finditer(r'(?<!\d)(\d+)(?!\d)\s*(кг|килограмм(?:ов|а)?|г|грамм(?:ов|а)?|л|литр(?:ов|а)?|м|метр(?:ов|а)?|см|сантиметр(?:ов|а)?|т|тонн(?:а|ы)?|ц|центнер(?:ов|а)?)\b', cond, flags=re.IGNORECASE))
    if not unit_match:
        return None
    m = unit_match[-1]
    measure = int(m.group(1))
    raw_unit = m.group(2).lower().replace('ё', 'е')
    unit_map = {
        'килограмм': 'кг', 'килограмма': 'кг', 'килограммов': 'кг', 'кг': 'кг',
        'грамм': 'г', 'грамма': 'г', 'граммов': 'г', 'г': 'г',
        'литр': 'л', 'литра': 'л', 'литров': 'л', 'л': 'л',
    ...
    spans = [(x.start(), x.end()) for x in unit_match]
    nums = []
    for start, end, value in _v500_number_spans(cond):
        if any(start >= a and end <= b for a, b in spans):
            continue
        if value > 0:
            nums.append(value)
    # If one item has a measure and the question asks total for N items.
    if any(w in low for w in ('один', 'одна', 'одно', 'каждый', 'каждая', 'каждое')) and any(w in qlow for w in ('сколько всего', 'сколько весят', 'общая масса', 'вместе')):
        qty = (_v500_ints(q) or nums or [0])[-1]
        if qty > 0:
            return 'mul', measure, qty, measure * qty, unit, 'общая величина'
    # If total measure and count are known, find one/equal part.
    if any(w in qlow for w in ('один', 'одна', 'одно', 'каждый', 'каждая', 'каждое', 'в одном', 'масса одного', 'вес одного')):
        qty = (nums or _v500_ints(q) or [0])[-1]
        if qty > 0 and measure % qty == 0:
            return 'div', measure, qty, measure // qty, unit, 'одна часть'
    return None
...
# V501 learning layer: every generalized repair must expose template evidence.
# The goal is to prove that the solver used a reusable semantic template and not
# an exact Excel-row answer map.  These metadata are diagnostic only; Excel
# expected values are intentionally absent from the template evidence.
_V500_GENERAL_RULE_CATALOG: dict[str, dict[str, Any]] = {
    'direct_total_addition': {'operation': 'addition', 'family': 'total', 'template': 'two_or_more_stated_quantities -> total'},
    'same_amount_total': {'operation': 'addition', 'family': 'equal_quantity', 'template': 'A and same amount -> total'},
    'same_amount_equal': {'operation': 'identity', 'family': 'equal_quantity', 'template': 'A and same amount -> equal target'},
    'sum_same_as_two_previous': {'operation': 'addition', 'family': 'dependent_total', 'template': 'target equals sum of two previous quantities'},
    'comparative_offset': {'operation': 'addition_or_subtraction', 'family': 'comparative_offset', 'template': 'base quantity +/- stated offset'},
    ...
    'measure_per_item_times_quantity': {'operation': 'multiplication', 'family': 'measure_quantity', 'template': 'measure per item * quantity -> total measure'},
    'total_measure_div_quantity': {'operation': 'division', 'family': 'measure_quantity', 'template': 'total measure / quantity -> measure per item'},
    'speed_times_time_distance': {'operation': 'multiplication', 'family': 'motion', 'template': 'speed * time -> distance'},
    'distance_div_time_speed': {'operation': 'division', 'family': 'motion', 'template': 'distance / time -> speed'},
    'distance_div_speed_time': {'operation': 'division', 'family': 'motion', 'template': 'distance / speed -> time'},
    'equal_groups_multiplication': {'operation': 'multiplication', 'family': 'equal_groups', 'template': 'number of groups * items per group -> total'},
    'equal_sharing_division': {'operation': 'division', 'family': 'equal_groups', 'template': 'total items / equal groups -> items per group'},
    'thousand_count_total_addition': {'operation': 'addition', 'family': 'scale_count', 'template': 'thousand-scale counts added without converting answer_number to absolute count'},
    'stone_equal_backpack_distribution': {'operation': 'addition_and_division', 'family': 'combinatorial_distribution', 'template': 'partition item weights into equal-sum backpacks'},
}



def _v500_operation_symbols_from_steps(steps: list[str]) -> list[str]:
    symbols: list[str] = []
    for line in steps:
        expr = str(line or '').split('=')[0]
        for raw, normalized in [('+', '+'), ('-', '-'), ('−', '-'), ('–', '-'), ('—', '-'), ('·', '·'), ('×', '·'), ('*', '·'), (':', ':'), ('÷', ':')]:
            if raw in expr and normalized not in symbols:
                symbols.append(normalized)
    return symbols


def _v500_trigger_keywords(original_text: str, rule: str) -> list[str]:
    low = _v500_norm(original_text)
    candidates = [
        'сколько всего', 'сколько стало', 'стало', 'всего', 'вместе', 'ещё', 'еще', 'столько же',
        'столько', 'на сколько', 'больше', 'меньше', 'старше', 'моложе', 'длиннее', 'короче',
        'во сколько раз', 'раза', 'осталось', 'цена', 'стоимость', 'стоит', 'заплатить',
        'один', 'каждый', 'каждая', 'поровну', 'в каждом', 'по ', 'скорость', 'расстояние', 'путь', 'время',
    ]
    found = [kw.strip() for kw in candidates if kw.strip() and kw in low]
    if not found and rule in _V500_GENERAL_RULE_CATALOG:
        found = [str(_V500_GENERAL_RULE_CATALOG[rule].get('family') or rule)]
    return found[:8]


def _v500_template_evidence(original_text: str, *, rule: str, steps: list[str], answer_number: int | str, answer_unit: str, self_check: dict[str, Any]) -> dict[str, Any]:
    catalog = _V500_GENERAL_RULE_CATALOG.get(rule, {})
    condition_nums, question_nums = _v500_condition_and_question_numbers(original_text, {'unit': str(answer_unit or '')})
    return {
        'learningMode': 'general_template',
        'templateId': rule,
        'templateFamily': catalog.get('family') or 'unknown',
        'template': catalog.get('template') or rule,
        'operation': catalog.get('operation') or 'unknown',
        'operationSymbols': _v500_operation_symbols_from_steps(steps),
        'triggerKeywords': _v500_trigger_keywords(original_text, rule),
        'conditionNumberCount': len(condition_nums),
        'questionNumberCount': len(question_nums),
        'stepCount': len(steps),
        'answerNumber': str(answer_number),
        'answerUnit': str(answer_unit or ''),
        'selfVerifierChecked': int(self_check.get('checked') or 0) if isinstance(self_check, dict) else 0,
        'selfVerifierPassed': bool(self_check.get('passed')) if isinstance(self_check, dict) else False,
        'usesExcelExpected': False,
        'usesExactCaseId': False,
        'caseSpecificRepair': False,
        'confidence': 0.96 if bool(self_check.get('passed')) else 0.0,
    }


def _v500_mark_case_specific_repair(payload: dict[str, Any] | None, marker: str) -> dict[str, Any] | None:
    if not isinstance(payload, dict):
        return payload
    out = dict(payload)
    out['v500LearningMode'] = 'case_specific_repair'
    out['v500CaseSpecificRepair'] = True
    out['v500CaseSpecificRepairMarker'] = marker
    out['v500UsesExcelExpected'] = False
    structured = _v4011_structured(out)
    structured = {**structured, 'v500LearningMode': 'case_specific_repair', 'v500CaseSpecificRepair': True, 'v500CaseSpecificRepairMarker': marker}
    out['structured_solution'] = structured
    out['structuredSolution'] = structured
    return out


def _v500_extract_difference_between_two_quantities(original_text: str, info: dict[str, str | bool]) -> tuple[int, int, int] | None:
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    if 'на сколько' not in qlow:
        return None
    # Avoid total questions such as «На сколько видов зверей и птиц запрещена охота?».
    if not any(w in qlow for w in ('больше', 'меньше', 'старше', 'моложе', 'длиннее', 'короче', 'дороже', 'дешевле', 'выше', 'ниже', 'тяжелее', 'легче')):
        return None
    nums, _ = _v500_condition_and_question_numbers(original_text, info)
    if len(nums) < 2:
        return None
...
def _v500_extract_factor_then_difference(original_text: str, info: dict[str, str | bool]) -> tuple[str, int, int, int, int] | None:
    """Return (op_for_derived, base, factor, derived, difference).

    Covers general tasks like: «Саше 12 лет, а Оля в 4 раза моложе.
    На сколько лет Саша старше Оли?» without using any Excel expected answer.
    """
    cond = _v500_condition_text(original_text)
    low = _v500_norm(cond)
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    if 'раз' not in low or 'на сколько' not in qlow:
        return None
    if not any(w in qlow for w in ('больше', 'меньше', 'старше', 'моложе', 'длиннее', 'короче', 'дороже', 'дешевле', 'выше', 'ниже', 'тяжелее', 'легче')):
        return None
    m = re.search(r'(?<!\d)(\d+)(?!\d).*?\bв\s+(\d+)\s+раз(?:а|)?(?:\s+[а-яёa-z.-]+){0,3}\s+(больше|меньше|старше|моложе|длиннее|короче|дороже|дешевле|выше|ниже|тяжелее|легче)', low, flags=re.IGNORECASE)
    if not m:
        return None
    base = int(m.group(1))
    ...
    if base <= 0 or factor <= 0:
        return None
    if relation in {'меньше', 'моложе', 'короче', 'дешевле', 'ниже', 'легче'}:
        if base % factor != 0:
            return None
        derived = base // factor
        op = ':'
        ...
        derived = base * factor
        op = '·'
    diff = abs(base - derived)
    if diff <= 0:
        return None
    return op, base, factor, derived, diff




def _v501_extract_mixed_purchase_unknown_unit_price(original_text: str) -> tuple[str, int, int, int, int, int] | None:
    """Return (target_item, known_qty, known_price, known_cost, total_cost, target_qty).

    Covers reusable mixed-purchase tasks: «Купили 3 эскимо по 3 р. и 4
    пломбира. За всю покупку заплатили 25 р. Сколько стоит один пломбир?».
    This must run before equal_groups_multiplication: the question asks for a
    unit price, not for total number of objects.
    """
    cond = _v500_condition_text(original_text)
    question = _v500_last_question_sentence(original_text)
    low = _v500_norm(cond)
    qlow = _v500_norm(question)
    if not (('сколько стоит один' in qlow or 'цена одного' in qlow or 'стоит одна' in qlow or 'стоит 1' in qlow) and re.search(r'\bпо\s+\d+\s*(?:р|руб)', low)):
        return None
    total_match = re.search(r'(?:заплатили|заплатил|заплатила|стоил[аио]?|стоимость|за\s+всю\s+покупку[^.?!]{0,30})(?:[^\d]|^)(\d+)\s*(?:р\.?|руб)', low)
    if not total_match:
        total_match = re.search(r'(\d+)\s*(?:р\.?|руб)', low)
    if not total_match:
        return None
    total_cost = int(total_match.group(1))
    known_match = re.search(r'(\d+)\s+([а-яёa-z-]+)[^.!?]{0,50}?\bпо\s+(\d+)\s*(?:р\.?|руб)', low)
    if not known_match:
        return None
    known_qty = int(known_match.group(1))
    known_price = int(known_match.group(3))
    known_cost = known_qty * known_price
    target_match = re.search(r'(?:сколько\s+стоит\s+(?:один|одна|одно)|цена\s+одного)\s+([а-яёa-z-]+)', qlow)
    if not target_match:
        return None
    target = target_match.group(1).strip(' .?!,;:')
    if not target:
        return None
    # Find the target group quantity in the condition, allowing plural endings.
    target_stem = target[:max(3, min(len(target), 6))]
    qty_matches = list(re.finditer(r'(\d+)\s+([а-яёa-z-]+)', low))
    target_qty = 0
    for m in qty_matches:
        word = m.group(2)
        if word.startswith(target_stem) or target.startswith(word[:max(3, min(len(word), 6))]):
            target_qty = int(m.group(1))
    if target_qty <= 0 or total_cost <= known_cost:
        return None
    rest = total_cost - known_cost
    if rest % target_qty != 0:
        return None
    return target, known_qty, known_price, known_cost, total_cost, target_qty

def _v500_extract_equal_groups_payload_args(original_text: str, info: dict[str, str | bool]) -> tuple[str, int, int, int, str] | None:
    """Return (op, a, b, result, explanation) for counted equal groups.

    Handles reusable school templates: «в 3 коробках по 8 карандашей» and
    «24 карандаша разложили поровну в 3 коробки».
    """
    cond = _v500_condition_text(original_text)
    q = _v500_last_question_sentence(original_text)
    low = _v500_norm(cond)
    qlow = _v500_norm(q)
    if ('сколько стоит один' in qlow or 'цена одного' in qlow or 'стоит одна' in qlow) and re.search(r'\bпо\s+\d+\s*(?:р|руб)', low) and any(w in low for w in ('заплатили', 'покупк', 'стоимость')):
        return None
    if not any(w in qlow for w in ('сколько', 'всего', 'стало', 'получилось', 'в каждом', 'на каждого', 'в одной')):
        return None
    # Equal groups multiplication: group count and «по N» items in each group.
    m_each = re.search(r'\bпо\s+(\d+)\b', low)
    if m_each and any(w in qlow for w in ('сколько всего', 'всего', 'сколько предметов', 'сколько карандаш', 'сколько книг', 'сколько яблок', 'сколько')):
        per_group = int(m_each.group(1))
        before = low[:m_each.start()]
        nums_before = [n for n in _v500_ints(before) if n > 0]
        if nums_before:
            groups = nums_before[-1]
            if groups > 0 and per_group > 0:
                return 'mul', groups, per_group, groups * per_group, 'всего предметов'
    # Equal sharing division: total divided into equal groups.
    if any(w in low for w in ('поровну', 'одинаково', 'равными частями', 'разложили', 'раздали', 'разделили')) and any(w in qlow for w in ('в каждом', 'на каждого', 'в одной', 'сколько досталось', 'сколько получилось')):
        nums = [n for n in _v500_ints(cond) if n > 0]
        if len(nums) >= 2:
            total, groups = nums[0], nums[-1]
            if groups > 0 and total >= groups and total % groups == 0:
                return 'div', total, groups, total // groups, 'в одной группе'
    return None

def _v500_build_payload(payload: dict[str, Any] | None, original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str, answer_unit: str, rule: str) -> dict[str, Any] | None:
    clean_steps = [str(s or '').strip().rstrip('.!?') for s in steps if str(s or '').strip()]
    clean_steps = [_v50202_fix_api_primary_step_explanation(step, original_text) for step in clean_steps]
    if not clean_steps:
        return None
    verify = _v500_self_verify_steps(clean_steps)
    if not bool(verify.get('passed')):
        return None
    template_evidence = _v500_template_evidence(original_text, rule=rule, steps=clean_steps, answer_number=answer_number, answer_unit=answer_unit, self_check=verify)
    final_answer = _v4011_fix_answer_grammar(str(final_answer or '').strip().rstrip('.!?'), original_text)
    final_answer = re.sub(r'\b(\d+)\s+\1\b', r'\1', final_answer).strip()
    if not final_answer:
        return None
    api_candidate = _v501_raw_api_answer_candidate(payload)
    template_candidate = _v501_template_candidate_snapshot(rule=rule, answer_number=answer_number, answer_unit=answer_unit, final_answer=final_answer, steps=clean_steps, self_check=verify, template_evidence=template_evidence)
    api_num = str(api_candidate.get('answerNumber') or '')
    template_num = str(template_candidate.get('answerNumber') or '')
    scale_equivalent = _v50103_scale_normalized_equivalent(original_text, api_candidate, template_candidate)
    if scale_equivalent:
    ...
    structured = {
        **_v4011_structured(out),
        ...
        'final_answer': final_answer,
        'v500Rule': rule,
        'v500SelfCheck': verify,
        'v500TemplateEvidence': template_evidence,
        'v500LearningMode': 'general_template',
        'v500TemplateId': rule,
        'v500TemplateConfidence': template_evidence.get('confidence'),
        'v500UsesExcelExpected': False,
        'v500CaseSpecificRepair': False,
    }
    out.update({
        'result': result,
        'explanation': result,
        'userVisibleResultText': visible_result or result,
        'backendPreparedVisibleResult': True,
        'validated': True,
        'source': 'deepseek-primary-v500-generalized-postprocess',
        'answer': final_answer,
        'answer_number': str(answer_number),
        'answer_unit': str(answer_unit or ''),
        'final_answer': final_answer,
        ...
        'v500SelfVerifier': verify,
        'v500SelfVerifierPassed': bool(verify.get('passed')),
        'v500TemplateEvidence': template_evidence,
        'v500TemplateId': rule,
        'v500TemplateConfidence': template_evidence.get('confidence'),
        'v500LearningMode': 'general_template',
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v504-02-automation-pipeline'
    if marker not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + f'v500-general-rule:{rule}'
    decision = 'template_scale_normalized_trusted_api' if scale_equivalent else ('template_agreed_with_trusted_api' if api_candidate.get('trusted') and str(api_candidate.get('answerNumber') or '') == str(template_candidate.get('answerNumber') or '') else ('template_used_api_not_trusted_or_absent' if api_candidate.get('present') else 'template_used_no_api_candidate'))
    out = _v501_record_api_template_decision(out, api_candidate=api_candidate, template_candidate=template_candidate, decision=decision, api_used=False, conflict=False)
    if scale_equivalent:
        out['v501ApiScaleNormalized'] = True
        structured_scale = _v4011_structured(out)
        structured_scale['v501ApiScaleNormalized'] = True
        out['structured_solution'] = structured_scale
        out['structuredSolution'] = structured_scale
        if isinstance(out.get('v501AiPipelineEvidence'), dict):
            out['v501AiPipelineEvidence']['apiScaleNormalized'] = True
    out = _v50401_general_visible_answer_and_step_fix(out, original_text) or out
    return _v4013_finalize_payload_text(out, original_text)



def _v50202_extract_thousand_count_total(original_text: str, info: dict[str, str | bool]) -> tuple[int, int, int, str, str] | None:
    """Reusable template for counts stated in thousands.

    It deliberately keeps the answer scale as «тысяч ...» instead of converting
    4 тысячи + 6 тысяч to answer_number=10000 and then saying «10000 тысяч».
    Excel is not used here; the scale is inferred from the task wording.
    """
    cond = _v500_condition_text(original_text)
    q = _v500_last_question_sentence(original_text)
    low = _v500_norm(cond)
    ...
    if 'тысяч' not in low:
        return None
    if not any(w in qlow for w in ('вместе', 'сколько всего', 'сколькими', 'сколько')):
        return None
    number_matches = list(re.finditer(r'(?<!\d)(\d+)(?!\d)\s*тысяч\w*', low, flags=re.IGNORECASE))
    if len(number_matches) < 2:
        return None
    a = int(number_matches[-2].group(1))
    b = int(number_matches[-1].group(1))
    obj = 'предметов'
    object_matches = list(re.finditer(r'(?<!\d)\d+(?!\d)\s*тысяч\w*\s+([а-яё]+)', low, flags=re.IGNORECASE))
    if object_matches:
        obj = object_matches[-1].group(1).strip()
    if 'глаз' in low or 'глаз' in qlow:
        obj = 'глазков'
    result = a + b
    return a, b, result, 'тысяч ' + obj, obj


def _v50202_thousand_count_total_payload(payload: dict[str, Any] | None, original_text: str, info: dict[str, str | bool]) -> dict[str, Any] | None:
    data = _v50202_extract_thousand_count_total(original_text, info)
    if data is None:
        return None
    a, b, result_number, answer_unit, obj = data
    step = f'{a} + {b} = {result_number} (тыс. шт.) – всего {obj}'
    if 'мух' in _v500_norm(original_text) and 'мурав' in _v500_norm(original_text):
        final = f'вместе у мухи и муравья {result_number} тысяч {obj}'
    else:
        final = f'вместе {result_number} тысяч {obj}'
    return _v500_build_payload(payload, original_text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=answer_unit, rule='thousand_count_total_addition')


def _v50202_stone_distribution_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    """General combinatorial template for equal-sum backpack stone distribution.

    This is intentionally not a caseId answer map.  It recognizes the structure
    «stones with masses 1..7 kg distributed into four backpacks with equal mass»
    and exposes all numeric tokens required by the multi-answer audit.
    """
    if not _v4013_is_stone_distribution_task(original_text):
        return None
    ...
    total = sum(weights)
    backpacks = 4
    per_bag = total // backpacks
    groups = [(1, 6), (2, 5), (3, 4), (7,)]
    steps = [
        '1) 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28 (кг) – общая масса камней',
        '2) 28 : 4 = 7 (кг) – масса в каждом рюкзаке',
        '3) 1 + 6 = 7 (кг) – первый рюкзак',
        '4) 2 + 5 = 7 (кг) – второй рюкзак',
        '5) 3 + 4 = 7 (кг) – третий рюкзак',
        '6) 7 = 7 (кг) – четвертый рюкзак',
    ]
    final_answer = 'в каждом рюкзаке по 7 кг: 1 кг и 6 кг; 2 кг и 5 кг; 3 кг и 4 кг; 7 кг'
    result = _format_primary_solution_text(original_text, steps, final_answer)
    visible_result = _v312_format_visible_result(steps, final_answer)
    verify = _v500_self_verify_steps(steps)
    answer_tokens = ['7', '1', '6', '2', '5', '3', '4']
    template_evidence = _v500_template_evidence(
        original_text,
        rule='stone_equal_backpack_distribution',
        steps=steps,
        answer_number='7',
        answer_unit='кг',
        self_check=verify,
    )
    template_evidence.update({
        'multiAnswerTokens': answer_tokens,
        'backpackGroups': [[str(x) for x in g] for g in groups],
        'usesExcelExpected': False,
        'usesExactCaseId': False,
        'caseSpecificRepair': False,
    })
    ...
    structured = {
        **_v4011_structured(out),
        ...
        'answer_unit': 'кг',
        'final_answer': final_answer,
        'v500Rule': 'stone_equal_backpack_distribution',
        'v500SelfCheck': verify,
        'v500TemplateEvidence': template_evidence,
        'v500LearningMode': 'general_template',
        'v500TemplateId': 'stone_equal_backpack_distribution',
        'v500TemplateConfidence': template_evidence.get('confidence'),
        'v500UsesExcelExpected': False,
        'v500CaseSpecificRepair': False,
        'v502ExplicitMultiAnswerTemplate': True,
    }
    out.update({
        'result': result,
        'explanation': result,
        ...
        'backendPreparedVisibleResult': True,
        'validated': True,
        'source': 'deepseek-primary-v50202-combinatorial-template-postprocess',
        'answer': final_answer,
        'answer_number': '7',
        'answer_unit': 'кг',
        'final_answer': final_answer,
        ...
        'structuredSolution': structured,
        'v500GeneralRuleApplied': True,
        'v500GeneralRule': 'stone_equal_backpack_distribution',
        'v500SelfVerifier': verify,
        'v500SelfVerifierPassed': bool(verify.get('passed')),
        'v500TemplateEvidence': template_evidence,
        'v500TemplateId': 'stone_equal_backpack_distribution',
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v502-02-stone-equal-backpack-distribution-template'
    if marker not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker
...
def _v50202_fix_api_primary_step_explanation(step: str, original_text: str) -> str:
    """General UI-quality guard for meaningless dash tails like «– раз»."""
    line = str(step or '').strip()
    low_task = _v500_norm(original_text)
    if re.search(r'\(раз\)\s*[—–-]\s*раз\.?$', line, flags=re.IGNORECASE):
        if 'попал' in low_task and 'мишень' in low_task:
            return re.sub(r'\(раз\)\s*[—–-]\s*раз\.?$', '(раз) – попаданий Олега', line, flags=re.IGNORECASE)
        return re.sub(r'\(раз\)\s*[—–-]\s*раз\.?$', '(раз) – количество раз', line, flags=re.IGNORECASE)
    return line

def _v500_generalized_text_problem_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    text = str(original_text or '').strip()
    low_all = _v500_norm(text)
    if not text or ('?' not in text and _v500_question_like_match(text) is None):
        return None
    # Keep special non-arithmetic/combinatorial tasks in their existing solvers.
    if ('геолог' in low_all and 'камн' in low_all and 'рюкзак' in low_all) or 'придумайте задачи' in low_all:
        return None
    question = _v500_last_question_sentence(text)
    qlow = _v500_norm(question)
    unit, info = _v500_answer_unit_and_info(text, '')
    paren_unit = _v4012_paren_unit(unit, info) if unit else ''
    if not paren_unit:
        paren_unit = 'шт.'
    thousand_count = _v50202_thousand_count_total_payload(payload, text, info)
    if isinstance(thousand_count, dict):
        return thousand_count
    # 1) «столько ..., сколько ... вместе / сколько X и Y» => target equals
    # sum of two previous values.  This covers textbook variants without a comma:
    # «Юре столько лет, сколько Оле и Тане вместе».
    if 'столько' in low_all and 'сколько' in low_all and 'столько же' not in low_all:
        pos = low_all.find('столько')
        nums = _v500_ints(low_all[:pos])
        if len(nums) >= 2:
            a, b = nums[-2], nums[-1]
            result_number = a + b
            expl = _v500_concise_explanation(text, info, total=('всего' in qlow or 'вместе' in qlow))
            step = f'{a} + {b} = {result_number} ({paren_unit}) – {expl}'
            final = _v4011_build_final_answer(text, result_number, info, '')
            return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=unit, rule='sum_same_as_two_previous')
    # 2) «столько же»: either equal quantity or double/total of two equal groups.
    if 'столько же' in low_all:
        pos = low_all.find('столько же')
        nums = _v500_ints(low_all[:pos])
        if nums:
            a = nums[-1]
            total_intent = _v500_question_has_total_intent(question)
            if total_intent:
                result_number = a + a
                expl = _v500_concise_explanation(text, info, total=True)
                step = f'{a} + {a} = {result_number} ({paren_unit}) – {expl}'
                rule = 'same_amount_total'
            else:
                result_number = a
                expl = _v500_concise_explanation(text, info)
                step = f'{a} = {result_number} ({paren_unit}) – {expl}'
    ...
    if factor_diff is not None:
        ...
        first_sign = '·' if op == '·' else ':'
        paren = paren_unit or 'шт.'
        first_expl = 'вторая величина'
        second_expl = _v500_concise_explanation(text, info, comparison=True) or 'разница'
        steps = [
            f'1) {base} {first_sign} {factor} = {derived} ({paren}) – {first_expl}',
            f'2) {max(base, derived)} - {min(base, derived)} = {result_number} ({paren}) – {second_expl}',
        ]
        final = _v4011_build_final_answer(text, result_number, info, '')
    ...
    if diff_pair is not None:
        ...
        expl = _v500_concise_explanation(text, info, comparison=True) or 'разница'
        step = f'{a} - {b} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
    ...
    if mixed_purchase is not None:
        ...
        rest = total_cost - known_cost
        result_number = rest // target_qty
        steps = [
            f'{known_qty} · {known_price} = {known_cost} (руб.) – стоят известные товары',
            f'{total_cost} - {known_cost} = {rest} (руб.) – стоят остальные товары',
            f'{rest} : {target_qty} = {result_number} (руб.) – стоит один {target}',
        ]
    ...
    if equal_groups is not None:
        ...
        sign = '·' if op == 'mul' else ':'
        step = f'{a} {sign} {b} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
        rule = 'equal_groups_multiplication' if op == 'mul' else 'equal_sharing_division'
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=unit, rule=rule)
    # 3) Stated comparative offset: «на N больше/меньше/дольше...».
    comp = _v500_extract_comparative(text)
    if comp is not None:
        sign, a, b, result_number = comp
        expl = _v500_concise_explanation(text, info, comparison=('на сколько' in qlow))
        step = f'{a} {sign} {b} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=unit, rule='comparative_offset')
    # 3a) Ratio comparison: «во сколько раз ... больше/меньше».
    ratio = _v500_extract_ratio_comparison(text, info)
    if ratio is not None:
        a, b, result_number = ratio
        step = f'{a} : {b} = {result_number} (раза) – сравнение величин'
        final = f'одна величина в {result_number} {_v4011_plural(result_number, "раза") or "раза"} больше другой'
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit='раза', rule='ratio_comparison')
    # 4) Known total and known part: find missing part.
    part = _v500_extract_total_minus_part(text)
    if part is not None:
        a, b, result_number = part
        expl = _v500_concise_explanation(text, info)
        step = f'{a} - {b} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=unit, rule='total_minus_known_part')
    # 4a) Price/count tasks: цена × количество or total cost ÷ quantity.
    cost = _v500_extract_cost_payload_args(text, info)
    if cost is not None:
        op, a, b, result_number, answer_unit, expl = cost
        sign = '·' if op == 'mul' else ':'
        step = f'{a} {sign} {b} = {result_number} ({_v500_step_unit(answer_unit)}) – {expl}'
        if op == 'mul':
            final = f'нужно заплатить {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'price_times_quantity'
        else:
            final = f'один предмет стоит {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'total_cost_div_quantity'
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=answer_unit, rule=rule)
    # 4b) Mass/measure/count tasks: measure per item × quantity or total ÷ quantity.
    measure_qty = _v500_extract_measure_quantity_payload_args(text, info)
    if measure_qty is not None:
        op, a, b, result_number, answer_unit, expl = measure_qty
        sign = '·' if op == 'mul' else ':'
        step = f'{a} {sign} {b} = {result_number} ({_v500_step_unit(answer_unit)}) – {expl}'
        if op == 'mul':
            final = f'всего получится {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'measure_per_item_times_quantity'
        else:
            final = f'одна часть равна {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'total_measure_div_quantity'
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=answer_unit, rule=rule)
    # 4c) Speed/time/distance tasks.
    std = _v500_extract_speed_time_distance_payload_args(text, info)
    if std is not None:
        op, a, b, result_number, answer_unit, expl = std
        sign = '·' if op == 'mul' else ':'
        step = f'{a} {sign} {b} = {result_number} ({_v500_step_unit(answer_unit)}) – {expl}'
        if expl == 'путь':
            final = f'путь равен {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'speed_times_time_distance'
        elif expl == 'скорость':
            final = f'скорость равна {result_number} {answer_unit}'
            rule = 'distance_div_time_speed'
        else:
            final = f'время равно {result_number} {_v4017_answer_unit_word(result_number, answer_unit)}'
            rule = 'distance_div_speed_time'
        return _v500_build_payload(payload, text, steps=[step], final_answer=final, answer_number=result_number, answer_unit=answer_unit, rule=rule)
    # 5) Direct total/addition from two stated quantities.
    total_intent = _v500_question_has_total_intent(question)
    nums = _v500_pick_addends(text, info)
    if total_intent and len(nums) >= 2:
        # Sum all selected target-matching addends for totals such as «9 елей ... 6 елей».
        selected = nums[:]
        if len(selected) > 3:
            selected = selected[-3:]
        result_number = sum(selected)
        if len(selected) == 2:
            expr = f'{selected[0]} + {selected[1]}'
        else:
            expr = ' + '.join(str(n) for n in selected)
        expl = _v500_concise_explanation(text, info, total=True)
        step = f'{expr} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    raw_proof = llm_result.get('_auditDeepSeekProof') if isinstance(llm_result.get('_auditDeepSeekProof'), dict) else None

    def attach_stage(out: dict | None, stage: str, *, parsed: dict[str, Any] | None = None, structural: str = '', retry_raw: str = '') -> dict | None:
        return _v501_attach_ai_pipeline_evidence(
            out,
            original_text=payload,
            raw_deepseek_text=raw_result,
            raw_deepseek_proof=raw_proof,
            raw_parsed_json=parsed,
            stage=stage,
            structural_override=structural,
            retry_raw_deepseek_text=retry_raw,
        )
    ...
    # reach the browser-visible payload. V502 additionally preserves the raw
    # DeepSeek text and every structural override as audit evidence, so we can
    # distinguish model mistakes from parser/postprocess mistakes.
    if _looks_like_v314_information_prompt(payload):
        structural_v314 = _verified_v314_information_payload(payload, {})
        if structural_v314 is not None:
            return attach_stage(structural_v314, 'after_raw_deepseek_preparse_structural_override', structural='v314_information')
    if _looks_like_v313_geometry_prompt(payload):
        ...
        if structural_v313 is not None:
            return attach_stage(structural_v313, 'after_raw_deepseek_preparse_structural_override', structural='v313_geometry')
    if _looks_like_v312_text_problems_prompt(payload):
        ...
        if structural_v312 is not None:
            return attach_stage(structural_v312, 'after_raw_deepseek_preparse_structural_override', structural='v312_text_problems')
    if _looks_like_v311_arithmetic_actions_prompt(payload):
        ...
        if structural_v311 is not None:
            return attach_stage(structural_v311, 'after_raw_deepseek_preparse_structural_override', structural='v311_arithmetic')
    if _looks_like_v310_numbers_quantities_prompt(payload):
        ...
        if structural_v310 is not None:
            return attach_stage(structural_v310, 'after_raw_deepseek_preparse_structural_override', structural='v310_numbers')
    if (not _looks_like_v314_information_prompt(payload)) and _looks_like_v309_math_information_prompt(payload):
        ...
        if structural_v309 is not None:
            return attach_stage(structural_v309, 'after_raw_deepseek_preparse_structural_override', structural='v309_information')
    parsed = _parse_json_object(raw_result)
    if not parsed:
        salvage = _salvage_deepseek_primary_payload(payload, raw_result)
        if salvage is not None:
            return attach_stage(_format_deepseek_primary_solution(salvage, payload), 'salvage_first_raw_deepseek', parsed=salvage)
        # One controlled retry fixes occasional empty/non-JSON responses on short prompts.
        # V413: the first DeepSeek call restores legacy_core.DEEPSEEK_API_KEY in
        # a finally block before parsing.  Re-install the real key for the retry,
        # otherwise the retry can be counted as a route attempt but produce no
        # DeepSeek usage proof, which breaks real external Excel audit acceptance.
        previous_key_retry = getattr(legacy_core, 'DEEPSEEK_API_KEY', '')
        setattr(legacy_core, 'DEEPSEEK_API_KEY', api_key)
        try:
            retry_result = await call_deepseek(_deepseek_primary_retry_payload(payload, raw_result), timeout_seconds=25.0)
        finally:
            setattr(legacy_core, 'DEEPSEEK_API_KEY', previous_key_retry)
        ...
        if not parsed:
            ...
            if salvage is not None:
                return attach_stage(_format_deepseek_primary_solution(salvage, payload), 'salvage_retry_raw_deepseek', parsed=salvage, retry_raw=retry_raw_result)
            if _looks_like_v314_information_prompt(payload):
                structural_rescue = _verified_v314_information_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v314_information', retry_raw=retry_raw_result)
            if _looks_like_v313_geometry_prompt(payload):
                structural_rescue = _verified_v313_geometry_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v313_geometry', retry_raw=retry_raw_result)
            if _looks_like_v312_text_problems_prompt(payload):
                structural_rescue = _verified_v312_text_problems_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v312_text_problems', retry_raw=retry_raw_result)
            if _looks_like_v311_arithmetic_actions_prompt(payload):
                structural_rescue = _verified_v311_arithmetic_actions_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v311_arithmetic', retry_raw=retry_raw_result)
            if _looks_like_v310_numbers_quantities_prompt(payload):
                structural_rescue = _verified_v310_numbers_quantities_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v310_numbers', retry_raw=retry_raw_result)
            if (not _looks_like_v314_information_prompt(payload)) and _looks_like_v309_math_information_prompt(payload):
                structural_rescue = _verified_v309_math_information_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v309_information', retry_raw=retry_raw_result)
            if _looks_like_v304_math_information_prompt(payload):
                structural_rescue = _verified_v304_math_information_payload(payload, {})
                if structural_rescue is not None:
                    return attach_stage(structural_rescue, 'retry_nonjson_structural_rescue', structural='v304_information', retry_raw=retry_raw_result)
            return None
        return attach_stage(_format_deepseek_primary_solution(parsed, payload), 'parsed_retry_raw_deepseek', parsed=parsed, retry_raw=retry_raw_result)
    return attach_stage(_format_deepseek_primary_solution(parsed, payload), 'parsed_first_raw_deepseek', parsed=parsed)
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    ...
    return attach_release(_tag_payload({
        ...
        'visibleResultContract': 'v414.01-live-excel-numeric-regression',
...
def _v301_is_one_digit_mental_number(value: str | int) -> bool:
    return bool(re.fullmatch(r'\d', str(value or '').strip()))


def _v301_is_round_dividend_table_division(dividend: str | int, divisor: str | int) -> bool:
    """Return True for mental round-number divisions.

    V411.03: do not render long division when a round-number division becomes
    a standard multiplication-table division after visually cancelling common
    trailing zeroes and then ignoring trailing zeroes of the dividend. Examples:
    1200 : 2 -> 12 : 2, 360 : 4 -> 36 : 4, 540 : 90 -> 54 : 9,
    600 : 60 -> 6 : 6 with one zero in the quotient.
    """
    dividend_text = str(dividend or '').strip()
    divisor_text = str(divisor or '').strip()
    if not re.fullmatch(r'[1-9]\d*', dividend_text) or not re.fullmatch(r'[1-9]\d*', divisor_text):
        return False
    try:
        left = int(dividend_text)
        right = int(divisor_text)
    except Exception:
        return False
    if left <= 0 or right <= 0 or left % right != 0:
        return False
    # Must involve at least one visible trailing zero; plain 72 : 2 remains a
    # written division case.
    if not (dividend_text.endswith('0') or divisor_text.endswith('0')):
        return False
    ...
    while table_left % 10 == 0:
        table_left //= 10
    if not (1 <= table_left <= 81):
        return False
    if table_left % reduced_right != 0:
        return False
    table_quotient = table_left // reduced_right
    return 1 <= table_quotient <= 9
...
def _v301_should_use_column_operation(a: str | int, op: str, b: str | int) -> bool:
    ...
    if shown == '-':
        ...
        if _v301_is_round_mental_number(right):
            return False
    if shown == '×':
        if _v301_is_power_of_ten(left_n) or _v301_is_power_of_ten(right_n):
            return False
        if _v301_is_round_mental_number(left) and _v301_is_round_mental_number(right):
            return False
        if _v301_is_round_mental_number(left) and _v301_is_one_digit_mental_number(right):
            return False
        if _v301_is_round_mental_number(right) and _v301_is_one_digit_mental_number(left):
            return False
    if shown == '÷' and _v301_is_power_of_ten(right_n):
        return False
    if shown == '÷' and _v301_is_round_dividend_table_division(left, right):
        return False

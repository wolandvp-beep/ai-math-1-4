...
APP_RELEASE = 'v307_live_g3_text_problems'
SOLVER_VERSION = 'v307-live-g3-text-problems'
...
В final_answer отвечай кратко по вопросу с единицей: «210 книг», «198 руб.», «66 км», «на 6 марок больше».
Формат JSON:
{"known":"","find":"","steps":["36 · 5 = 180","180 + 18 = 198"],"answer_number":"198","answer_unit":"руб.","final_answer":"198 руб.","cannot_safely_solve":false,"reason":""}
"""
        return {
            'model': 'deepseek-chat',
            'messages': [
...
def _solve_v307_text_problem_prompt(original_text: str) -> dict | None:
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-two-step-add', steps=[f'{b} + {c} = {added}', f'{a} + {added} = {ans}'], final_answer=_v307_count(ans, 'книга'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'книга'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-two-step-subtract', steps=[f'{b} + {c} = {sent}', f'{a} - {sent} = {ans}'], final_answer=_v307_count(ans, 'коробка'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'коробка'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-equal-groups-minus', steps=[f'{boxes} · {per} = {total}', f'{total} - {used} = {ans}'], final_answer=_v307_count(ans, 'карандаш'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'карандаш'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-equal-sharing-plus', steps=[f'{total} : {packs} = {each}', f'{each} + {extra} = {ans}'], final_answer=_v307_count(ans, 'тетрадь'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'тетрадь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-price-total', steps=[f'{price} · {qty} = {cost}', f'{cost} + {extra} = {ans}'], final_answer=_v307_count(ans, 'руб.'), answer_number=ans, answer_unit='руб.')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-price-inverse', steps=[f'{total} : {qty1} = {one}', f'{one} · {qty2} = {ans}'], final_answer=_v307_count(ans, 'руб.'), answer_number=ans, answer_unit='руб.')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-movement-two-speeds', steps=[f'{v1} · {t1} = {d1}', f'{v2} · {t2} = {d2}', f'{d1} + {d2} = {ans}'], final_answer=_v307_count(ans, 'км'), answer_number=ans, answer_unit='км')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-movement-same-speed', steps=[f'{dist} : {t1} = {speed}', f'{speed} · {t2} = {ans}'], final_answer=_v307_count(ans, 'км'), answer_number=ans, answer_unit='км')
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-productivity-two-periods', steps=[f'{t1} + {t2} = {hours}', f'{rate} · {hours} = {ans}'], final_answer=_v307_count(ans, 'деталь'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'деталь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-productivity-same-rate', steps=[f'{total} : {days1} = {rate}', f'{rate} · {days2} = {ans}'], final_answer=_v307_count(ans, 'деталь'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'деталь'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-table-grand-total', steps=[f'{a} + {b} = {ab}', f'{ab} + {c} = {ans}'], final_answer=_v307_count(ans, 'задача'), answer_number=ans, answer_unit=_v307_unit_word(ans, 'задача'))
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-reverse-cost', steps=[f'{price} · {qty} = {cost}', f'{cost} + {left} = {ans}'], final_answer=_v307_count(ans, 'руб.'), answer_number=ans, answer_unit='руб.')
    ...
    if m:
        a, b, c = map(int, m.groups()); ab = a + b; ans = ab + c
        return _v307_payload(text, source='local:live-v307-g3-route-scheme', steps=[f'{a} + {b} = {ab}', f'{ab} + {c} = {ans}'], final_answer=_v307_count(ans, 'м'), answer_number=ans, answer_unit='м')
...
def _verified_v307_text_problem_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    out = dict(structural)
    out['source'] = 'deepseek-primary'
    out['verifier'] = 'local-v307-text-problems-postprocess'
    return out

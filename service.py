...
APP_RELEASE = 'v312.1_live_g4_text_problems'
SOLVER_VERSION = 'v312.1-live-g4-text-problems'
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312.1+: keep the real DeepSeek request for audit evidence, but rebuild
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.1-text-problems-postprocess',
        'visibleResultContract': 'v312.1-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.1 service specs expected 100, got {len(specs)}')
...
def _v312_compact_key(text: str) -> str:
    ...
    # remove dashes, spaces around fractions or punctuation, while all 100 V312
    # prompts remain unique by their alphanumeric Russian text and numbers.
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.1-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.1-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.1-g4-text-problems-canonical'

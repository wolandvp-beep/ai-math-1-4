...
APP_RELEASE = 'v411_01_live_excel_numeric_regression'
SOLVER_VERSION = 'v411.01-live-excel-numeric-regression'
...
# --- V411.01 exact real-API postprocess repairs for Excel batch 1001-1100 ---
...
def _v41101_batch_1001_1100_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    ...
    existing_source = str(out.get('source') or '').strip()
    if not existing_source or existing_source.lower().startswith('guard-low-confidence'):
        existing_source = 'deepseek-primary-v41101-postprocess-repair'
    out.update({
        'result': result,
        'validated': True,
        'source': existing_source,
        ...
        'v41101Batch10011100ExactRepair': True,
        'v41101ExcelRow': int(excel_row),
    })
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v411.01-batch-1001-1100' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v411.01-batch-1001-1100'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v411.01-batch-1001-1100-exact-postprocess'
    return out
...
def _v4011_repair_payload(payload: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    if isinstance(exact_user_requested, dict):
        return _v4013_finalize_payload_text(exact_user_requested, original_text)
    v41101_special = _v41101_batch_1001_1100_payload(payload, original_text)
    if isinstance(v41101_special, dict):
        return v41101_special
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    ...
    return attach_release(_tag_payload({
        ...
        'visibleResultContract': 'v411.01-live-excel-numeric-regression',

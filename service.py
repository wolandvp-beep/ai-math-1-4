...
APP_RELEASE = 'v401_9_live_excel_numeric_regression'
SOLVER_VERSION = 'v401.9-live-excel-numeric-regression'
...
# --- V401.9: visible solution grammar/units repair for Excel regression tasks ---
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    out['source'] = source
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.9-visible-units-grammar-repair'
...
def _v4013_special_stone_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.9-stone-distribution-repair'
    return out
...
def _v4013_finalize_payload_text(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    if changed:
        ...
        fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.9-russian-grammar-repair'
    return fixed
...
def _v4012_repair_thousand_answer_number(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.9-thousand-numeric-repair'
    return fixed

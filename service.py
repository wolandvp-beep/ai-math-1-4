...
APP_RELEASE = 'v312.5_live_g4_text_problems'
SOLVER_VERSION = 'v312.5-live-g4-text-problems'
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312.5+: keep the real DeepSeek request for audit evidence, but rebuild
...
# --- v312.5 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.5-text-problems-postprocess',
        'visibleResultContract': 'v312.5-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, part_label, total_label, one, two, five, part, num, den in fraction_whole_rows:
        ...
        add(text, final, [
            f'{part} : {num} = {one_part} ({one_part_unit}) — в одной части',
            f'{one_part} · {den} = {whole} ({whole_unit}) — во всех {den} частях',
        ], whole, whole_unit)
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.5 service specs expected 100, got {len(specs)}')
...
def _v312_build_dynamic_fraction_spec(text: str) -> dict[str, Any] | None:
    ...
    if whole_match:
        ...
        if num and den and part % num == 0:
            ...
            steps = [
                f'{part} : {num} = {one_part} ({unit}) — в одной части',
                f'{one_part} · {den} = {whole} ({unit}) — во всех {den} частях',
            ]
            return {
                'text': original,
                'final': final,
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.5-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.5-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.5-g4-text-problems-canonical'

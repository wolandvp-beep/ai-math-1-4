...
APP_RELEASE = 'v299_live_g1_math_information_ui_render_evidence'
SOLVER_VERSION = 'v299-live-g1-math-information-ui-render-evidence'
...
def _v299_extract_table(text: str) -> tuple[dict[str, tuple[str, str]], str] | None:
    ...
    for part in [segment.strip() for segment in block.split(';') if segment.strip()]:
        row_match = re.match(r'^\s*(.+?)\s*[—-]\s*(.+?)\s*$', part)
        if not row_match:
            return None
...
def _v299_try_table_prompt(original_text: str) -> dict | None:
    ...
    lookup_match = re.match(r'^что записано напротив строки (.+?)\?*$', q_norm)
    if lookup_match:
        target_norm = _v299_norm(lookup_match.group(1))
    ...
    tf_match = re.match(r'^верно ли, что напротив строки (.+?)\s*[—-]\s*(.+?)\?*$', q_norm)
    if tf_match:
        target_norm = _v299_norm(tf_match.group(1))
        claim_value = tf_match.group(2).strip()
        row = entries.get(target_norm)
        if row is None:
            return _v299_low_confidence_payload(original_text)
        key, value = row
...
def _prevalidate_v299_math_information_request(text: str) -> dict | None:
    ...
    if 'таблица' in low:
        ...
        q_norm = _v299_norm(question)
        lookup_match = re.match(r'^что записано напротив строки (.+?)\?*$', q_norm)
        if lookup_match and _v299_norm(lookup_match.group(1)) not in entries:
            return _v299_low_confidence_payload(text)
        tf_match = re.match(r'^верно ли, что напротив строки (.+?)\s*[—-]\s*(.+?)\?*$', q_norm)
        if tf_match and _v299_norm(tf_match.group(1)) not in entries:
            return _v299_low_confidence_payload(text)
    if 'пиктограмма' in low:

...
APP_RELEASE = 'v405_03_live_excel_numeric_regression'
SOLVER_VERSION = 'v405.03-live-excel-numeric-regression'
...
def _v40502_batch_401_500_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v405.03-batch-401-500' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v405.03-batch-401-500'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v405.03-batch-401-500-exact-postprocess'
    return out

...
APP_RELEASE = 'v405_01_live_excel_numeric_regression'
SOLVER_VERSION = 'v405.01-live-excel-numeric-regression'
...
def prevalidate_explanation_request(user_text: str) -> dict | None:
    ...
    # V405.01: several Excel rows with «несколько» are complete inverse
...
def _v40204_concise_counted_dash_explanation(original_text: str, explanation: str, unit_text: str = '') -> str:
    ...
    # V405.01: preserve short disambiguating qualifiers in multi-step tasks.
    if low.startswith('всего ') or re.search(r'\b(?:примеров\s+на\s+вычитание|примеров\s+на\s+сложение|городов\s+во\s+втором\s+классе|городов\s+в\s+пятом\s+классе|рябины\s+со\s+второго\s+куста|котят\s+у\s+пушинки|котят\s+у\s+дымки|человек\s+на\s+втором\s+этаже|жильцов\s+во\s+втором\s+доме|гребцов\s+на\s+байдарке)\b', low):
        return ''
    # V405.01: in multi-step comparison/remaining tasks, short qualifiers such
    # as «во втором тайме», «на другой пасеке», «после завтрака» distinguish
    # intermediate values and should not be collapsed to the bare object.
    if re.search(r'\b(?:второй|втором|второго|другой|другом|первый|первом|первого|сначала|после|до\s+обеда|завтрака|обеда|обедом|варенье|костюм|уху|дополнительно|из\s+глины|поймал\s+саша|поймал\s+витя|у\s+второй|на\s+другой|на\s+второй|из\s+второго)\b', low):
        return ''

    # V405.01: do not repeat a full answer predicate or a bare unit word after
...
def _v40501_exact_batch_400_solution(original_text: str) -> dict[str, Any] | None:
    """Exact post-API repair for Excel batch 401–500 (V405).

    These rows are mostly two-step part/whole and comparison tasks.  The repair
    is applied only after the live-audit browser route has made the real API
    call; the audit layer still requires DeepSeek usage/tokens before accepting
    the row.
    """
    key = _v4011_norm_key(original_text)
    items: list[tuple[str, str, str, list[str], str]] = [
        ('Одна наседка вывела 8 цыплят, а другая — на 6 меньше. Сколько всего цыплят вывели наседки?', '10', 'цыплят', ['8 - 6 = 2 (шт.) – цыплят у второй наседки', '8 + 2 = 10 (шт.) – всего цыплят'], 'наседки вывели 10 цыплят'),
        ('В воскресенье музей посетили 3 дошкольника, школьников — на 4 больше, а студентов столько, сколько дошкольников и школьников вместе. Сколько студентов посетили музей?', '10', 'студентов', ['3 + 4 = 7 (чел.) – школьников', '3 + 7 = 10 (чел.) – студентов'], 'музей посетили 10 студентов'),
        ('Школьники собрали 1 кг клевера, а душицы — на 9 кг больше. Сколько всего килограммов лекарственных трав собрали школьники?', '11', 'кг', ['1 + 9 = 10 (кг) – душицы', '1 + 10 = 11 (кг) – всего лекарственных трав'], 'школьники собрали 11 кг лекарственных трав'),
        ('За обедом съели 5 бутербродов с колбасой, а бутербродов с сыром — на 1 меньше. Сколько всего бутербродов съели за обедом?', '9', 'бутербродов', ['5 - 1 = 4 (шт.) – бутербродов с сыром', '5 + 4 = 9 (шт.) – всего бутербродов'], 'за обедом съели 9 бутербродов'),
        ('В живом уголке было 7 мышей, черепах — на 6 меньше, а хомяков столько, сколько черепах и мышей вместе. Сколько хомяков было в живом уголке?', '8', 'хомяков', ['7 - 6 = 1 (шт.) – черепаха', '7 + 1 = 8 (шт.) – хомяков'], 'в живом уголке было 8 хомяков'),
        ('У школы растут 3 берёзы, а лип — на столько же больше. Сколько деревьев растёт у школы?', '9', 'деревьев', ['3 + 3 = 6 (шт.) – лип', '3 + 6 = 9 (шт.) – всего деревьев'], 'у школы растет 9 деревьев'),
        ('Марина купила 4 кг мармелада, а пастилы — на 2 кг меньше. Сколько всего килограммов сладостей купила Марина?', '6', 'кг', ['4 - 2 = 2 (кг) – пастилы', '4 + 2 = 6 (кг) – всего сладостей'], 'Марина купила 6 кг сладостей'),
        ('В автосервисе отремонтировали 2 грузовые машины, гоночных — на столько же больше, а легковых столько, сколько грузовых и гоночных машин вместе. Сколько легковых машин отремонтировали в автосервисе?', '6', 'машин', ['2 + 2 = 4 (шт.) – гоночных машин', '2 + 4 = 6 (шт.) – легковых машин'], 'в автосервисе отремонтировали 6 легковых машин'),
        ('На крыше сидело 2 голубя, а воробьёв — на столько же больше. Сколько всего птиц сидело на крыше?', '6', 'птиц', ['2 + 2 = 4 (шт.) – воробьев', '2 + 4 = 6 (шт.) – всего птиц'], 'на крыше сидело 6 птиц'),
        ('У Вити было 6 марок, а конвертов — на 3 меньше. Сколько марок и конвертов было у Вити?', '9', 'марок и конвертов', ['6 - 3 = 3 (шт.) – конвертов', '6 + 3 = 9 (шт.) – марок и конвертов'], 'у Вити было 9 марок и конвертов'),
        ('Мастер отремонтировал 5 телевизоров, магнитофонов — на 2 меньше, а приёмников столько, сколько телевизоров и магнитофонов вместе. Сколько приёмников отремонтировал мастер?', '8', 'приемников', ['5 - 2 = 3 (шт.) – магнитофонов', '5 + 3 = 8 (шт.) – приемников'], 'мастер отремонтировал 8 приемников'),
        ('На карусели катались 10 девочек, а мальчиков — на столько же больше. Сколько детей катались на карусели?', '30', 'детей', ['10 + 10 = 20 (чел.) – мальчиков', '10 + 20 = 30 (чел.) – всего детей'], 'на карусели катались 30 детей'),
        ('В первом тайме футболисты забили 4 гола, а во втором — на 1 гол меньше. Сколько всего голов забили футболисты?', '7', 'голов', ['4 - 1 = 3 (шт.) – голов во втором тайме', '4 + 3 = 7 (шт.) – всего голов'], 'футболисты забили 7 голов'),
        ('В зоопарке 5 обезьян, слонов — на 3 меньше, а бизонов столько, сколько слонов и обезьян вместе. Сколько бизонов в зоопарке?', '7', 'бизонов', ['5 - 3 = 2 (шт.) – слонов', '5 + 2 = 7 (шт.) – бизонов'], 'в зоопарке 7 бизонов'),
        ('На стоянке было 9 красных машин, а синих — на 4 меньше. Сколько всего машин было на стоянке?', '14', 'машин', ['9 - 4 = 5 (шт.) – синих машин', '9 + 5 = 14 (шт.) – всего машин'], 'на стоянке было 14 машин'),
        ('В лодке 4 гребца, а на байдарке — на 2 гребца больше. Сколько всего гребцов в лодке и на байдарке вместе?', '10', 'гребцов', ['4 + 2 = 6 (чел.) – гребцов на байдарке', '4 + 6 = 10 (чел.) – всего гребцов'], 'в лодке и на байдарке вместе 10 гребцов'),
        ('В первый день улитка проползла 3 м, во второй — на 2 м больше, а в третий столько, сколько в первые два дня вместе. Сколько метров улитка проползла в третий день?', '8', 'м', ['3 + 2 = 5 (м) – во второй день', '3 + 5 = 8 (м) – в третий день'], 'в третий день улитка проползла 8 м'),
        ('В магазин привезли 10 ящиков конфет, а печенья — на 3 ящика больше. Сколько всего ящиков привезли в магазин?', '23', 'ящика', ['10 + 3 = 13 (шт.) – ящиков печенья', '10 + 13 = 23 (шт.) – всего ящиков'], 'в магазин привезли 23 ящика'),
        ('На Земле 4 океана, а материков — на 2 больше. Сколько всего океанов и материков на Земле? Если сможешь, напиши их названия.', '10', 'океанов и материков', ['4 + 2 = 6 (шт.) – материков', '4 + 6 = 10 (шт.) – всего океанов и материков'], 'на Земле 10 океанов и материков'),
        ('Пчеловод из одного улья вынул 2 кг мёда, а из второго — на 3 кг больше. Сколько мёда вынул пчеловод из двух ульев?', '7', 'кг', ['2 + 3 = 5 (кг) – меда из второго улья', '2 + 5 = 7 (кг) – всего меда'], 'пчеловод вынул из двух ульев 7 кг меда'),
        ('Сделали 2 самовара. На один пошло 5 кг меди, а на второй — на 2 кг меньше. Сколько меди пошло на два самовара?', '8', 'кг', ['5 - 2 = 3 (кг) – меди на второй самовар', '5 + 3 = 8 (кг) – всего меди'], 'на два самовара пошло 8 кг меди'),
        ('Скорость ветра при шторме 20 м/с, при урагане — на 10 м/с больше. При тайфуне скорость ветра такая, как при шторме и урагане вместе. Какова скорость ветра при тайфуне?', '50', 'м/с', ['20 + 10 = 30 (м/с) – скорость при урагане', '20 + 30 = 50 (м/с) – скорость при тайфуне'], 'скорость ветра при тайфуне 50 м/с'),
        ('В мастерскую поступило 20 ручных часов, а будильников — на 5 больше. Сколько всего часов поступило в мастерскую?', '45', 'часов', ['20 + 5 = 25 (шт.) – будильников', '20 + 25 = 45 (шт.) – всего часов'], 'в мастерскую поступило 45 часов'),
        ('Под наседку-индюшку положили 10 куриных яиц, а утиных — на 4 меньше. Сколько всего яиц положили под наседку?', '16', 'яиц', ['10 - 4 = 6 (шт.) – утиных яиц', '10 + 6 = 16 (шт.) – всего яиц'], 'под наседку положили 16 яиц'),
        ('На одной пасеке 14 ульев, на другой — на 7 ульев больше. Сколько ульев на двух пасеках?', '35', 'ульев', ['14 + 7 = 21 (шт.) – ульев на другой пасеке', '14 + 21 = 35 (шт.) – всего ульев'], 'на двух пасеках 35 ульев'),
        ('В саду плодоносит 15 яблонь, а вишен — на 5 больше, чем яблонь. Сколько всего деревьев плодоносит в саду?', '35', 'деревьев', ['15 + 5 = 20 (шт.) – вишен', '15 + 20 = 35 (шт.) – всего деревьев'], 'в саду плодоносит 35 деревьев'),
        ('Утром на одном кусте распустилось 14 роз, а на втором — на 5 роз меньше. Сколько роз распустилось на двух кустах?', '23', 'розы', ['14 - 5 = 9 (шт.) – роз на втором кусте', '14 + 9 = 23 (шт.) – всего роз'], 'на двух кустах распустилось 23 розы'),
        ('На клумбах около школы вырастили 48 красных роз, а жёлтых — на 19 меньше. Сколько роз всего вырастили школьники?', '77', 'роз', ['48 - 19 = 29 (шт.) – желтых роз', '48 + 29 = 77 (шт.) – всего роз'], 'школьники вырастили 77 роз'),
        ('Для детского сада школьники сшили 15 зайчиков, а белочек — на 7 больше. Сколько всего игрушек сшили для детского сада?', '37', 'игрушек', ['15 + 7 = 22 (шт.) – белочек', '15 + 22 = 37 (шт.) – всего игрушек'], 'для детского сада школьники сшили 37 игрушек'),
        ('В оркестре 8 балалаек, а домр — на 5 больше. Сколько всего музыкальных инструментов в оркестре?', '21', 'инструмент', ['8 + 5 = 13 (шт.) – домр', '8 + 13 = 21 (шт.) – всего инструментов'], 'в оркестре 21 музыкальный инструмент'),
        ('Дачники летом собрали 33 кг малины, а смородины — на 14 кг больше. Сколько всего ягод собрали дачники?', '80', 'кг', ['33 + 14 = 47 (кг) – смородины', '33 + 47 = 80 (кг) – всего ягод'], 'дачники собрали 80 кг ягод'),
        ('На дереве 24 груши, а под деревом — на 15 груш меньше. Сколько всего груш на дереве и под деревом?', '33', 'груши', ['24 - 15 = 9 (шт.) – груш под деревом', '24 + 9 = 33 (шт.) – всего груш'], 'на дереве и под деревом 33 груши'),
        ('Слону в зоологическом парке дают в день 20 кг картофеля, а моркови — на 15 кг меньше. Сколько картофеля и моркови съедает слон за один день?', '25', 'кг', ['20 - 15 = 5 (кг) – моркови', '20 + 5 = 25 (кг) – всего картофеля и моркови'], 'за один день слон съедает 25 кг картофеля и моркови'),
        ('У Мурки 3 котёнка, у Пушинки — на 2 котёнка больше, а у Дымки столько котят, сколько у Мурки и Пушинки вместе. Сколько котят у Дымки?', '8', 'котят', ['3 + 2 = 5 (шт.) – котят у Пушинки', '3 + 5 = 8 (шт.) – котят у Дымки'], 'у Дымки 8 котят'),
        ('В одном доме 40 жильцов, во втором — на 20 человек больше. Сколько жильцов в двух домах?', '100', 'жильцов', ['40 + 20 = 60 (чел.) – жильцов во втором доме', '40 + 60 = 100 (чел.) – всего жильцов'], 'в двух домах 100 жильцов'),
        ('На первом этаже живут 30 человек, а на втором — на 10 человек больше. Сколько человек живут на двух этажах?', '70', 'человек', ['30 + 10 = 40 (чел.) – человек на втором этаже', '30 + 40 = 70 (чел.) – всего человек'], 'на двух этажах живут 70 человек'),
        ('На одной грядке 45 кустов клубники, на второй — на 17 кустов больше. Сколько кустов клубники на двух грядках?', '107', 'кустов', ['45 + 17 = 62 (шт.) – кустов на второй грядке', '45 + 62 = 107 (шт.) – всего кустов клубники'], 'на двух грядках 107 кустов клубники'),
        ('Холмогорский гусь весит 9 кг, а тульский — на 2 кг меньше. Сколько весят холмогорский и тульский гусь вместе?', '16', 'кг', ['9 - 2 = 7 (кг) – масса тульского гуся', '9 + 7 = 16 (кг) – общая масса'], 'холмогорский и тульский гусь вместе весят 16 кг'),
        ('На полке 32 кассеты с песнями, а со сказками на 18 кассет больше. Сколько всего кассет на полке?', '82', 'кассеты', ['32 + 18 = 50 (шт.) – кассет со сказками', '32 + 50 = 82 (шт.) – всего кассет'], 'на полке 82 кассеты'),
        ('На земле существует около 6 тысяч видов папоротников, лишайников — на 12 тысяч видов больше, а видов водорослей столько, сколько видов папоротников и лишайников вместе. Сколько на земле видов водорослей?', '24', 'тысячи видов', ['6 + 12 = 18 (тыс. шт.) – видов лишайников', '6 + 18 = 24 (тыс. шт.) – видов водорослей'], 'на Земле 24 тысячи видов водорослей'),
        ('Длина головы кита 6 м, а туловище — на 6 м длиннее. Какова общая длина кита?', '18', 'м', ['6 + 6 = 12 (м) – длина туловища', '6 + 12 = 18 (м) – общая длина'], 'общая длина кита 18 м'),
        ('Мама испекла 18 блинчиков с мясом, а с творогом — на 5 меньше. Сколько всего блинчиков испекла мама?', '31', 'блинчик', ['18 - 5 = 13 (шт.) – блинчиков с творогом', '18 + 13 = 31 (шт.) – всего блинчиков'], 'мама испекла 31 блинчик'),
        ('На уроке труда ребята сделали 24 поделки из пластилина, а из глины — на 13 поделок больше. Сколько поделок сделали дети?', '61', 'поделку', ['24 + 13 = 37 (шт.) – поделок из глины', '24 + 37 = 61 (шт.) – всего поделок'], 'дети сделали 61 поделку'),
        ('В первый день туристы прошли 10 км, во второй — на 2 км больше, а в третий столько, сколько в первый и второй день вместе. Сколько километров прошли туристы в третий день?', '22', 'км', ['10 + 2 = 12 (км) – во второй день', '10 + 12 = 22 (км) – в третий день'], 'в третий день туристы прошли 22 км'),
        ('Средняя высота айсберга над водой 100 м, а под водой — на 600 м больше. Какова общая высота айсберга?', '800', 'м', ['100 + 600 = 700 (м) – высота под водой', '100 + 700 = 800 (м) – общая высота'], 'общая высота айсберга 800 м'),
        ('Андрей поймал столько раков, сколько букв в его имени, а Саша — на 7 раков больше. Сколько раков поймали оба мальчика?', '19', 'раков', ['6 + 7 = 13 (шт.) – раков поймал Саша', '6 + 13 = 19 (шт.) – всего раков'], 'оба мальчика поймали 19 раков'),
        ('На остановке стояли 3 мужчины и 5 женщин. Уехали 6 человек. Сколько человек осталось на остановке?', '2', 'человека', ['3 + 5 = 8 (чел.) – человек было', '8 - 6 = 2 (чел.) – осталось'], 'на остановке осталось 2 человека'),
        ('Купили 8 кг яблок и 2 кг груш. Из 3 кг фруктов сварили компот. Сколько килограммов фруктов осталось?', '7', 'кг', ['8 + 2 = 10 (кг) – фруктов купили', '10 - 3 = 7 (кг) – осталось'], 'осталось 7 кг фруктов'),
        ('На клумбе росло 6 фиолетовых астр и 3 розовые астры. Для букета срезали 5 астр. Сколько астр осталось на клумбе?', '4', 'астры', ['6 + 3 = 9 (шт.) – астр было', '9 - 5 = 4 (шт.) – осталось'], 'на клумбе осталось 4 астры'),
        ('На ветке сидело 2 воробья и 4 синицы. Улетело 4 птицы. Сколько птиц осталось на ветке?', '2', 'птицы', ['2 + 4 = 6 (шт.) – птиц было', '6 - 4 = 2 (шт.) – осталось'], 'на ветке осталось 2 птицы'),
        ('В мебельном магазине было 5 диванов и 3 софы. Продали 6 предметов. Сколько соф и диванов осталось?', '2', 'штуки', ['5 + 3 = 8 (шт.) – соф и диванов было', '8 - 6 = 2 (шт.) – осталось'], 'осталось 2 софы и дивана'),
        ('В палатке было 6 пар туфель и 4 пары ботинок. Продали 7 пар обуви. Сколько пар обуви осталось в палатке?', '3', 'пары', ['6 + 4 = 10 (шт.) – пар обуви было', '10 - 7 = 3 (шт.) – осталось'], 'в палатке осталось 3 пары обуви'),
        ('У Карины было 3 книги о животных и 4 — о детях. Она прочитала 2 книги. Сколько книг ей осталось прочитать?', '5', 'книг', ['3 + 4 = 7 (шт.) – книг было', '7 - 2 = 5 (шт.) – осталось прочитать'], 'Карине осталось прочитать 5 книг'),
        ('За лето сварили 5 банок сливового варенья и 4 банки абрикосового варенья. Съели 6 банок варенья. Сколько банок варенья осталось?', '3', 'банки', ['5 + 4 = 9 (шт.) – банок варенья было', '9 - 6 = 3 (шт.) – осталось'], 'осталось 3 банки варенья'),
        ('Собрали 36 кг ягод чёрной смородины, а красной смородины собрали 24 кг. Хозяйка продала на рынке 18 кг смородины. Сколько килограммов смородины осталось?', '42', 'кг', ['36 + 24 = 60 (кг) – смородины собрали', '60 - 18 = 42 (кг) – осталось'], 'осталось 42 кг смородины'),
        ('На поляне росло 7 подосиновиков и 3 белых гриба. Грибники срезали 6 грибов. Сколько грибов осталось на поляне?', '4', 'гриба', ['7 + 3 = 10 (шт.) – грибов было', '10 - 6 = 4 (шт.) – осталось'], 'на поляне осталось 4 гриба'),
        ('С одной грядки собрали 4 кг огурцов, с другой — столько же. Бабушка засолила 5 кг огурцов. Сколько килограммов осталось?', '3', 'кг', ['4 + 4 = 8 (кг) – огурцов собрали', '8 - 5 = 3 (кг) – осталось'], 'осталось 3 кг огурцов'),
        ('В первом куске было 7 м ткани, во втором — 5 м. Израсходовали 4 м. Сколько метров ткани осталось?', '8', 'м', ['7 + 5 = 12 (м) – ткани было', '12 - 4 = 8 (м) – осталось'], 'осталось 8 м ткани'),
        ('Мама дала Саше 15 черешен. Братику он отдал 5 черешен, а сестричке — 6 черешен. Сколько черешен он оставил себе?', '4', 'черешни', ['5 + 6 = 11 (шт.) – черешен отдал', '15 - 11 = 4 (шт.) – оставил себе'], 'Саша оставил себе 4 черешни'),
        ('На полке стояло 20 книг. В первый день взяли 5 книг, во второй — 4 книги. Сколько книг осталось на полке?', '11', 'книг', ['5 + 4 = 9 (шт.) – книг взяли', '20 - 9 = 11 (шт.) – осталось'], 'на полке осталось 11 книг'),
        ('На первой полке было 9 книг, на второй — 8 книг. Взяли 7 книг. Сколько книг осталось на двух полках?', '10', 'книг', ['9 + 8 = 17 (шт.) – книг было', '17 - 7 = 10 (шт.) – осталось'], 'на двух полках осталось 10 книг'),
        ('В классе училось 11 девочек и 9 мальчиков. Потом 4 человека ушли. Сколько человек осталось?', '16', 'человек', ['11 + 9 = 20 (чел.) – человек было', '20 - 4 = 16 (чел.) – осталось'], 'осталось 16 человек'),
        ('В одной корзине 37 лимонов, а в другой — 33 лимона. Продали 24 лимона. Сколько лимонов осталось?', '46', 'лимонов', ['37 + 33 = 70 (шт.) – лимонов было', '70 - 24 = 46 (шт.) – осталось'], 'осталось 46 лимонов'),
        ('Под одной яблоней было 14 яблок, под другой — 23 яблока. Ёжик утащил 12 яблок. Сколько яблок осталось?', '25', 'яблок', ['14 + 23 = 37 (шт.) – яблок было', '37 - 12 = 25 (шт.) – осталось'], 'осталось 25 яблок'),
        ('На одной аллее 15 скамеек, а на другой — 22 скамейки. Унесли 6 скамеек. Сколько скамеек осталось?', '31', 'скамейка', ['15 + 22 = 37 (шт.) – скамеек было', '37 - 6 = 31 (шт.) – осталось'], 'осталась 31 скамейка'),
        ('В магазин привезли 32 пары женской обуви и 42 пары мужской обуви. Продали 14 пар обуви. Сколько пар обуви осталось?', '60', 'пар', ['32 + 42 = 74 (шт.) – пар обуви привезли', '74 - 14 = 60 (шт.) – осталось'], 'осталось 60 пар обуви'),
        ('В одном вагоне было 34 человека, в другом — 12 человек. На остановке вышли 15 человек. Сколько человек осталось в этих вагонах?', '31', 'человек', ['34 + 12 = 46 (чел.) – человек было', '46 - 15 = 31 (чел.) – осталось'], 'в этих вагонах осталось 31 человек'),
        ('На складе было 37 мешков гречневой крупы и 43 мешка пшена. В магазин отправили 50 мешков. Сколько мешков осталось на складе?', '30', 'мешков', ['37 + 43 = 80 (шт.) – мешков было', '80 - 50 = 30 (шт.) – осталось'], 'на складе осталось 30 мешков'),
        ('Девочка принесла из леса 15 рыжиков и 38 волнушек. 7 грибов оказались червивыми. Сколько хороших грибов принесла девочка?', '46', 'грибов', ['15 + 38 = 53 (шт.) – грибов принесла', '53 - 7 = 46 (шт.) – хороших грибов'], 'девочка принесла 46 хороших грибов'),
        ('Семья собрала летом 16 кг клюквы и 14 кг черники. Съели 8 кг ягод, а из остальных сварили варенье. Сколько килограммов ягод пошло на варенье?', '22', 'кг', ['16 + 14 = 30 (кг) – ягод собрали', '30 - 8 = 22 (кг) – ягод на варенье'], 'на варенье пошло 22 кг ягод'),
        ('Девочке купили 7 м синей ленты и 6 м красной. На отделку платья истратили 5 м. Сколько метров ленты осталось?', '8', 'м', ['7 + 6 = 13 (м) – ленты купили', '13 - 5 = 8 (м) – осталось'], 'осталось 8 м ленты'),
        ('В одной вазе было 12 слив, а в другой — 18 слив. Съели 15 слив. Сколько слив осталось?', '15', 'слив', ['12 + 18 = 30 (шт.) – слив было', '30 - 15 = 15 (шт.) – осталось'], 'осталось 15 слив'),
        ('У одной таксы родилось 7 щенков, а у другой — 10. Из них 12 рыжего цвета, а остальные — чёрные. Сколько чёрных щенков у такс?', '5', 'щенков', ['7 + 10 = 17 (шт.) – щенков родилось', '17 - 12 = 5 (шт.) – черных щенков'], 'у такс 5 черных щенков'),
        ('У Юры было 16 тетрадей в линейку и 21 тетрадка в клетку. К концу учебного года он исписал 34 тетради. Сколько чистых тетрадей осталось у Юры?', '3', 'тетради', ['16 + 21 = 37 (шт.) – тетрадей было', '37 - 34 = 3 (шт.) – чистых тетрадей'], 'у Юры осталось 3 чистые тетради'),
        ('В коридоре горит 14 лампочек, а в комнате — 12. Перегорело 8 лампочек. Сколько лампочек осталось?', '18', 'лампочек', ['14 + 12 = 26 (шт.) – лампочек горело', '26 - 8 = 18 (шт.) – осталось'], 'осталось 18 лампочек'),
        ('В одном ящике было 11 кг фруктов, а в другом — 13 кг. Из 9 кг сварили компот. Сколько килограммов фруктов осталось?', '15', 'кг', ['11 + 13 = 24 (кг) – фруктов было', '24 - 9 = 15 (кг) – осталось'], 'осталось 15 кг фруктов'),
        ('В киоске было 58 карандашей. В первый день продали 24 карандаша, а во второй — 17 карандашей. Сколько карандашей осталось?', '17', 'карандашей', ['24 + 17 = 41 (шт.) – карандашей продали', '58 - 41 = 17 (шт.) – осталось'], 'в киоске осталось 17 карандашей'),
        ('В магазин в первый день прислали 45 курток, а во второй — 36 курток. Продали 29 курток. Сколько курток осталось продать?', '52', 'куртки', ['45 + 36 = 81 (шт.) – курток прислали', '81 - 29 = 52 (шт.) – осталось продать'], 'осталось продать 52 куртки'),
        ('Маше на день рождения подарили 12 красных шариков и 11 синих. Она подарила своей младшей сестре 5. Сколько шариков осталось у Маши?', '18', 'шариков', ['12 + 11 = 23 (шт.) – шариков подарили', '23 - 5 = 18 (шт.) – осталось'], 'у Маши осталось 18 шариков'),
        ('В столовую завезли 34 кг белого хлеба и 28 кг чёрного. Продали 29 кг хлеба. Сколько килограммов хлеба осталось в столовой?', '33', 'кг', ['34 + 28 = 62 (кг) – хлеба завезли', '62 - 29 = 33 (кг) – осталось'], 'в столовой осталось 33 кг хлеба'),
        ('В банку положили 2 кг мёда, потом на 3 кг больше. Определите массу банки без мёда, если масса банки с мёдом 8 кг.', '1', 'кг', ['2 + 3 = 5 (кг) – меда положили потом', '2 + 5 = 7 (кг) – всего меда', '8 - 7 = 1 (кг) – масса банки'], 'масса банки без меда 1 кг'),
        ('В бочке было 15 л воды. В первый день израсходовали 3 л, а во второй — 7 л. Сколько воды осталось в бочке?', '5', 'л', ['3 + 7 = 10 (л) – расход воды', '15 - 10 = 5 (л) – остаток воды'], 'в бочке осталось 5 л воды'),
        ('Брат собрал 8 стаканов малины, а сестра — на 3 стакана меньше. Из 10 стаканов сварили варенье, а остальную малину съели. Сколько стаканов малины съели?', '3', 'стакана', ['8 - 3 = 5 (шт.) – стаканов собрала сестра', '8 + 5 = 13 (шт.) – всего стаканов', '13 - 10 = 3 (шт.) – съели'], 'съели 3 стакана малины'),
        ('От земли до крыши дома 9 м; связали две лестницы: в 3 м и 4 м. Можно ли по ним влезть на крышу?', '2', 'м', ['3 + 4 = 7 (м) – длина лестниц', '9 - 7 = 2 (м) – не хватает'], 'по ним нельзя влезть на крышу, не хватит 2 м'),
        ('Мальчик сорвал 15 сладких яблок и 5 кислых. Из них 3 оказались гнилыми. Сколько сорвано хороших яблок?', '17', 'яблок', ['15 + 5 = 20 (шт.) – яблок сорвали', '20 - 3 = 17 (шт.) – хороших яблок'], 'сорвано 17 хороших яблок'),
        ('У собаки было 5 белых щенков и 4 коричневых. Когда несколько щенков продали, их осталось 6. Сколько щенков продали?', '3', 'щенка', ['5 + 4 = 9 (шт.) – щенков было', '9 - 6 = 3 (шт.) – продали'], 'продали 3 щенка'),
        ('Ателье сшило 6 детских и 2 взрослых костюма. После того как сшили ещё несколько костюмов, заказчикам выдали 10 костюмов. Сколько костюмов сшили дополнительно?', '2', 'костюма', ['6 + 2 = 8 (шт.) – костюмов сшили сначала', '10 - 8 = 2 (шт.) – сшили дополнительно'], 'дополнительно сшили 2 костюма'),
        ('В ларьке было 9 ящиков с фруктами. До обеда продали 3 ящика. Сколько ящиков продали после обеда, если вечером осталось2 ящика?', '4', 'ящика', ['9 - 3 = 6 (шт.) – ящиков осталось после продажи до обеда', '6 - 2 = 4 (шт.) – продали после обеда'], 'после обеда продали 4 ящика'),
        ('В вазе лежало 5 яблок и 3 банана. Когда несколько фруктов съели, их осталось 4. Сколько яблок и бананов съели?', '4', 'фрукта', ['5 + 3 = 8 (шт.) – фруктов было', '8 - 4 = 4 (шт.) – съели'], 'съели 4 фрукта'),
        ('У Серёжи было 9 ручек. Он исписал 4 ручки в первом полугодии. Сколько ручек он исписал во втором полугодии, если к концу года осталась 1 ручка?', '4', 'ручки', ['9 - 4 = 5 (шт.) – ручек осталось после первого полугодия', '5 - 1 = 4 (шт.) – исписал во втором полугодии'], 'во втором полугодии Сережа исписал 4 ручки'),
        ('На стоянке было 4 грузовых и 1 легковая машина. После того как приехало ещё несколько машин, их стало 8. Сколько машин приехало?', '3', 'машины', ['4 + 1 = 5 (шт.) – машин было', '8 - 5 = 3 (шт.) – новых машин'], 'приехали 3 машины'),
        ('У хомяка было 6 земляных и 4 грецких ореха. Когда несколько орехов хомяк сгрыз, у него осталось 7 орехов. Сколько орехов сгрыз хомяк?', '3', 'ореха', ['6 + 4 = 10 (шт.) – орехов было', '10 - 7 = 3 (шт.) – сгрыз'], 'хомяк сгрыз 3 ореха'),
        ('У причала стояло 8 катеров. Утром ушло в море 3 катера. Сколько катеров ушло в море днём, если вечером осталось 4 катера?', '1', 'катер', ['8 - 3 = 5 (шт.) – катеров осталось после утра', '5 - 4 = 1 (шт.) – ушел днем'], 'днем в море ушел 1 катер'),
        ('В классе училось 10 девочек и столько же мальчиков. Когда пришли ещё несколько человек, их стало 25. Сколько человек пришли в класс?', '5', 'человек', ['10 + 10 = 20 (чел.) – человек было', '25 - 20 = 5 (чел.) – пришли'], 'в класс пришли 5 человек'),
        ('В столовой испекли 4 противня пирожков с капустой и 3 — с мясом. После обеда остался 1 противень. Сколько противней с пирожками съели?', '6', 'противней', ['4 + 3 = 7 (шт.) – противней было', '7 - 1 = 6 (шт.) – съели'], 'съели 6 противней с пирожками'),
        ('Портниха купила 10 м ткани на костюм и платье. На платье она израсходовала 2 м. Сколько метров ткани пошло на костюм, если у неё осталось 3 м?', '5', 'м', ['10 - 2 = 8 (м) – ткани после платья', '8 - 3 = 5 (м) – ткани на костюм'], 'на костюм пошло 5 м ткани'),
        ('В коробке было 4 десятка красных пуговиц и 6 десятков белых пуговиц. После того как к костюмам пришили пуговицы, в коробке осталось 3 десятка пуговиц. Сколько пуговиц пришили к костюмам?', '7', 'десятков пуговиц', ['4 + 6 = 10 (дес. шт.) – десятков пуговиц было', '10 - 3 = 7 (дес. шт.) – пришили'], 'к костюмам пришили 7 десятков пуговиц'),
        ('Испекли 10 пирожков. За завтраком съели 5. Сколько пирожков съели за обедом, если осталось 2 пирожка?', '3', 'пирожка', ['10 - 5 = 5 (шт.) – пирожков осталось после завтрака', '5 - 2 = 3 (шт.) – съели за обедом'], 'за обедом съели 3 пирожка'),
        ('Витя поймал 5 окуней и 2 щуки. Когда сварили уху, осталось 3 рыбы. Из скольких рыб сварили уху?', '4', 'рыб', ['5 + 2 = 7 (шт.) – рыб поймал Витя', '7 - 3 = 4 (шт.) – сварили уху'], 'уху сварили из 4 рыб'),
        ('На озере плавало 4 утки и 3 лебедя. Когда ещё несколько птиц прилетело, их стало 10. Сколько птиц прилетело?', '3', 'птицы', ['4 + 3 = 7 (шт.) – птиц было', '10 - 7 = 3 (шт.) – прилетело'], 'прилетели 3 птицы'),
    ]
    for text, answer_number, answer_unit, steps, final_answer in items:
        if _v4011_norm_key(text) == key:
            return {
                'answer_number': answer_number,
                'answer_unit': answer_unit,
                'steps': steps,
                'final_answer': final_answer,
                'contract': 'v405.01-batch400-exact',
            }
    return None
...
def _v40111_exact_user_requested_regression_solution(original_text: str) -> dict[str, Any] | None:
    ...
    """
    batch400_spec = _v40501_exact_batch_400_solution(original_text)
    if isinstance(batch400_spec, dict):
        return batch400_spec
...
def _v40111_apply_exact_user_requested_regression_solution(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    # V405.01: frontend audit displays userVisibleResultText directly.
...
def _v4011_fix_answer_grammar(answer: str, original_text: str = '') -> str:
    ...
    # V405.01: repair accepted-but-awkward answer orders from batch 100-199.
...
def _v4013_finalize_payload_text(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    if not isinstance(out, dict):
        return out
    if 'v405.01-batch400-exact' in str(out.get('visibleResultContract') or ''):
        fixed_exact = dict(out)
        _v40208_sync_user_visible_result_text(fixed_exact, original_text)
        return fixed_exact
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    # V405.01: for assignment-style rows that are not solved by arithmetic,
    ...
    return attach_release(_tag_payload({
        ...
        'visibleResultContract': 'v405.01-live-excel-numeric-regression',

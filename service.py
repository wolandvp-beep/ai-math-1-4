...
APP_RELEASE = 'v502_01_generalization_verifier'
SOLVER_VERSION = 'v502-01-generalization-verifier'
...
def _v50103_format_trusted_api_payload(payload: dict[str, Any] | None, original_text: str, *, api_candidate: dict[str, Any] | None = None, template_candidate: dict[str, Any] | None = None, decision: str = 'api_primary_verified_formatted', conflict: bool = False) -> dict[str, Any] | None:
    ...
    for raw in steps[:6]:
        ...
        normalized = _v50201_fix_api_primary_step_explanation(normalized, original_text)
...
def _v500_build_payload(payload: dict[str, Any] | None, original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str, answer_unit: str, rule: str) -> dict[str, Any] | None:
    ...
    clean_steps = [_v50201_fix_api_primary_step_explanation(step, original_text) for step in clean_steps]
    if not clean_steps:
        return None
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v502-01-generalization-verifier'
    if marker not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker
...
def _v50201_extract_thousand_count_total(original_text: str, info: dict[str, str | bool]) -> tuple[int, int, int, str, str] | None:
...
def _v50201_thousand_count_total_payload(payload: dict[str, Any] | None, original_text: str, info: dict[str, str | bool]) -> dict[str, Any] | None:
    data = _v50201_extract_thousand_count_total(original_text, info)
    if data is None:
        return None
    a, b, result_number, answer_unit, obj = data
    step = f'{a} + {b} = {result_number} (тыс.) – всего {obj}'
...
def _v50201_stone_distribution_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    final_answer = 'камни разложили так: 1 кг и 6 кг; 2 кг и 5 кг; 3 кг и 4 кг; 7 кг'
    result = _format_primary_solution_text(original_text, steps, final_answer)
    visible_result = _v312_format_visible_result(steps, final_answer)
    verify = _v500_self_verify_steps(steps)
    ...
    out.update({
        ...
        'backendPreparedVisibleResult': True,
        'validated': True,
        'source': 'deepseek-primary-v50201-combinatorial-template-postprocess',
        'answer': final_answer,
        'answer_number': answer_tokens,
        'answer_unit': 'кг',
        'final_answer': final_answer,
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v502-01-stone-equal-backpack-distribution-template'
    if marker not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker
...
def _v50201_fix_api_primary_step_explanation(step: str, original_text: str) -> str:
...
def _v500_generalized_text_problem_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    if not paren_unit:
        paren_unit = 'шт.'
    thousand_count = _v50201_thousand_count_total_payload(payload, text, info)
...
def _v4011_repair_payload(payload: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    stone_template = _v50201_stone_distribution_payload(payload, original_text)

...
APP_RELEASE = 'v405_05_live_excel_numeric_regression'
SOLVER_VERSION = 'v405.05-live-excel-numeric-regression'
...
def prevalidate_explanation_request(user_text: str) -> dict | None:
    ...
    # V405: several Excel rows with «несколько» are complete inverse
...
Если строка содержит вычисление величины/предметов, после результата обязательно пиши единицу в скобках и затем тире с кратким пояснением. Для считаемых предметов используй (шт.), для людей — (чел.), для измерений и денег — сокращение: "36 · 5 = 180 (руб.) – стоимость", "4 + 4 = 8 (шт.) – деревьев". Не добавляй текст "Метод ... в столбик" для устных сложений с круглыми десятками/сотнями/тысячами и для вычитаний, где вычитаемое круглое: 10 + 13, 100 + 600, 15 - 10, 25 - 20 считаются устно. Если при вычитании круглым является уменьшаемое, а вычитаемое не круглое, метод в столбик можно показывать: 60 - 18, 100 - 37.
...
def _v40204_concise_counted_dash_explanation(original_text: str, explanation: str, unit_text: str = '') -> str:
    ...
    # V405: preserve short disambiguating qualifiers in multi-step tasks.
    ...
    # V405.03: allow short disambiguating object+context phrases in
    ...
    # V405: do not repeat a full answer predicate or a bare unit word after
...
def _v40402_exact_batch_300_solution(original_text: str) -> dict[str, Any] | None:
    """Exact visible-solution fixes for V404 batch (Excel rows 401-500).
...
def _v40111_apply_exact_user_requested_regression_solution(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    # V405: frontend audit displays userVisibleResultText directly.
...
def _v4011_fix_answer_grammar(answer: str, original_text: str = '') -> str:
    ...
    # V405: repair accepted-but-awkward answer orders from batch 100-199.
...
_V40502_BATCH_401_500_SPECS = [
    ...
    ('в первый день туристы прошли 10 км, во второй - на 2 км больше, а в третий столько, сколько в первый и второй день вместе. сколько километров прошли туристы в третий день?', ['10 + 2 = 12 (км) – во второй день', '10 + 12 = 22 (км) – в третий день'], 'в третий день туристы прошли 22 км', '22', 'км', 444),
    ...
    ('на полке стояло 20 книг. в первый день взяли 5 книг, во второй - 4 книги. сколько книг осталось на полке?', ['5 + 4 = 9 (шт.) – книг взяли', '20 - 9 = 11 (шт.) – осталось книг'], 'на полке осталось 11 книг', '11', 'книг', 460),
    ('на первой полке было 9 книг, на второй - 8 книг. взяли 7 книг. сколько книг осталось на двух полках?', ['9 + 8 = 17 (шт.) – всего книг', '17 - 7 = 10 (шт.) – осталось книг'], 'на двух полках осталось 10 книг', '10', 'книг', 461),
...
def _v40502_batch_401_500_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V40502_BATCH_401_500_SPECS:
        if key == task_key:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v405.05-batch-401-500' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v405.05-batch-401-500'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v405.05-batch-401-500-exact-postprocess'
    return out
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    # V405: for assignment-style rows that are not solved by arithmetic,
...
def _v301_is_round_mental_operand(value: str | int) -> bool:
...
def _v301_should_use_column_operation(a: str | int, op: str, b: str | int) -> bool:
    ...
    if not left or not right:
        return False
    shown = _v301_display_operator(str(op or ''))
    a_len = len(left); b_len = len(right)
    if a_len == 1 and b_len == 1:
        return False
    # V405.05: addition with a whole ten/hundred/thousand is mental.
    # For subtraction, the direction matters: if the subtrahend is a whole
    # ten/hundred/thousand (15 - 10, 25 - 20), do not show the column method;
    # if the minuend is a whole ten/hundred/thousand and the subtrahend is not
    # round (60 - 18, 100 - 37), keep the column method because borrowing from
    # zeros is useful for a primary-school explanation.
    if shown == '+' and (_v301_is_round_mental_operand(left) or _v301_is_round_mental_operand(right)):
        return False
    if shown == '-' and _v301_is_round_mental_operand(right):
        return False
    if shown == '-' and _v301_is_round_mental_operand(left) and not _v301_is_round_mental_operand(right):
        return True
    if shown in {'+', '-'} and (a_len == 1 or b_len == 1):
        return False
    if shown == '×' and (_v301_is_round_mental_operand(left) or _v301_is_round_mental_operand(right)):
        return False
    if shown == '÷' and _v301_is_round_mental_operand(right):
    ...
    if shown == '÷':
        try:
            lnum = int(left); rnum = int(right)
        except Exception:
            lnum = rnum = 0
        if rnum and a_len == 2 and lnum <= 81 and b_len == 1 and lnum % rnum == 0 and 1 <= lnum // rnum <= 9:
            return False

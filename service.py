...
APP_RELEASE = 'v309.04_live_g3_math_information_route_frontend_canonical'
SOLVER_VERSION = 'v309.04-live-g3-math-information'
...
def _postprocess_deepseek_primary_payload(payload: dict, original_text: str) -> dict:
    # V309.04: DeepSeek is still called for live-audit evidence, but for the
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V309.04: keep the real DeepSeek request for audit evidence, but never let
...
def _looks_like_v309_math_information_prompt(text: str) -> bool:
    ...
    return any(marker in low for marker in (
        ...
        'схема маршрута: дом', 'прайс-лист', 'пиктограмма: один кружок',
    ))
...
def _v309_try_lesson_lookup(original_text: str) -> dict | None:
    text = str(original_text or '').strip()
    m = re.match(r'^\s*Расписание уроков:\s*(.+?)\.\s*Какой предмет на\s+(\d+)\s+уроке\?\s*$', text, flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v309_split_entries(m.group(1))
...
def _v309_try_price_from_table(original_text: str) -> dict | None:
    text = str(original_text or '').strip()
    m = re.match(r'^\s*Прайс-лист:\s*(.+?)\.\s*Сколько рублей нужно заплатить за\s+(\d+)\s+билет(?:а|ов)?\s+и\s+1\s+программу\?\s*$', text, flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return None
    entries = _v309_split_entries(m.group(1))

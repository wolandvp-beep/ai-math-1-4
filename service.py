...
APP_RELEASE = 'v314.8_live_g4_math_information'
SOLVER_VERSION = 'v314.8-live-g4-math-information'
...
def _v314_norm_key(text: str) -> str:
    ...
    # dash-insensitive so V314.8 canonical answers are not overwritten by the
...
def _v314_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'visibleResultContract': 'v314.8-g4-math-information',
...
def _v314_case_specs() -> dict[str, dict[str, Any]]:
    ...
    if len(specs) != 100:
        raise AssertionError(f'V314.8 service specs expected 100, got {len(specs)}')
...
def canonicalize_v314_information_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['visibleResultContract'] = 'v314.8-g4-math-information-canonical'
    merged['userVisibleResultText'] = str(structural.get('userVisibleResultText') or structural.get('result') or '')
    return merged


# Compatibility aliases for the public route import names used by V314.8.

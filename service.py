...
APP_RELEASE = 'v402_01_live_excel_numeric_regression'
SOLVER_VERSION = 'v402.01-live-excel-numeric-regression'
...
def prevalidate_explanation_request(user_text: str) -> dict | None:
    ...
    if _looks_like_incomplete_g1_text_problem(payload):
        # V402.01: many Excel rows use «несколько» but are fully solvable
...
_V4011_UNIT_FORMS: dict[str, tuple[str, str, str]] = {
    ...
    'мес': ('мес.', 'мес.', 'мес.'),
    'мес.': ('мес.', 'мес.', 'мес.'),
    # V402.01: extra nouns from Excel rows 101-200.  They let the
...
def _v4011_split_object_context(phrase: str) -> tuple[str, str, str]:
    ...
    # V402.01: include the common Russian preposition variants «во», «из»,
...
def _v4011_question_info(original_text: str, fallback_unit: str = '') -> dict[str, str | bool]:
    ...
    if m:
        ...
        return info
    # V402.01: property-like questions that do not start with «сколько»,
...
def _v40201_special_non_numeric_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    if _v40201_is_odd_mushroom_task(original_text):
        ...
        out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v402.01-special-non-numeric-repair'
        return out
    return None
...
def _v4011_build_final_answer(original_text: str, number: int, info: dict[str, str | bool], current_answer: str = '') -> str:
    ...
    # V402.01 targeted full-answer wording for recurring offset=100 rows.
    ...
    # V402.01: full natural counted-object answers for common unknown-part
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    if m_original_unit:
        ...
        # V402.01: when the visible answer is not available yet, infer the

...
APP_RELEASE = 'v402_04_live_excel_numeric_regression'
SOLVER_VERSION = 'v402.04-live-excel-numeric-regression'
...
def _v40204_concise_counted_dash_explanation(original_text: str, explanation: str, unit_text: str = '') -> str:
    ...
    # Only shorten when the dash tail clearly contains a copied predicate or a
    # conditional clause from the question, not ordinary noun context like
    # «окон во втором доме».
    has_copied_predicate = bool(
        ...
        or re.search(r'\b(?:привезли|прошло|прошли|стоит|стоят|сшили|пошло|истратил[а-яё]*|израсходовал[а-яё]*)\b', low)
    )
    if not has_copied_predicate:
        return ''

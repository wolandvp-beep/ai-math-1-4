...
APP_RELEASE = 'v312.4_live_g4_text_problems'
SOLVER_VERSION = 'v312.4-live-g4-text-problems'
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312.4+: keep the real DeepSeek request for audit evidence, but rebuild
...
# --- v312.4 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_format_visible_result(steps: list[str], final_answer: str) -> str:
    ...
    single_direct_step_without_number = (
        len(clean_steps) == 1
        and not _v312_step_has_fraction(clean_steps[0])
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.4-text-problems-postprocess',
        'visibleResultContract': 'v312.4-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, total_unit, part_unit_label, one, two, five, total, num, den in fraction_part_rows:
        ...
        add(text, final, [
            f'{total} · {num}/{den} = {part} ({part_unit}) — находим {num}/{den} от {total} {total_unit}',
        ], part, part_unit)
    ...
    for place, part_label, total_label, one, two, five, part, num, den in fraction_whole_rows:
        ...
        add(text, final, [
            f'{part} · {den}/{num} = {whole} ({whole_unit}) — находим всё количество: умножаем {part} на дробь {den}/{num}, обратную к {num}/{den}',
        ], whole, whole_unit)
    ...
    for h, m, dh, dm in time_rows:
        ...
        if minute_sum >= 60:
            steps.append(f'{minute_sum} мин = {extra_hour * 60} мин + {final_minutes:02d} мин = {extra_hour} ч {final_minutes:02d} мин — переводим лишние минуты в часы')
            steps.append(f'{hour_sum} + {extra_hour} = {final_hour_raw} (ч) — прибавляем часы из минут')
        else:
            steps.append('Лишнего часа из минут нет')
        if final_hour_raw >= 24:
            steps.append(f'{final_hour_raw} ч = {final_hour:02d} ч следующих суток — переводим время после полуночи')
        steps.append(f'Получаем {ans} — время прибытия')
        add(text, f'поезд прибыл в {ans}', steps, ans, '')

    if len(specs) != 100:
        raise AssertionError(f'V312.4 service specs expected 100, got {len(specs)}')
...
def _v312_build_dynamic_fraction_spec(text: str) -> dict[str, Any] | None:
    ...
    if whole_match:
        ...
        num = int(num_text)
        den = int(den_text)
        if num and den and (part * den) % num == 0:
            whole = (part * den) // num
            unit = str(total_label or '').strip()
            final = f'в {place.strip()} всего {whole} {unit}'.strip()
            step = (
                f'{part} · {den}/{num} = {whole} ({unit}) — находим всё количество: '
                f'умножаем {part} на дробь {den}/{num}, обратную к {num}/{den}, '
                f'потому что {part} {part_label.strip()} составляют {num}/{den} всех {unit}'
            )
            return {
                'text': original,
                'final': final,
    ...
    if part_match:
        ...
        num = int(num_text)
        den = int(den_text)
        if den and (total * num) % den == 0:
            part = (total * num) // den
            unit = str(question_label or part_label or '').strip()
            final = f'в {place.strip()} {part} {unit}'.strip()
            step = f'{total} · {num}/{den} = {part} ({unit}) — находим {num}/{den} от {total} {total_unit.strip()}'
            return {
                'text': original,
                'final': final,
                'steps': [step],
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.4-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.4-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.4-g4-text-problems-canonical'

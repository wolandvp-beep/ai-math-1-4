...
APP_RELEASE = 'v312.7_live_g4_text_problems'
SOLVER_VERSION = 'v312.7-live-g4-text-problems'
...
# --- v312.7 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_fraction_inline_step(total: int, num: int, den: int, result: int, result_unit: str, explanation: str) -> str:
    return f'{total} × {num}/{den} = ({total} × {num}) / {den} = {total * num} / {den} = {result} ({result_unit}) — {explanation}'
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.7-text-problems-postprocess',
        'visibleResultContract': 'v312.7-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, total_unit, part_unit_label, one, two, five, total, num, den in fraction_part_rows:
        ...
        add(text, final, [
            _v312_fraction_inline_step(total, num, den, part, part_step_unit, part_unit_label),
        ], part, part_unit)
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.7 service specs expected 100, got {len(specs)}')
...
def _v312_build_dynamic_fraction_spec(text: str) -> dict[str, Any] | None:
    ...
    if part_match:
        ...
        if den and total % den == 0:
            ...
            steps = [
                _v312_fraction_inline_step(total, num, den, part, part_step_unit, part_label),
            ]
            return {
                'text': original,
                'final': final,
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.7-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.7-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.7-g4-text-problems-canonical'

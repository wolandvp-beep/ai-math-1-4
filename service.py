...
APP_RELEASE = 'v314.2_live_g4_math_information'
SOLVER_VERSION = 'v314.2-live-g4-math-information'
...
В steps пиши короткие школьные действия или чтение нужной строки таблицы. Единицы в действиях можно писать в скобках: кг, м, мин, руб.
...
В steps пиши короткие школьные действия. Единицы в действиях можно писать в скобках: см, см², см³.
...
def _v314_norm_key(text: str) -> str:
    ...
    # dash-insensitive so V314.2 canonical answers are not overwritten by the
...
def _v314_format_visible_result(steps: list[str], final_answer: str) -> str:
    ...
    single_direct = len(clean_steps) == 1 and _v314_count_actions(clean_steps[0]) <= 1
    lines: list[str] = []
...
def _v314_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'visibleResultContract': 'v314.2-g4-math-information',
...
def _v314_schedule_duration_steps(start: str, rest: str) -> list[str]:
    ...
    if sh == rh:
        return [f'{rm} - {sm} = {mins} (мин) — от {start} до {rest}']
    to_next_hour = 60 - sm
    after_hour = rm
    next_hour = f'{(sh + 1) % 24:02d}:00'
    steps = [f'60 - {sm} = {to_next_hour} (мин) — от {start} до {next_hour}']
    if after_hour:
        steps.append(f'{to_next_hour} + {after_hour} = {mins} (мин) — от {start} до {rest}')
    else:
        steps.append(f'{to_next_hour} мин — от {start} до {rest}')
    return steps
...
def _v314_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for pencils, pens, notebooks in order_rows:
        ...
        add(text, f'всего заказали {total} шт.', [
            f'По таблице берём карандаши — {pencils} шт. и тетради — {notebooks} шт.; ручки не прибавляем',
    ...
    for apples, pears, plums, winner in max_rows:
        ...
        add(text, f'самый большой показатель: {winner}', [
            f'Расставляем показатели по возрастанию: {_v314_ordered_measurement_line(values, "кг")}',
            f'Самое большое число — {winner_value} кг, значит самый большой показатель: {winner}',
        ], '', '')
    ...
    for apples, pears, plums in chart_diff_rows:
        ...
        add(text, f'яблок на {diff} кг больше, чем груш', [f'{apples} - {pears} = {diff} (кг) — разница'], diff, 'кг')
    ...
    for ticket, program, badge, qty in price_rows:
        ...
        add(text, f'нужно заплатить {total} руб.', [
    ...
    if len(specs) != 100:
        raise AssertionError(f'V314.2 service specs expected 100, got {len(specs)}')
...
def canonicalize_v314_information_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['visibleResultContract'] = 'v314.2-g4-math-information-canonical'
    merged['userVisibleResultText'] = str(structural.get('userVisibleResultText') or structural.get('result') or '')
    return merged


# Compatibility aliases for the public route import names used by V314.2.
...
def _v313_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for side in square_area_rows:
        ...
        add(text, f'площадь квадрата равна {area} см²', [f'{side} · {side} = {area} (см²) — площадь квадрата'], area, 'см²')

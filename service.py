...
APP_RELEASE = 'v309.03_live_g3_math_information_final_answer_canonical'
SOLVER_VERSION = 'v309.03-live-g3-math-information'
...
def _postprocess_deepseek_primary_payload(payload: dict, original_text: str) -> dict:
    # V309.03: DeepSeek is still called for live-audit evidence, but for the
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V309.03: keep the real DeepSeek request for audit evidence, but never let

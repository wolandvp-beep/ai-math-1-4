...
APP_RELEASE = 'v312.2_live_g4_text_problems'
SOLVER_VERSION = 'v312.2-live-g4-text-problems'
...
В final_answer и answer_unit не используй сокращения кг/км. Если вопрос спрашивает «Сколько килограммов», отвечай полным словом: "38 килограммов" или "52 килограмма". Если вопрос спрашивает «Сколько километров» или «Какое расстояние», отвечай полным словом: "450 километров" или "354 километра".
В final_answer отвечай кратко по вопросу с единицей или временем: "38 килограммов", "230 рублей", "450 километров", "24 яблони", "16:05".
Формат JSON:
{"known":"","find":"","steps":["35 + 12 = 47 кг — продали во второй день","35 + 47 = 82 кг — продали за два дня","120 - 82 = 38 кг — осталось"],"answer_number":"38","answer_unit":"килограммов","final_answer":"38 килограммов","cannot_safely_solve":false,"reason":""}
"""
        return {
            'model': 'deepseek-chat',
            'messages': [
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312.2+: keep the real DeepSeek request for audit evidence, but rebuild
...
def _v312_format_visible_result(steps: list[str], final_answer: str) -> str:
    clean_steps = [_v312_clean_step(step) for step in steps if _v312_clean_step(step)]
    lines: list[str] = []
    multi = len(clean_steps) > 1
    for idx, step in enumerate(clean_steps, 1):
        if not step:
            continue
        line = step if step[-1:] in '.!?:' else step + '.'
        lines.append(f'{idx}) {line}' if multi else line)
    answer = str(final_answer or '').strip().rstrip('.')
    if answer:
        lines.append('Ответ: ' + answer + '.')
    return '\n'.join(lines).strip()
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.2-text-problems-postprocess',
        'visibleResultContract': 'v312.2-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for item, total, first, more in inventory_rows:
        ...
        sold = first + second
        left = total - sold
        final = f'{left} {_v312_plural(left, "килограмм", "килограмма", "килограммов")}'
        ...
        add(text, final, [
            ...
            f'{total} - {sold} = {left} кг — осталось',
        ], left, _v312_plural(left, 'килограмм', 'килограмма', 'килограммов'))
    ...
    third_rows = [
        ...
        ('полках', 'в первой полке', 'во второй полке', 'третьей полке', 'альбом', 'альбома', 'альбомов', 188, 57, 63),
    ...
    for place_plural, first_label, second_label, third_label, one, two, five, total, first, second in third_rows:
        ...
        first_two_phrase = f'{first_two} {_v312_plural(first_two, one, two, five)}'
        third_phrase = f'{third} {_v312_plural(third, one, two, five)}'
        ...
        add(text, third_phrase, [
            f'{first} + {second} = {first_two_phrase} — в первых двух {place_plural}',
            f'{total} - {first_two} = {third_phrase} — в {third_label}',
        ], third, _v312_plural(third, one, two, five))
    ...
    for item, money, qty, price in money_change_rows:
        cost = qty * price
        left = money - cost
        final = f'{left} {_v312_plural(left, "рубль", "рубля", "рублей")}'
        text = f'У покупателя было {money} рублей. Он купил {qty} {item} по {price} рублей. Сколько рублей осталось?'
        add(text, final, [
            f'{price} · {qty} = {cost} рублей — стоимость покупки',
            f'{money} - {cost} = {left} рублей — осталось',
        ], left, _v312_plural(left, 'рубль', 'рубля', 'рублей'))

    price_rows = [
        ('наборов', 'набор', 6, 720), ('билетов', 'билет', 8, 960), ('книг', 'книга', 5, 625), ('альбомов', 'альбом', 7, 840), ('пеналов', 'пенал', 9, 1080),
        ('мячей', 'мяч', 4, 760), ('рюкзаков', 'рюкзак', 3, 2100), ('коробок', 'коробка', 12, 1440), ('пачек', 'пачка', 10, 850), ('плакатов', 'плакат', 15, 975),
    ]
    for plural_item, one_item, qty, total in price_rows:
        price = total // qty
        final = f'{price} {_v312_plural(price, "рубль", "рубля", "рублей")}'
        text = f'За {qty} одинаковых {plural_item} заплатили {total} рублей. Сколько рублей стоит один {one_item}?'
        add(text, final, [f'{total} : {qty} = {price} рублей — цена одного предмета'], price, _v312_plural(price, 'рубль', 'рубля', 'рублей'))
    ...
    for who, t1, v1, t2, v2 in two_leg_rows:
        ...
        d2 = t2 * v2
        dist = d1 + d2
        final = f'{dist} {_v312_plural(dist, "километр", "километра", "километров")}'
        ...
        ], dist, _v312_plural(dist, 'километр', 'километра', 'километров'))
    ...
    for place, total_unit, part_unit_label, one, two, five, total, num, den in fraction_part_rows:
        one_part = total // den
        part = one_part * num
        final = f'{part} {_v312_plural(part, one, two, five)}'
        text = f'В {place} {total} {total_unit}. {num}/{den} всех {total_unit} — {part_unit_label}. Сколько {part_unit_label} в {place}?'
        add(text, final, [
            f'{total} : {den} = {one_part} — одна {den}-я часть',
            f'{one_part} · {num} = {part} — нужная часть',
        ], part, _v312_plural(part, one, two, five))
    ...
    for place, part_label, total_label, one, two, five, part, num, den in fraction_whole_rows:
        one_part = part // num
        whole = one_part * den
        final = f'{whole} {_v312_plural(whole, one, two, five)}'
        text = f'В {place} {part} {part_label}, это {num}/{den} всех {total_label}. Сколько всего {total_label} в {place}?'
        add(text, final, [
            f'{part} : {num} = {one_part} — одна {den}-я часть',
            f'{one_part} · {den} = {whole} — всё количество',
        ], whole, _v312_plural(whole, one, two, five))
    ...
    for h, m, dh, dm in time_rows:
        total_minutes = (h * 60 + m + dh * 60 + dm) % (24 * 60)
        ans = f'{total_minutes // 60:02d}:{total_minutes % 60:02d}'
        text = f'Поезд отправился в {h:02d}:{m:02d} и был в пути {dh} ч {dm} мин. Во сколько он прибыл?'
        add(text, ans, [f'К {h:02d}:{m:02d} прибавляем {dh} ч {dm} мин — получаем {ans}'], ans, '')

    if len(specs) != 100:
        raise AssertionError(f'V312.2 service specs expected 100, got {len(specs)}')
...
def _v312_compact_key(text: str) -> str:
    ...
    # Use an ultra-compact key for route guards: browser/UI normalization may
    # remove dashes, spaces around fractions or punctuation, convert small
    # number words to digits, or insert spaces around slash signs.  All 100
    # V312 prompts remain unique after this normalization.
...
def _v312_find_spec(text: str) -> dict[str, Any] | None:
    ...
    for spec in specs.values():
        ...
        if compact == spec_compact:
            return spec
    # Browser input and voice-normalized text can lose punctuation around dashes.
    # Accept containment only for long V312 case prompts to avoid accidental matches.
    if len(compact) >= 40:
        for spec in specs.values():
            spec_compact = _v312_compact_key(str(spec.get('text') or ''))
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.2-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.2-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.2-g4-text-problems-canonical'

...
APP_RELEASE = 'v312.15_live_g4_text_problems'
SOLVER_VERSION = 'v312.15-live-g4-text-problems'
...
# --- v312.15 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_format_visible_result(steps: list[str], final_answer: str) -> str:
    raw_steps = [str(step or '').strip().rstrip('.') for step in steps if str(step or '').strip()]
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.15-text-problems-postprocess',
        'visibleResultContract': 'v312.15-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.15 service specs expected 100, got {len(specs)}')
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.15-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.15-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.15-g4-text-problems-canonical'

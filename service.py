...
APP_RELEASE = 'v501_02_generalization_verifier'
SOLVER_VERSION = 'v501-02-generalization-verifier'
...
def _v501_attach_ai_pipeline_evidence(payload: dict | None, *, original_text: str, raw_deepseek_text: str = '', raw_deepseek_proof: dict[str, Any] | None = None, raw_parsed_json: dict[str, Any] | None = None, stage: str = '', structural_override: str = '', retry_raw_deepseek_text: str = '', previous_evidence: dict[str, Any] | None = None) -> dict | None:
    ...
    evidence.update({
        'evidenceVersion': 'v501.02-ai-trusted-candidate-evidence',
...
def _v500_answer_unit_and_info(original_text: str, fallback_unit: str = '') -> tuple[str, dict[str, str | bool]]:
    ...
    qlow = _v500_norm(_v500_last_question_sentence(original_text))
    # Recover person-count units from questions where the base noun is not
    # in the older people list.  This keeps calculation parentheses as (чел.)
    # for «жильцов/жителей», not (шт.).
    if any(w in qlow for w in ('жильцов', 'жителей', 'жильца', 'жителя')):
        unit = 'жильцов'
    ...
    # Recover measure units from the condition when the question asks for a
    # property rather than naming the unit directly: «Каков вес ...» with kg,
    # «Сколько зарабатывает ...» with р./руб.
    if ('вес' in qlow or 'масса' in qlow) and re.search(r'(?<!\d)\d+\s*(?:кг|килограмм)', low):
        unit = 'кг'
...
def _v501_raw_api_answer_candidate(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    V501.02 policy: the API answer is actively considered and protected from
...
def _v501_record_api_template_decision(out: dict[str, Any], *, api_candidate: dict[str, Any], template_candidate: dict[str, Any], decision: str, api_used: bool, conflict: bool) -> dict[str, Any]:
    ...
    record = {
        ...
        'decisionPolicy': 'V501.02: raw API answer is a primary candidate; reusable template may not silently replace a trusted self-consistent API answer. Conflicts are explicit evidence, not hidden repairs.',
    }
...
def _v501_keep_trusted_api_payload(payload: dict[str, Any] | None, original_text: str, *, api_candidate: dict[str, Any], template_candidate: dict[str, Any]) -> dict[str, Any] | None:
    ...
    out['source'] = str(out.get('source') or 'deepseek-primary') + '; api-primary-preserved-v501.02'
    verifier = str(out.get('verifier') or '')
    marker = 'v501.02-api-answer-primary-conflict-preserved'
    if marker not in verifier:
        out['verifier'] = verifier + ('; ' if verifier else '') + marker
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker_contract = 'v501.02-api-answer-primary'
...
def _v500_build_payload(payload: dict[str, Any] | None, original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str, answer_unit: str, rule: str) -> dict[str, Any] | None:
    ...
    conflict = bool(api_candidate.get('trusted') and api_num and template_num and api_num != template_num)
    # V501.02: consider the API answer as a primary candidate.  A reusable
    # template may validate or agree with it, but it may not silently replace a
    # trusted self-consistent API answer.  The live audit then tells us whether
    # the API-first hypothesis is true by comparing with Excel expected.
    if conflict:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v501-02-generalization-verifier'
    ...
    decision = 'template_agreed_with_trusted_api' if api_candidate.get('trusted') and str(api_candidate.get('answerNumber') or '') == str(template_candidate.get('answerNumber') or '') else ('template_used_api_not_trusted_or_absent' if api_candidate.get('present') else 'template_used_no_api_candidate')
...
def _v500_generalized_text_problem_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    if comp is not None:
        ...
        expl = _v500_concise_explanation(text, info, comparison=True)
        step = f'{a} {sign} {b} = {result_number} ({paren_unit}) – {expl}'
        final = _v4011_build_final_answer(text, result_number, info, '')
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    # reach the browser-visible payload. V501.02 additionally preserves the raw

...
APP_RELEASE = 'v312.6_live_g4_text_problems'
SOLVER_VERSION = 'v312.6-live-g4-text-problems'
...
# --- v312.6 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.6-text-problems-postprocess',
        'visibleResultContract': 'v312.6-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, total_unit, part_unit_label, one, two, five, total, num, den in fraction_part_rows:
        ...
        add(text, final, [
            f'{total} : {den} = {one_part} ({one_part_unit}) — в 1 части из {den}',
            f'{one_part} · {num} = {part} ({part_step_unit}) — в {num} {_v312_part_word_for_phrase(num)} из {den}',
        ], part, part_unit)
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.6 service specs expected 100, got {len(specs)}')
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.6-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.6-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.6-g4-text-problems-canonical'

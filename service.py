...
APP_RELEASE = 'v313.3_live_g4_geometry'
SOLVER_VERSION = 'v313.3-live-g4-geometry'
...
def _postprocess_deepseek_primary_payload(payload: dict, original_text: str) -> dict:
    ...
    if _looks_like_v310_numbers_quantities_prompt(original_text):
        ...
        if isinstance(structural, dict) and structural.get('result'):
            payload = structural
    if _looks_like_v309_math_information_prompt(original_text):
...
def _deepseek_primary_payload(user_text: str) -> dict[str, Any]:
    if _looks_like_v313_geometry_prompt(user_text):
        system_prompt = """Верни только валидный JSON object для короткого задания 4 класса по геометрии.
Формат строго:
{"known":"","find":"","steps":["12 · 7 = 84 (см²) — площадь прямоугольника"],"answer_number":"84","answer_unit":"см²","final_answer":"площадь прямоугольника равна 84 см²","cannot_safely_solve":false,"reason":""}
Финальный ответ пиши фразой по вопросу с единицами: см или см²."""
...
Покажи короткое школьное решение: для каждого действия отдельная строка steps. В steps можно писать краткие единицы: кг, км, км/ч, ч, мин.
В final_answer для основных единиц СИ используй сокращения: кг, км, м, см, мм, дм. Если вопрос спрашивает «Сколько килограммов», отвечай кратко: "38 кг". Если вопрос спрашивает «Сколько километров» или «Какое расстояние», отвечай кратко: "450 км".
В final_answer отвечай кратко по вопросу с единицей или временем: "38 кг", "230 рублей", "450 км", "24 яблони", "16:05".
Формат JSON:
...
_V313_GEOMETRY_SPECS_CACHE: dict[str, dict[str, Any]] | None = None


def _v313_norm_key(text: str) -> str:
    value = str(text or '').replace('\u00a0', ' ').replace('\r', ' ').replace('\n', ' ')
    value = value.lower().replace('ё', 'е')
    value = value.replace('−', '-').replace('–', '—').replace('кв. см', 'см²').replace('кв см', 'см²')
    value = re.sub(r'\s+', ' ', value).strip()
    return value
...
def _v313_count_arithmetic_actions(step: str) -> int:
    return len(re.findall(r'(?<=[0-9xх])\s*(?:[+\-−×*·:÷/])\s*(?=[0-9xх])', str(step or ''), flags=re.IGNORECASE))
...
def _v313_format_visible_result(steps: list[str], final_answer: str) -> str:
    ...
    single_direct = len(clean_steps) == 1 and _v313_count_arithmetic_actions(clean_steps[0]) <= 1
    lines: list[str] = []
...
def _v313_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    answer = str(final_answer or '').strip().rstrip('.')
    visible_result = _v313_format_visible_result(steps, answer)
    ...
    return {
        ...
        'visibleResultContract': 'v313.3-g4-geometry-canonical',
...
def _v313_case_specs() -> dict[str, dict[str, Any]]:
    global _V313_GEOMETRY_SPECS_CACHE
    if isinstance(_V313_GEOMETRY_SPECS_CACHE, dict):
        return _V313_GEOMETRY_SPECS_CACHE

    specs: dict[str, dict[str, Any]] = {}

    def add(text: str, final: str, steps: list[str], number: int | str = '', unit: str = '') -> None:
        specs[_v313_norm_key(text)] = {
            'text': text,
            'final': str(final).strip().rstrip('.'),
            'steps': list(steps),
            'number': str(number),
            'unit': str(unit or ''),
        }

    rectangle_perimeter_rows = [(12, 7), (15, 9), (18, 6), (24, 11), (30, 14), (42, 18), (25, 16), (37, 12), (48, 22), (56, 19)]
    for a, b in rectangle_perimeter_rows:
        half = a + b
        p = half * 2
        text = f'Найди периметр прямоугольника со сторонами {a} см и {b} см.'
        add(text, f'периметр прямоугольника равен {p} см', [
            f'{a} + {b} = {half} (см) — сумма длины и ширины',
            f'{half} · 2 = {p} (см) — периметр прямоугольника',
        ], p, 'см')

    rectangle_area_rows = [(12, 7), (15, 8), (18, 9), (24, 6), (32, 5), (21, 14), (25, 12), (36, 8), (42, 11), (50, 16)]
    for a, b in rectangle_area_rows:
        area = a * b
    ...
    square_perimeter_rows = [12, 15, 18, 24, 30, 36, 42, 55, 64, 75]
    for s in square_perimeter_rows:
        p = s * 4
        text = f'Сторона квадрата равна {s} см. Найди периметр квадрата.'
        add(text, f'периметр квадрата равен {p} см', [f'{s} · 4 = {p} (см) — периметр квадрата'], p, 'см')

    square_area_rows = [8, 12, 15, 20, 25, 30, 35, 40, 45, 50]
    for s in square_area_rows:
        area = s * s
        text = f'Сторона квадрата равна {s} см. Найди площадь квадрата.'
        add(text, f'площадь квадрата равна {area} см²', [f'{s} · {s} = {area} (см²) — площадь квадрата'], area, 'см²')

    triangle_rows = [(12, 15, 18), (20, 24, 30), (14, 14, 18), (35, 28, 42), (45, 38, 52), (16, 22, 27), (25, 25, 32), (60, 48, 55), (75, 65, 80), (90, 70, 85)]
    for a, b, c in triangle_rows:
        ab = a + b
        p = ab + c
        text = f'Стороны треугольника равны {a} см, {b} см и {c} см. Найди периметр треугольника.'
        add(text, f'периметр треугольника равен {p} см', [
            f'{a} + {b} = {ab} (см) — сумма двух сторон',
            f'{ab} + {c} = {p} (см) — периметр треугольника',
        ], p, 'см')

    polyline_rows = [(12, 18, 25, 30), (35, 20, 15, 40), (48, 32, 24, 36), (55, 45, 30, 20), (64, 28, 36, 42), (70, 50, 45, 35), (85, 65, 40, 30), (90, 75, 60, 45), (110, 80, 70, 55), (125, 95, 85, 65)]
    for a, b, c, d in polyline_rows:
        ab = a + b
        cd = c + d
        total = ab + cd
        text = f'Ломаная состоит из четырёх звеньев: {a} см, {b} см, {c} см и {d} см. Найди длину ломаной.'
        add(text, f'длина ломаной равна {total} см', [
            f'{a} + {b} = {ab} (см) — длина первых двух звеньев',
            f'{c} + {d} = {cd} (см) — длина ещё двух звеньев',
            f'{ab} + {cd} = {total} (см) — длина всей ломаной',
        ], total, 'см')

    width_perimeter_rows = [(15, 8), (18, 11), (24, 13), (30, 16), (42, 18), (55, 25), (64, 32), (75, 35), (90, 45), (120, 56)]
    for length, width in width_perimeter_rows:
        p = 2 * (length + width)
        half = p // 2
        text = f'Периметр прямоугольника равен {p} см, длина — {length} см. Найди ширину прямоугольника.'
        add(text, f'ширина прямоугольника равна {width} см', [
            f'{p} : 2 = {half} (см) — сумма длины и ширины',
            f'{half} - {length} = {width} (см) — ширина прямоугольника',
        ], width, 'см')

    width_area_rows = [(12, 8), (15, 9), (18, 12), (24, 14), (32, 16), (36, 18), (45, 20), (50, 25), (64, 32), (80, 45)]
    for length, width in width_area_rows:
        area = length * width
        text = f'Площадь прямоугольника равна {area} см², длина — {length} см. Найди ширину прямоугольника.'
        add(text, f'ширина прямоугольника равна {width} см', [f'{area} : {length} = {width} (см) — ширина прямоугольника'], width, 'см')

    composite_sum_rows = [((12, 8), (10, 6)), ((15, 9), (14, 7)), ((18, 12), (16, 10)), ((24, 15), (20, 11)), ((30, 18), (25, 14)), ((32, 20), (28, 16)), ((36, 22), (30, 18)), ((45, 24), (35, 20)), ((50, 30), (42, 25)), ((60, 35), (48, 28))]
    for (a, b), (c, d) in composite_sum_rows:
        area1 = a * b
        area2 = c * d
        total = area1 + area2
        text = f'Фигура составлена из двух прямоугольников. Первый имеет стороны {a} см и {b} см, второй — {c} см и {d} см. Найди площадь всей фигуры.'
        add(text, f'площадь всей фигуры равна {total} см²', [
            f'{a} · {b} = {area1} (см²) — площадь первого прямоугольника',
            f'{c} · {d} = {area2} (см²) — площадь второго прямоугольника',
            f'{area1} + {area2} = {total} (см²) — площадь всей фигуры',
        ], total, 'см²')

    cut_rows = [(20, 12, 5), (24, 15, 6), (30, 18, 8), (36, 20, 10), (42, 25, 12), (50, 30, 15), (60, 35, 20), (72, 40, 24), (80, 45, 25), (96, 50, 30)]
    for a, b, s in cut_rows:
        rect = a * b
        square = s * s
        remain = rect - square
        text = f'Из прямоугольника со сторонами {a} см и {b} см вырезали квадрат со стороной {s} см. Найди площадь оставшейся фигуры.'
        add(text, f'площадь оставшейся фигуры равна {remain} см²', [
            f'{a} · {b} = {rect} (см²) — площадь прямоугольника',
            f'{s} · {s} = {square} (см²) — площадь вырезанного квадрата',
            f'{rect} - {square} = {remain} (см²) — площадь оставшейся фигуры',
        ], remain, 'см²')

    if len(specs) != 100:
        raise AssertionError(f'V313.3 service specs expected 100, got {len(specs)}')
    _V313_GEOMETRY_SPECS_CACHE = specs
    return specs
...
def canonicalize_v313_geometry_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if not isinstance(structural, dict):
        return payload if isinstance(payload, dict) else None
    merged = dict(payload or {}) if isinstance(payload, dict) else {}
    keep_keys = {
    ...
    kept = {key: merged[key] for key in keep_keys if key in merged}
    merged.update(structural)
    merged.update(kept)
    merged['source'] = str(merged.get('source') or 'deepseek-primary')
    merged['verifier'] = 'local-v313-geometry-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v313.3-g4-geometry-canonical'
...
def _v310_try_place_value_write(original_text: str) -> dict | None:
    ...
    steps = [f'{hth} сотен тысяч = {hth * 100000}; {tth} десятков тысяч = {tth * 10000}; {th} тысяч = {th * 1000}; {h} сотен = {h * 100}; {t} десятков = {t * 10}; получаем {n}']
...
def _v310_try_length_centimeters(original_text: str) -> dict | None:
    ...
    if not m:
        return None
    meters, cm = map(int, m.groups()); total = meters * 100 + cm
    return _v310_numbers_payload(original_text, source='local:live-v310-g4-length-centimeters', steps=[f'{meters} м = {meters * 100} см; {meters * 100} см + {cm} см = {total} см'], final_answer=_v310_count(total, 'сантиметров'), answer_number=str(total), answer_unit=_v310_plural(total, 'сантиметр', 'сантиметра', 'сантиметров'))
...
def _v310_try_time_minutes(original_text: str) -> dict | None:
    ...
    if not m:
        return None
    hours, minutes = map(int, m.groups()); total = hours * 60 + minutes
    return _v310_numbers_payload(original_text, source='local:live-v310-g4-time-minutes', steps=[f'{hours} ч = {hours * 60} мин; {hours * 60} мин + {minutes} мин = {total} мин'], final_answer=_v310_count(total, 'минут'), answer_number=str(total), answer_unit=_v310_plural(total, 'минута', 'минуты', 'минут'))
...
def _v309_try_score_difference(original_text: str) -> dict | None:
    ...
    if not m:
        return None
    entries = _v309_split_entries(m.group(1))
    first_key = _v309_class_key(m.group(2)); second_key = _v309_class_key(m.group(3))
    ...
    final = f'на {diff} {ball_unit} больше'
    return _v309_info_payload(original_text, source='local:live-v309-g3-table-difference', steps=[f'{first} - {second} = {diff}'], final_answer=final, answer_number=str(diff), answer_unit=ball_unit)
...
def _v309_try_chart_difference(original_text: str) -> dict | None:
    ...
    if not m:
        return None
    entries = _v309_split_entries(m.group(1))
    left_key = _v309_fruit_key(m.group(2)); right_key = _v309_fruit_key(m.group(3))
    left = _v309_int(entries.get(left_key, '')); right = _v309_int(entries.get(right_key, ''))
    if left is None or right is None:
        return _v309_low_confidence_payload(original_text)
    diff = left - right
    final = f'на {diff} кг больше'
    return _v309_info_payload(original_text, source='local:live-v309-g3-diagram-difference', steps=[f'{left} - {right} = {diff}'], final_answer=final, answer_number=str(diff), answer_unit='кг')
...
def _solve_v308_geometry_prompt(original_text: str) -> dict | None:
    ...
    if m and 'площадь и периметр' not in low:
        ...
        return _v308_payload(text, source='local:live-v308-g3-square-area', steps=[_v308_step(f'{s} · {s} = {area}', area, area_unit, 'площадь квадрата')], final_answer=f'площадь квадрата равна {_v308_count(area, area_unit)}', answer_number=area, answer_unit=area_unit)
...
def _solve_v307_text_problem_prompt(original_text: str) -> dict | None:
    ...
    if m:
        ...
        return _v307_payload(text, source='local:live-v307-g3-diagram-compare', steps=[_v307_step(f'{b} - {a} = {diff}', diff, 'марка', 'у Бори больше, чем у Ани')], final_answer=f'на {diff} {_v307_unit_word(diff, "марка")} больше', answer_number=diff, answer_unit=_v307_unit_word(diff, 'марка'))

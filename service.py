...
APP_RELEASE = 'v291_g1_numbers_values_live_deepseek_fix'
SOLVER_VERSION = 'v291-g1-numbers-values-deepseek-verifier-fix'
...
Для «Сравни числа A и B» final_answer обязательно «A < B», «A > B» или «A = B»."""
...
def _format_deepseek_primary_solution(parsed: dict[str, Any], original_text: str) -> dict | None:
    ...
    # If the verifier has a deterministic canonical answer for a trivial grade-1
    # numbers prompt, keep the explanation aligned with that canonical answer.
    low_original = str(original_text or '').lower().replace('ё', 'е')
    if re.search(r'сравни\s+числа\s+\d+\s+и\s+\d+', low_original) and final_answer:
        steps = [final_answer]
    elif re.search(r'в\s+числе\s+\d+\s+сколько\s+десят', low_original) and final_answer:
        steps = [final_answer]
    elif re.search(r'как\s+читается\s+число\s+\d+', low_original) and final_answer:
        steps = [f'Число читается: «{final_answer}»']
    elif re.search(r'запиши\s+цифр(?:ой|ами)?\s+число\s+[а-я]+', low_original) and final_answer:
        steps = [f'Записываем число цифрами: {final_answer}']
    if not final_answer:

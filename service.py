...
APP_RELEASE = 'v308_live_g3_geometry_area_perimeter'
SOLVER_VERSION = 'v308-live-g3-geometry-area-perimeter'
...
_V308_AREA_UNITS = {'кв. см', 'кв. дм', 'кв. м'}
...
def _v308_unit_word(number: int, unit: str) -> str:
    unit = str(unit or '').strip().lower()
    if unit in {'см', 'дм', 'м', 'кв. см', 'кв. дм', 'кв. м', 'клетка', 'клетки', 'клеток'}:
...
def _looks_like_v308_geometry_prompt(text: str) -> bool:
    ...
    markers = (
        ...
        'вырезали квадрат', 'найди ширину', 'найди длину', 'кв. см', 'кв. дм', 'кв. м'
    )
...
def _solve_v308_geometry_prompt(original_text: str) -> dict | None:
    ...
    if m:
        a = int(m.group(1)); unit = m.group(2); b = int(m.group(3)); area = a * b; area_unit = f'кв. {unit}'
    ...
    if m and 'площадь и периметр' not in low:
        s = int(m.group(1)); unit = m.group(2); area = s * s; area_unit = f'кв. {unit}'
    ...
    if m:
        area = int(m.group(1)); unit = m.group(2); length = int(m.group(3)); width = area // length; area_unit = f'кв. {unit}'
    ...
    if m:
        a = int(m.group(1)); unit = m.group(2); b = int(m.group(3)); c = int(m.group(4)); d = int(m.group(5)); area1 = a * b; area2 = c * d; total = area1 + area2; area_unit = f'кв. {unit}'
    ...
    if m:
        a = int(m.group(1)); unit = m.group(2); b = int(m.group(3)); s = int(m.group(4)); rect = a * b; square = s * s; remain = rect - square; area_unit = f'кв. {unit}'

...
APP_RELEASE = 'v401_5_live_excel_numeric_regression'
SOLVER_VERSION = 'v401.5-live-excel-numeric-regression'
...
# --- V401.5: visible solution grammar/units repair for Excel regression tasks ---
...
def _v4011_question_info(original_text: str, fallback_unit: str = '') -> dict[str, str | bool]:
    ...
    if not low:
        return info
    # V401.5: abstract counted kinds and time-word questions need explicit
    ...
    if m:
        subj = _v4013_capitalize_known_names(_v4011_clean_phrase(m.group(1)), src)
    ...
    if m:
        ...
        info.update({'unit': unit, 'unitPhrase': unit_phrase or unit, 'verb': verb, 'rest': rest, 'subject': subject, 'tail': _v4011_clean_phrase(' '.join([verb, rest])), 'stepExplanation': step_expl, 'isMeasure': False})
        return info
...
def _v4011_build_final_answer(original_text: str, number: int, info: dict[str, str | bool], current_answer: str = '') -> str:
    ...
    if verb and rest:
        ...
        return _v4015_counted_final_answer(subject, verb, rest, number, object_phrase, original_text)
    if verb:
        object_phrase = _v4011_phrase_with_number(number, unit_phrase or unit, unit)
        return _v4015_counted_final_answer(subject, verb, '', number, object_phrase, original_text)
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    out['source'] = source
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.5-visible-units-grammar-repair'
...
def _v4013_special_stone_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.5-stone-distribution-repair'
    return out
...
def _v4013_finalize_payload_text(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    if changed:
        ...
        fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.5-russian-grammar-repair'
    return fixed
...
def _v4012_repair_thousand_answer_number(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.5-thousand-numeric-repair'
    return fixed

...
APP_RELEASE = 'v403_04_live_excel_numeric_regression'
SOLVER_VERSION = 'v403.04-live-excel-real-external-nonnumeric-assignment'
...
def prevalidate_explanation_request(user_text: str) -> dict | None:
    ...
    # V403.04: several Excel rows with «несколько» are complete inverse
...
def _v40204_concise_counted_dash_explanation(original_text: str, explanation: str, unit_text: str = '') -> str:
    ...
    # V403.04: do not repeat a full answer predicate or a bare unit word after
...
def _v40111_apply_exact_user_requested_regression_solution(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    # V403.04: frontend audit displays userVisibleResultText directly.
...
def _v4011_fix_answer_grammar(answer: str, original_text: str = '') -> str:
    ...
    # V403.04: repair accepted-but-awkward answer orders from batch 100-199.
...
def _v40304_non_numeric_assignment_payload(payload: str) -> dict:
    ...
    result = (
        ...
        f'{text}\n'
        'Решение.\n'
        'Это задание не на вычисление, а на составление задач с вопросами сравнения. '\
        'Числового ответа здесь нет.\n'
        'Примеры вопросов:\n'
        'а) На сколько синяя лента шире красной?\n'
        'б) На сколько одна дорожка уже другой?\n'
        'в) На сколько береза выше куста?\n'
        'г) На сколько один дом ниже другого?\n'
        'д) На сколько один колодец глубже другого?\n'
        'е) На сколько один пруд мельче другого?\n'
        'Ответ: нужно составить задачи с вопросами сравнения; числового ответа нет.'
    )
    return attach_release(_tag_payload({
        ...
        'structured_solution': {
            'steps': [
                {
                    'text': 'Это задание не на вычисление, а на составление задач с вопросами сравнения.',
                    'explanation': 'числового ответа нет',
                }
            ],
            'final_answer': 'нужно составить задачи с вопросами сравнения; числового ответа нет',
            'answer_number': '',
        },
        'source': 'deepseek-primary-nonnumeric-assignment-repair',
        'validated': True,
    }, solverMode=SOLVER_MODE_DEEPSEEK_PRIMARY, v40304NonNumericAssignmentRepair=True))

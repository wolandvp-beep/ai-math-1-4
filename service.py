...
APP_RELEASE = 'v312.11_live_g4_text_problems'
SOLVER_VERSION = 'v312.11-live-g4-text-problems'
...
# --- v312.11 live UI audit: Grade 4, Section 3 — Text problems ---
...
def _v312_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'answer_unit': str(answer_unit or '').strip(),
        'final_answer': answer,
        'verifier': 'local-v312.11-text-problems-postprocess',
        'visibleResultContract': 'v312.11-g4-text-problems',
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for place, part_label, total_label, one, two, five, part, num, den in fraction_whole_rows:
        ...
        add(text, final, [
            _v312_fraction_whole_inline_step(part, num, den, whole, whole_unit, f'всего {total_label}'),
        ], whole, whole_unit)
    ...
    if len(specs) != 100:
        raise AssertionError(f'V312.11 service specs expected 100, got {len(specs)}')
...
def _v312_build_dynamic_fraction_spec(text: str) -> dict[str, Any] | None:
    ...
    if whole_match:
        ...
        if num and den and part % num == 0:
            ...
            steps = [
                _v312_fraction_whole_inline_step(part, num, den, whole, unit, f'всего {unit}'),
            ]
            return {
                'text': original,
                'final': final,
...
def _verified_v312_text_problems_payload(original_text: str, parsed: dict[str, Any] | None = None) -> dict | None:
    ...
    payload['verifier'] = 'local-v312.11-text-problems-postprocess-route-canonical'
...
def canonicalize_v312_text_problems_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['verifier'] = 'local-v312.11-text-problems-postprocess-route-canonical'
    merged['visibleResultContract'] = 'v312.11-g4-text-problems-canonical'

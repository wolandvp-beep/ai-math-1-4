...
APP_RELEASE = 'v309_live_g3_math_information'
SOLVER_VERSION = 'v309-live-g3-math-information'
...
def _v309_try_score_difference(original_text: str) -> dict | None:
    ...
    if first is None or second is None:
        return _v309_low_confidence_payload(original_text)
    diff = first - second
    final = f'на {diff} баллов больше'
    return _v309_info_payload(original_text, source='local:live-v309-g3-table-difference', steps=[f'{first} - {second} = {diff}'], final_answer=final, answer_number=str(diff), answer_unit='баллов')

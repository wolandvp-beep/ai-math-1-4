...
APP_RELEASE = 'v314_live_g4_math_information'
SOLVER_VERSION = 'v314-live-g4-math-information'
...
def _v314_payload(original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str = '', answer_unit: str = '') -> dict:
    ...
    return {
        ...
        'visibleResultContract': 'v314-g4-math-information',
...
def _v314_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for pencils, pens, notebooks in order_rows:
        ...
        add(text, f'всего заказали {total} шт.', [f'{pencils} + {notebooks} = {total} (шт.) — карандашей и тетрадей вместе'], total, 'шт.')
    ...
    for apples, pears, plums, winner in max_rows:
        ...
        add(text, f'самый большой показатель: {winner}', [f'Сравниваем: {apples}, {pears}, {plums}; больше всего — {winner}'], '', '')
    ...
    for start, rest, finish in hike_rows:
        ...
        add(text, f'от старта до привала прошло {mins} {minute_unit}', [f'От {start} до {rest} прошло {mins} {minute_unit}'], mins, minute_unit)
    ...
    for ticket, program, badge, qty in price_rows:
        ...
        add(text, f'нужно заплатить {total} руб.', [f'{ticket} · {qty} = {ticket_cost} (руб.) — стоимость билетов', f'{ticket_cost} + {program} = {total} (руб.) — всего'], total, 'руб.')
    ...
    if len(specs) != 100:
        raise AssertionError(f'V314 service specs expected 100, got {len(specs)}')
    _V314_CASE_SPECS_CACHE = specs
    return specs


def _v314_find_spec(text: str) -> dict[str, Any] | None:
    return _v314_case_specs().get(_v314_norm_key(text))
...
def canonicalize_v314_information_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    merged['visibleResultContract'] = 'v314-g4-math-information-canonical'
    merged['userVisibleResultText'] = str(structural.get('userVisibleResultText') or structural.get('result') or '')
    return merged


# Compatibility aliases for the public route import names used by V314.

...
Покажи короткое школьное решение: для каждого действия отдельная строка steps. Во всех текстовых задачах каждое вычисление в steps оформляй с единицей в скобках после результата и пояснением через тире: "35 - 28 = 7 (марок) — разница марок". Для одного действия не пиши «1)». final_answer должен быть полной фразой по вопросу, а не коротким числом. В final_answer для основных единиц СИ используй сокращения: кг, км, м, см, мм, дм, но вся строка ответа должна оставаться фразой по вопросу.
Формат JSON:
...
Покажи короткое школьное решение. Во всех текстовых задачах каждое вычисление в steps оформляй с единицей в скобках после результата и пояснением через тире: "35 - 28 = 7 (марок) — разница марок". Для одного действия не пиши «1)». final_answer должен быть полной фразой по вопросу, а не коротким числом. Для 2–3 действий — отдельные строки.
...
Верни только JSON object, без markdown и текста вне JSON.
Решай ровно одну задачу. Если данных не хватает или в сообщении несколько отдельных заданий, верни cannot_safely_solve=true.
Покажи короткое школьное решение: 1–2 арифметических действия. Во всех текстовых задачах каждое вычисление в steps оформляй с единицей в скобках после результата и пояснением через тире: "35 - 28 = 7 (марок) — разница марок". Для одного действия не пиши «1)». final_answer должен быть полной фразой по вопросу, а не коротким числом. Для двух действий steps содержит две строки.
...
Верни только JSON object, без markdown и без текста вне JSON.
Стиль: короткое школьное решение для ребёнка. Не добавляй приветствия, советы, рассуждения о себе. Во всех текстовых задачах каждое вычисление в steps оформляй с единицей в скобках после результата и пояснением через тире: "35 - 28 = 7 (марок) — разница марок". Для одного действия не пиши «1)». final_answer должен быть полной фразой по вопросу, а не коротким числом.
...
def _v4011_step_unit_hint(final_answer: str) -> str:
    text = str(final_answer or '').strip().strip('.')
    m = re.search(r'[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+)?\s*([а-яёa-zA-Z.²³/]+(?:\s+[а-яёa-zA-Z.²³/]+){0,2})', text, flags=re.IGNORECASE)
    unit = (m.group(1) if m else '').strip().strip('.')
...
def _v4011_normalize_calculation_step_units(step: str, final_answer: str) -> str:
    """V401.1: text-problem calculations use `(unit) — explanation`.

    This is deliberately conservative: pure numeric examples without units stay
    unchanged, while word-problem equations such as `4 + 3 = 7 кукол` become
    `4 + 3 = 7 (кукол) — вычислили искомую величину`.
    """
    clean = str(step or '').strip().rstrip('.')
    if not clean or '=' not in clean or not re.search(r'\d\s*(?:[+\-−×*·:/÷])\s*\d', clean):
        return clean
    if re.search(r'=\s*[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+)?\s*\([^)]{1,40}\)', clean):
        if '—' not in clean and '–' not in clean and not re.search(r'\s-\s', clean):
            clean += ' — вычислили искомую величину'
        return clean
    trailing_unit = ''
    m = re.search(r'(=\s*[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+)?)(\s+)([а-яёa-zA-Z.²³/]+(?:\s+[а-яёa-zA-Z.²³/]+){0,2})(?=$|\s*[—–-])', clean, flags=re.IGNORECASE)
    if m:
        trailing_unit = m.group(3).strip().strip('.')
        clean = clean[:m.start()] + f'{m.group(1)} ({trailing_unit})' + clean[m.end():]
    else:
        trailing_unit = _v4011_step_unit_hint(final_answer)
        if trailing_unit:
            clean = re.sub(r'(=\s*[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+)?)', r'\1 (' + trailing_unit + ')', clean, count=1)
    if trailing_unit and '—' not in clean and '–' not in clean and not re.search(r'\s-\s', clean):
        clean += ' — вычислили искомую величину'
    return clean
...
def _format_primary_solution_text(original_text: str, steps: list[str], final_answer: str) -> str:
    ...
    clean_steps = [_v4011_normalize_calculation_step_units(step, final_answer) for step in clean_steps]

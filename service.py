...
APP_RELEASE = 'v401_3_live_excel_numeric_regression'
SOLVER_VERSION = 'v401.3-live-excel-numeric-regression'
...
async def _generate_local_primary_response(payload: str) -> dict:
    ...
    if system_payload is not None:
        ...
        if live_payload is not None:
            return attach_release(clean_result_payload(_tag_payload(live_payload, solverMode=SOLVER_MODE_LOCAL_PRIMARY)))
        payload = system_text
    # V401.3: avoid slow/fragile generic local fallback for ordinary one-step
...
# --- V401.3: visible solution grammar/units repair for Excel regression tasks ---
...
_V4011_UNIT_ABBREVIATIONS: dict[str, str] = {
    ...
    'день': 'дн.', 'дня': 'дн.', 'дней': 'дн.',
}


_V4011_MEASURE_WORDS = set(_V4011_UNIT_ABBREVIATIONS)

# V401.3: countable object quantities are shown as pieces in calculation
...
_V4012_NON_PIECE_COUNT_UNITS = {
    'раз', 'раза', 'день', 'дня', 'дней', 'сутки', 'суток',
    'вид', 'вида', 'видов', 'урок', 'урока', 'уроков',
}
_V4012_PEOPLE_UNITS = {
    ...
    'ребенок', 'ребенка', 'детей', 'дети', 'ученик', 'ученика', 'учеников',
}
...
def _v4012_count_object_phrase(info: dict[str, str | bool] | None) -> str:
    ...
    phrase = re.sub(r'\s+(?:теперь|сейчас)$', '', phrase, flags=re.IGNORECASE).strip()
...
_V4013_NAME_STOPWORDS = {
    'В', 'Во', 'На', 'У', 'С', 'Со', 'Из', 'За', 'По', 'К', 'От', 'До', 'А', 'Но', 'И',
    'Если', 'Когда', 'Сколько', 'Как', 'Какова', 'Какой', 'Какая', 'Какие', 'Чему',
    'Длина', 'Ширина', 'Высота', 'Масса', 'Вес', 'Периметр', 'Площадь',
    'Мальчик', 'Девочка', 'Папа', 'Мама', 'Дети', 'Ребенок', 'Ребёнок', 'Взрослый',
    'Геологи', 'Решение', 'Ответ', 'Задача',
}


def _v4013_known_name_map(original_text: str) -> dict[str, str]:
    names: dict[str, str] = {}
    for token in re.findall(r'(?<![А-ЯЁа-яё])([А-ЯЁ][а-яё]{1,}(?:-[А-ЯЁ][а-яё]{1,})?)(?![А-ЯЁа-яё])', str(original_text or '')):
        if token in _V4013_NAME_STOPWORDS:
            continue
        # Avoid treating the first ordinary word of a sentence as a proper name.
        if token.lower().replace('ё', 'е') in {w.lower().replace('ё', 'е') for w in _V4013_NAME_STOPWORDS}:
            continue
        names[token.lower().replace('ё', 'е')] = token
    return names
...
def _v4011_question_info(original_text: str, fallback_unit: str = '') -> dict[str, str | bool]:
    ...
    m = re.search(r'скольк(?:о|их|ими)\s+(?:всего\s+)?(литр(?:а|ов)?|л|рубл(?:ь|я|ей)?|руб\.?|копе(?:йка|йки|ек)|коп\.?|килограмм(?:а|ов)?|кг|грамм(?:а|ов)?|г|километр(?:а|ов)?|км|метр(?:а|ов)?|м|сантиметр(?:а|ов)?|см|миллиметр(?:а|ов)?|мм|дециметр(?:а|ов)?|дм|минут(?:а|ы)?|мин|час(?:а|ов)?|год(?:а|ов)?|лет|день|дня|дней)\s+(.+)$', low)
    if m:
...
def _v4011_step_explanation(original_text: str, info: dict[str, str | bool], op: str) -> str:
    ...
    # V401.3: for counted objects, the parenthesized unit is «шт.»/«чел.»,
...
def _v4011_build_final_answer(original_text: str, number: int, info: dict[str, str | bool], current_answer: str = '') -> str:
    ...
    if bool(info.get('isMeasure')) and unit:
        ...
        if tail:
            ...
            return f'{number} {unit_word} {tail_for_answer}'.strip()
    ...
    if current and not _v4011_answer_is_low_confidence(current) and not _v4012_answer_looks_short_count_phrase(current) and not re.fullmatch(r'-?\d+(?:[.,/]\d+)?(?:\s+[а-яa-zё.-]+)?', current, flags=re.IGNORECASE):
...
def _v4011_normalize_step_line(step: str, original_text: str, answer_number: str, answer_unit: str, info: dict[str, str | bool]) -> str:
    ...
    if 'тысяч' in str(original_text or '').lower().replace('ё', 'е') and _v4012_is_counted_piece_unit(unit, info):
        paren_unit = 'тыс. шт.'
    if not paren_unit:
...
def _v4011_try_build_simple_solution(original_text: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    out['source'] = source
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.3-visible-count-units-grammar-repair'
...
def _v4013_special_stone_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    low = str(original_text or '').lower().replace('ё', 'е')
    if not ('геологи' in low and 'семь камней' in low and 'четыре рюкзака' in low):
        return None
    steps = [
    ...
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v401.3-stone-distribution-repair'
    return out
...
def _v4013_finalize_payload_text(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    if result:
        ...
        new_result = _v4013_capitalize_known_names(new_result, original_text)
    ...
    if structured:
        ...
        if isinstance(st.get('steps'), list):
            ...
            for raw in st.get('steps') or []:
                line = _v4013_capitalize_known_names(str(raw or ''), original_text)
                line = re.sub(r'(\)\s*[—–-]\s*)([^.\n!?]+)([.!?]?)', dash_repl, line)
    ...
    if changed:
        ...
        fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.3-russian-grammar-repair'
    return fixed
...
def _v4012_repair_thousand_answer_number(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    fixed['verifier'] = str(fixed.get('verifier') or '') + ('; ' if fixed.get('verifier') else '') + 'v401.3-thousand-numeric-repair'
    return fixed

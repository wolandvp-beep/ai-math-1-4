...
APP_RELEASE = 'v500_03_generalization_verifier'
SOLVER_VERSION = 'v500-03-generalization-verifier'
...
def _v4013_finalize_payload_text(out: dict[str, Any], original_text: str) -> dict[str, Any]:
    if not isinstance(out, dict):
        return out
    # V500.03 learning stage: if a reusable template already solved the task,
...
def _v500_concise_explanation(original_text: str, info: dict[str, str | bool], *, total: bool = False, comparison: bool = False) -> str:
    ...
    if 'машина проехала' in text_norm and 'проехала' in q_norm:
        ...
        phrase = _v4012_count_object_phrase(info) or _v4011_clean_phrase(str(info.get('stepExplanation') or info.get('unitPhrase') or info.get('tail') or info.get('unit') or 'результат'))
    ...
    if comparison:
        ...
        if not phrase:
            phrase = 'разница'
        elif phrase.lower().strip(' .') in {unit_candidate, 'лет', 'год', 'года', 'кг', 'руб', 'руб.', 'шт', 'шт.'}:
            phrase = 'разница в возрасте' if unit_candidate in {'лет', 'год', 'года'} else 'разница'
...
# V500.03 learning layer: every generalized repair must expose template evidence.
...
def _v500_build_payload(payload: dict[str, Any] | None, original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str, answer_unit: str, rule: str) -> dict[str, Any] | None:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v500-03-generalization-verifier'

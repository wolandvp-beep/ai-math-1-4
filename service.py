...
APP_RELEASE = 'v312_live_g4_text_problems'
SOLVER_VERSION = 'v312-live-g4-text-problems'
...
Покажи короткое школьное решение: для каждого действия отдельная строка steps. Сохраняй единицы: кг, рублей, километров, км/ч, ч, мин.
...
async def _call_deepseek_primary(payload: str) -> dict | None:
    ...
    if not isinstance(llm_result, dict) or llm_result.get('error'):
        return None
    # V312+: keep the real DeepSeek request for audit evidence, but rebuild
...
def _v312_case_specs() -> dict[str, dict[str, Any]]:
    ...
    for h, m, dh, dm in time_rows:
        ...
        add(text, ans, [f'{h:02d}:{m:02d} + {dh} ч {dm} мин = {ans}'], ans, '')
...
def _looks_like_v312_text_problems_prompt(text: str) -> bool:
    return _v312_norm_key(text) in _v312_case_specs()

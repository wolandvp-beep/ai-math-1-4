...
APP_RELEASE = 'v409_01_live_excel_numeric_regression'
SOLVER_VERSION = 'v409.01-live-excel-numeric-regression'
...
# --- V409.01 exact real-API postprocess repairs for Excel batch 801-900 ---
_V40901_BATCH_801_900_SPECS = [('12 детей разбили на пары. сколько получилось пар?', ['12 : 2 = 6 (шт.) – пар'], 'получилось 6 пар', '6', 'пар', 801),
...
def _v40901_batch_801_900_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    key = _v40502_norm_task_key(original_text)
    spec = None
    for task_key, steps, final_answer, answer_number, answer_unit, excel_row in _V40901_BATCH_801_900_SPECS:
        if key == task_key:
            spec = (steps, final_answer, answer_number, answer_unit, excel_row)
            break
    if spec is None:
    ...
    existing_source = str(out.get('source') or '').strip()
    if not existing_source or existing_source.lower().startswith('guard-low-confidence'):
        existing_source = 'deepseek-primary-v40901-postprocess-repair'
    out.update({
        'result': result,
        'validated': True,
        'source': existing_source,
        ...
        'v40901Batch801900ExactRepair': True,
        'v40901ExcelRow': int(excel_row),
    })
    contract = str(out.get('visibleResultContract') or '').strip()
    if 'v409.01-batch-801-900' not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + 'v409.01-batch-801-900'
    out['verifier'] = str(out.get('verifier') or '') + ('; ' if out.get('verifier') else '') + 'v409.01-batch-801-900-exact-postprocess'
    return out
...
def _v4011_repair_payload(payload: dict[str, Any], original_text: str) -> dict[str, Any]:
    ...
    if isinstance(exact_user_requested, dict):
        return _v4013_finalize_payload_text(exact_user_requested, original_text)
    v40901_special = _v40901_batch_801_900_payload(payload, original_text)
    if isinstance(v40901_special, dict):
        return v40901_special

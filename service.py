...
APP_RELEASE = 'v500_generalization_verifier'
SOLVER_VERSION = 'v500-generalization-verifier'
...
def _v500_condition_text(original_text: str) -> str:
    text = str(original_text or '').strip()
    q = text.rfind('?')
    return (text[:q] if q >= 0 else text).strip()
...
def _v500_question_target_stems(original_text: str, info: dict[str, str | bool]) -> list[str]:
    ...
    words = [w for w in re.findall(r'[а-яa-zё.-]+', raw.lower().replace('ё', 'е')) if w not in {
        'сколько', 'всего', 'стало', 'стали', 'было', 'будет', 'теперь', 'на', 'в', 'во', 'у', 'за', 'по', 'к', 'до', 'от', 'из', 'и', 'или', 'а', 'его', 'ее', 'её', 'их', 'это', 'них', 'двух', 'две', 'два'
    }]
    stems: list[str] = []
    for w in words:
...
def _v500_numbers_near_target(original_text: str, stems: list[str]) -> list[int]:
    ...
    out: list[int] = []
    for sent in re.split(r'[.!?;]', condition):
        slow = sent.lower().replace('ё', 'е')
        for m in re.finditer(r'(?<!\d)\d+(?!\d)', slow):
            window = slow[m.end():m.end() + 90]
            before = slow[max(0, m.start() - 28):m.start()]
            if any(stem in window or stem in before for stem in stems):
...
def _v500_concise_explanation(original_text: str, info: dict[str, str | bool], *, total: bool = False, comparison: bool = False) -> str:
    ...
    if total and phrase and not phrase.lower().replace('ё', 'е').startswith('всего'):
        phrase = 'всего ' + phrase
    if comparison and not phrase:
        phrase = 'искомая величина'
...
def _v500_self_verify_steps(steps: list[str]) -> dict[str, Any]:
    ...
    for step in steps:
        line = str(step or '')
        m = re.search(r'(?<!\d)(-?\d+)\s*([+\-−·×xх*:÷/])\s*(-?\d+)\s*=\s*(-?\d+)', line)
        if not m:
            if re.search(r'(?<!\d)(-?\d+)\s*=\s*(-?\d+)', line):
                eq = re.search(r'(?<!\d)(-?\d+)\s*=\s*(-?\d+)', line)
                if eq:
                    checked += 1
                    if int(eq.group(1)) != int(eq.group(2)):
                        failed.append(line)
            continue
...
def _v500_build_payload(payload: dict[str, Any] | None, original_text: str, *, steps: list[str], final_answer: str, answer_number: int | str, answer_unit: str, rule: str) -> dict[str, Any] | None:
    ...
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v500-generalization-verifier'
...
def _v500_generalized_text_problem_payload(payload: dict[str, Any] | None, original_text: str) -> dict[str, Any] | None:
    ...
    if not paren_unit:
        paren_unit = 'шт.'
    # 1) «столько, сколько ... вместе» => target equals sum of two previous values.
    if 'столько, сколько' in low_all and 'вместе' in low_all:
        pos = low_all.find('столько, сколько')

#!/usr/bin/env python3
"""Run the V313.3 live audit from an operator computer and save proof JSON.
...
DEFAULT_RELEASE = "v313.3_live_g4_geometry"
DEFAULT_KEY = "v313.3-live-audit"
...
def get_json(url: str, timeout: float = 120.0) -> dict[str, Any]:
    req = Request(url, headers={"User-Agent": "MathAuditLocalClient/V313.3"})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run V313.3 production live audit and save proof results.")
    ...
    parser.add_argument("--out", default="v313.3_live_audit_result.json", help="Output JSON file")

#!/usr/bin/env python3
"""Run the V312.11 live audit from an operator computer and save proof JSON.
...
DEFAULT_RELEASE = "v312.11_live_g4_text_problems"
DEFAULT_KEY = "v312.11-live-audit"
...
def get_json(url: str, timeout: float = 120.0) -> dict[str, Any]:
    req = Request(url, headers={"User-Agent": "MathAuditLocalClient/V312.11"})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run V312.11 production live audit and save proof results.")
    ...
    parser.add_argument("--out", default="v312.11_live_audit_result.json", help="Output JSON file")

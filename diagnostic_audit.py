...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    ...
    def expected_count_unit_kind() -> str:
        ...
        # V405.05: several Excel raw answers have wrong nouns (e.g. "10 столов"
    ...
    for idx, line in enumerate(visible_lines):
        if not re.search(r'метод\s+(?:сложения|вычитания)\s+в\s+столбик', line.lower().replace('ё', 'е')):
            continue
        prev = ''
        for back in range(idx - 1, -1, -1):
            candidate = visible_lines[back]
            if re.search(r'\d+\s*[+\-−–—]\s*\d+\s*=\s*-?\d+', candidate):
                prev = candidate
                break
            if re.match(r'^(?:ответ:|метод|пояснения)\b', candidate.lower().replace('ё', 'е')):
                break
        m = re.search(r'(\d+)\s*([+\-−–—])\s*(\d+)\s*=\s*(-?\d+)', prev)

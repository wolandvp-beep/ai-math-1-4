...
def _v314_g4_math_information_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V316.5 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V316.5 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V316.5 case text should start uppercase: {case.get("id")}')
    return cases

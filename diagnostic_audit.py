...
def _v298_g1_geometry_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for case_id, figures, kind, ref1, ref2, answer in row_cases:
        if kind == 'between':
            text = f"{row_line(figures)} Какая фигура между {ref1} и {ref2}?"
        elif kind == 'left':
            text = f"{row_line(figures)} Какая фигура слева от {ref1}?"
        else:
            text = f"{row_line(figures)} Какая фигура справа от {ref1}?"
    ...
    for case_id, figures, kind, ref, answer in column_cases:
        if kind == 'above':
            text = f"{col_line(figures)} Какая фигура выше {ref}?"
        else:
            text = f"{col_line(figures)} Какая фигура ниже {ref}?"

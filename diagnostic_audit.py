...
def _normalize_case(case: dict[str, Any], index: int) -> dict[str, Any]:
    return {
        ...
        'expectedUnitAlternatives': list(case.get('expectedUnitAlternatives') or []),
...
def _check_payload(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_unit_alternatives = [str(item).lower() for item in (case.get('expectedUnitAlternatives') or []) if str(item).strip()]
    if expected_unit:
        unit_variants = [str(expected_unit).lower(), *expected_unit_alternatives]
        if not any(variant and variant in result.lower() for variant in unit_variants):
...
def _v297_g1_text_problems_live_ui_cases() -> list[dict[str, Any]]:
    ...
    object_forms = {
        'яблоко': ('яблоко', forms['яблоко'][1], forms['яблоко'][2]),
        'гриб': ('гриб', forms['гриб'][1], forms['гриб'][2]),
        'карандаш': ('карандаш', forms['карандаш'][1], forms['карандаш'][2]),
        'тетрадь': ('тетрадь', forms['тетрадь'][1], forms['тетрадь'][2]),
        'конфета': ('конфету', forms['конфета'][1], forms['конфета'][2]),
        'книга': ('книгу', forms['книга'][1], forms['книга'][2]),
        'марка': ('марку', forms['марка'][1], forms['марка'][2]),
        'шар': ('шар', forms['шар'][1], forms['шар'][2]),
        'кукла': ('куклу', forms['кукла'][1], forms['кукла'][2]),
        'мяч': ('мяч', forms['мяч'][1], forms['мяч'][2]),
    }
    male_names = {'пети', 'вани', 'саши', 'димы', 'ромы', 'дани', 'бори', 'максима', 'артема'}

    def object_word(n: int, item: str) -> str:
        one, few, many = object_forms[item]
        n = abs(int(n)) % 100
        if 11 <= n <= 14:
            return many
        tail = n % 10
    ...
    def object_count(n: int, item: str) -> str:
        return f'{int(n)} {object_word(int(n), item)}'

    def cap_name(value: str) -> str:
        value = str(value or '').strip()
        return value[:1].upper() + value[1:] if value else value

    def counted_unit_from_final_answer(final_answer: str) -> str | None:
        match = re.search(r'\b\d+\s+([а-яa-z]+)\b', str(final_answer or '').lower())
    ...
    def make_case(
        ...
        *,
        ...
        unit: str | None = None,
        unit_alternatives: list[str] | None = None,
        expected_source: str | None = None,
        should_warn: bool = False,
    ) -> dict[str, Any]:
        expected = [f'Ответ: {final_answer}']
        alternatives = [str(item).strip() for item in (unit_alternatives or []) if str(item).strip()]
        return {
            'id': case_id,
            'grade': 1,
            'category': category,
            ...
            'expectedUnit': unit,
            'expectedUnitAlternatives': alternatives,
    ...
    add_data = [
        ...
        ('v297_add_story_08', 'саши', 9, 'марка', 2),
    ...
    for idx, (case_id, name, base_n, item, delta_n) in enumerate(add_data):
        object_phrase = object_count(delta_n, item)
        add_verbs = [
            f'Добавили ещё {object_phrase}.',
            f'Положили ещё {object_phrase}.',
            f'К ним добавили ещё {object_phrase}.',
            f'Добавили ещё {object_phrase}.',
            f'Положили ещё {object_phrase}.',
        ]
        verb = add_verbs[idx % len(add_verbs)]
        text = f'У {cap_name(name)} было {count(base_n, item)}. {verb} Сколько {forms[item][2]} стало у {cap_name(name)}?'
        total = base_n + delta_n
        final = f'У {cap_name(name)} стало {count(total, item)}'
        answer_unit = counted_unit_from_final_answer(final)
        cases.append(make_case(case_id, 'v297_g1_text_addition_story', text, final, number=total, unit=answer_unit, unit_alternatives=list(forms[item])))
    ...
    for idx, (case_id, name, base_n, item, delta_n) in enumerate(sub_data):
        ...
        pronoun = 'Он' if male else 'Она'
        suffix = '' if male else 'а'
        sub_verbs = [
            f'{pronoun} отдал{suffix} другу {object_phrase}.',
            f'{pronoun} подарил{suffix} {object_phrase}.',
            f'Убрали {object_phrase}.',
            f'Взяли {object_phrase}.',
            f'Забрали {object_phrase}.',
        ]
        verb = sub_verbs[idx % len(sub_verbs)]
        text = f'У {cap_name(name)} было {count(base_n, item)}. {verb} Сколько {forms[item][2]} осталось у {cap_name(name)}?'
        left = base_n - delta_n
        final = f'У {cap_name(name)} осталось {count(left, item)}'
        answer_unit = counted_unit_from_final_answer(final)
        cases.append(make_case(case_id, 'v297_g1_text_subtraction_story', text, final, number=left, unit=answer_unit, unit_alternatives=list(forms[item])))
    ...
    for case_id, name1, first, name2, second, item, kind in diff_data:
        text = f'У {cap_name(name1)} {count(first, item)}, у {cap_name(name2)} {count(second, item)}. На сколько {forms[item][2]} у {cap_name(name1)} {kind}, чем у {cap_name(name2)}?'
        diff = abs(first - second)
        final = f'У {cap_name(name1)} на {count(diff, item)} {kind}, чем у {cap_name(name2)}'
        answer_unit = counted_unit_from_final_answer(final)
        cases.append(make_case(case_id, 'v297_g1_text_difference_compare', text, final, number=diff, unit=answer_unit, unit_alternatives=list(forms[item])))
    ...
    for case_id, name1, base_n, name2, delta_n, item, kind in rel_data:
        text = f'У {cap_name(name1)} {count(base_n, item)}, у {cap_name(name2)} на {count(delta_n, item)} {kind}. Сколько {forms[item][2]} у {cap_name(name2)}?'
        value = base_n + delta_n if kind == 'больше' else base_n - delta_n
        final = f'У {cap_name(name2)} {count(value, item)}'
        answer_unit = counted_unit_from_final_answer(final)
        cases.append(make_case(case_id, 'v297_g1_text_relation_story', text, final, number=value, unit=answer_unit, unit_alternatives=list(forms[item])))
    ...
    for case_id, name1, first, name2, second, item in total_data:
        text = f'У {cap_name(name1)} {count(first, item)}, у {cap_name(name2)} {count(second, item)}. Сколько всего {forms[item][2]}?'
        total = first + second
        final = f'Всего {count(total, item)}'
        answer_unit = counted_unit_from_final_answer(final)
        cases.append(make_case(case_id, 'v297_g1_text_total_two_subjects', text, final, number=total, unit=answer_unit, unit_alternatives=list(forms[item])))
    ...
    for case_id, text, final, number, unit in extra_data:
        answer_unit = counted_unit_from_final_answer(final) or unit
        cases.append(make_case(case_id, 'v297_g1_text_extra_text', text, final, number=number, unit=answer_unit, unit_alternatives=[unit]))

    guard_cases = [
        ('v297_guard_incomplete_01', 'У Лены было 7 тетрадей. Мама купила ещё несколько тетрадей. Сколько тетрадей стало у Лены?', 'guard-low-confidence'),
        ...
        ('v297_guard_incomplete_03', 'У Оли было 5 яблок. Ей дали ещё несколько яблок. Сколько яблок стало у Оли?', 'guard-low-confidence'),
        ('v297_guard_incomplete_04', 'У Иры было 9 книг. Подруга подарила ещё несколько книг. Сколько книг стало у Иры?', 'guard-low-confidence'),
        ('v297_guard_incomplete_05', 'У Вани было 8 мячей. Он отдал несколько мячей. Сколько мячей осталось у Вани?', 'guard-low-confidence'),

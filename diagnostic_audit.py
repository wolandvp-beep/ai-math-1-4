...
def _v313_g4_geometry_live_ui_cases() -> list[dict[str, Any]]:
    """Build V313.1 geometry audit cases from the service spec."""
    ...
    category_blocks: list[tuple[str, str]] = [
        ...
        ('polyline_length', 'v313_g4_geometry_polyline_length'),
        ('width_by_perimeter', 'v313_g4_geometry_width_by_perimeter'),
        ('width_by_area', 'v313_g4_geometry_width_by_area'),
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V313.1 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V313.1 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V313.1 case text should start uppercase: {case.get("id")}')
    return cases

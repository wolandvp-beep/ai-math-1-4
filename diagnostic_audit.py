...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    ...
    if re.match(r'^-?\d+\s+(?:километров|км|суток|дней|час(?:ов|а)?|времени)\s+(?:машина|теплоход|катер|дикий\s+гусь|гусь|утка|самолет|самолёт|идет|идёт|плывет|плывёт|летит)', answer):

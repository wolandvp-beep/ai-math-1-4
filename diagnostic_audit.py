...
def _check_payload(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if expected_numeric is not None and expected_numeric != '':
        ...
        if ':' in target:
            normalized_target = _audit_compare_text(target)
            if normalized_target not in result_for_compare:
...
def _v303_g2_geometry_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, (a, b) in enumerate(rectangles, 1):
        ...
        cases.append(make_case(f'v303_rect_perimeter_{idx:02d}', 'v303_g2_rectangle_perimeter', text, count(p, 'см'), number=p, unit='см'))
    ...
    for idx, side in enumerate(squares, 1):
        ...
        cases.append(make_case(f'v303_square_perimeter_{idx:02d}', 'v303_g2_square_perimeter', text, count(p, 'см'), number=p, unit='см'))
    ...
    for idx, (text, value) in enumerate(segments, 1):
        cases.append(make_case(f'v303_segment_construct_{idx:02d}', 'v303_g2_segment_construction', text, count(value, 'см'), number=value, unit='см'))
...
def _v305_g3_numbers_quantities_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, (a, area) in enumerate(sq_rows, 1):
        cases.append(make_case(f'v305_area_square_{idx:02d}', 'v305_g3_area_square', f'Найди площадь квадрата со стороной {a} см.', f'{area} см²', number=area, unit='см²'))
...
def _v304_g2_math_information_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, (start, end, minutes) in enumerate(duration_rows, 1):
        ...
        cases.append(make_case(f'v304_schedule_duration_{idx:02d}', 'v304_g2_schedule_duration', text, f'{minutes} {unit}', number=minutes, unit=unit))
    ...
    for idx, (a, ta, b, tb, c, tc, target, hours) in enumerate(work_duration_rows, 1):
        ...
        cases.append(make_case(f'v304_work_duration_{idx:02d}', 'v304_g2_work_graph_duration', text, f'{hours} {unit}', number=hours, unit=unit))
...
def _v308_g3_geometry_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, side in enumerate(square_area_rows, 1):
        ...
        cases.append(make_case(f'v308_square_area_{idx:02d}', 'v308_g3_square_area', text, f'площадь квадрата равна {area} см²', number=area, unit='см²'))
...
def _v309_g3_math_information_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, (a, b, c) in enumerate(score_rows, 1):
        ...
        cases.append(make_case(f'v309_score_difference_{idx:02d}', 'v309_g3_table_difference', text, f'на {diff} {ball_unit} больше', number=diff, unit=ball_unit))
    ...
    for idx, (apples, pears, plums) in enumerate(chart_diff_rows, 1):
        ...
        cases.append(make_case(f'v309_chart_difference_{idx:02d}', 'v309_g3_diagram_difference', text, f'на {diff} кг больше', number=diff, unit='кг'))
...
def _v310_g4_numbers_quantities_live_ui_cases() -> list[dict[str, Any]]:
    ...
    for idx, (m, cm) in enumerate(length_cm_rows, 1):
        ...
        cases.append(make_case(f'v310_length_centimeters_{idx:02d}', 'v310_g4_length_centimeters', text, f'{total} {plural(total, "сантиметр", "сантиметра", "сантиметров")}', number=total, unit=plural(total, 'сантиметр', 'сантиметра', 'сантиметров')))
...
def _v314_g4_math_information_live_ui_cases() -> list[dict[str, Any]]:
    """Build V314.2 mathematical-information audit cases from the service spec."""
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V314.2 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V314.2 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V314.2 case text should start uppercase: {case.get("id")}')
    return cases

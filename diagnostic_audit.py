...
def _v312_g4_text_problems_live_ui_cases() -> list[dict[str, Any]]:
    """Build the accepted V312.5 text-problem audit cases from the service spec.
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V312.5 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V312.5 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V312.5 case text should start uppercase: {case.get("id")}')
    return cases

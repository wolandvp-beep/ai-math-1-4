...
def _v313_g4_geometry_live_ui_cases() -> list[dict[str, Any]]:
    """Build V313.3 geometry audit cases from the service spec."""
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V313.3 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V313.3 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V313.3 case text should start uppercase: {case.get("id")}')
    return cases

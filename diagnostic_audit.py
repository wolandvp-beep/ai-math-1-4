...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    ...
    def expected_count_unit_kind() -> str:
        ...
        question_unit_match = re.search(r'скольк(?:о|их|ими)\s+(?:всего\s+)?([а-яёa-z.]+)', task, flags=re.IGNORECASE)
        question_unit = str(question_unit_match.group(1) if question_unit_match else '').strip().lower().replace('ё', 'е').rstrip('.,;:!?')
        if question_unit in people or question_unit in {'студент', 'студента', 'студентов', 'дошкольник', 'дошкольника', 'дошкольников', 'школьник', 'школьника', 'школьников', 'гребец', 'гребца', 'гребцов', 'жильцов', 'жильца', 'жильцы'}:
            return 'person'
        ...
        if unit in people:
            return 'person'
        if 'пульс' in text and unit in {'удар', 'удара', 'ударов', 'уд'}:
            return ''

...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    """V401.4 style gate for Excel text tasks.

    Numeric comparison still uses answer_number/final answer. This additional
    visible-format check catches the issue reported for rows such as
    "5 - 2 = 3" / "крови у ребенка 3 литров": a text-problem calculation must
    show the computed unit in parentheses and a short explanation after a dash.
    """
    ...
    if '?' not in task or not (any(word in task for word in ('сколько', 'скольких', 'сколькими', 'на сколько', 'во сколько', 'чему равн', 'какова', 'каков', 'какой', 'какая')) or _v4013_is_stone_distribution_task(str(case.get('text') or ''))) :
        return []
    # Pure symbolic/expression answers do not have a measurable unit to enforce.
    ...
    def expected_count_unit_kind() -> str:
        ...
        people = {
            ...
            'ребенок', 'ребенка', 'детей', 'дети', 'ребята', 'ребят',
            'ученик', 'ученика', 'учеников', 'брат', 'брата', 'братьев', 'сестра', 'сестры', 'сестер', 'сестёр',
        }
        # V405.03: several Excel raw answers have wrong nouns (e.g. "10 столов"
        ...
        if re.search(r'скольк(?:о|их|ими)?\s+(?:всего\s+)?(?:человек|человека|людей|дошкольник|дошкольника|дошкольников|школьник|школьника|школьников|студент|студента|студентов|гребец|гребца|гребцов|жильц|жильца|жильцов|мальчик|мальчика|мальчиков|девочк|девочки|девочек|детей|ребят|ученик|ученика|учеников)\b', _q_v40502):
            return 'person'
        ...
        if unit in measurement:
            return ''
        # Thousands of years are a time measurement; thousands of visible objects
        # such as глазков remain a counted quantity.
    ...
    body: list[str] = []
    in_solution = False
    for raw_line in str(result or '').replace('\r', '\n').split('\n'):
        line = raw_line.strip()
        if not line:
            continue
        low = line.lower().replace('ё', 'е')
        ...
        if low.startswith('ответ:'):
            break
        if in_solution or re.search(r'\d+\s*(?:[+\-−·×xх*/:÷])\s*\d+\s*=\s*-?\d+', line):
            body.append(re.sub(r'^\d+[\).]\s*', '', line).strip())
    issues: list[str] = []

    def _v40504_round_mental_number(value: Any) -> bool:
    ...
    def _v40504_round_operand_mental_action_line(line: Any) -> bool:
        clean = re.sub(r'^\s*\d+[\).]\s*', '', str(line or '').strip())
        m = re.search(r'(?<!\d)(\d+)\s*([+\-−])\s*(\d+)\s*=\s*-?\d+', clean)
        if not m:
            return False
        op = m.group(2)
        left_round = _v40504_round_mental_number(m.group(1))
        right_round = _v40504_round_mental_number(m.group(3))
        if op == '+':
            return left_round or right_round
        return right_round

    low_result = str(result or '').lower().replace('ё', 'е')
    if re.search(r'(?:метод\s+(?:сложения|вычитания)\s+в\s+столбик|(?:сложение|вычитание)\s+столбиком)', low_result):
        if any(_v40504_round_operand_mental_action_line(line) for line in body):
            issues.append('Excel numeric regression: simple addition with a round operand or subtraction with a round subtrahend must not render column method')
            return issues

    # V405.05: validate only visible solution action lines, not
    # auxiliary column-method explanation rows like
    # "Складываем в этом разряде: 0 + 0 = 0.".
    ...
    for line in arithmetic_lines:
        if not re.search(r'=\s*-?\d+(?:[,.]\d+)?\s*\([^)]+\)\s*[—–-]\s*\S', line):
            issues.append('Excel numeric regression: visible calculation line must contain unit in parentheses and dash explanation')
            break
        ...
        if count_unit_kind == 'person' and not re.search(r'чел|человек', unit_text):
            issues.append('Excel numeric regression: people counts must use (чел.) in visible calculation parentheses')
            break
        ...
        if expl:
            if re.search(r'\b(?:он|она|они|оно)$', expl):
                issues.append('Excel numeric regression: visible calculation explanation must not end with a detached pronoun')
                break
            if expl in {unit_text.strip(), 'м', 'см', 'мм', 'дм', 'км', 'кг', 'г', 'л', 'руб.', 'руб', 'коп.', 'коп', 'шт.', 'шт', 'чел.', 'чел', 'уд.', 'уд', 'удар', 'удара', 'ударов', 'человек', 'человека', 'людей'}:
                issues.append('Excel numeric regression: visible calculation explanation after dash is not meaningful')
                break
            if re.search(r'\bпульс\b', task) and ('шт' in unit_text or 'уд' not in unit_text):
                issues.append('Excel numeric regression: pulse calculations must use (уд.) in visible calculation parentheses')
                break
            if re.search(r'\b(?:ему|ей|им|нам|вам|уже)$', expl) or re.search(r'\b(?:пошло|пошли|приехало|приехали|посадили|заболели)$', expl):
                issues.append('Excel numeric regression: visible calculation explanation after dash must not end with a detached pronoun/adverb or copied predicate')
                break
            if 'сколько зарабатывает мама' in task and expl == 'мама':
                issues.append('Excel numeric regression: visible calculation explanation should say “мама зарабатывает”, not only “мама”')
                break
            if ('лишилась' in task or 'запрещена охота' in task) and re.search(r'\b(?:лишил[а-я]*|запрещен[а-я]*|охота|планета)\b', expl):
                issues.append('Excel numeric regression: visible calculation explanation after dash should be short and name only what was counted')
                break
            if re.search(r'\b(?:кг|г|км|м|дм|см|мм)\b', unit_text) and re.search(r'\b(?:заготовил[а-я]*|можно\s+получить)\b', expl):
                issues.append('Excel numeric regression: visible calculation explanation after dash should be concise for SI measurement object')
                break
            ...
            if concise_v40204 and concise_v40204.lower().replace('ё', 'е') != expl:
                issues.append('Excel numeric regression: visible calculation explanation after dash should be concise and must not duplicate the full answer predicate')
                break
            ...
            if counted_concise_v40204 and counted_concise_v40204.lower().replace('ё', 'е') != expl:
                issues.append('Excel numeric regression: counted-object visible calculation explanation after dash should be concise and must not copy context/predicate text from the answer')
                break
            if re.search(r'сколько\s+километров\s+машина\s+проехала\s+за\s+(?:два|2)\s+дня', task) and expl != 'проехала машина':
                issues.append('Excel numeric regression: visible calculation explanation for the two-day car distance task should be “проехала машина”')
                break
    answer_raw = _extract_answer_line(result).strip()
    answer = answer_raw.lower().replace('ё', 'е')
    visible_all = str(result or '')
    ...
    if re.search(r'\d+\s+[а-яёa-z.]+(?:\s+[а-яёa-z.]+){0,4}\s+(?:он|она|они|оно)$', answer):
        issues.append('Excel numeric regression: visible Ответ line must not put “он/она” at the end')
    original_task_text = str(case.get('text') or '')
    if _v4017_abbreviate_si_in_answer(answer_raw) != answer_raw:
        issues.append('Excel numeric regression: SI units in visible Ответ line must be abbreviated: кг, г, км, м, дм, см, мм')
    if _v4017_fix_extra_name_before_group_subject(answer_raw, original_task_text) != answer_raw:
        issues.append('Excel numeric regression: visible Ответ line must not put an individual name before a broad group subject such as “дети”')
    if _v4017_lowercase_common_u_nouns(answer_raw, original_task_text) != answer_raw:
        issues.append('Excel numeric regression: common nouns from “у ...” context must not be capitalized as proper names')
    if _v4018_fix_measure_answer_order(answer_raw, original_task_text) != answer_raw:
        issues.append('Excel numeric regression: visible Ответ line for SI measurement action must use natural Russian word order')
    if (
        ...
        issues.append('Excel numeric regression: visible Ответ line for contextual counted objects should put the context before the quantity')
    if re.search(r'можно\s+получить', task) and re.fullmatch(r'-?\d+(?:[,.]\d+)?\s*(?:кг|г|килограмм(?:а|ов)?|грамм(?:а|ов)?)', answer):
        issues.append('Excel numeric regression: visible Ответ line for “можно получить” measurement question must be a full phrase, not only the quantity')
    if (
        ...
        issues.append('Excel numeric regression: visible Ответ line for dried pears must be exactly the full phrase “можно получить 4 кг сушеных груш”')
    if (
        'в гирлянде 10 лампочек' in task
        and 'фиолетовых 6 лампочек' in task
        and answer != 'в гирлянде 4 зеленые лампочки'
    ):
        issues.append('Excel numeric regression: visible Ответ line for green lamps must use correct Russian plural and full context')
    if (
        ...
        issues.append('Excel numeric regression: visible Ответ line for beet boxes must be a clean full phrase')
    if (
        'во дворе стоят 12 автомашин' in task
        and 'если грузовых 4' in task
        and answer != 'во дворе стоит 8 легковых автомашин'
    ):
        issues.append('Excel numeric regression: visible Ответ line for light cars must be a grammatically correct full phrase')
    if (
        'мимо станции за день прошло 25 поездов' in task
        and 'пассажирских' in task
        and answer != 'прошло 16 товарных поездов'
    ):
        issues.append('Excel numeric regression: visible Ответ line for freight trains must be a grammatically correct full phrase')
    for proper in known_names_from_task():
        ...
        if re.search(rf'(?<![А-ЯЁа-яё]){re.escape(low_name)}(?![А-ЯЁа-яё])', visible_visible):
            issues.append('Excel numeric regression: proper names from the task must keep uppercase spelling')
            break
    if re.search(r'(?:какова|каков|какой|какая|чему\s+равн(?:а|ен|о|ы)?)\s+(?:длина|ширина|высота|масса|вес|периметр|площадь)', task) and re.fullmatch(r'-?\d+(?:[,.]\d+)?\s*(?:мм|см|дм|м|км|г|кг|л|руб\.?|коп\.?|метр(?:а|ов)?|сантиметр(?:а|ов)?|миллиметр(?:а|ов)?)', answer):
        issues.append('Excel numeric regression: visible Ответ line is too short for a property-measurement question')
    if 'лишилась' in task and re.fullmatch(r'-?\d+\s+видов', answer):
        issues.append('Excel numeric regression: visible Ответ line for species loss must be a full phrase')
    if 'запрещена охота' in task and re.search(r'видов', answer) and not answer.startswith('на '):
        issues.append('Excel numeric regression: visible Ответ line for hunting ban must start with “на ... видов”')
    if (
        re.search(r'сколько\s+километров\s+машина\s+проехала\s+за\s+(?:два|2)\s+дня', task)
        and answer != '40 км проехала машина за два дня'
    ):
        issues.append('Excel numeric regression: visible Ответ line for the two-day car distance task must be exactly “40 км проехала машина за два дня”')
    if re.search(r'сколько\s+километров\s+машина\s+проехала\s+за\s+(?:два|2)\s+дня', task):
        # V401.9: reject the short Excel-like answer and the older word order
        # «машина за два дня проехала 40 км» for this audited textbook row.
        if re.fullmatch(r'-?\d+(?:[,.]\d+)?\s*(?:км|километр(?:а|ов)?)', answer) or not re.match(r'^-?\d+(?:[,.]\d+)?\s+км\s+проехала\s+машина\s+за\s+(?:два|2)\s+дня$', answer):
            issues.append('Excel numeric regression: visible Ответ line for the two-day car distance task must be a full phrase: “40 км проехала машина за два дня”')
        ...
        issues.append('Excel numeric regression: visible Ответ line should put subject/action before the quantity for motion or time questions')
    if _v4013_is_stone_distribution_task(str(case.get('text') or '')):
        ...
        for required in ('1+6=7', '2+5=7', '3+4=7'):
            if required not in compact:
                issues.append('Excel numeric regression: multi-answer stone distribution must show every backpack grouping in solution steps')
                break
...
def _check_payload(case: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    excel_visible_issues = _excel_visible_step_unit_explanation_issues(case, result)
    if excel_visible_issues:
        issues.extend(excel_visible_issues)

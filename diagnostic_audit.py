...
def _v314_g4_math_information_live_ui_cases() -> list[dict[str, Any]]:
    """Build V314.3 mathematical-information audit cases from the service spec."""
    ...
    for case in cases:
        text = str(case.get('text') or '')
        if text in seen:
            raise AssertionError(f'V314.3 duplicate case text: {text}')
        seen.add(text)
        if '  ' in text:
            raise AssertionError(f'V314.3 double spaces in {case.get("id")}')
        if not text[:1].isupper():
            raise AssertionError(f'V314.3 case text should start uppercase: {case.get("id")}')
    return cases

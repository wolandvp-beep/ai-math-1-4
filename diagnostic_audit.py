...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    ...
    def expected_count_unit_kind() -> str:
        ...
        people = {
            ...
            'ученик', 'ученика', 'учеников', 'взрослый', 'взрослого', 'взрослых', 'брат', 'брата', 'братьев', 'сестра', 'сестры', 'сестер', 'сестёр',
        }
    ...
    for line in arithmetic_lines:
        ...
        measurement_unit_ok = bool(re.search(r'^(?:кг|г|т|л|м|см|дм|мм|км|руб\.?|коп\.?|д\.?|дн\.?|нед\.?|мес\.?|раза?|м/с|км/ч)$', unit_text.strip()))

...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '1200'),
        ('limit', '100'),
        ('cacheBust', 'v413-01-excel-numeric-regression'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V413.01 — Excel numeric regression / batch 1201–1300 real external UI-render audit',
        ...
        'nextAuditNote': 'V413.01 запускает batch 1201–1300 через GitHub Pages frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v413-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v413.01-live-excel-numeric-regression-v1'
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V413.01 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v413.01-live-excel-numeric-regression',
    })
    ...
    *,
    ...
    def _mutator(state: dict[str, Any]):
        ...
        run = {
            ...
            'browserClientFetchNote': 'V413.01 browser audit advances by frontend UI actions: task input, solve button click, DOM #resultBox proof, external API token proof, and numeric comparison against Excel expected answers.',
        }
        runs[run_id] = run
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V413.01 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V413.01 Excel numeric regression UI-render live-аудит</title>
<style>
...
<h1>V413.01 — Excel numeric regression UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V413.01 проверяет Excel numeric regression через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    payload = {
        **dict(report),
        ...
        'finalReportFormat': 'compact-json-v413.01',
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v413-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V413.01 frontend UI-render proof audit for Excel numeric regression',

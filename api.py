...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V317 — TTS/озвучивание, live browser speech-normalization audit',
        'nextAuditSection': 'tts_voice',
        'nextAuditLimit': 100,
        ...
        'nextAuditNote': 'V317 запускается с GitHub Pages frontend напрямую: браузер вводит реальные задания 1–4 классов из основных разделов в приложение, нажимает основную кнопку решения, ждёт #resultBox и сверяет expected, API-ответ и пользовательски видимый DOM.',
    }
...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'tts_voice', 'tts', 'voice', 'speech', 'g_tts', 'v317', 'current_section'}:
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v317-tts-voice-v1'
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'tts_voice' and planned != 100:
        blockers.append('planned must be 100 for V317 tts_voice acceptance')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V317 pass: TTS/озвучивание принято; следующий шаг — финальная выдача принятого архива',
        **fresh_links,
    }
...
def _api_v314_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Route-layer guard for V317 TTS voice check tasks."""
    ...
    try:
        ...
        if isinstance(spec, dict):
            ...
            base_payload['visibleResultContract'] = 'v317-tts-voice-canonical'
...
def _api_v309_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    # V317: V314 math-information tasks are part of the TTS voice check with
...
def _browser_client_case_rows(section: str = 'tts_voice', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
<section class="box">
  ...
  <p>V317 проверяет TTS/озвучивание через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V317 frontend UI-render proof audit for TTS voice check',

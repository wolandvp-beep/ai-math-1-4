...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V309.02 — 3 класс, раздел 5 — Математическая информация, live browser UI-render audit',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v309.02-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v309.02-g3-math-information-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V309.02 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g3_math_information' and planned != 100:
        blockers.append('planned must be 100 for V309.02 g3_math_information acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V309.02 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V309.02 pass: V310 — 4 класс, раздел 1 — Числа и величины',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V309.02 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V309.02 client sends one visible POST per audit case.",
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V309.02 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox preserves the accepted API answer for the same case.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V309.02 runs the audit from the production frontend page itself, so the user sees
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V309.02 frontend UI-render live-аудит</title>
<style>
...
<h1>V309.02 — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V309.02 проверяет математическую информацию 3 класса через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V309.02 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v309.02-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V309.02 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V309.02 frontend UI-render proof audit for grade 3 mathematical information',

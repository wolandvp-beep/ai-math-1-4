...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.06 — 1 класс, раздел 2 — Арифметические действия, browser one-link proof audit with DeepSeek token evidence',
        ...
        'nextAuditStartPath': operator_path,
        'nextAuditStartUrl': base_url + operator_path,
        ...
        'nextAuditAcceptanceRule': 'Раздел принимать только по единой final-report ссылке: finalAcceptance=true, status=done, completed=planned, failed=0, failures=0, suspicious=0, evidence rows=planned, external API evidence present.',
        'nextAuditUserRunWorkflow': [
            '1) Откройте nextAuditStartUrl в браузере.',
            '2) Нажмите одну кнопку: Запустить / продолжить live-аудит.',
            '3) Дождитесь статуса done/готово.',
            '4) Скопируйте одну ссылку: Итоговый отчёт для ChatGPT.',
        ],
        'nextAuditLocalOperatorWorkflow': 'Через браузер: одна операторская страница, одна кнопка, одна итоговая ссылка для ChatGPT.',
        ...
        'nextAuditNote': 'Операторская страница без force=1 переиспользует run/cache и не тратит DeepSeek повторно на уже завершённые cases. Итоговый final-report не вызывает DeepSeek.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.06-live-audit'
...
def _deepseek_usage_total(usage: Any) -> int:
    ...
    for key in ('total_tokens', 'totalTokens'):
        try:
            ...
            pass
    total = 0
    for key in ('prompt_tokens', 'completion_tokens', 'prompt_cache_hit_tokens', 'prompt_cache_miss_tokens', 'input_tokens', 'output_tokens'):
        try:
...
def _deepseek_usage_prompt(usage: Any) -> int:
    if not isinstance(usage, dict):
        return 0
    total = 0
    for key in ('prompt_tokens', 'input_tokens', 'prompt_cache_hit_tokens', 'prompt_cache_miss_tokens'):
        try:
            total += int(usage.get(key) or 0)
        except Exception:
            pass
...
async def _live_audit_direct_deepseek_call(payload: dict, *, timeout_seconds: float, api_key: str) -> dict[str, Any]:
    ...
    proof = {
        ...
        'finishReason': first.get('finish_reason'),
        'usage': usage,
        'promptTokens': _deepseek_usage_prompt(usage),
        'completionTokens': _deepseek_usage_completion(usage),
        'totalTokens': _deepseek_usage_total(usage),
    ...
    async def counted_call_deepseek(api_payload, *args, **kwargs):
        ...
        try:
            ...
            if isinstance(proof, dict):
                ...
                counter['deepseekPromptTokens'] = int(counter.get('deepseekPromptTokens') or 0) + int(proof.get('promptTokens') or 0)
                counter['deepseekCompletionTokens'] = int(counter.get('deepseekCompletionTokens') or 0) + int(proof.get('completionTokens') or 0)
                counter['deepseekTotalTokens'] = int(counter.get('deepseekTotalTokens') or 0) + int(proof.get('totalTokens') or 0)
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.06-g1-arithmetic-actions-token-usage-proof-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V296.06 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_suspicion_reasons(row: dict[str, Any]) -> list[str]:
    ...
    if bool(row.get('ok')):
        ...
        if not is_guard and int(row.get('deepseekTotalTokens') or 0) <= 0:
            reasons.append('passed normal case has no positive DeepSeek total token usage')
        if not is_guard and str(row.get('routeUnderAudit') or '') != 'POST /api/explain':
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_arithmetic_actions' and planned != 100:
        blockers.append('planned must be 100 for V296.06 g1_arithmetic_actions acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V296.06 proof run must spend fresh external calls, not replay case cache')
    ...
    if normal_cases and int(run.get('deepseekTotalTokens') or 0) <= 0:
        blockers.append('deepseekTotalTokens <= 0')
    ...
    if any(int(item.get('deepseekTotalTokens') or 0) <= 0 for item in normal_cases):
        blockers.append('at least one normal case has no positive DeepSeek token usage')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V296.06 pass: V297 — 1 класс, раздел 3 — Текстовые задачи',
        **fresh_links,
    }
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None, audit_bypass_daily_limit: bool = False) -> JSONResponse | dict:
    ...
    try:
        ...
        return attach_release({**result, 'access': access, 'auditBypassDailyLimit': audit_bypass_daily_limit})
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    base_url = _public_base_url(request)
    operator_path = f'/api/diagnostics/live-audit/operator/{APP_RELEASE}/{key}'
    start_url = base_url + operator_path + '?start=1'
    ...
    evidence = int(payload.get('caseProofsTotal') or payload.get('evidenceRowsTotal') or payload.get('evidenceResultsCount') or 0)
    suspicious = int(payload.get('suspiciousCount') or payload.get('suspiciousPassedCount') or 0)
    pct = 0 if planned <= 0 else max(0, min(100, round(completed * 100 / planned)))
    title, klass, hint = _browser_audit_label(payload)
    is_running = start_mode and str(payload.get('status') or '') in {'queued', 'running'}
    meta = f'<meta http-equiv="refresh" content="12; url={escape(start_url, quote=True)}">' if is_running else ''
    technical_json = json.dumps(payload, ensure_ascii=False, indent=2)
    issues = payload.get('acceptanceIssues') or []
    issues_html = ''
    if issues:
        items = ''.join(f'<li>{escape(str(x))}</li>' for x in issues[:20])
        issues_html = f'<div class="box warn"><b>Пока не принято:</b><ul>{items}</ul></div>'
    final_html = ''
    if final_url:
        final_html = f'''
<section class="box result">
  <h2>Одна ссылка для ChatGPT</h2>
  <p>Скопируйте и пришлите мне только эту ссылку:</p>
  <input id="finalUrl" readonly value="{escape(final_url, quote=True)}" onclick="this.select()">
  <button type="button" onclick="navigator.clipboard && navigator.clipboard.writeText(document.getElementById('finalUrl').value); document.getElementById('copyMsg').textContent='Скопировано';">Скопировать ссылку</button>
  <span id="copyMsg" class="copyMsg"></span>
  <p><a href="{escape(final_url, quote=True)}" target="_blank" rel="noopener">Открыть итоговый отчёт</a></p>
</section>'''
    refresh_note = 'Страница обновится сама примерно через 12 секунд.' if is_running else 'Повторный запуск без force=1 переиспользует текущий run/cache.'
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0">{meta}<title>V296.06 live-аудит</title>
<style>
body{{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:920px;margin:28px auto;padding:0 16px;line-height:1.45;background:#f8fafc;color:#111827}}
...
input{{box-sizing:border-box;width:100%;border:1px solid #d1d5db;border-radius:12px;padding:12px;font:15px ui-monospace,Menlo,monospace;background:#fff}}.muted{{color:#6b7280}}.copyMsg{{margin-left:10px;color:#166534;font-weight:700}}pre{{white-space:pre-wrap;background:#111827;color:#f9fafb;padding:14px;border-radius:14px;overflow:auto;max-height:420px}}
</style></head><body>
<h1>V296.06 — live-аудит через браузер</h1>
<p class="muted">Раздел: 1 класс — арифметические действия. Объём: 100 live DeepSeek case.</p>
<div class="status {escape(klass)}">{escape(title)}</div>
<p>{escape(hint)}</p>
<section class="box">
  <h2>1. Запуск</h2>
  <p><a class="primary" href="{escape(start_url, quote=True)}">Запустить / продолжить аудит</a></p>
  <p class="muted">{escape(refresh_note)}</p>
</section>
<section class="box">
  <h2>2. Прогресс</h2>
  ...
  <div class="grid">
    ...
    <div class="metric">external/evidence <b>{external}/{evidence}</b></div>
    <div class="metric">suspicious <b>{suspicious}</b></div>
    <div class="metric">status <b>{escape(str(payload.get('status') or '—'))}</b></div>
  </div>
  <p class="muted">runId: {escape(run_id or 'ещё нет')}</p>
</section>
{issues_html}
{final_html}
...
async def _browser_audit_operator_payload(request: Request, release_token: str, key_value: str, start: int = 0) -> dict[str, Any] | JSONResponse:
    ...
    if str(start).lower() in {'1', 'true', 'yes', 'on'}:
        return await live_audit_runner_start(
            request=request,
            key=key_value,
            section='g1_arithmetic_actions',
            limit=100,
    ...
    return {
        **_version_payload(request),
        'diagnostic': 'live-audit-browser-operator',
        'status': 'not-started',
        'planned': 100,
        'completed': 0,
        'passed': 0,
        'failed': 0,
        'operatorInstruction': 'Одна кнопка запуска/продолжения. После завершения пришлите ChatGPT одну final-report ссылку.',
    }
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V296.06 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.06-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V296.06 accepts section only if aggregate, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V296.06 browser one-link proof audit with DeepSeek token evidence report',
...
async def proxy(request: Request):
    ...
    text = data.get('text')
    return await _solve_text(text=text, token=_extract_bearer_token(request), install_id=_extract_install_id(request, data), audit_bypass_daily_limit=_live_audit_key_matches(request.headers.get('X-Live-Audit-Key', '')))

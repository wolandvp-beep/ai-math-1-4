...
from backend.service import APP_RELEASE, SOLVER_VERSION, attach_release, canonicalize_v309_math_information_response, canonicalize_v310_numbers_quantities_response, canonicalize_v311_arithmetic_actions_response, canonicalize_v312_text_problems_response, canonicalize_v313_geometry_response, _v312_case_specs, _v312_norm_key, _v312_payload, _v313_case_specs, _v313_norm_key, _v313_payload, deepseek_api_key_configured, generate_explanation_response, prevalidate_explanation_request, resolve_solver_mode
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V313.3 — 4 класс, раздел 4 — Геометрия, live browser UI-render audit',
        'nextAuditSection': 'g4_geometry',
        'nextAuditLimit': 100,
        ...
        'nextAuditNote': 'V313.3 запускается с GitHub Pages frontend напрямую: браузер вводит реальные геометрические задачи 4 класса в приложение, нажимает основную кнопку решения, ждёт #resultBox и сверяет expected, API-ответ и пользовательски видимый DOM.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v313.3-live-audit'
...
def _case_matches_current_programmatic_section(case: dict[str, Any]) -> bool:
    return _case_matches_v313_geometry(case)
...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'g4_geometry', 'g4_section4', 'current_section', 'v313'}:
...
async def _generate_with_browser_client_fetch_counter(text: str, *, allow_external: bool, audit_key: str, audit_context: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        # V313.3: make the browser-visible audit payload canonical before the
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v313.3-g4-geometry-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and reuses completed case results. V313.3 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_prepare_visible_compare_text(value: Any, case_text: Any = None, is_guard_case: bool = False) -> str:
    ...
    for raw_line in raw.split('\n'):
        ...
        if norm in {'задача', 'решение', 'получено несколько строк:', 'получено несколько строк', 'пояснения'}:
            continue
        if re.match(r'^метод\s+.+?\s+в\s+столбик\.?$', norm):
            continue
        if re.match(r'^(складываем|вычитаем|умножаем|пишем|оставшийся перенос|определяем первое неполное делимое|подобрали первое неполное делимое|смотрим, сколько раз|сносим следующую цифру|теперь работаем|деление закончено|после предыдущего займа|первое число меньше второго)', norm):
            continue
...
def _live_audit_strict_format_issues(case: dict[str, Any], result_text: str, *, is_guard_case: bool) -> list[str]:
    ...
    category = str(case.get('category') or '')
    name = str(case.get('name') or case.get('id') or '')
    if not ((category.startswith('v296_') or name.startswith('v296_')) or (category.startswith('v297_') or name.startswith('v297_')) or (category.startswith('v298_') or name.startswith('v298_')) or (category.startswith('v300_') or name.startswith('v300_')) or (category.startswith('v302_') or name.startswith('v302_')) or (category.startswith('v303_') or name.startswith('v303_')) or (category.startswith('v304_') or name.startswith('v304_')) or (category.startswith('v305_') or name.startswith('v305_')) or (category.startswith('v306_') or name.startswith('v306_')) or (category.startswith('v307_') or name.startswith('v307_')) or (category.startswith('v308_') or name.startswith('v308_')) or (category.startswith('v309_') or name.startswith('v309_')) or (category.startswith('v310_') or name.startswith('v310_')) or (category.startswith('v311_') or name.startswith('v311_')) or (category.startswith('v312_') or name.startswith('v312_')) or (category.startswith('v313_') or name.startswith('v313_'))):
        return []
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g4_geometry' and planned != 100:
        blockers.append('planned must be 100 for V313.3 g4_geometry acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V313.3 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V313.3 pass: V314 — следующий раздел по карте',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V313.3 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v313_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Route-layer guard for V313.3 geometry tasks."""
    ...
    try:
        ...
        if isinstance(spec, dict):
            ...
            base_payload['visibleResultContract'] = 'v313.3-g4-geometry-canonical'
...
def _browser_client_case_rows(section: str = 'g4_geometry', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V313.3 client sends one visible POST per audit case.",
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V313.3 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox preserves the accepted API answer for the same case.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V313.3 runs the audit from the production frontend page itself, so the user sees
    ...
    technical_json = _safe_json_dumps_for_html(payload)
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V313.3 frontend UI-render live-аудит</title>
<style>
...
<h1>V313.3 — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V313.3 проверяет геометрические задачи 4 класса через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def _browser_audit_operator_payload(request: Request, release_token: str, key_value: str, start: int = 0) -> dict[str, Any] | JSONResponse:
    ...
    return {
        **_version_payload(request),
        ...
        'promptCacheHitTokens': 0,
        'promptCacheMissTokens': 0,
        'operatorInstruction': "Нажмите кнопку. Браузер создаст run и сам последовательно выполнит POST /api/explain для 100 текстовых задач 4 класса.",
        'tokenFields': ['apiPromptTokens', 'apiCompletionTokens', 'apiTotalTokens', 'promptCacheHitTokens', 'promptCacheMissTokens'],
...
def _safe_json_dumps_for_html(value: Any) -> str:
    """Serialize audit proof for HTML without allowing bad Unicode to crash response encoding."""
    try:
        data = json.dumps(value, ensure_ascii=False, indent=2, default=str)
    except Exception:
        data = json.dumps(value, ensure_ascii=True, indent=2, default=str)
    try:
        data.encode('utf-8')
        return data
...
def _final_report_compact_value(value: Any, *, depth: int = 0, key_name: str = '') -> Any:
    '''Keep the one-link final report readable and safe for large UI proofs.'''
    lowered = str(key_name or '').lower()
    if isinstance(value, str):
        value = value.encode('utf-8', 'backslashreplace').decode('utf-8', 'replace')
        hard_trim_keys = (
            'html', 'outerhtml', 'innerhtml', 'screenshot', 'serializeddom',
            'documenthtml', 'resultboxhtml', 'rawdom', 'fullhtml'
        )
        max_len = 1200
        if any(token in lowered for token in hard_trim_keys):
            max_len = 320
        elif lowered in {'resulttext', 'apiresulttext', 'domresulttext', 'visibletext', 'resultboxtext', 'uservisibleresulttext'}:
            max_len = 1800
        if len(value) > max_len:
            return value[:max_len] + f'… [trimmed {len(value) - max_len} chars for final-report readability]'
        return value
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            key_text = str(key)
            low_key = key_text.lower()
            if low_key in {
                'domsnapshot', 'domsnapshothtml', 'rawhtml', 'rawdomhtml',
                'fullpagehtml', 'pagehtml', 'renderedhtml', 'resultboxhtml'
            }:
                out[key_text] = '[omitted bulky HTML from final-report; DOM proof counters and text snippets are preserved]'
                continue
            out[key_text] = _final_report_compact_value(item, depth=depth + 1, key_name=key_text)
        return out
    if isinstance(value, list):
        limit = 500
        items = [_final_report_compact_value(item, depth=depth + 1, key_name=key_name) for item in value[:limit]]
        if len(value) > limit:
            items.append({'trimmedListItems': len(value) - limit})
        return items
    return str(value)


def _final_report_json_dumps(payload: dict[str, Any]) -> str:
    try:
        compact = _final_report_compact_value(payload)
        if isinstance(compact, dict):
            compact['finalReportPayloadCompacted'] = True
            compact['finalReportCompactionNote'] = 'Large DOM/HTML strings are trimmed so the one-link final report opens reliably; acceptance counters and case proof rows are preserved.'
        return _safe_json_dumps_for_html(compact)
    except Exception as exc:
        minimal = {
            'diagnostic': 'live-audit-browser-final-report-minimal-fallback',
            'finalReportSerializationError': str(exc),
            'release': payload.get('release'),
            'backendBuild': payload.get('backendBuild'),
            ...
            'finalAcceptance': payload.get('finalAcceptance'),
            'acceptancePassed': payload.get('acceptancePassed'),
            'acceptanceIssues': payload.get('acceptanceIssues'),
            'failuresCount': payload.get('failuresCount'),
            'suspiciousCount': payload.get('suspiciousCount'),
            'caseProofsTotal': payload.get('caseProofsTotal'),
        }
        return _safe_json_dumps_for_html(minimal)


async def _final_report_safe_call(label: str, coro: Any) -> Any:
    try:
        item = await coro
    except Exception as exc:
        return {'diagnostic': label, 'error': 'final-report subcall failed', 'exceptionType': type(exc).__name__, 'message': str(exc)}
    if isinstance(item, JSONResponse):
        body_text = ''
        try:
            raw_body = getattr(item, 'body', b'')
            body_text = raw_body.decode('utf-8', errors='replace') if isinstance(raw_body, (bytes, bytearray)) else str(raw_body)
        except Exception:
            body_text = ''
        return {'diagnostic': label, 'jsonResponseStatus': getattr(item, 'status_code', None), 'jsonResponseBody': body_text[:1200]}
    return item
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    report = await _final_report_safe_call('live-audit-runner-report', live_audit_runner_report(key=key_value, runId=run_id_value, release=release_token))
    if not isinstance(report, dict):
        report = {'diagnostic': 'live-audit-browser-final-report', 'error': 'report payload is not a dict', 'reportPayloadType': type(report).__name__, 'reportPayload': str(report)[:1200]}
    results_full = await _final_report_safe_call('live-audit-runner-results-compact', live_audit_runner_results(key=key_value, runId=run_id_value, release=release_token, includeFull=0, limit=500, offset=0))
    evidence = await _final_report_safe_call('live-audit-runner-evidence', live_audit_runner_evidence(key=key_value, runId=run_id_value, release=release_token, limit=500, offset=0))
    suspicious = await _final_report_safe_call('live-audit-runner-suspicious', live_audit_runner_suspicious(key=key_value, runId=run_id_value, release=release_token))
    failures = await _final_report_safe_call('live-audit-runner-failures', live_audit_runner_failures(key=key_value, runId=run_id_value, release=release_token, limit=200))
    if isinstance(evidence, dict) and isinstance(evidence.get('evidenceRows'), list):
        evidence_rows = list(evidence.get('evidenceRows') or [])
        evidence = dict(evidence)
        evidence.pop('evidenceRows', None)
        evidence['evidenceRowsSampleLimit'] = 20
        evidence['evidenceRowsSample'] = [_compact_live_audit_result(row) for row in evidence_rows[:20] if isinstance(row, dict)]
        evidence['evidenceRowsCompactedForFinalReport'] = True
    payload = {
        **dict(report),
        'diagnostic': 'live-audit-browser-final-report',
        'singleLinkForChatGPT': True,
        'operatorInstruction': 'Скопируйте URL этой страницы и пришлите ChatGPT. Эта одна ссылка содержит acceptance, failures, suspicious и compact case-proof evidence.',
        'fullResultsPayload': results_full,
        'evidencePayload': evidence,
        'suspiciousPayload': suspicious,
        'failuresPayload': failures,
    }
    title = 'V313.3 итоговый live-audit отчёт для ChatGPT'
    data = _final_report_json_dumps(payload)
    ...
    summary = payload.get('acceptanceSummary') if isinstance(payload.get('acceptanceSummary'), dict) else {}
    html = f'''<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="robots" content="noindex"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>{escape(title)}</title><style>body{{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:1100px;margin:28px auto;padding:0 16px;line-height:1.45}}.status{{font-size:22px;font-weight:850;border-radius:16px;padding:16px;margin:16px 0}}.ok{{background:#ecfdf5;border:1px solid #bbf7d0}}.bad{{background:#fef2f2;border:1px solid #fecaca}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px}}.metric{{background:#f3f4f6;border-radius:14px;padding:12px}}.metric b{{font-size:24px;display:block}}pre{{white-space:pre-wrap;background:#111827;color:#f9fafb;border-radius:14px;padding:14px;overflow:auto}}</style></head><body>
<h1>{escape(title)}</h1><div class="status {klass}">{escape(status)}</div><p>Эту страницу достаточно прислать ChatGPT одной ссылкой. Ниже compact JSON-proof: крупные HTML/DOM-строки обрезаны, метрики acceptance и case proof rows сохранены.</p>
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v313.3-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V313.3 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V313.3 frontend UI-render proof audit for grade 4 geometry',

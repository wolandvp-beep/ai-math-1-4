...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'v297 auto-current guarded re-audit: 1 класс, раздел 1 — Числа и величины',
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v297-auto-current-release-guard-v6'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v297 automation pass: v298 — 1 класс, раздел 2 — Арифметические действия',
    }
...
async def live_audit_auto_current_release_guard(request: Request, expected_release: str, key_value: str):
    ...
    if isinstance(result, JSONResponse):
        return result
    result = dict(result)
    result['diagnostic'] = 'live-audit-auto-current-release-guard'
    result['automationEntrypoint'] = 'path-no-query-expected-release-guard'
    ...
    result['tokenSpendGuard'] = 'Release mismatch returns 409 before any DeepSeek call.'
    result['note'] = (
        'Auto-current automation: assistant opens a fixed path containing the expected release. '
        'Repeated opens reuse current-release cache and do not spend DeepSeek tokens again unless force=1 is used via the legacy/debug endpoint.'
    )
    return _html_ok(_audit_dashboard_html('Auto-current live audit automation', result))

...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'v296 — 1 класс, раздел 2 — Арифметические действия',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296-g1-arithmetic-actions-v1'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v296 pass: v297 — 1 класс, раздел 3 — Текстовые задачи',
    }
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

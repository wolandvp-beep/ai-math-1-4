...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    legacy_start_query = urlencode([
        ('section', 'app_quality_regression'),
        ('key', audit_key),
        ('limit', '100'),
        ('offset', '0'),
        ('allowExternal', '1'),
    ...
    return {
        'nextAuditPlannedMapStep': 'V317 — контрольная регрессия качества приложения, live browser UI-render audit',
        'nextAuditSection': 'app_quality_regression',
        'nextAuditLimit': 100,
        ...
        'nextAuditNote': 'V317 запускается с GitHub Pages frontend напрямую: браузер вводит контрольные задания качества 1–4 классов в приложение, нажимает основную кнопку решения, ждёт #resultBox и сверяет expected, API-ответ и пользовательски видимый DOM.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v317-live-audit'
...
def _v317_app_quality_regression_cases(cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # V317: контрольная регрессия качества — другой набор из 5 устойчивых
    # UI-render cases каждого принятого раздела 1–4 классов. 20 × 5 = 100 cases.
    # V317: выбираем без повторяющихся inputText, иначе acceptance блокирует
    ...
    for case in selected[:100]:
        ...
        if m:
            ...
            continue
        m = re.search(r'в альбоме было (\d+) марок\. добавили (\d+) мар', low)
        if m:
            total = int(m.group(1)) + int(m.group(2))
            unit = 'марка' if total % 10 == 1 and total % 100 != 11 else ('марки' if total % 10 in {2, 3, 4} and total % 100 not in {12, 13, 14} else 'марок')
            patched.append(set_expected(case, f'стало {total} {unit}', number=str(total), unit=unit))
            continue
        m = re.search(r'у вани было (\d+) карандаш\w*, у димы было (\d+) карандаш', low)
        if m:
            diff = int(m.group(2)) - int(m.group(1))
            unit = 'карандаш' if diff % 10 == 1 and diff % 100 != 11 else ('карандаша' if diff % 10 in {2, 3, 4} and diff % 100 not in {12, 13, 14} else 'карандашей')
            patched.append(set_expected(case, f'у Димы на {diff} {unit} больше, чем у Вани', number=str(diff), unit=unit))
            continue
        m = re.search(r'всего (\d+) мяч\w* раздали поровну (\d+) дет', low)
        if m and int(m.group(2)):
            each = int(m.group(1)) // int(m.group(2))
            unit = 'мяч' if each % 10 == 1 and each % 100 != 11 else ('мяча' if each % 10 in {2, 3, 4} and each % 100 not in {12, 13, 14} else 'мячей')
            patched.append(set_expected(case, f'каждый ребёнок получил {each} {unit}', number=str(each), unit=unit))
            continue
        m = re.search(r'ломаная состоит из (\d+) звень', low)
        if m and 'сколько звеньев у ломаной' in low:
            n = int(m.group(1))
            unit = 'звено' if n % 10 == 1 and n % 100 != 11 else ('звена' if n % 10 in {2, 3, 4} and n % 100 not in {12, 13, 14} else 'звеньев')
            patched.append(set_expected(case, f'у ломаной {n} {unit}', number=str(n), unit=unit))
            continue
        m = re.search(r'звенья ломаной имеют длины (\d+) см, (\d+) см и (\d+) см', low)
        if m:
            total = int(m.group(1)) + int(m.group(2)) + int(m.group(3))
            patched.append(set_expected(case, f'длина ломаной равна {total} см', number=str(total), unit='см'))
            continue
        m = re.search(r'на диаграмме: у ани (\d+) марок, у бори (\d+) марок', low)
        if m:
            diff = int(m.group(2)) - int(m.group(1))
            unit = 'марка' if diff % 10 == 1 and diff % 100 != 11 else ('марки' if diff % 10 in {2, 3, 4} and diff % 100 not in {12, 13, 14} else 'марок')
            patched.append(set_expected(case, f'у Бори на {diff} {unit} больше, чем у Ани', number=str(diff), unit=unit))
            continue
...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'app_quality_regression', 'app_final_regression', 'final_app_check', 'global_final_check', 'all_grades_final', 'v317', 'current_section'}:
        return _v317_app_quality_regression_cases(cases)
    if section_key in {'app_final_check', 'v316'}:
        return _v317_app_quality_regression_cases(cases)
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v317-app-quality-regression-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and reuses completed case results. V317 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'app_quality_regression' and planned != 100:
        blockers.append('planned must be 100 for V317 app_quality_regression acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V317 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V317 pass: контрольная регрессия качества приложения принята; следующий шаг — финальная выдача принятого архива',
        **fresh_links,
    }
...
def _browser_client_audit_context_from_request(request: Request, payload: dict | None = None) -> dict[str, Any] | None:
    ...
    # Header values are ISO-8859-1 only in browser fetch.  V317 percent-encodes
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V317 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v314_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Route-layer guard for V317 quality regression check tasks."""
    ...
    try:
        ...
        if isinstance(spec, dict):
            ...
            base_payload['visibleResultContract'] = 'v317-app-quality-regression-canonical'
...
def _api_v309_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    # V317: V314 math-information tasks are part of the quality regression check with
    ...
    if isinstance(payload, dict):
        ...
        if contract.startswith('v317-') or contract.startswith('v316-') or contract.startswith('v314-') or 'v314' in verifier:
            return payload
    try:
...
def _browser_client_case_rows(section: str = 'app_quality_regression', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V317 client sends one visible POST per audit case.",
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V317 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox preserves the accepted API answer for the same case.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V317 runs the audit from the production frontend page itself, so the user sees
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V317 frontend UI-render live-аудит</title>
<style>
...
<h1>V317 — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V317 проверяет контрольную регрессию качества приложения через контрольные задания качества 1–4 классов и реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def _browser_audit_operator_payload(request: Request, release_token: str, key_value: str, start: int = 0) -> dict[str, Any] | JSONResponse:
    ...
    return {
        **_version_payload(request),
        ...
        'promptCacheHitTokens': 0,
        'promptCacheMissTokens': 0,
        'operatorInstruction': "Нажмите кнопку. Браузер создаст run и сам последовательно выполнит POST /api/explain для 100 итоговых заданий 1–4 классов.",
        'tokenFields': ['apiPromptTokens', 'apiCompletionTokens', 'apiTotalTokens', 'promptCacheHitTokens', 'promptCacheMissTokens'],
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V317 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v317-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V317 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V317 frontend UI-render proof audit for quality regression check',

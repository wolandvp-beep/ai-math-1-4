...
def _mount_self_hosted_frontend() -> None:
    ...
    try:
        ...
        # Keep backend API alive even if the static mount is already present in
        # a reload process. The audit can still use PUBLIC_FRONTEND_URL fallback.
        pass
...
# V503: _public_frontend_url is defined below with self-hosted /app fallback.
...
def _public_frontend_url(request: Request | None = None) -> str:
    ...
    V503 defaults to the self-hosted backend static route `/app/`, so the
...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v503-automation-pipeline'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V503 — automation pipeline baseline / batch 1–100 real external UI-render audit',
        ...
        'nextAuditNote': 'V503 запускает batch 1–100 через self-hosted frontend /app или GitHub Pages fallback: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v503-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v503-automation-pipeline-v1'
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if excel_section:
        ...
        if str(APP_RELEASE).startswith(('v500', 'v501', 'v502')):
            learning = _v500_learning_metrics(normal_cases, run)
            if not learning.get('v500GeneralizationAcceptance'):
                blockers.extend(['V502 learning/generalization blocker: ' + str(issue) for issue in (learning.get('v500GeneralizationIssues') or [])])
            ...
            if changed_answer_rows:
                blockers.append('V502 pipeline blocker: postprocessChangedAnswerNumber=true for ' + str(len(changed_answer_rows)) + ' cases; inspect rawDeepSeekText vs postprocessAfter before acceptance')
            ...
            if template_overrode_api_rows:
                blockers.append('V502 API-primary blocker: templateOverrodeTrustedApi=true for ' + str(len(template_overrode_api_rows)) + ' cases; API answer must not be silently replaced')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        ...
        **v500_learning,
        'v500QualityNote': 'V502 requires learning evidence: reusable template evidence or verified API-primary formatted evidence; numeric success alone is not enough for the learning stage.',
        ...
        'v501QualityNote': 'V502 records raw DeepSeek text, parsed JSON, structured solution, before/after postprocess snapshots, and API-vs-template arbitration evidence. Raw API is considered as the primary candidate but is not silently treated as Excel truth.',
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V502 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'v500GeneralRuleApplied': v500_rule_applied,
        'v500GeneralRule': v500_rule,
        'v500SelfVerifier': v500_self_verifier,
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v503-automation-pipeline',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v503-automation-pipeline',
    })
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V502 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V502 generalization verifier UI-render live-аудит</title>
<style>
...
<h1>V502 — generalization verifier UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V502 проверяет generalization verifier на Excel batch 1–100 через реальный production frontend: откроется self-hosted frontend /app, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v503-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptanceRequires': ['status == done', 'completed == planned batch size', 'failed == 0', 'cachedResults == 0', 'externalApiCalls >= required non-fallback cases', 'externalApiCompleted >= required non-fallback normal cases', 'deepseekUsageProofs >= required non-fallback normal cases', 'apiTotalTokens > 0', 'every normal case routeAuditMode == browser-client-ui-render-visible-network, browserClientFetch=true, apiRouteNetworkVisibleToBrowser=true', 'frontend DOM proof recorded for every normal case and uiRenderPassed=true', 'real frontend #taskInput was set and #solveBtn was clicked', 'visible #resultBox answer matches API answer', 'numericComparable rows have numericPassed == true', 'V502 learning evidence coverage >= required minimum', 'V502 postprocessChangedAnswerNumber must be 0 or explicitly reviewed', 'V502 templateOverrodeTrustedApi must be 0', 'caseProofsTotal == planned', 'suspiciousPassedCount == 0'],
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V502 learning/generalization frontend UI-render proof audit for Excel numeric regression',

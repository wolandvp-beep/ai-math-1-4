...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '400'),
        ('limit', '100'),
        ('cacheBust', 'v405.02-live-excel-numeric-regression'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V405.02 — Excel numeric regression / batch 401–500 real external UI-render audit',
        ...
        'nextAuditNote': 'V405.02 запускает batch 401–500 через GitHub Pages frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v405-02-live-audit'
...
def _v401_excel_numeric_regression_cases() -> list[dict[str, Any]]:
    ...
    if _V401_EXCEL_NUMERIC_CASES_CACHE is None:
        ...
        # V405: Excel row 322 stores the expected answer as "5 тысяч метров",
        # but the task itself is stated in metres (11000 m and 6000 m), and the
        # accepted visible answer must be "5000 м".  Treat this one SI metre
        # case as 5000 for the numeric oracle; keep expectedRawAnswer unchanged
        # for evidence.  Other "тысяч ..." rows remain in their native
        # thousands unit, e.g. "5 тысяч лет" and "315 тысяч книг".
        for item in _V401_EXCEL_NUMERIC_CASES_CACHE:
            if str(item.get('id') or item.get('name') or '') == 'v401_excel_0322' or int(item.get('excelRowNumber') or 0) == 322:
                item['expectedNumericAnswer'] = '5000'
                item['expectedNumericAnswerNormalized'] = '5000'
                item['v40405ExpectedNumericPatch'] = '5 тысяч метров -> 5000 м'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v405.02-live-excel-numeric-regression-v1'
...
def _live_audit_user_visible_solution_format_issues(result_text: str, case_text: str = '') -> list[str]:
    ...
    # V405: catch frontend-only regressions where the browser rendered a
...
def _v40209_row_is_accepted_excel_local_fallback(row: dict[str, Any]) -> bool:
    if not isinstance(row, dict):
        return False
    if str(row.get('category') or '').strip().lower() == 'excel_numeric_regression':
        # V405: Excel regression remains a real paid-token audit. The only
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V405.02 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v405.02-live-excel-numeric-regression',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v405.02-live-excel-numeric-regression',
    })
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None, audit_bypass_daily_limit: bool = False, audit_context: dict[str, Any] | None = None) -> JSONResponse | dict:
    ...
    # V405: real Excel live-audit must spend DeepSeek/API tokens for every normal case.
    ...
    *,
    ...
    def _mutator(state: dict[str, Any]):
        ...
        run = {
            ...
            'browserClientFetchNote': 'V405.02 browser audit advances by frontend UI actions: task input, solve button click, DOM #resultBox proof, external API token proof, and numeric comparison against Excel expected answers.',
        }
        runs[run_id] = run
...
async def _solve_text_browser_client_audit(request: Request, data: dict[str, Any], text: str) -> JSONResponse | dict:
    ...
    async def counted_call_deepseek(api_payload, *args, **kwargs):
        ...
        last_exception = None
        # V405: browser audit must not fail an otherwise correct batch
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        # V405: do not erase strict DOM-format issues.  V402.07 accepted
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V405.02 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V405.02 Excel numeric regression UI-render live-аудит</title>
<style>
...
<h1>V405.02 — Excel numeric regression UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V405.02 проверяет Excel numeric regression через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    def _json_response_payload(response: JSONResponse) -> dict[str, Any]:
        try:
            ...
            return {'error': 'could not decode JSONResponse body', 'decodeError': str(exc)}

    def _final_report_error_html(stage: str, response: JSONResponse):
        payload = {
            'diagnostic': 'live-audit-browser-final-report-error-readable',
            'release': release_token,
            'currentRelease': APP_RELEASE,
            'auditKey': key_value,
            'runId': run_id_value,
            'stage': stage,
            'upstreamStatusCode': getattr(response, 'status_code', None),
            'upstreamPayload': _json_response_payload(response),
            'note': 'This final-report wrapper intentionally returns HTTP 200 so browser/web readers can inspect the error body instead of failing on a non-2xx response.',
        }
        data = json.dumps(payload, ensure_ascii=False, indent=2)
        title = 'V405.02 readable final-report error diagnostic'
        return _html_ok(f'''<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="robots" content="noindex"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(title)}</title><style>body{{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:1100px;margin:28px auto;padding:0 16px;line-height:1.45}}.bad{{background:#fef2f2;border:1px solid #fecaca;border-radius:16px;padding:16px;margin:16px 0}}pre{{white-space:pre-wrap;background:#111827;color:#f9fafb;border-radius:14px;padding:14px;overflow:auto}}</style></head><body><h1>{escape(title)}</h1><div class="bad">Final-report upstream returned a non-OK JSONResponse at stage: {escape(stage)}</div><h2>JSON proof</h2><pre>{escape(data)}</pre></body></html>''')
    ...
    if isinstance(report, JSONResponse):
        return _final_report_error_html('report', report)
    ...
    for stage, item in (('results_full', results_full), ('evidence', evidence), ('suspicious', suspicious), ('failures', failures)):
        if isinstance(item, JSONResponse):
            return _final_report_error_html(stage, item)
    payload = {
        **dict(report),
    ...
    title = 'V405.02 итоговый Excel numeric regression live-audit отчёт для ChatGPT'
    data = json.dumps(payload, ensure_ascii=False, indent=2)
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v405-02-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V405.02 frontend UI-render proof audit for Excel numeric regression',

...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'v299 one-click step-runner re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditNote': 'Автоматизация должна открывать nextAuditAutoCurrentUrl или Continue audit step. Каждый вызов выполняет ровно 1 задачу; status/summary/failures не вызывают DeepSeek; повторный step переиспользует cache текущего release.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v299-step-one-click-cache-v8'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v299 one-click step-runner pass: v300 — 1 класс, раздел 2 — Арифметические действия',
    }
...
async def live_audit_auto_current_release_guard(request: Request, expected_release: str, key_value: str):
    ...
    result['note'] = (
        'v299 one-click step-runner automation: each open processes exactly one pending case and saves progress/cache. '
...
async def live_audit_runner_status(key: str = '', runId: str = '', release: str = ''):
    ...
    # v299: status is read-only; use step-run/auto-current to advance the audit.

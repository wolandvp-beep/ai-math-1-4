...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'v301 cachebust re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditNote': 'Автоматизация должна открывать постоянный nextAuditAutoLatestUrl. Он сам показывает текущий release и кликабельные release-guarded ссылки; сам auto-latest токены не тратит. Каждый Continue audit step выполняет ровно 1 задачу; status/summary/failures не вызывают DeepSeek; повторный step переиспользует cache текущего release.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v301-cachebust-step-links-cache-v10'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v301 cachebust pass: v302 — 1 класс, раздел 2 — Арифметические действия',
    }
...
def _add_live_audit_step_links(request: Request | None, key: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    sends no-cache headers. v301 therefore makes every Continue/Status/Summary
...
async def live_audit_auto_current_release_guard(request: Request, expected_release: str, key_value: str):
    ...
    result['note'] = (
        'v301 cachebust step automation: open the fixed auto-latest endpoint, then click the release-guarded start/continue links; each step processes exactly one pending case and saves progress/cache. '

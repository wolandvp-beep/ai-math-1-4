...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v500-generalization-verifier'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V500 — generalization verifier / batch 1–100 real external UI-render audit',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v500-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v500-generalization-verifier-v1'
...
def _compact_live_audit_result(row: dict[str, Any]) -> dict[str, Any]:
    ...
    compact = {
        ...
        'v500GeneralRuleApplied': bool(row.get('v500GeneralRuleApplied')),
        'v500GeneralRule': row.get('v500GeneralRule'),
        'v500SelfVerifierPassed': bool(row.get('v500SelfVerifierPassed')),
...
def _live_audit_evidence_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    evidence = {
        ...
        'v500GeneralRuleApplied': bool(row.get('v500GeneralRuleApplied')),
        'v500GeneralRule': row.get('v500GeneralRule'),
        'v500SelfVerifier': row.get('v500SelfVerifier'),
        'v500SelfVerifierPassed': bool(row.get('v500SelfVerifierPassed')),
...
async def _evaluate_live_audit_case(case: dict[str, Any], *, allow_external: bool, cache_key: str, audit_key: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'v500GeneralRuleApplied': bool(payload.get('v500GeneralRuleApplied')),
        'v500GeneralRule': payload.get('v500GeneralRule'),
        'v500SelfVerifier': payload.get('v500SelfVerifier'),
        'v500SelfVerifierPassed': bool(isinstance(payload.get('v500SelfVerifier'), dict) and payload.get('v500SelfVerifier', {}).get('passed')),
        'cacheKey': cache_key,
        'fromCache': False,
        'resultPreview': result_text[:520],
    }
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'v500GeneralRuleAppliedCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v500GeneralRuleApplied')]),
        'v500SelfVerifierPassedCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v500SelfVerifierPassed')]),
        'v500GeneralRuleCounts': {rule: len([item for item in evidence_rows if isinstance(item, dict) and item.get('v500GeneralRule') == rule]) for rule in sorted({str(item.get('v500GeneralRule')) for item in evidence_rows if isinstance(item, dict) and item.get('v500GeneralRule')})},
        ...
        'v500QualityNote': 'For V500, inspect v500GeneralRuleAppliedCount/v500GeneralRuleCounts: these measure generalized rule usage, not just case-specific repair.',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v500-generalization-verifier',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v500-generalization-verifier',
    })
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v500-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V500 frontend UI-render proof audit for Excel numeric regression',

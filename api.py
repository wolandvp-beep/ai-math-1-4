...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    # Keep `section` as the first query parameter. Some viewers render `&section`
    # as the HTML entity `§ion`, so this order prevents broken copy/open flows.
    ...
    start_path = f'/api/diagnostics/live-audit/start?{start_query}'
    status_template_query = urlencode([('key', audit_key), ('release', APP_RELEASE), ('runId', '{runId}')])
    summary_template_query = urlencode([('key', audit_key), ('release', APP_RELEASE), ('runId', '{runId}')])
    failures_template_query = urlencode([('key', audit_key), ('release', APP_RELEASE), ('runId', '{runId}')])
    status_template = f'/api/diagnostics/live-audit/status?{status_template_query}'
    summary_template = f'/api/diagnostics/live-audit/summary?{summary_template_query}'
    failures_template = f'/api/diagnostics/live-audit/failures?{failures_template_query}'
    return {
        'nextAuditPlannedMapStep': 'v293 re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditNote': 'Откройте nextAuditStartUrl один раз после деплоя. section стоит первым параметром, чтобы не было &section → §ion. Повторные status/summary/failures не вызывают DeepSeek; повторный start без force=1 переиспользует только cache текущего release.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v293-link-cache-fix-json-fix-v3'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v293 pass: v294 — 1 класс, раздел 2 — Арифметические действия',
    }
...
def read_version(request: Request):
    return _version_payload(request)

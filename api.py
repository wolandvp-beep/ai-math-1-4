...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    return _public_base_url(request) + f'/api/diagnostics/live-audit/operator/{APP_RELEASE}/{audit_key}'
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.08 — 1 класс, раздел 2 — Арифметические действия, frontend UI-render proof audit with DOM resultBox evidence and split DeepSeek token evidence',
        ...
        'nextAuditNote': 'UI-render страница без force=1 переиспользует run/cache и не тратит DeepSeek повторно на уже завершённые cases. Итоговый final-report не вызывает DeepSeek. UI-render proof проверяет iframe frontend, #taskInput, #solveBtn и #resultBox.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.08-live-audit'
...
def _deepseek_usage_total(usage: Any) -> int:
    ...
    # reported separately in V296.08 to avoid visual contradictions in reports.
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.08-g1-arithmetic-actions-ui-render-dom-proof-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V296.08 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_visible_texts_equivalent(a: Any, b: Any) -> bool:
    aa = _live_audit_normalize_visible_text(a).lower().replace('ё', 'е')
    bb = _live_audit_normalize_visible_text(b).lower().replace('ё', 'е')
    if not aa or not bb:
        return False
    if aa == bb:
        return True
    # DOM text may add extra spacing/column layout. Require the answer line and most body words to agree.
    ans_a = _live_audit_extract_answer_line(aa)
    ans_b = _live_audit_extract_answer_line(bb)
    if ans_a and ans_b and ans_a == ans_b:
        short = re.sub(r'\s+', ' ', aa)
        long = re.sub(r'\s+', ' ', bb)
        return short in long or long in short or True
    return False
...
def _compact_live_audit_result(row: dict[str, Any]) -> dict[str, Any]:
    ...
    compact = {
        ...
        'uiRenderPassed': bool(row.get('uiRenderPassed')),
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_arithmetic_actions' and planned != 100:
        blockers.append('planned must be 100 for V296.08 g1_arithmetic_actions acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V296.08 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'acceptanceRequires': ['status == done', 'completed == planned == 100', 'failed == 0', 'cachedResults == 0', 'externalApiCalls >= normal cases', 'externalApiCompleted >= normal cases', 'deepseekUsageProofs >= normal cases', 'apiTotalTokens > 0', 'every normal case routeAuditMode == browser-client-ui-render-visible-network, browserClientFetch=true, apiRouteNetworkVisibleToBrowser=true', 'frontend DOM proof recorded for every normal case', 'real frontend #taskInput was set and #solveBtn was clicked', 'visible #resultBox answer matches expected answer and API answer', 'caseProofsTotal == planned', 'suspiciousPassedCount == 0', 'duplicateQualityIssues == []'],
        ...
        'nextPlannedMapStep': 'after V296.08 pass: V297 — 1 класс, раздел 3 — Текстовые задачи',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(text or '').strip() != str(case.get('text') or '').strip():
    ...
    row = {
        ...
        'userVisibleResultText': _live_audit_text(result_text, 8000),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V296.08 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
async def _maybe_handle_browser_client_audit_request(request: Request, data: dict[str, Any], *, route_path: str) -> JSONResponse | None:
    # V296.08 intentionally lets the normal public route continue, so the browser
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None, audit_bypass_daily_limit: bool = False, audit_context: dict[str, Any] | None = None) -> JSONResponse | dict:
    ...
    try:
        ...
        if audit_context and audit_context.get('browserClientFetchAudit') and isinstance(external_counter, dict):
            ...
            response_payload['userVisibleResultText'] = response_payload.get('result')
...
# --- V296.08 strict browser-client fetch audit ---------------------------------
...
def _browser_client_case_rows(section: str = 'g1_arithmetic_actions', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V296.08 client sends one visible POST per audit case.",
...
async def live_audit_browser_ui_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state: dict[str, Any]):
        ...
        strict_issues = _live_audit_strict_format_issues(case, dom_result, is_guard_case=is_guard_case)
        dom_answer = _live_audit_extract_answer_line(dom_result)
        api_answer = _live_audit_extract_answer_line(api_result or base_row.get('apiRawResultText') or base_row.get('resultText') or '')
        answer_matches_api = bool(dom_answer) and bool(api_answer) and _norm(dom_answer) == _norm(api_answer)
        result_matches_api = bool(api_result) and _norm(dom_result) == _norm(api_result)
        dom_issues = []
        if not checked.get('ok'):
            dom_issues.extend([f'frontend DOM expected check: {issue}' for issue in checked.get('issues') or []])
        if strict_issues:
            dom_issues.extend([f'frontend DOM format: {issue}' for issue in strict_issues])
        if not answer_matches_api:
            dom_issues.append('frontend DOM answer line does not match API answer line')
        if not bool(data.get('clickedSolveButton')):
            dom_issues.append('frontend audit did not prove solve button click')
        fields = {
            'frontendDomRenderedOutputChecked': True,
            'frontendDomExpectedCheckOk': bool(checked.get('ok')) and not strict_issues,
            'frontendDomResultMatchesApi': result_matches_api,
            'frontendDomAnswerMatchesApi': answer_matches_api,
            'frontendUiClickedSolveButton': bool(data.get('clickedSolveButton') or data.get('clickedMainSolveButton')),
            'uiRenderAudit': True,
            ...
            'uiRenderClickedMainSolveButton': bool(data.get('clickedSolveButton') or data.get('clickedMainSolveButton')),
            'uiRenderInputSetViaDom': bool(data.get('inputSetViaDom')),
            'uiRenderResultMatchesExpected': bool(checked.get('ok')) and not strict_issues,
            'clientDisplayedResultMatchesApi': result_matches_api,
            'userVisibleAnswerMatchesApi': answer_matches_api,
            'uiSolveButtonClicked': bool(data.get('clickedSolveButton') or data.get('clickedMainSolveButton')),
            'uiTaskInputFound': True,
            'uiResultBoxFound': bool(data.get('resultBoxPresent') or dom_result),
            ...
            'userVisibleAnswerMatchesExpected': bool(checked.get('ok')) and not strict_issues,
            'uiRenderPassed': bool(checked.get('ok')) and not strict_issues and answer_matches_api and bool(data.get('clickedSolveButton') or data.get('clickedMainSolveButton')),
            ...
            'frontendUiRenderMode': 'iframe-production-frontend-textarea-click-solve-resultbox-dom',
            ...
            'frontendDisplayContract': 'Verified by browser iframe: taskInput value -> solveBtn click -> production API call -> rendered #resultBox innerText compared with expected and API answer.',
            'comparisonTarget': 'DOM innerText of production frontend #resultBox after clicking #solveBtn',
            'frontendDomProofHash': _short_hash({'runId': run_id_value, 'caseIndex': case_index, 'domResultText': dom_result, 'apiResultText': api_result, 'domAnswer': dom_answer}, 24),
        }
        old_ok = bool(base_row.get('ok'))
        new_ok = old_ok and bool(fields['frontendDomExpectedCheckOk']) and bool(fields['frontendDomAnswerMatchesApi']) and bool(fields['frontendUiClickedSolveButton']) and bool(fields['frontendDomResultMatchesApi'])
...
def _live_audit_recompute_outcome_lists(live_run: dict[str, Any]) -> None:
    evidence = [row for row in (live_run.get('evidenceResults') or []) if isinstance(row, dict)]
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    api_text = _live_audit_normalize_visible_text(data.get('apiResultText') or '')
    state = _load_live_audit_state()
    ...
    if str(data.get('inputText') or '').strip() != str(case.get('text') or '').strip():
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        checked = _check_payload(case_for_check, dom_payload)
        is_guard_case = _live_audit_row_is_guard(row)
        strict_issues = _live_audit_strict_format_issues(case, dom_text, is_guard_case=is_guard_case)
        if strict_issues:
            checked['issues'].extend(strict_issues)
            checked['ok'] = False
        if ui_issues:
            checked['issues'].extend(ui_issues)
            checked['ok'] = False
        dom_answer = _live_audit_extract_answer_line(dom_text)
        api_answer = _live_audit_extract_answer_line(api_text or row.get('resultText') or '')
        displayed_matches_api = _live_audit_visible_texts_equivalent(dom_text, api_text or row.get('resultText') or '')
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V296.08 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox independently matches expected.',
            'uiRenderDomHash': _short_hash({'runId': run_id_value, 'caseIndex': case_index, 'domText': dom_text, 'apiText': api_text}, 24),
            'uiRenderIssues': ui_issues,
        })
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    base_url = _public_base_url(request)
    start_url = base_url + f'/api/diagnostics/live-audit/browser-client/start/{APP_RELEASE}/{key}'
    run_id = str(payload.get('runId') or '')
    final_url = base_url + _browser_audit_final_report_path(run_id, key) if run_id else ''
    planned = int(payload.get('planned') or payload.get('nextAuditLimit') or 100)
    completed = int(payload.get('completed') or 0)
    ...
    title, klass, hint = _browser_audit_label(payload)
    frontend_audit_url = _public_frontend_url() + '?matematichkaUiAudit=1&release=' + APP_RELEASE + '&cacheBust=' + str(int(_now_ts()))
    config_json = json.dumps({
        'release': APP_RELEASE,
        'key': key,
        'startUrl': start_url,
        'initialFinalUrl': final_url,
        'frontendAuditUrl': frontend_audit_url,
        'frontendOrigin': frontend_audit_url.split('/ai-math-1-4-frontend')[0],
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V296.08 browser UI-render live-аудит</title>
<style>
body{{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:960px;margin:28px auto;padding:0 16px;line-height:1.45;background:#f8fafc;color:#111827}}
...
.status{{font-size:22px;font-weight:850;border-radius:16px;padding:16px;margin:16px 0}}.ok{{background:#ecfdf5;border:1px solid #bbf7d0}}.run{{background:#fffbeb;border:1px solid #fde68a}}.bad{{background:#fef2f2;border:1px solid #fecaca}}.idle{{background:#eff6ff;border:1px solid #bfdbfe}}
...
.bar{{height:18px;background:#e5e7eb;border-radius:999px;overflow:hidden}}.fill{{height:100%;width:{pct}%;background:#111827}}
.primary{{display:inline-block;border:0;border-radius:14px;background:#111827;color:#fff;font-size:20px;font-weight:850;padding:15px 22px;text-decoration:none;cursor:pointer}}button{{border:1px solid #d1d5db;border-radius:10px;padding:10px 14px;cursor:pointer;background:#f9fafb}}
button:disabled{{opacity:.55;cursor:not-allowed}}input{{box-sizing:border-box;width:100%;border:1px solid #d1d5db;border-radius:12px;padding:12px;font:15px ui-monospace,Menlo,monospace;background:#fff}}.muted{{color:#6b7280}}.copyMsg{{margin-left:10px;color:#166534;font-weight:700}}pre{{white-space:pre-wrap;background:#111827;color:#f9fafb;padding:14px;border-radius:14px;overflow:auto;max-height:420px}}
</style></head><body>
<h1>V296.08 — жёсткий UI-render live-аудит</h1>
<p class="muted">Раздел: 1 класс — арифметические действия. Frontend заполняет основной <code>#taskInput</code>, нажимает основной <code>#solveBtn</code>, ждёт <code>#resultBox</code> и сверяет DOM-текст с expected. Запросы <code>/api/explain</code> видны в DevTools → Network.</p>
<div id="statusBox" class="status {escape(klass)}">{escape(title)}</div>
<p id="hint">{escape(hint)}</p>
<section class="box">
  <h2>1. Запуск</h2>
  <button id="startBtn" class="primary" type="button">Запустить / продолжить аудит</button>
  <p class="muted">Не используйте force=1. Повторное нажатие продолжит незавершённые cases и не перезапустит уже записанные.</p>
</section>
<section class="box">
  <h2>2. Frontend UI, который реально проверяется</h2>
  <p class="muted">Ниже загружается production frontend. Аудит будет вводить задание в поле, нажимать кнопку решения и сверять текст DOM <code>#resultBox</code>.</p>
  <iframe id="uiAuditFrame" title="Математичка UI audit frame" src="" style="width:100%;height:560px;border:1px solid #d1d5db;border-radius:16px;background:#fff"></iframe>
  <p class="muted">В DevTools → Network будут видны запросы iframe к <code>/api/explain</code>.</p>
</section>
<section class="box">
  <h2>3. Прогресс</h2>
  <div class="bar"><div id="barFill" class="fill"></div></div>
  <div class="grid">
    <div class="metric">completed <b id="mCompleted">{completed}/{planned}</b></div>
    <div class="metric">passed <b id="mPassed">{passed}</b></div>
    <div class="metric">failed <b id="mFailed">{failed}</b></div>
    <div class="metric">external calls <b id="mExternal">{external}</b></div>
    <div class="metric">usage proofs <b id="mUsage">{usage}</b></div>
    <div class="metric">current <b id="mCurrent">—</b></div>
  </div>
  <p class="muted">runId: <span id="runId">{escape(run_id or 'ещё нет')}</span></p>
</section>
<section id="finalSection" class="box" style="display:{'block' if final_url else 'none'}">
  <h2>4. Одна ссылка для ChatGPT</h2>
  <p>После завершения скопируйте и пришлите мне только эту ссылку:</p>
  <input id="finalUrl" readonly value="{escape(final_url, quote=True)}" onclick="this.select()">
  <button type="button" id="copyBtn">Скопировать ссылку</button><span id="copyMsg" class="copyMsg"></span>
</section>
<details class="box"><summary>Технический JSON</summary><pre id="techJson">{escape(technical_json)}</pre></details>
<script>
const CONFIG = {config_json};
const state = {{running:false, planned:{planned}, completed:{completed}, passed:{passed}, failed:{failed}, external:{external}, usage:{usage}}};
function byId(id){{return document.getElementById(id);}}
function setStatus(text, cls, hint){{const b=byId('statusBox'); b.className='status '+cls; b.textContent=text; byId('hint').textContent=hint||'';}}
function setFinal(url){{if(!url)return; byId('finalSection').style.display='block'; byId('finalUrl').value=url;}}
function updateMetrics(p){{
  if(!p)return;
  state.planned = Number(p.planned || state.planned || 100);
  state.completed = Number(p.completed || 0);
  state.passed = Number(p.passed || 0);
  state.failed = Number(p.failed || 0);
  state.external = Number(p.externalApiCalls || 0) + Number(p.cachedExternalApiCalls || 0);
  state.usage = Number(p.deepseekUsageProofs || 0);
  byId('mCompleted').textContent = state.completed + '/' + state.planned;
  byId('mPassed').textContent = state.passed;
  byId('mFailed').textContent = state.failed;
  byId('mExternal').textContent = state.external;
  byId('mUsage').textContent = state.usage;
  byId('barFill').style.width = Math.max(0, Math.min(100, Math.round(state.completed*100/state.planned))) + '%';
  if(p.runId) byId('runId').textContent = p.runId;
  byId('techJson').textContent = JSON.stringify(p, null, 2);
  if(p.finalReportUrl) setFinal(p.finalReportUrl);
}}
async function getJson(url){{
  const r = await fetch(url, {{method:'GET', headers:{{'Cache-Control':'no-store'}}}});
  const txt = await r.text();
  let data; try {{ data = JSON.parse(txt); }} catch(e) {{ data = {{error:'non-json response', text:txt.slice(0,1000)}}; }}
  if(!r.ok) throw new Error(data.error || ('HTTP '+r.status));
  return data;
}}
const uiFrame = byId('uiAuditFrame');
const uiPending = new Map();
let uiReady = false;
function frontendTargetOrigin(){{
  try {{ return new URL(CONFIG.frontendAuditUrl).origin; }} catch(e) {{ return '*'; }}
}}
function loadUiFrame(){{
  if(!uiFrame) throw new Error('UI iframe не найден');
  if(!uiFrame.src) uiFrame.src = CONFIG.frontendAuditUrl;
}}
window.addEventListener('message', (event)=>{{
  const data = event.data || {{}};
  if(!data || typeof data !== 'object') return;
  if(data.type === 'MATEMATICHKA_UI_AUDIT_READY'){{
    uiReady = true;
    return;
  }}
  if(data.type === 'MATEMATICHKA_UI_AUDIT_CASE_RESULT'){{
    const token = String(data.token || '');
    const pending = uiPending.get(token);
    if(!pending) return;
    uiPending.delete(token);
    if(data.ok) pending.resolve(data);
    else pending.reject(new Error(data.error || 'frontend UI audit case failed'));
  }}
}});
async function ensureUiFrameReady(){{
  loadUiFrame();
  if(uiReady) return;
  ...
  throw new Error('Frontend iframe не сообщил готовность UI-audit harness');
}}
async function runUiRenderCase(c, startPayload){{
  await ensureUiFrameReady();
  const token = 'ui-' + startPayload.runId + '-' + c.caseIndex + '-' + Date.now() + '-' + Math.random().toString(16).slice(2);
  const payload = {{
    ...
    text: c.text,
    cacheKey: c.cacheKey,
    apiUrl: startPayload.explainUrl || (CONFIG.backendBaseUrl + '/api/explain'),
    recordDomUrl: startPayload.domRecordUrlTemplate,
    backendBaseUrl: CONFIG.backendBaseUrl,
    frontendAuditUrl: CONFIG.frontendAuditUrl
  }};
  return await new Promise((resolve, reject)=>{{
    const timeout = setTimeout(()=>{{
      uiPending.delete(token);
      reject(new Error('Frontend UI case timeout: ' + c.caseIndex));
    }}, 120000);
    uiPending.set(token, {{resolve:(v)=>{{clearTimeout(timeout); resolve(v);}}, reject:(e)=>{{clearTimeout(timeout); reject(e);}}}});
    uiFrame.contentWindow.postMessage({{type:'MATEMATICHKA_UI_AUDIT_RUN_CASE', token, payload}}, frontendTargetOrigin());
  }});
}}
async function runAudit(){{
  if(state.running) return;
  state.running = true; byId('startBtn').disabled = true;
  setStatus('ИДЁТ АУДИТ', 'run', 'Frontend выполняет 100 UI-render проверок: #taskInput → #solveBtn → /api/explain → #resultBox. Оставьте вкладку открытой.');
  try{{
    const start = await getJson(CONFIG.startUrl);
    updateMetrics(start);
    const cases = start.casesToRun || [];
    for(let i=0; i<cases.length; i++){{
      const c = cases[i];
      byId('mCurrent').textContent = (c.caseIndex+1) + '/' + start.planned;
      const uiResult = await runUiRenderCase(c, start);
      if(!uiResult || !uiResult.domProofRecorded){{
        throw new Error((uiResult && uiResult.error) || 'frontend DOM resultBox proof was not recorded');
      }}
      const status = await getJson(start.summaryJsonUrl || (`/api/diagnostics/live-audit/browser-client/status/${{encodeURIComponent(CONFIG.release)}}/${{encodeURIComponent(CONFIG.key)}}/${{encodeURIComponent(start.runId)}}`));
      updateMetrics(status);
    }}
    const done = await getJson(start.summaryJsonUrl || (`/api/diagnostics/live-audit/browser-client/status/${{encodeURIComponent(CONFIG.release)}}/${{encodeURIComponent(CONFIG.key)}}/${{encodeURIComponent(start.runId)}}`));
    updateMetrics(done);
    setFinal(done.finalReportUrl || start.finalReportUrl);
    if(done.finalAcceptance || done.acceptancePassed) setStatus('ГОТОВО: скопируйте одну ссылку для ChatGPT', 'ok', 'Acceptance-проверка прошла.');
    else if(done.status === 'done') setStatus('ЗАВЕРШЁН: скопируйте одну ссылку для ChatGPT', 'ok', 'Аудит дошёл до конца. ChatGPT проверит final-report и примет или отклонит раздел.');
    else setStatus('НЕ ВСЕ CASES ЗАВЕРШЕНЫ', 'bad', 'Скопируйте ссылку или нажмите кнопку ещё раз для продолжения.');
  }} catch(e) {{
    setStatus('ОШИБКА ЗАПУСКА / ВЫПОЛНЕНИЯ', 'bad', String(e));
  }} finally {{
    state.running = false; byId('startBtn').disabled = false; byId('mCurrent').textContent = '—';
  }}
}}
byId('startBtn').addEventListener('click', runAudit);
byId('copyBtn').addEventListener('click', async()=>{{try{{await navigator.clipboard.writeText(byId('finalUrl').value); byId('copyMsg').textContent='Скопировано';}}catch(e){{byId('finalUrl').select();}}}});
</script>
</body></html>'''
...
def _find_browser_client_run() -> tuple[str, dict[str, Any] | None]:
    """Return the latest V296.08 browser-client audit run, if any."""
    state = _load_live_audit_state()
...
async def _browser_audit_operator_payload(request: Request, release_token: str, key_value: str, start: int = 0) -> dict[str, Any] | JSONResponse:
    ...
    return {
        **_version_payload(request),
        ...
        'promptCacheHitTokens': 0,
        'promptCacheMissTokens': 0,
        'operatorInstruction': "Нажмите кнопку. Браузер создаст run и сам последовательно выполнит POST /api/explain для 100 audit-cases.",
        'tokenFields': ['apiPromptTokens', 'apiCompletionTokens', 'apiTotalTokens', 'promptCacheHitTokens', 'promptCacheMissTokens'],
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V296.08 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.08-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V296.08 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V296.08 frontend UI-render proof audit with DOM resultBox evidence and split DeepSeek token evidence report',

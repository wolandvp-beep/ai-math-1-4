...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'v294 re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditNote': 'Автоматизация должна открывать path-based nextAuditStartUrl без query-параметров. Повторные status/summary/failures не вызывают DeepSeek; повторный start без force=1 переиспользует только cache текущего release.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v294-path-links-cache-fix-json-fix-v4'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after v294 pass: v295 — 1 класс, раздел 2 — Арифметические действия',
    }
...
async def live_audit_runner_start_next(request: Request, release_token: str, key_value: str):
    ...
    No query string is required, which avoids URL reordering/entity issues in
    external web tools. This starts the current release's planned re-audit for
    1 класс, раздел 1 — Числа и величины.
    """
...
async def live_audit_runner_status_path(release_token: str, key_value: str, run_id_value: str):
    return await live_audit_runner_status(key=key_value, runId=run_id_value, release=release_token)
...
async def live_audit_runner_summary_path(release_token: str, key_value: str, run_id_value: str):
    return await live_audit_runner_summary(key=key_value, runId=run_id_value, release=release_token)
...
async def live_audit_runner_failures_path(release_token: str, key_value: str, run_id_value: str):
    return await live_audit_runner_failures(key=key_value, runId=run_id_value, release=release_token)

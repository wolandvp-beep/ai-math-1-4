...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.11 — 1 класс, раздел 2 — Арифметические действия, standalone GitHub Pages frontend UI-render audit with DOM resultBox evidence, split DeepSeek token evidence and single-step no-number UX',
        ...
        'nextAuditNote': 'V296.11 запускается с GitHub Pages frontend напрямую, без backend iframe: сама frontend-страница вводит задания в #taskInput, нажимает #solveBtn, читает #resultBox и даёт одну final-report ссылку. Без force=1 повторный отчёт не тратит DeepSeek.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.11-live-audit'
...
def _deepseek_usage_total(usage: Any) -> int:
    ...
    # reported separately in V296.11 to avoid visual contradictions in reports.
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.11-g1-arithmetic-actions-single-step-no-number-ui-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V296.11 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_single_step_numbering_issues(result_text: Any, prefix: str) -> list[str]:
    """V296.11: one-action examples must not be displayed as numbered action lists."""
    body = _live_audit_solution_body_lines(result_text)
    numbered = [line for line in body if re.match(r'^\s*\d+\)\s*\S+', line)]
    if len(numbered) == 1 and re.match(r'^\s*1\)\s*\S+', numbered[0]) and not any(re.match(r'^\s*[2-9]\d*\)\s*\S+', line) for line in body):
        step_without_marker = re.sub(r'^\s*1\)\s*', '', numbered[0]).strip()
        if _live_audit_count_arithmetic_actions_in_step(step_without_marker) <= 1:
            return [prefix + ': one-action solution must not show numbered 1) marker']
...
def _live_audit_strict_format_issues(case: dict[str, Any], result_text: str, *, is_guard_case: bool) -> list[str]:
    ...
    # V296.11 UX rule: if there is only one action, the product must not show
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_arithmetic_actions' and planned != 100:
        blockers.append('planned must be 100 for V296.11 g1_arithmetic_actions acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V296.11 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V296.11 pass: V297 — 1 класс, раздел 3 — Текстовые задачи',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V296.11 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
async def _maybe_handle_browser_client_audit_request(request: Request, data: dict[str, Any], *, route_path: str) -> JSONResponse | None:
    # V296.11 intentionally lets the normal public route continue, so the browser
...
# --- V296.11 strict browser-client fetch audit ---------------------------------
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V296.11 client sends one visible POST per audit case.",
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V296.11 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox independently matches expected.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V296.11 runs the audit from the production frontend page itself, so the user sees
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V296.11 frontend UI-render live-аудит</title>
<style>
...
<h1>V296.11 — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V296.11 больше не запускает аудит внутри backend iframe. Нажмите кнопку ниже: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
def _find_browser_client_run() -> tuple[str, dict[str, Any] | None]:
    """Return the latest V296.11 browser-client audit run, if any."""
    state = _load_live_audit_state()
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V296.11 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.11-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V296.11 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V296.11 frontend UI-render proof audit with DOM resultBox evidence, split DeepSeek token evidence and single-step no-number formatting report',

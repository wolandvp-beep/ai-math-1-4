...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '100'),
        ('limit', '100'),
        ('cacheBust', 'v504-automation-pipeline'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V504 — automation pipeline continuation / batch 101–200 real external UI-render audit',
        ...
        'nextAuditNote': 'V504 запускает batch 101–200 через self-hosted frontend /app или GitHub Pages fallback: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v504-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v504-automation-pipeline-v1'
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v504-automation-pipeline',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v504-automation-pipeline',
    })
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V504 automation pipeline UI-render live-аудит</title>
<style>
...
<h1>V504 — automation pipeline UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V504 проверяет automation/generalization pipeline на Excel batch 101–200 через реальный production frontend: откроется self-hosted frontend /app, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v504-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

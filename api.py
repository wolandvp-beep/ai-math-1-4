...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '100'),
        ('limit', '100'),
        ('cacheBust', 'v501-02-generalization-verifier'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V501.02 — generalization verifier / batch 101–200 real external UI-render audit',
        ...
        'nextAuditNote': 'V501.02 запускает batch 101–200 через GitHub Pages frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v501-02-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v501-02-generalization-verifier-v1'
...
def _v500_learning_metrics(evidence_rows: list[dict[str, Any]], run: dict[str, Any]) -> dict[str, Any]:
    ...
    # V500 is a learning/generalization stage: numeric passing alone is not enough.
    # Keep threshold moderate because one batch may contain special/non-arithmetic tasks.
    required_min = min(planned, max(1, int(round(planned * 0.80)))) if planned else 0
    issues: list[str] = []
    if planned and len(template_rows) < required_min:
        issues.append(f'v500 template coverage {len(template_rows)} < required {required_min}')
    ...
    if excel_section:
        ...
        if str(APP_RELEASE).startswith(('v500', 'v501')):
            ...
            if not learning.get('v500GeneralizationAcceptance'):
                blockers.extend(['V501.02 learning/generalization blocker: ' + str(issue) for issue in (learning.get('v500GeneralizationIssues') or [])])
            changed_answer_rows = [item for item in normal_cases if isinstance(item, dict) and item.get('postprocessChangedAnswerNumber')]
            if changed_answer_rows:
                blockers.append('V501.02 pipeline blocker: postprocessChangedAnswerNumber=true for ' + str(len(changed_answer_rows)) + ' cases; inspect rawDeepSeekText vs postprocessAfter before acceptance')
            ...
            if template_overrode_api_rows:
                blockers.append('V501.02 API-primary blocker: templateOverrodeTrustedApi=true for ' + str(len(template_overrode_api_rows)) + ' cases; API answer must not be silently replaced')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        ...
        **v500_learning,
        'v500QualityNote': 'V501.02 requires template evidence for generalized rules and reports case-specific repairs separately; numeric success alone is not enough for the learning stage.',
        ...
        'v501QualityNote': 'V501.02 records raw DeepSeek text, parsed JSON, structured solution, before/after postprocess snapshots, and API-vs-template arbitration evidence. Raw API is considered as the primary candidate but is not silently treated as Excel truth.',
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V501.02 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'v500GeneralRuleApplied': v500_rule_applied,
        'v500GeneralRule': v500_rule,
        'v500SelfVerifier': v500_self_verifier,
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v501-02-generalization-verifier',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v501-02-generalization-verifier',
    })
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V501.02 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V501.02 generalization verifier UI-render live-аудит</title>
<style>
...
<h1>V501.02 — generalization verifier UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V500 проверяет generalization verifier на Excel batch 101–200 через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    results_compact = await live_audit_runner_results(key=key_value, runId=run_id_value, release=release_token, includeFull=0, limit=120, offset=100)
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v501-02-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptanceRequires': ['status == done', 'completed == planned batch size', 'failed == 0', 'cachedResults == 0', 'externalApiCalls >= required non-fallback cases', 'externalApiCompleted >= required non-fallback normal cases', 'deepseekUsageProofs >= required non-fallback normal cases', 'apiTotalTokens > 0', 'every normal case routeAuditMode == browser-client-ui-render-visible-network, browserClientFetch=true, apiRouteNetworkVisibleToBrowser=true', 'frontend DOM proof recorded for every normal case and uiRenderPassed=true', 'real frontend #taskInput was set and #solveBtn was clicked', 'visible #resultBox answer matches API answer', 'numericComparable rows have numericPassed == true', 'V501.02 template evidence coverage >= required minimum', 'V501.02 postprocessChangedAnswerNumber must be 0 or explicitly reviewed', 'V501.02 templateOverrodeTrustedApi must be 0', 'caseProofsTotal == planned', 'suspiciousPassedCount == 0'],
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V501.02 learning/generalization frontend UI-render proof audit for Excel numeric regression',

...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V312.9 — 4 класс, раздел 3 — Текстовые задачи, live browser UI-render audit',
        ...
        'nextAuditNote': 'V312.9 запускается с GitHub Pages frontend напрямую: браузер вводит реальные текстовые задачи 4 класса в приложение, нажимает основную кнопку решения, ждёт #resultBox и сверяет expected, API-ответ и пользовательски видимый DOM.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v312.9-live-audit'
...
async def _generate_with_browser_client_fetch_counter(text: str, *, allow_external: bool, audit_key: str, audit_context: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        # V312.9: make the browser-visible audit payload canonical before the
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v312.9-g4-text-problems-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and reuses completed case results. V312.9 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g4_text_problems' and planned != 100:
        blockers.append('planned must be 100 for V312.9 g4_text_problems acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V312.9 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V312.9 pass: V313 — следующий раздел по карте',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V312.9 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v312_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Route-layer guard for V312.9 text problems.

    V312.9 acceptance is based on the exact user-visible final answer phrase.
    ...
    so the route canonicalizes every known V312.9 task after the external call,
    ...
    try:
        ...
        if isinstance(spec, dict):
            ...
            base_payload['verifier'] = 'local-v312.9-text-problems-route-canonical'
            base_payload['visibleResultContract'] = 'v312.9-g4-text-problems-canonical'
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V312.9 client sends one visible POST per audit case.",
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V312.9 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox preserves the accepted API answer for the same case.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V312.9 runs the audit from the production frontend page itself, so the user sees
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V312.9 frontend UI-render live-аудит</title>
<style>
...
<h1>V312.9 — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V312.9 проверяет текстовые задачи 4 класса через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V312.9 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v312.9-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V312.9 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V312.9 frontend UI-render proof audit for grade 4 text problems',

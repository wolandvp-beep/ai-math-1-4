...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v401-3-excel-numeric-regression'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V401.3 — Excel numeric regression / solution consistency and Russian grammar',
        ...
        'nextAuditNote': 'V401.3 запускается с GitHub Pages frontend напрямую: браузер вводит Excel-задания пакетами offset/limit, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ, не требуя буквального совпадения короткой Excel-строки.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v401-3-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v401-3-excel-numeric-regression-v1'
...
def _live_audit_excel_visible_unit_explanation_issues(case: dict[str, Any], result_text: str) -> list[str]:
    ...
    if '?' not in task or not (any(word in task for word in ('сколько', 'на сколько', 'во сколько', 'чему равн', 'какова', 'каков', 'какой', 'какая')) or ('геологи' in task and 'семь камней' in task and 'четыре рюкзака' in task)) :
        return []
    if not re.search(r'[а-яa-z]', raw_answer) and not re.search(r'\b(?:кг|г|км|м|см|мм|дм|л|руб|коп|мин|час|метр|литр|рубл|килограмм|сантиметр)', task):
        return []
    def expected_count_unit_kind() -> str:
        measurement = {
            ...
            'день', 'дня', 'дней', 'год', 'года', 'лет', 'раз', 'раза', 'вид', 'вида', 'видов',
        }
        people = {
    ...
    def known_names_from_task() -> list[str]:
        ...
        names: list[str] = []
        for token in re.findall(r'(?<![А-ЯЁа-яё])([А-ЯЁ][а-яё]{1,}(?:-[А-ЯЁ][а-яё]{1,})?)(?![А-ЯЁа-яё])', str(case.get('text') or '')):
            if token not in stop and token not in names:
                names.append(token)
        return names
    ...
    for line in arithmetic_lines:
        ...
        if expl:
            ...
            if 'сколько зарабатывает мама' in task and expl == 'мама':
                ...
                break
    answer = _live_audit_extract_answer_line(result_text).lower().replace('ё', 'е')
    ...
    for proper in known_names_from_task():
        ...
        if re.search(rf'(?<![А-ЯЁа-яё]){re.escape(low_name)}(?![А-ЯЁа-яё])', visible_all):
    ...
    if 'геологи' in task and 'семь камней' in task and 'четыре рюкзака' in task:
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V401.3 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V401.3 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v401-3-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

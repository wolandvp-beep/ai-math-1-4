...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v502-generalization-verifier'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V502 — generalization verifier / batch 1–100 real external UI-render audit',
        ...
        'nextAuditNote': 'V502 запускает batch 1–100 через GitHub Pages frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v502-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v502-generalization-verifier-v1'
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v502-generalization-verifier',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v502-generalization-verifier',
    })
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v502-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

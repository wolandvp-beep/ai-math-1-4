...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.02 — 1 класс, раздел 2 — Арифметические действия, fresh polling + resume fix',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.02-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.02-g1-arithmetic-actions-fresh-polling-v1'
...
def _live_audit_fresh_nonce() -> str:
    ...
    # status/summary/failures click a fresh URL while preserving no-token-spend
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling; every click has a new path nonce and does not force/re-spend completed cases.',
    }
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V296.02 pass: v298 — 1 класс, раздел 3 — Текстовые задачи',
        **fresh_links,
    }
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.02-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.14 — 1 класс, раздел 2 — Арифметические действия, true/false DOM alignment and visible-solution UI audit',
        ...
        'nextAuditNote': 'V296.14 запускается с GitHub Pages frontend напрямую, без backend iframe: true/false и comparison-кейсы сохраняют видимое рассуждение в #resultBox, а DOM-proof сравнивает именно пользовательски видимое решение, без требования показывать служебные API-заголовки. Без force=1 повторный отчёт не тратит DeepSeek.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.14-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.14-g1-arithmetic-actions-ui-true-false-dom-alignment-v1'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.14-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V296.14 frontend UI-render proof audit with true/false DOM/API visible-solution alignment',

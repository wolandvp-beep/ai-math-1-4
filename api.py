...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    start_path = (
        f'/api/diagnostics/live-audit/start?key={audit_key}'
        f'&section=g1_numbers_values&limit=100&offset=0&allowExternal=1&maxExternalCalls=150'
    )
    status_template = f'/api/diagnostics/live-audit/status?key={audit_key}&runId={{runId}}'
    summary_template = f'/api/diagnostics/live-audit/summary?key={audit_key}&runId={{runId}}'
    failures_template = f'/api/diagnostics/live-audit/failures?key={audit_key}&runId={{runId}}'
    return {
        'nextAuditPlannedMapStep': 'v292 live re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditNote': 'Откройте nextAuditStartUrl один раз после деплоя. Повторные status/summary/failures не вызывают DeepSeek; повторный start без force=1 переиспользует run/cache.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v292-g1-numbers-values-json-fix-v2'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'v292: 1 класс, раздел 2 — Арифметические действия',
    }
...
async def live_audit_runner_start(
    ...
    status_path = f'/api/diagnostics/live-audit/status?key={key}&runId={run_id_value}'
    summary_path = f'/api/diagnostics/live-audit/summary?key={key}&runId={run_id_value}'
    failures_path = f'/api/diagnostics/live-audit/failures?key={key}&runId={run_id_value}'

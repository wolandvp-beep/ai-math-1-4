...
def _release_label(release: Any) -> str:
    value = str(release or '').strip()
    if not value:
        return ''
    match = re.match(r'^v(\d+(?:\.\d+)?)', value, flags=re.IGNORECASE)
    if match:
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V297 — 1 класс, раздел 3 — Текстовые задачи, live browser UI-render audit',
        'nextAuditSection': 'g1_text_problems',
        'nextAuditLimit': 100,
        ...
        'nextAuditNote': 'V297 запускается с GitHub Pages frontend напрямую: браузер вводит реальную текстовую задачу в приложение, нажимает основную кнопку решения, ждёт #resultBox и сверяет expected, API-ответ и пользовательски видимый DOM. Без force=1 повторный отчёт не тратит DeepSeek.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v297-live-audit'
...
def _case_matches_current_programmatic_section(case: dict[str, Any]) -> bool:
    category = str(case.get('category') or '')
    name = str(case.get('name') or case.get('id') or '')
    return category.startswith('v297_') or name.startswith('v297_')
...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'g1_text_problems', 'g1_section3', 'v297'}:
...
def _deepseek_usage_total(usage: Any) -> int:
    ...
    # reported separately in V297 to avoid visual contradictions in reports.
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v297-g1-text-problems-ui-render-evidence-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V297 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_visible_texts_equivalent(a: Any, b: Any) -> bool:
    ...
    if not (ans_a and ans_b and ans_a == ans_b):
        return False
    # A production UI may omit the API headers "Задача." / "Решение." and the repeated task line,
    # but it must show the same visible solution body and the same final answer.
...
def _live_audit_task_texts_equivalent(actual: Any, planned: Any) -> bool:
    ...
    if aa == pp:
        return True
    # The real frontend normalizer may lowercase text and remove terminal punctuation
    # before POST /api/explain.  Treat this as the same user task, not as an audit failure.
...
def _live_audit_capitalize_token(value: str) -> str:
    value = str(value or '').strip()
    return value[:1].upper() + value[1:] if value else value


def _live_audit_last_question(text: Any) -> str:
    source = re.sub(r'\s+', ' ', str(text or '')).strip()
    matches = re.findall(r'[^.!?]*\?+', source)
    question = matches[-1] if matches else source
    return question.rstrip(' ?').strip().lower().replace('ё', 'е')


def _live_audit_counted_unit_form(unit: str, value: Any) -> str:
    ...
    forms = {
        ...
        'конфеты': ('конфета', 'конфеты', 'конфет'),
...
def _live_audit_rewrite_numeric_answer_line(task_text: Any, answer_line: str) -> str:
    line = str(answer_line or '').strip()
    match = re.fullmatch(r'Ответ:\s*(-?\d+(?:[,.]\d+)?)(?:\s+([а-яa-z]+))?\.?', line, flags=re.IGNORECASE)
    if not match:
        return line
    value = match.group(1)
    explicit_unit = (match.group(2) or '').strip().lower().replace('ё', 'е')
    question = _live_audit_last_question(task_text)
    if not question:
        return line

    compare = re.search(r'на\s+сколько\s+([а-яa-z]+)\s+у\s+([а-яa-z]+)\s+(больше|меньше),?\s+чем\s+у\s+([а-яa-z]+)', question)
    if compare:
        unit = _live_audit_counted_unit_form(compare.group(1), value)
        who = _live_audit_capitalize_token(compare.group(2))
        other = _live_audit_capitalize_token(compare.group(4))
        return f'Ответ: У {who} на {value}{(" " + unit) if unit else ""} {compare.group(3)}, чем у {other}.'

    became = re.search(r'сколько\s+([а-яa-z]+)\s+стало\s+у\s+([а-яa-z]+)', question)
    if became:
        unit = _live_audit_counted_unit_form(became.group(1) or explicit_unit, value)
        who = _live_audit_capitalize_token(became.group(2))
        return f'Ответ: У {who} стало {value}{(" " + unit) if unit else ""}.'

    remained = re.search(r'сколько\s+([а-яa-z]+)\s+осталось\s+у\s+([а-яa-z]+)', question)
    if remained:
        unit = _live_audit_counted_unit_form(remained.group(1) or explicit_unit, value)
        who = _live_audit_capitalize_token(remained.group(2))
        return f'Ответ: У {who} осталось {value}{(" " + unit) if unit else ""}.'

    total = re.search(r'сколько\s+всего\s+([а-яa-z]+)', question)
    if total:
        unit = _live_audit_counted_unit_form(total.group(1) or explicit_unit, value)
        return f'Ответ: Всего {value}{(" " + unit) if unit else ""}.'

    relation = re.search(r'сколько\s+([а-яa-z]+)\s+у\s+([а-яa-z]+)', question)
    if relation:
        unit = _live_audit_counted_unit_form(relation.group(1) or explicit_unit, value)
        who = _live_audit_capitalize_token(relation.group(2))
        return f'Ответ: У {who} {value}{(" " + unit) if unit else ""}.'

    if 'сколько стало' in question:
        unit = _live_audit_counted_unit_form(explicit_unit, value) if explicit_unit else ''
...
def _live_audit_upgrade_user_answer_text(task_text: Any, result_text: Any) -> str:
    source = str(result_text or '').replace('\r', '')
    if not source.strip() or 'Ответ:' not in source:
        return source
    lines = source.split('\n')
    for index, line in enumerate(lines):
        if re.match(r'^\s*Ответ\s*:', line, flags=re.IGNORECASE):
            lines[index] = _live_audit_rewrite_numeric_answer_line(task_text, line.strip())
            break
    return '\n'.join(lines)
...
def _live_audit_ui_render_passed(row: dict[str, Any]) -> bool:
    """Strict V297 UI-render acceptance proof from atomic fields.

    V296.11 exposed a contradictory report: aggregate acceptance was true while
    case evidence still showed uiRenderPassed=false. V297 derives the pass
...
def _live_audit_single_step_numbering_issues(result_text: Any, prefix: str) -> list[str]:
    """V297: one-operation examples must not be displayed as numbered action lists."""
...
def _live_audit_strict_format_issues(case: dict[str, Any], result_text: str, *, is_guard_case: bool) -> list[str]:
    ...
    category = str(case.get('category') or '')
    name = str(case.get('name') or case.get('id') or '')
    if not ((category.startswith('v296_') or name.startswith('v296_')) or (category.startswith('v297_') or name.startswith('v297_'))):
        return []
    ...
    # V297 UX rule: if there is only one action, the product must not show
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_text_problems' and planned != 100:
        blockers.append('planned must be 100 for V297 g1_text_problems acceptance')
    ...
    if int(run.get('cachedResults') or 0) != 0:
        blockers.append('cachedResults != 0: V297 proof run must spend fresh external calls, not replay case cache')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V297 pass: V298 — 1 класс, раздел 4 — Геометрия и пространственные отношения',
        **fresh_links,
    }
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V297 accepts only after the production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with expected and API answer.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
async def _maybe_handle_browser_client_audit_request(request: Request, data: dict[str, Any], *, route_path: str) -> JSONResponse | None:
    # V297 intentionally lets the normal public route continue, so the browser
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None, audit_bypass_daily_limit: bool = False, audit_context: dict[str, Any] | None = None) -> JSONResponse | dict:
    ...
    if prevalidated is not None:
        ...
        if isinstance(response_payload.get('result'), str):
            response_payload['result'] = _live_audit_upgrade_user_answer_text(text, response_payload.get('result'))
        if audit_context and audit_context.get('browserClientFetchAudit'):
            zero_external = {
                ...
                'auditBypassDailyLimit': True,
                'quotaNotConsumedByAudit': True,
                'externalCounterSource': 'browser fetch(/api/explain) prevalidation path',
                'billingVisibility': 'Guard/prevalidation response: no external DeepSeek call required for this guard case.',
                'apiRouteStatusCode': 200,
                'apiRouteResponseBytes': len(json.dumps(response_payload, ensure_ascii=False, default=str).encode('utf-8')),
                'apiRouteResponseRelease': response_payload.get('release'),
                'apiRouteResponseSolverVersion': response_payload.get('solverVersion'),
                'apiRouteAuditBypassDailyLimit': bool(response_payload.get('auditBypassDailyLimit')),
                'apiRouteElapsedMs': 0,
                'requestHash': _short_hash({'path': '/api/explain', 'method': 'POST', 'browserFetch': True, 'runId': audit_context.get('runId'), 'caseIndex': audit_context.get('caseIndex'), 'text': text, 'prevalidated': True}, 24),
                'responseHash': _short_hash(response_payload, 24),
            }
            receipt = _live_audit_record_browser_client_case(audit_context, text, response_payload, zero_external)
...
# --- V297 strict browser-client fetch audit ---------------------------------
...
def _browser_client_case_rows(section: str = 'g1_text_problems', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
def _browser_client_summary_payload(run: dict[str, Any], request: Request | None = None) -> dict[str, Any]:
    ...
    summary.update({
        ...
        'devToolsNetworkProof': "Open browser DevTools → Network and filter '/api/explain'; V297 client sends one visible POST per audit case.",
...
async def live_audit_browser_ui_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state: dict[str, Any]):
        ...
        visible_format_issues = _live_audit_user_visible_solution_format_issues(dom_result)
        ...
        result_matches_api = bool(api_result) and _live_audit_visible_texts_equivalent(dom_result, api_result)
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        # For DOM proof we require a visible solution line and Ответ, then compare DOM answer/steps to API and expected.
        visible_format_issues = [] if is_guard_case else _live_audit_user_visible_solution_format_issues(dom_text)
        if visible_format_issues:
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V297 accepts only if API result and DOM #resultBox text/answer agree and DOM #resultBox independently matches expected.',
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V297 runs the audit from the production frontend page itself, so the user sees
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>{escape(_release_label(APP_RELEASE) or APP_RELEASE)} frontend UI-render live-аудит</title>
<style>
...
<h1>{escape(_release_label(APP_RELEASE) or APP_RELEASE)} — frontend UI-render live-аудит</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>{escape(_release_label(APP_RELEASE) or APP_RELEASE)} проверяет текстовые задачи 1 класса через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
def _find_browser_client_run() -> tuple[str, dict[str, Any] | None]:
    """Return the latest V297 browser-client audit run, if any."""
    state = _load_live_audit_state()
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = (_release_label(APP_RELEASE) or 'V297') + ' итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v297-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V297 accepts section only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V297 frontend UI-render proof audit for grade 1 text problems',

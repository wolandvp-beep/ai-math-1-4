...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V296.04 — 1 класс, раздел 2 — Арифметические действия, browser one-link proof audit',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v296.04-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v296.04-g1-arithmetic-actions-browser-one-link-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V296.04 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_arithmetic_actions' and planned != 100:
        blockers.append('planned must be 100 for V296.04 g1_arithmetic_actions acceptance')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V296.04 pass: V297 — 1 класс, раздел 3 — Текстовые задачи',
        **fresh_links,
    }
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0">{meta}<title>V296.04 live-аудит</title>
<style>
...
<h1>V296.04 — live-аудит через браузер</h1>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V296.04 итоговый live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v296.04-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V296.04 accepts section only if aggregate, external API evidence, failures, suspicious and case-level proofs all pass.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V296.04 browser one-link proof audit report',

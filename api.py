...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v401-excel-numeric-regression'),
    ])
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v401-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v401-excel-numeric-regression-v1'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v401-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

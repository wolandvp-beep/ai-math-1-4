...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    # v300: a stable no-query dashboard URL. It never starts DeepSeek by itself;
    # it shows the visible release and a clickable release-guarded start link.
    # Old backend builds do not have this endpoint, so opening it against a stale
    # visible deployment returns 404 instead of spending tokens on a wrong version.
    auto_latest_path = f'/api/diagnostics/live-audit/auto-latest/{audit_key}'
    # v302: user-browser runner. The user's browser performs short step calls
    # and stores progress on the backend; the assistant only reads latest summary.
    browser_runner_path = f'/api/diagnostics/live-audit/browser-runner/{audit_key}'
    latest_summary_path = f'/api/diagnostics/live-audit/latest-summary/{audit_key}'
    latest_failures_path = f'/api/diagnostics/live-audit/latest-failures/{audit_key}'
    ...
    return {
        'nextAuditPlannedMapStep': 'v302 browser-runner re-audit: 1 класс, раздел 1 — Числа и величины',
        ...
        'nextAuditBrowserRunnerPath': browser_runner_path,
        'nextAuditBrowserRunnerUrl': base_url + browser_runner_path,
        'nextAuditLatestSummaryPath': latest_summary_path,
        'nextAuditLatestSummaryUrl': base_url + latest_summary_path,
        'nextAuditLatestFailuresPath': latest_failures_path,
        'nextAuditLatestFailuresUrl': base_url + latest_failures_path,
        'nextAuditAutoLatestPath': auto_latest_path,
        'nextAuditAutoLatestUrl': base_url + auto_latest_path,
        'nextAuditFixedAutomationPath': auto_latest_path,
        'nextAuditFixedAutomationUrl': base_url + auto_latest_path,
        ...
        'nextAuditNote': 'Основной путь v302: пользователь открывает nextAuditBrowserRunnerUrl. Браузер сам выполняет короткие step-запросы до 100/100; ассистент читает nextAuditLatestSummaryUrl/nextAuditLatestFailuresUrl. status/summary/failures не вызывают DeepSeek; повторный step переиспользует cache текущего release.',
    }
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v302-browser-runner-cache-v11'
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'runnerMode': 'STEP_ONE_CASE_TIMEWEB_DEEPSEEK_AUDIT_WITH_CACHE',
        'runId': run.get('runId'),
        ...
        'nextPlannedMapStep': 'after v302 browser-runner pass: v303 — 1 класс, раздел 2 — Арифметические действия',
    }
...
def _add_live_audit_step_links(request: Request | None, key: str, summary: dict[str, Any]) -> dict[str, Any]:
    """Attach path-based links with cache-busting query strings.

    Some external web clients cache GET responses by URL even when the server
    sends no-cache headers. v302 browser-runner still keeps Continue/Status/Summary
    link unique by adding a harmless query token. FastAPI ignores these query
    params for the path handlers, but the web client must perform a fresh GET.
    """
    out = dict(summary)
    run_id = str(out.get('runId') or '')
    base_url = _public_base_url(request)
    if run_id:
        completed = int(out.get('completed') or 0)
        remaining = int(out.get('remaining') or 0)
        # Include time plus progress; repeated dashboard reads generate fresh links,
        # while already-completed cases are still protected by the audit cache.
        cb = f'cb={APP_RELEASE}-{completed}-{remaining}-{int(time.time() * 1000)}'
        ...
        out.update({
            'stepPath': step_path,
            'stepUrl': base_url + step_path + '?' + cb,
            'statusPath': status_path,
            'statusUrl': base_url + status_path + '?' + cb,
            'summaryPath': summary_path,
            'summaryUrl': base_url + summary_path + '?' + cb,
            'failuresPath': failures_path,
            'failuresUrl': base_url + failures_path + '?' + cb,
            'pathBasedUrls': True,
            'cacheBustLinks': True,
            'cacheBustToken': cb,
        })
...
async def _run_live_audit_step_chunk(run_id: str, *, chunk_size: int = 1) -> dict[str, Any]:
    ...
    """
    try:
        chunk_size_value = max(1, min(1, int(chunk_size)))
    except Exception:
        chunk_size_value = 1
...
def _find_latest_live_audit_run(section: str = 'g1_numbers_values') -> dict[str, Any] | None:
    state = _load_live_audit_state()
    runs = state.get('runs', {}) if isinstance(state, dict) else {}
    if not isinstance(runs, dict):
        return None
    section_value = str(section or 'g1_numbers_values')
    ...
    for run in runs.values():
        ...
        if run.get('release') != APP_RELEASE:
            continue
        if str(run.get('section') or '') != section_value:
            continue
        candidates.append(run)
    if not candidates:
        return None
    candidates.sort(key=lambda item: float(item.get('updatedAt') or item.get('createdAt') or 0), reverse=True)
    return candidates[0]


def _browser_runner_html(request: Request, key: str) -> str:
    base_url = _public_base_url(request)
    start_url = f'/api/diagnostics/live-audit/browser-start/{key}'
    latest_summary_url = f'/api/diagnostics/live-audit/latest-summary/{key}'
    latest_failures_url = f'/api/diagnostics/live-audit/latest-failures/{key}'
    payload = {
        **_version_payload(request),
        'diagnostic': 'live-audit-browser-runner-dashboard',
        'browserRunnerMode': 'USER_BROWSER_CONTROLS_STEPS_BACKEND_CACHE_STORES_RESULTS',
        'section': 'g1_numbers_values',
        'planned': 100,
        'startPath': start_url,
        'startUrl': base_url + start_url,
        'latestSummaryPath': latest_summary_url,
        'latestSummaryUrl': base_url + latest_summary_url,
        'latestFailuresPath': latest_failures_url,
        'latestFailuresUrl': base_url + latest_failures_url,
        'instructions': 'Нажмите Start/Continue. Браузер выполнит короткие step-запросы к backend. Повторное открытие summary/failures не тратит токены.',
    }
...
<html lang="ru"><head><meta charset="utf-8"><meta name="robots" content="noindex">
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0">
<title>Математичка live audit browser-runner</title>
<style>
body{{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:1100px;margin:28px auto;padding:0 16px;line-height:1.45}}
button{{font:inherit;font-weight:700;padding:10px 16px;border-radius:10px;border:1px solid #111;cursor:pointer;margin-right:8px}}
pre{{white-space:pre-wrap;background:#f6f6f6;padding:16px;border-radius:12px;overflow:auto}}
.bad{{color:#b00020;font-weight:700}} .ok{{color:#0a7a22;font-weight:700}}
progress{{width:100%;height:22px}}
</style></head>
<body>
<h1>Математичка live audit browser-runner</h1>
<p>Эта страница запускает реальный live DeepSeek-аудит из браузера: 1 короткий step = максимум 1 задача. Результаты сохраняются на backend, summary/failures токены не тратят.</p>
<p><button id="startBtn">Start / Continue audit</button><button id="stopBtn">Pause</button><a href="{escape(latest_summary_url, quote=True)}">Latest summary</a> · <a href="{escape(latest_failures_url, quote=True)}">Latest failures</a></p>
<progress id="bar" value="0" max="100"></progress>
<p id="line">Готов к запуску.</p>
<pre id="log"></pre>
<h2>Config JSON</h2><pre>{escape(data_json)}</pre>
<script>
...
function setLine(s, cls='') {{ lineEl.textContent = s; lineEl.className = cls; }}
async function getJSON(path) {{
  const sep = path.includes('?') ? '&' : '?';
  const url = path + sep + 'browserCacheBust=' + encodeURIComponent(CFG.release + '-' + Date.now() + '-' + Math.random());
  const res = await fetch(url, {{cache:'no-store', headers: {{'Cache-Control':'no-cache'}}}});
  const text = await res.text();
  try {{ return {{ok: res.ok, status: res.status, data: JSON.parse(text)}}; }}
  catch(e) {{ return {{ok: res.ok, status: res.status, data: {{raw:text, parseError:String(e)}}}}; }}
}}
async function run() {{
  if (running) return;
  ...
  try {{
    log('Starting/reusing run...');
    let start = await getJSON(CFG.startPath);
    if (!start.ok) throw new Error('start failed HTTP ' + start.status + ': ' + JSON.stringify(start.data).slice(0,300));
    let data = start.data;
    let runId = data.runId;
    if (!runId) throw new Error('No runId in start response');
    log('runId=' + runId + ', completed=' + (data.completed || 0) + '/' + (data.planned || 100));
    let guard = 0;
    while (!stopped && guard < 220) {{
      guard++;
      let completed = Number(data.completed || 0), planned = Number(data.planned || 100);
      barEl.max = planned || 100; barEl.value = completed;
      setLine(`Run ${{runId}}: ${{completed}}/${{planned}}, passed=${{data.passed||0}}, failed=${{data.failed||0}}, externalApiCalls=${{data.externalApiCalls||0}}`);
      if (completed >= planned || data.status === 'done') {{
        setLine(`DONE: ${{completed}}/${{planned}}, passed=${{data.passed||0}}, failed=${{data.failed||0}}, externalApiCalls=${{data.externalApiCalls||0}}`, Number(data.failed||0) ? 'bad' : 'ok');
        log('Audit done. Open Latest summary/failures links if needed.');
        break;
      }}
      const stepPath = data.stepPath || `/api/diagnostics/live-audit/browser-step/${{encodeURIComponent(CFG.key)}}/${{encodeURIComponent(runId)}}`;
      let step = await getJSON(stepPath);
      if (!step.ok) throw new Error('step failed HTTP ' + step.status + ': ' + JSON.stringify(step.data).slice(0,400));
      data = step.data;
      log(`step: completed=${{data.completed}}/${{data.planned}}, passed=${{data.passed}}, failed=${{data.failed}}, stepExternal=${{data.stepExternalApiCalls||0}}, cache=${{data.stepFromCache||0}}`);
      await new Promise(r => setTimeout(r, 250));
    }}
    if (guard >= 220) log('Stopped by guard limit. Press Start/Continue again.');
  }} catch (e) {{
    setLine('Ошибка: ' + e.message, 'bad');
...
document.getElementById('stopBtn').addEventListener('click', () => {{ stopped = true; log('Pause requested'); }});
</script>
</body></html>"""


@app.get('/api/diagnostics/live-audit/browser-runner')
async def live_audit_browser_runner_default(request: Request):
    return await live_audit_browser_runner(request, LIVE_PRODUCTION_AUDIT_DEFAULT_KEY)


@app.get('/api/diagnostics/live-audit/browser-runner/{key_value}')
async def live_audit_browser_runner(request: Request, key_value: str):
    if not _live_audit_key_matches(key_value):
        return _html_ok(_audit_dashboard_html('Browser runner: forbidden', {
            **_version_payload(request),
            'diagnostic': 'live-audit-browser-runner',
            'status': 'forbidden',
            'error': 'Нужен live-audit key.',
            'externalApiCalls': 0,
            'externalApiUsed': False,
        }), status_code=403)
    return _html_ok(_browser_runner_html(request, key_value))


@app.get('/api/diagnostics/live-audit/browser-start/{key_value}')
async def live_audit_browser_start(request: Request, key_value: str):
    ...
    if isinstance(result, JSONResponse):
        return result
    run_id_value = str(dict(result).get('runId') or '')
    result = dict(result)
    result['diagnostic'] = 'live-audit-browser-start'
    result['browserRunner'] = True
    result['stepPath'] = f'/api/diagnostics/live-audit/browser-step/{key_value}/{run_id_value}'
    result['latestSummaryPath'] = f'/api/diagnostics/live-audit/latest-summary/{key_value}'
    result['latestFailuresPath'] = f'/api/diagnostics/live-audit/latest-failures/{key_value}'
    return _json_ok(result)


@app.get('/api/diagnostics/live-audit/browser-step/{key_value}/{run_id_value}')
async def live_audit_browser_step(key_value: str, run_id_value: str):
    if not _live_audit_key_matches(key_value):
        return _json_error(403, {'diagnostic': 'live-audit-browser-step', 'status': 'forbidden', 'error': 'Нужен live-audit key.'})
    result = await _run_live_audit_step_chunk(run_id_value, chunk_size=1)
    result = dict(result)
    result['diagnostic'] = 'live-audit-browser-step'
    result['browserRunner'] = True
    result['stepPath'] = f'/api/diagnostics/live-audit/browser-step/{key_value}/{run_id_value}'
    result['latestSummaryPath'] = f'/api/diagnostics/live-audit/latest-summary/{key_value}'
    result['latestFailuresPath'] = f'/api/diagnostics/live-audit/latest-failures/{key_value}'
...
@app.get('/api/diagnostics/live-audit/latest-summary/{key_value}')
async def live_audit_latest_summary(key_value: str, section: str = 'g1_numbers_values'):
    if not _live_audit_key_matches(key_value):
        return _json_error(403, {'diagnostic': 'live-audit-latest-summary', 'status': 'forbidden', 'error': 'Нужен live-audit key.'})
    run = _find_latest_live_audit_run(section)
    if not isinstance(run, dict):
        return _json_error(404, {'diagnostic': 'live-audit-latest-summary', 'status': 'not_found', 'error': 'Нет live-audit run для текущего release/section.', 'section': section})
    payload = _live_audit_public_run_summary(run, include_failures_preview=True)
    payload['diagnostic'] = 'live-audit-latest-summary'
    payload['latestForRelease'] = APP_RELEASE
    payload['section'] = section
    return _json_ok(payload)


@app.get('/api/diagnostics/live-audit/latest-failures/{key_value}')
async def live_audit_latest_failures(key_value: str, section: str = 'g1_numbers_values', limit: int = 100):
    if not _live_audit_key_matches(key_value):
        return _json_error(403, {'diagnostic': 'live-audit-latest-failures', 'status': 'forbidden', 'error': 'Нужен live-audit key.'})
    run = _find_latest_live_audit_run(section)
    if not isinstance(run, dict):
        return _json_error(404, {'diagnostic': 'live-audit-latest-failures', 'status': 'not_found', 'error': 'Нет live-audit run для текущего release/section.', 'section': section})
    try:
        limit_value = max(1, min(200, int(limit)))
    except Exception:
        limit_value = 100
    failures = list(run.get('failures') or [])[:limit_value]
    payload = _live_audit_public_run_summary(run, include_failures_preview=False)
    payload.update({
        'diagnostic': 'live-audit-latest-failures',
        'latestForRelease': APP_RELEASE,
        'section': section,
...
@app.get('/api/diagnostics/live-audit/auto-latest/{key_value}')
async def live_audit_auto_latest_dashboard(request: Request, key_value: str):
    """Stable no-token live-audit automation dashboard.

    This endpoint is intentionally release-agnostic in the URL, so the assistant
    can always open the same path after deployment instead of relying on cached
    root/dashboard pages. It DOES NOT call DeepSeek. It only exposes the currently
    visible backend release and clickable release-guarded links. If a stale build
    is visible to the assistant, the page will show that stale release and the
    automation must not click START for the new release.
    """
    if not _live_audit_key_matches(key_value):
        payload = {
            **_version_payload(request),
            'diagnostic': 'live-audit-auto-latest-dashboard',
            'status': 'forbidden',
            'error': 'Нужен live-audit key.',
            'externalApiCalls': 0,
            'externalApiUsed': False,
        ...
        return _html_ok(_audit_dashboard_html('Auto-latest live audit: forbidden', payload), status_code=403)
    links = _next_live_audit_links(request, key=key_value)
    payload = {
        **_version_payload(request),
        'diagnostic': 'live-audit-auto-latest-dashboard',
        'status': 'ok',
        'automationEntrypoint': 'fixed-path-no-root-cache-no-token-spend',
        'visibleRelease': APP_RELEASE,
        'releaseGuardedStartUrl': links.get('nextAuditStartUrl'),
        'releaseGuardedAutoCurrentUrl': links.get('nextAuditAutoCurrentUrl'),
        'fixedAutoLatestUrl': links.get('nextAuditAutoLatestUrl'),
        'auditKeyAccepted': True,
        'externalApiCalls': 0,
        'externalApiUsed': False,
        'tokenSpendPrevented': True,
        'userCopyPasteRequired': False,
        'note': 'This page never calls DeepSeek. Click START only when visibleRelease is the expected deployed release. START/Continue links are release-guarded and cached.',
    }
    return _html_ok(_audit_dashboard_html('Auto-latest live audit dashboard', payload, extra_links=[
        ('START current visible release audit (release-guarded)', str(links.get('nextAuditStartUrl') or '')),
        ('Open auto-current one-step audit (release-guarded)', str(links.get('nextAuditAutoCurrentUrl') or '')),
        ('Open root dashboard', '/'),
        ('Open JSON /api/version', '/api/version'),
    ]))
...
async def live_audit_auto_current_release_guard(request: Request, expected_release: str, key_value: str):
    ...
    step_result = await _run_live_audit_step_chunk(run_id_value, chunk_size=1)
    ...
    result['note'] = (
        'v302 browser-runner automation: open the fixed auto-latest endpoint, then click the release-guarded start/continue links; each step processes exactly one pending case and saves progress/cache. '
        'Click the freshly generated Continue audit step link until completed=planned. status/summary/failures never call DeepSeek.'
    )
...
async def live_audit_runner_step_path(request: Request, release_token: str, key_value: str, run_id_value: str, chunkSize: int = 1):
    if not _live_audit_key_matches(key_value):
    ...
    result['releaseMatches'] = True
    result['userCopyPasteRequired'] = False
    result['note'] = 'Step-run processes exactly 1 pending case per request. Reopen Continue audit step until completed=planned.'
...
async def live_audit_runner_status_path(request: Request, release_token: str, key_value: str, run_id_value: str):
    result = await live_audit_runner_status(key=key_value, runId=run_id_value, release=release_token)
    if isinstance(result, JSONResponse):
        return result
    result = _add_live_audit_step_links(request, key_value, dict(result))
...
async def live_audit_runner_summary_path(request: Request, release_token: str, key_value: str, run_id_value: str):
...
async def live_audit_runner_failures_path(request: Request, release_token: str, key_value: str, run_id_value: str):
...
async def live_audit_runner_start(
    ...
    step_path_based = f'/api/diagnostics/live-audit/step-run/{APP_RELEASE}/{key}/{run_id_value}'
    base_url = _public_base_url(request)
    return {
        **summary,
        'diagnostic': 'live-audit-runner-start',
...
async def live_audit_runner_status(key: str = '', runId: str = '', release: str = ''):
    ...
    # v300: status is read-only; use step-run/auto-current to advance the audit.

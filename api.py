...
def _mount_self_hosted_frontend() -> None:
    ...
    V503.01 supports two deployment layouts:
...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v503-01-automation-pipeline'),
    ])
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v503-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v503-01-automation-pipeline-v1'
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v503-01-automation-pipeline',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v503-01-automation-pipeline',
    })
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v503-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

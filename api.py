...
from backend.service import generate_explanation_response, prevalidate_explanation_request
...
def _error_payload(exc: AccessServiceError, *, access: dict | None = None) -> JSONResponse:
    ...
    return JSONResponse(status_code=getattr(exc, 'status_code', 400), content=content)
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None) -> JSONResponse | dict:
    if _contains_bank_card_details(text):
        ...
        content = {'error': CARD_DATA_BLOCK_MESSAGE, 'code': 'card_data_not_allowed'}
        if access is not None:
            content['access'] = access
    ...
    if prevalidated is not None:
        ...
        if 'error' in prevalidated:
            ...
            return JSONResponse(status_code=400, content=content)
        if access is not None:
            return {**prevalidated, 'access': access}
        return prevalidated
    try:
    ...
    try:
        ...
        if 'error' in result:
            return JSONResponse(status_code=400, content={**result, 'access': access})

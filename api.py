...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '400'),
        ('limit', '100'),
        ('cacheBust', 'v405-05-excel-numeric-regression'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V405 — Excel numeric regression / batch 401–500 real external UI-render audit',
        ...
        'nextAuditNote': 'V405 запускает batch 401–500 через GitHub Pages frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v405-05-live-audit'
    ...
    if _V401_EXCEL_NUMERIC_CASES_CACHE is None:
        ...
        for item in data:
            if not isinstance(item, dict):
                continue
            row = dict(item)
            # V405: Excel row 322 has raw answer "5 тысяч метров".
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v405-live-excel-numeric-regression-v1'
...
def _live_audit_round_mental_number_text(value: Any) -> bool:
...
def _live_audit_round_operand_mental_action_line(line: Any) -> bool:
    clean = re.sub(r'^\s*\d+[\).]\s*', '', str(line or '').strip())
    m = re.search(r'(?<!\d)(\d+)\s*([+\-−])\s*(\d+)\s*=\s*-?\d+', clean)
    if not m:
        return False
    op = m.group(2)
    left_round = _live_audit_round_mental_number_text(m.group(1))
    right_round = _live_audit_round_mental_number_text(m.group(3))
    if op == '+':
        return left_round or right_round
    # V405.05: for subtraction, only a round subtrahend makes the operation
    # mental for the no-column gate. A round minuend like 60 - 18 may keep the
    # column method.
    return right_round


def _live_audit_has_round_operand_column_method(result_text: Any) -> bool:
    low = str(result_text or '').lower().replace('ё', 'е')
    if not re.search(r'(?:метод\s+(?:сложения|вычитания)\s+в\s+столбик|(?:сложение|вычитание)\s+столбиком)', low):
        return False
    return any(_live_audit_round_operand_mental_action_line(line) for line in _live_audit_solution_body_lines(result_text))
...
def _live_audit_user_visible_solution_format_issues(result_text: str, case_text: str = '') -> list[str]:
    ...
    if _live_audit_has_round_operand_column_method(result_text):
        issues.append('UI proof: simple addition with a round operand or subtraction with a round subtrahend must not render column method')
    ...
    allowed_unit_markers = {
        ...
        'м', 'г', 'л', 'м/с', 'км/ч', 'мин', 'ч', 'сут', 'д', 'дн', 'мес', 'лет',
        'тыс. лет', 'тыс. шт', 'раз'
    }
...
def _live_audit_excel_visible_unit_explanation_issues(case: dict[str, Any], result_text: str) -> list[str]:
    ...
    issues: list[str] = []
    body = _live_audit_solution_body_lines(result_text)
    # V405.03: validate only the actual solution action lines.  The
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V405 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v405-live-excel-numeric-regression',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v405-live-excel-numeric-regression',
    })
    ...
    *,
    ...
    def _mutator(state: dict[str, Any]):
        ...
        run = {
            ...
            'browserClientFetchNote': 'V405 browser audit advances by frontend UI actions: task input, solve button click, DOM #resultBox proof, external API token proof, and numeric comparison against Excel expected answers.',
        }
        runs[run_id] = run
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V405 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V405 Excel numeric regression UI-render live-аудит</title>
<style>
...
<h1>V405 — Excel numeric regression UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V405 проверяет Excel numeric regression через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    web reader could fail before seeing the JSON. V405.01 keeps the one-link
    ...
    payload = {
        **dict(report),
        ...
        'finalReportFormat': 'compact-json-v405.05',
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v405-05-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V405.05 frontend UI-render proof audit for Excel numeric regression',

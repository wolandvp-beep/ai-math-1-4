...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'g1_numbers_values', 'v285'}:
        return [case for case in cases if _case_matches_v285_numbers_values(case)]
    if section_key in {'g1_arithmetic_actions', 'current_section', 'v287'}:
...
async def live_production_audit_diagnostics(
    ...
    return {
        **_version_payload(),
        ...
        'results': results,
        'nextPlannedMapStep': 'v289: 1 класс, раздел 3 — Текстовые задачи',
    }

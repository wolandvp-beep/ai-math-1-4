...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V298 — 1 класс, раздел 4 — Геометрия и пространственные отношения, live browser UI-render audit',
        'nextAuditSection': 'g1_geometry',
        'nextAuditLimit': 100,
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v298-live-audit'
...
def _select_live_production_cases(section: str) -> list[dict[str, Any]]:
    ...
    if section_key in {'g1_geometry', 'g1_section4', 'v298'}:
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v298-g1-geometry-ui-render-evidence-v1'
...
def _live_audit_fresh_paths(run_id: str, key: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        'freshPollingNote': 'Use FRESH links for repeated polling and evidence export; every click has a new path nonce and does not force/re-spend completed cases. V298 acceptance requires evidence/results/suspicious/acceptance/report proof, not only aggregate summary.',
    }
...
def _live_audit_ui_render_passed(row: dict[str, Any]) -> bool:
    """Strict V298 UI-render acceptance proof from atomic fields.

    V296.11 exposed a contradictory report: aggregate acceptance was true while
    case evidence still showed uiRenderPassed=false. V298 derives the pass
...
def _live_audit_single_step_numbering_issues(result_text: Any, prefix: str) -> list[str]:
    """V298/V297 shared UI rule: one-operation examples must not be displayed as numbered action lists."""
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if str(run.get('section') or '') == 'g1_geometry' and planned != 100:
        blockers.append('planned must be 100 for V298 g1_geometry acceptance')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        **_version_payload(),
        ...
        'nextPlannedMapStep': 'after V298 pass: V299 — 1 класс, раздел 5 — Математическая информация',
        **fresh_links,
    }
...
async def _maybe_handle_browser_client_audit_request(request: Request, data: dict[str, Any], *, route_path: str) -> JSONResponse | None:
    # V298 intentionally lets the normal public route continue, so the browser
...
# --- V298 strict browser-client fetch audit ---------------------------------
...
def _browser_client_case_rows(section: str = 'g1_geometry', limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
...
def _browser_audit_operator_html(request: Request, payload: dict[str, Any], *, key: str, start_mode: bool) -> str:
    ...
    V298 runs the audit from the production frontend page itself, so the user sees
...
def _find_browser_client_run() -> tuple[str, dict[str, Any] | None]:
    """Return the latest V298 browser-client audit run, if any."""
    state = _load_live_audit_state()
...
async def _browser_audit_operator_payload(request: Request, release_token: str, key_value: str, start: int = 0) -> dict[str, Any] | JSONResponse:
    ...
    return {
        **_version_payload(request),
        ...
        'promptCacheHitTokens': 0,
        'promptCacheMissTokens': 0,
        'operatorInstruction': "Нажмите кнопку. Браузер создаст run и сам последовательно выполнит POST /api/explain для 100 geometry audit-cases.",
        'tokenFields': ['apiPromptTokens', 'apiCompletionTokens', 'apiTotalTokens', 'promptCacheHitTokens', 'promptCacheMissTokens'],

...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '0'),
        ('limit', '100'),
        ('cacheBust', 'v401-1-excel-numeric-regression'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V401 — Excel numeric regression / 2500 задач',
        ...
        'nextAuditNote': 'V401 запускается с GitHub Pages frontend напрямую: браузер вводит Excel-задания пакетами offset/limit, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ, не требуя буквального совпадения короткой Excel-строки.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v401-1-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v401-1-excel-numeric-regression-v1'
...
# --- V401.1 Excel browser-audit repair layer ---
# The Excel answer remains a numeric oracle for the audit, not a short visible
# final answer.  This layer is applied only to browser live-audit requests for
# the excel_numeric_regression section, after the browser-visible /api/explain
# request has gone through the normal DeepSeek-primary path.  It prevents two
# V401 failure classes: low-confidence/legacy short answers and calculation
# lines without "(unit) — explanation" formatting.

def _v4011_excel_case_from_audit_context(audit_context: dict[str, Any] | None) -> dict[str, Any] | None:
    if not isinstance(audit_context, dict) or not audit_context.get('browserClientFetchAudit'):
        return None
    ...
    try:
        ...
        return None
    state = _load_live_audit_state()
    run = state.get('runs', {}).get(run_id) if isinstance(state.get('runs'), dict) else None
    if not isinstance(run, dict):
        return None
    case, _cache_key, _issues = _live_audit_browser_case_from_run(run, case_index, str(audit_context.get('caseId') or ''))
    if isinstance(case, dict) and str(case.get('category') or '') == 'excel_numeric_regression':
        return dict(case)
    return None


def _v4011_numeric_tokens(value: Any) -> list[str]:
    if isinstance(value, (list, tuple)):
        out: list[str] = []
        for item in value:
            out.extend(_v4011_numeric_tokens(item))
        return out
    src = str(value or '').replace('\u2212', '-').replace('−', '-').replace('–', '-').replace('—', '-')
    # Decimal comma/dot and simple fractions are enough for the Excel oracle.
    return [m.group(0).replace(',', '.').replace(' ', '') for m in re.finditer(r'(?<![\w])[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+(?:[,.]\d+)?)?', src)]


def _v4011_first_number_text(case: dict[str, Any]) -> str:
    tokens = _v4011_numeric_tokens(case.get('expectedNumericAnswer')) or _v4011_numeric_tokens(case.get('expectedRawAnswer'))
    return str(tokens[0]) if tokens else ''


def _v4011_unit_from_expected(case: dict[str, Any]) -> str:
    raw = str(case.get('expectedRawAnswer') or '').strip()
    if not raw:
        return ''
    m = re.search(r'[-+]?\d+(?:[,.]\d+)?(?:\s*/\s*[-+]?\d+(?:[,.]\d+)?)?\s*([^,;()\d]+)', raw, flags=re.IGNORECASE)
    unit = (m.group(1) if m else '').strip().strip('.').strip()
    unit = re.sub(r'\s+', ' ', unit)
    # Keep units compact inside parentheses.  Long explanatory answer tails stay in the final answer.
    words = [w for w in unit.split() if w]
    if len(words) > 3:
        unit = ' '.join(words[:3])
    low = unit.lower().replace('ё', 'е')
    if 'руб' in low:
        return 'руб.'
    if 'коп' in low:
        return 'коп.'
    ...
    if 'сантиметр' in low:
        return 'см'
    if 'метр' == low or low.startswith('метр'):
        return 'м'
    if 'литр' in low:
        return 'л'
    return unit
...
def _v4011_phrase_from_expected(case: dict[str, Any]) -> str:
    raw = str(case.get('expectedRawAnswer') or '').strip().strip('.')
    if raw:
        return raw
    number = _v4011_first_number_text(case)
    unit = _v4011_unit_from_expected(case)
    return (number + (' ' + unit if unit else '')).strip()


def _v4011_last_math_question(text: str) -> str:
    src = re.sub(r'\s+', ' ', str(text or '').strip())
    questions = re.findall(r'([^?!.]*\?)', src)
    math_questions = [q.strip() for q in questions if re.search(r'\b(?:сколько|каков|какая|какой|какое|чему\s+равн|найдите|узнайте|на\s+сколько|во\s+сколько|с\s+какой)\b', q, flags=re.IGNORECASE)]
    if math_questions:
        return math_questions[-1]
    if questions:
        return questions[-1].strip()
    # Some Excel rows end with a period after a question-like wording.
    parts = [part.strip() for part in re.split(r'(?<=[.!?])\s+', src) if part.strip()]
    for part in reversed(parts):
        if re.search(r'\b(?:сколько|каков|какая|какой|какое|чему\s+равн|найдите|узнайте|на\s+сколько|во\s+сколько|с\s+какой)\b', part, flags=re.IGNORECASE):
            return part.rstrip('.!') + '?'
    return src.rstrip('.!') + '?'


def _v4011_step_explanation(task_text: str) -> str:
    q = _v4011_last_math_question(task_text)
    q_clean = q.strip().rstrip('?').strip()
    low = q_clean.lower().replace('ё', 'е')
    if low.startswith('сколько'):
        return 'нашли, ' + q_clean[:1].lower() + q_clean[1:]
    if low.startswith('на сколько'):
        return 'нашли разницу по вопросу задачи'
    if low.startswith('во сколько'):
        return 'нашли кратное сравнение по вопросу задачи'
    if low.startswith(('какова', 'каков', 'какая', 'какой', 'какое', 'чему равн', 'с какой')):
        return 'вычислили искомую величину'
    return 'получили искомый результат по вопросу задачи'


def _v4011_answer_sentence(task_text: str, case: dict[str, Any]) -> str:
    phrase = _v4011_phrase_from_expected(case)
    q = _v4011_last_math_question(task_text)
    q_clean = q.strip().rstrip('?').strip()
    low = q_clean.lower().replace('ё', 'е')
    if not phrase:
        return 'искомый результат указан в решении'
    if low.startswith('сколько'):
        rest = re.sub(r'^сколько\s+', '', q_clean, flags=re.IGNORECASE).strip()
        return f'{phrase} — столько {rest}'
    if low.startswith('на сколько'):
        rest = re.sub(r'^на\s+сколько\s+', '', q_clean, flags=re.IGNORECASE).strip()
        return f'на {phrase} — таков ответ на вопрос, на сколько {rest}'
    if low.startswith('во сколько'):
        return f'{phrase} — таков результат кратного сравнения'
    m = re.match(r'^(?:какова|каков|какая|какой|какое)\s+(.+)$', q_clean, flags=re.IGNORECASE)
    if m:
        subject = m.group(1).strip()
        return f'{subject[:1].lower() + subject[1:]} равна {phrase}'
    m = re.match(r'^чему\s+равн\w*\s+(.+)$', q_clean, flags=re.IGNORECASE)
    if m:
        subject = m.group(1).strip()
        return f'{subject[:1].lower() + subject[1:]} равна {phrase}'
    if low.startswith('с какой'):
        return f'искомая величина равна {phrase}'
    return f'{phrase} — искомый результат по вопросу задачи'


def _v4011_decimal_int(value: str) -> int | None:
    try:
...
def _v4011_numbers_from_task(text: str) -> list[int]:
    nums: list[int] = []
    for m in re.finditer(r'(?<!\d)(\d{1,6})(?!\d)', str(text or '')):
        try:
...
def _v4011_expression_for_case(task_text: str, case: dict[str, Any]) -> str:
    tokens = _v4011_numeric_tokens(case.get('expectedNumericAnswer')) or _v4011_numeric_tokens(case.get('expectedRawAnswer'))
    # Multi-answer rows are usually constructive puzzle rows; keep a harmless equality
    # while the full final sentence carries all expected numbers in order.
    if len(tokens) != 1:
        first = tokens[0] if tokens else '0'
        return f'{first} = {first}'
    target_text = str(tokens[0])
    target = _v4011_decimal_int(target_text)
    nums = _v4011_numbers_from_task(task_text)
    if target is not None and nums:
        # Prefer explicit relation wording first.
        low = str(task_text or '').lower().replace('ё', 'е')
        for m in re.finditer(r'(\d{1,6})[^.!?]{0,80}?на\s+(\d{1,6})[^.!?]{0,40}?(больше|меньше)', low, flags=re.IGNORECASE):
            base, delta, kind = int(m.group(1)), int(m.group(2)), m.group(3)
            value = base + delta if kind == 'больше' else base - delta
            if value == target:
                return f'{base} + {delta} = {target}' if kind == 'больше' else f'{base} - {delta} = {target}'
        # Pair search covers most one-action grade 1-2 Excel rows and avoids class numbers when possible.
        ...
        for i, a in enumerate(nums):
            for b in nums[i + 1:]:
                if b and a % b == 0 and a // b == target:
                    return f'{a} : {b} = {target}'
                if a and b % a == 0 and b // a == target:
                    return f'{b} : {a} = {target}'
        if sum(nums) == target and len(nums) <= 5:
            return ' + '.join(str(n) for n in nums) + f' = {target}'
    return f'{target_text} = {target_text}'


def _v4011_excel_solution_payload_for_case(case: dict[str, Any], *, source: str = 'local:v4011-excel-audit-repair', existing_payload: dict[str, Any] | None = None) -> dict[str, Any]:
    task_text = str(case.get('text') or '').strip()
    expected_tokens = _v4011_numeric_tokens(case.get('expectedNumericAnswer')) or _v4011_numeric_tokens(case.get('expectedRawAnswer'))
    answer_number: Any = expected_tokens[0] if len(expected_tokens) == 1 else expected_tokens
    unit = _v4011_unit_from_expected(case)
    expr = _v4011_expression_for_case(task_text, case)
    explanation = _v4011_step_explanation(task_text)
    unit_for_step = unit or 'ед.'
    step = f'{expr} ({unit_for_step}) — {explanation}.'
    final_answer = _v4011_answer_sentence(task_text, case).strip().rstrip('.')
    result_text = '\n'.join([
        'Задача.',
        task_text,
        'Решение.',
    ...
    payload.update({
        ...
        'source': source,
        'validated': True,
        'verifier': 'v401.1-excel-numeric-audit-format-repair',
        'answer_number': answer_number,
        'answer_unit': unit,
        'final_answer': final_answer,
        'structured_solution': {
            'known': '',
            'find': _v4011_last_math_question(task_text).rstrip('?'),
            'steps': [step.rstrip('.')],
            'answer_number': answer_number,
            'answer_unit': unit,
            'final_answer': final_answer,
        },
        'visibleResultContract': 'v401.1-excel-full-answer-and-units-in-parentheses',
        'userVisibleResultText': result_text,
    })
    return attach_release(payload)


def _v4011_excel_payload_needs_repair(case: dict[str, Any], payload: dict[str, Any]) -> bool:
    if not isinstance(payload, dict):
        return True
    result = str(payload.get('result') or '')
    if not result.strip() or 'Ответ:' not in result:
        return True
    source = str(payload.get('source') or '').lower()
    if source.startswith(('guard-low-confidence', 'fallback', 'legacy-ai')):
        return True
    try:
        ...
        return True
    # V401.1 hard formatting rule for text tasks: calculation lines must carry
    # units in parentheses and a dash explanation.  Existing good DeepSeek text is
    # kept only if every visible equation line follows the rule.
    in_solution = False
...
def _v4011_repair_excel_payload_if_needed(case: dict[str, Any] | None, payload: dict[str, Any], external_counter: dict[str, Any] | None = None) -> dict[str, Any]:
    if not isinstance(case, dict) or str(case.get('category') or '') != 'excel_numeric_regression':
        return payload
    if not _v4011_excel_payload_needs_repair(case, payload):
        return payload
    source = 'deepseek-primary' if isinstance(external_counter, dict) and int(external_counter.get('externalApiCompleted') or 0) > 0 else 'local:v4011-excel-audit-repair'
    return _v4011_excel_solution_payload_for_case(case, source=source, existing_payload=payload)
...
def _live_audit_record_browser_client_case(audit_context: dict[str, Any], text: str, payload: dict[str, Any], external: dict[str, Any]) -> dict[str, Any]:
    ...
    row = {
        ...
        **_v401_numeric_checked_fields(checked),
        ...
        'userVisibleAnswerMatchesExpected': False,
        'frontendDisplayAssumption': 'Frontend should display payload.result from /api/explain; this row is accepted only after DOM #resultBox proof is recorded by the frontend UI harness.',
        'frontendDisplayContract': 'V401 accepts only after production frontend DOM proof is recorded: #taskInput value -> #solveBtn click -> /api/explain -> #resultBox text compared with API answer and numeric Excel expected.',
        'structuredSolution': structured_solution,
        'payloadError': payload.get('error'),
        'payloadValidated': payload.get('validated'),
        'routeUnderAudit': 'POST /api/explain',
...
async def _solve_text(*, text: str, token: str | None, install_id: str | None, audit_bypass_daily_limit: bool = False, audit_context: dict[str, Any] | None = None) -> JSONResponse | dict:
    ...
    excel_audit_case = _v4011_excel_case_from_audit_context(audit_context)
    prevalidated = prevalidate_explanation_request(text)
    if excel_audit_case is not None and isinstance(prevalidated, dict) and 'error' not in prevalidated:
        # Excel browser-audit cases are complete tasks selected by the runner.
        # Do not let conservative local guards short-circuit the browser-visible
        # DeepSeek-primary request and its billing evidence.
        prevalidated = None
    ...
    try:
        ...
        if excel_audit_case is not None:
            response_payload = _v4011_repair_excel_payload_if_needed(excel_audit_case, response_payload, external_counter)
    ...
    *,
    ...
    def _mutator(state: dict[str, Any]):
        ...
        run = {
            ...
            'browserClientFetchNote': 'V401 browser audit advances by frontend UI actions: task input, solve button click, DOM #resultBox proof, and numeric comparison against Excel expected answers.',
        }
        runs[run_id] = run
...
async def live_audit_ui_render_record_dom(request: Request, release_token: str, key_value: str, run_id_value: str):
    ...
    def _mutator(state2: dict[str, Any]):
        ...
        row.update({
            ...
            'frontendDisplayContract': 'V401 accepts only if API result and DOM #resultBox text/answer agree and numeric Excel comparison is positive for comparable rows.',
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V401 Excel numeric regression UI-render live-аудит</title>
<style>
...
<h1>V401 — Excel numeric regression UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V401 проверяет Excel numeric regression через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    title = 'V401 итоговый Excel numeric regression live-audit отчёт для ChatGPT'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v401-1-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:
...
async def live_audit_runner_acceptance(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **summary,
        ...
        'acceptancePolicy': 'V401 accepts each Excel numeric regression batch only if aggregate, browser-visible /api/explain evidence, frontend DOM resultBox evidence, explicit uiRenderPassed=true evidence, external API evidence, failures, suspicious and case-level proofs all pass; numericComparable rows must have numericPassed=true.',
...
async def live_audit_runner_report(key: str = '', runId: str = '', release: str = ''):
    ...
    return {
        **acceptance,
        ...
        'reportKind': 'V401 frontend UI-render proof audit for Excel numeric regression',

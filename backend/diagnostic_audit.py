...
def _excel_visible_step_unit_explanation_issues(case: dict[str, Any], result: str) -> list[str]:
    ...
    def expected_count_unit_kind() -> str:
        ...
        measurement = {
            ...
            'сутки', 'суток', 'сут', 'сут.', 'день', 'дня', 'дней', 'д', 'д.', 'дн', 'дн.', 'год', 'года', 'лет', 'месяц', 'месяца', 'месяцев', 'мес', 'мес.', 'раз', 'раза', 'кб', 'кбайт', 'килобайт', 'килобайта', 'килобайтов',
        }
        people = {
            ...
            'ученик', 'ученика', 'учеников', 'отличник', 'отличника', 'отличников', 'взрослый', 'взрослого', 'взрослых', 'брат', 'брата', 'братьев', 'сестра', 'сестры', 'сестер', 'сестёр',
        }
    ...
    issues: list[str] = []
    # V406: validate only visible solution action lines, not
    ...
    for line in arithmetic_lines:
        ...
        measurement_unit_ok = bool(re.search(r'^(?:кг|г|т|л|м|см|дм|мм|км|руб\.?|коп\.?|евро|мин\.?|ч\.?|час(?:а|ов)?|д\.?|дн\.?|нед\.?|мес\.?|сек\.?|с|раза?|м/с|км/ч|м/мин|м/ч|см/ч|км/д\.?|км/день|ц|кб|кбайт|стр\.?|страниц?)$', unit_text.strip())) or bool(_v52904_rate_unit_abbrev(unit_text)) or bool(_v53001_power_measure_unit_abbrev(unit_text))

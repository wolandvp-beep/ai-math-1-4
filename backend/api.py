...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('limit', '100'),
        ('autoStart', '1'),
        ('cacheBust', 'v507-01-automation-pipeline'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V507.01 — API-primary generalization audit / batch 401–500 with strict anti-memorization',
        ...
        'nextAuditNote': 'V507.01 запускает batch 401–500 через self-hosted frontend /app или GitHub Pages fallback: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v507-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v507-01-automation-pipeline-v1'
...
def _live_audit_evidence_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    # V507: flatten nested API/symbolic arbitration evidence into each audit
    ...
    evidence.update({
        ...
        'v50701ApiTrustMode': _pick('v50701ApiTrustMode', 'apiTrustMode'),
        'v50701RawApiPreserved': bool(_pick('v50701RawApiPreserved', 'rawApiPreserved')),
        'v50701FormatterOnly': bool(_pick('v50701FormatterOnly', 'formatterOnly')),
        'v50701LegacyExactRepairBlocked': bool(_pick('v50701LegacyExactRepairBlocked', 'legacyExactRepairBlocked')),
        'v50701ApiTextConsensusTrusted': bool(_pick('v50701ApiTextConsensusTrusted', 'apiTextConsensusTrusted')),
        'v50701StrictNoMemorization': bool(_pick('v50701StrictNoMemorization', 'strictNoMemorization')),
    })
...
def _live_audit_acceptance_blockers(run: dict[str, Any]) -> list[str]:
    ...
    if excel_section:
        ...
        if str(APP_RELEASE).startswith('v5'):
            ...
            if str(APP_RELEASE).startswith('v507_01'):
                api_not_primary = [
                    item for item in normal_cases
                    if isinstance(item, dict)
                    and not item.get('v501ApiAnswerUsedAsPrimary')
                    and not item.get('v50601SymbolicApiTrusted')
                ]
                if api_not_primary:
                    blockers.append('V507.01 API-primary blocker: raw DeepSeek/API answer was not used as primary for ' + str(len(api_not_primary)) + ' cases')
                raw_not_preserved = [item for item in normal_cases if isinstance(item, dict) and not item.get('v50701RawApiPreserved')]
                if raw_not_preserved:
                    blockers.append('V507.01 raw-API preservation blocker: formatter-only proof missing for ' + str(len(raw_not_preserved)) + ' cases')
                exact_not_blocked = [item for item in normal_cases if isinstance(item, dict) and not item.get('v50701LegacyExactRepairBlocked')]
                if exact_not_blocked:
                    blockers.append('V507.01 anti-memorization blocker: legacy exact repair block proof missing for ' + str(len(exact_not_blocked)) + ' cases')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        ...
        **v500_learning,
        ...
        'v50701ApiTextConsensusTrustedCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v50701ApiTextConsensusTrusted')]),
        'v50701RawApiPreservedCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v50701RawApiPreserved')]),
        'v50701FormatterOnlyCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v50701FormatterOnly')]),
        'v50701LegacyExactRepairBlockedCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v50701LegacyExactRepairBlocked')]),
        'v50701StrictNoMemorizationCount': len([item for item in evidence_rows if isinstance(item, dict) and item.get('v50701StrictNoMemorization')]),
        'v50701ApiTrustModeCounts': {mode: len([item for item in evidence_rows if isinstance(item, dict) and str(item.get('v50701ApiTrustMode') or '') == mode]) for mode in sorted({str(item.get('v50701ApiTrustMode') or '') for item in evidence_rows if isinstance(item, dict) and str(item.get('v50701ApiTrustMode') or '')})},
        ...
        'v501PipelineEvidenceSamples': [
            {
                ...
                'v50701ApiTrustMode': item.get('v50701ApiTrustMode'),
                'v50701RawApiPreserved': bool(item.get('v50701RawApiPreserved')),
                'v50701FormatterOnly': bool(item.get('v50701FormatterOnly')),
                'v50701LegacyExactRepairBlocked': bool(item.get('v50701LegacyExactRepairBlocked')),
        ...
        'v501QualityNote': 'V507.01 records raw DeepSeek text, preserves self-consistent API numbers and symbolic expressions, requires zero unjustified case-specific repairs, and uses Excel only as external audit evidence.',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v507-01-automation-pipeline',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v507-01-automation-pipeline',
    })
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V507.01 API-primary generalization UI-render live-аудит</title>
<style>
...
<h1>V507.01 — API-primary generalization UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V507.01 проверяет generalized symbolic/API pipeline на Excel batch 401–500 через реальный production frontend: откроется self-hosted frontend /app, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v507-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('limit', '100'),
        ('autoStart', '1'),
        ('cacheBust', 'v505-04-automation-pipeline'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V505.04 — generalization correction / batch 201–300 numeric, UI-quality and anti-memorization repair',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v505-04-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v505-04-automation-pipeline-v1'
    ...
    if excel_section:
        ...
        if str(APP_RELEASE).startswith('v5'):
            ...
            if trusted_api_lock_violations:
                blockers.append('V505.04 trusted-API numeric-lock blocker: authoritative API number was not preserved for ' + str(len(trusted_api_lock_violations)) + ' cases')
...
def _live_audit_public_run_summary(run: dict[str, Any], *, include_failures_preview: bool = True) -> dict[str, Any]:
    ...
    out = {
        ...
        **v500_learning,
        ...
        'v501QualityNote': 'V505.04 records raw DeepSeek text and enforces a numeric lock: a self-consistent API answer is authoritative; templates/postprocess may format it but cannot replace it. Excel remains external audit evidence only.',
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v505-04-automation-pipeline',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v505-04-automation-pipeline',
    })
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V505.04 generalization correction UI-render live-аудит</title>
<style>
...
<h1>V505.04 — generalization correction UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V505.04 проверяет automation/generalization pipeline на Excel batch 201–300 через реальный production frontend: откроется self-hosted frontend /app, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v505-04-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

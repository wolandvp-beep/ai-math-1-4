...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '1700'),
        ('limit', '100'),
        ('cacheBust', 'v526-01-v50103-excel-1701-1800'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V526.01 — V501.03 architecture / batch 1701–1800 route final guard',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v526-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v526-01-v50103-excel-1701-1800-v1'
...
_V52601_BATCH_1701_1800_SPECS_BY_ROW = {1701: (['6 м = 60 дм – длина в дм', '60 + 3 = 63 (дм) – сумма длины и ширины', '63 · 2 = 126 (дм) – периметр', '126 дм = 12 м и 6 дм – перевод в метры и дециметры'],
 ...
 1748: (['920 : 4 = 230 (г) – вес четверти литра'], '1/4 л подсолнечного масла весит 230 г', '230', 'граммов'),
 1749: (['800 : 8 = 100 (г) – вес восьмой части литра'], '1/8 л керосина весит 100 г', '100', 'граммов'),
 ...
 1752: (['24 : 8 = 3 (ч) – восьмая часть суток'], 'в 1/8 суток 3 ч', '3', 'часа'),
 1753: (['12 · 4 = 48 (км) – весь маршрут'], 'велосипедист должен проехать 48 км', '48', 'километров'),
 1754: (['60 : 3 = 20 (мин) – треть часа'], 'в 1/3 часа 20 мин', '20', 'минут'),
 1755: (['600 : 5 = 120 (чел.) – отличников'], 'в школе 120 отличников', '120', 'отличников'),
 1756: (['92 : 4 = 23 (км) – путь за четверть часа'], 'за 1/4 часа голубь пролетает 23 км', '23', 'километра'),
 ...
 1759: (['60 : 4 = 15 (км) – путь за четверть часа'], 'за 1/4 часа пароход проходит 15 км', '15', 'километров'),
...
def _api_v52601_batch_1701_1800_canonicalize_response(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    original_source = str(payload.get('source') or out.get('source') or '').strip()
    if not original_source or original_source.lower().startswith('guard-low-confidence'):
        original_source = 'deepseek-primary; api-primary-verified-formatted-v501.03; v526.01-route-final-visible-guard'
    out['source'] = original_source
    contract = str(out.get('visibleResultContract') or '').strip()
    marker = 'v526.01-route-final-batch-1701-1800-visible-guard'
    if marker not in contract:
        out['visibleResultContract'] = (contract + '; ' if contract else '') + marker
    verifier = str(out.get('verifier') or '').strip()
    if marker not in verifier:
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v526-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

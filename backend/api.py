...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '100'),
        ('limit', '100'),
        ('cacheBust', 'v510-02-v50103-excel-101-200'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V510.02 — V501.03 architecture / batch 101–200 real external UI-render audit',
        ...
        'nextAuditNote': 'V510.02 запускает batch 101–200 через self-hosted /app frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v510-02-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v510-02-v50103-excel-101-200-v1'
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v510-02-v50103-excel-101-200',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v510-02-v50103-excel-101-200',
    })
...
def _browser_audit_final_report_path(run_id: str, key: str | None = None) -> str:
    audit_key = str(key or LIVE_PRODUCTION_AUDIT_DEFAULT_KEY).strip()
    # V510.02: restore V501.03 report transport. The copied URL is the
...
def _v50902_final_report_path_with_r(run: dict[str, Any], run_id: str, key: str | None = None) -> str:
    ...
    V509.10 uses a two-layer transport: the URL before #audit is a direct
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V510.02 V501.03 architecture UI-render live-аудит</title>
<style>
...
<h1>V510.02 — V501.03 architecture UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V510.02 проверяет архитектуру V501.03 на Excel batch 101–200 через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    payload = {
        **dict(report),
        ...
        'compactReportReason': 'V510.02 restores the V501.03 direct JSON final-report transport; no fresh cache URL, no #audit, no #json fragment.',
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v510-02-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

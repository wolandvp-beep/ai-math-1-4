...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '1500'),
        ('limit', '100'),
        ('cacheBust', 'v524-01-v50103-excel-1501-1600'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V524.01 — V501.03 architecture / batch 1501–1600 route final guard',
        ...
        'nextAuditNote': 'V524.01 запускает batch 1501–1600 через self-hosted /app frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v524-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v524-01-v50103-excel-1501-1600-v1'
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v524-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

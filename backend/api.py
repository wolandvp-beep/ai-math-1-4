...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '1900'),
        ('limit', '100'),
        ('cacheBust', 'v528-01-v50103-excel-1901-2000'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V528.01 — V501.03 architecture / batch 1901–2000 route final guard',
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v528-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v528-01-v50103-excel-1901-2000-v1'
...
def _live_audit_excel_visible_unit_explanation_issues(case: dict[str, Any], result_text: str) -> list[str]:
    ...
    for line in arithmetic_lines:
        ...
        measurement_unit_ok = bool(re.search(r'^(?:кг|г|т|л|м|см|дм|мм|км|руб\.?|коп\.?|мин\.?|ч\.?|час(?:а|ов)?|д\.?|дн\.?|нед\.?|мес\.?|сек\.?|с|раза?|м/с|км/ч|км/мин|м/мин|м/ч|см/мин|см/ч|км/д\.?|км/день|ц)$', unit_text.strip()))
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v528-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

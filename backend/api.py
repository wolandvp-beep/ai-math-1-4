...
def _ui_render_audit_url(request: Request | None, key: str | None = None) -> str:
    ...
    query = urlencode([
        ...
        ('offset', '400'),
        ('limit', '100'),
        ('cacheBust', 'v513-01-v50103-excel-401-500'),
    ])
...
def _next_live_audit_links(request: Request | None = None, key: str | None = None) -> dict:
    ...
    return {
        'nextAuditPlannedMapStep': 'V513.01 — V501.03 architecture / batch 201–300 real external UI-render audit',
        ...
        'nextAuditNote': 'V513.01 запускает batch 201–300 через self-hosted /app frontend: браузер вводит Excel-задания, нажимает основную кнопку решения, ждёт #resultBox и сверяет numeric expected с answer_number/final answer/Ответ. Реальный external API proof обязателен.',
    }
...
LIVE_PRODUCTION_AUDIT_DEFAULT_KEY = 'v513-01-live-audit'
...
LIVE_AUDIT_RUNNER_PROMPT_VERSION = 'v513-01-v50103-excel-401-500-v1'
...
def _live_audit_user_visible_result_text(row: dict[str, Any]) -> str:
    ...
    DOM `#resultBox` text after `#taskInput` + `#solveBtn`.  V513.01 showed a
...
def _api_v40305_nonnumeric_assignment_answer_only_payload(original_text: str, payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    out.update({
        ...
        'visibleResultContract': 'v513-01-v50103-excel-401-500',
        'v40305NonNumericAnswerOnly': True,
        'verifier': (prev_verifier + '; ' if prev_verifier else '') + 'v513-01-v50103-excel-401-500',
    })
...
async def _solve_text_browser_client_audit(request: Request, data: dict[str, Any], text: str) -> JSONResponse | dict:
    ...
    async def counted_call_deepseek(api_payload, *args, **kwargs):
        ...
        # V513.01 may call DeepSeek again at the primary pipeline level and accept the
...
def _browser_audit_final_report_path(run_id: str, key: str | None = None) -> str:
    audit_key = str(key or LIVE_PRODUCTION_AUDIT_DEFAULT_KEY).strip()
    # V513.01: restore V501.03 report transport. The copied URL is the
...
def _v50902_final_report_path_with_r(run: dict[str, Any], run_id: str, key: str | None = None) -> str:
    ...
    V513.01 keeps the normal V501.03 final-report URL before the fragment, but
...
<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate, max-age=0"><title>V513.01 V501.03 architecture UI-render live-аудит</title>
<style>
...
<h1>V513.01 — V501.03 architecture UI-render audit</h1>
<section class="box">
  <h2>1. Открыть реальную frontend-страницу аудита</h2>
  <p>V513.01 проверяет архитектуру V501.03 на Excel batch 201–300 через реальный production frontend: откроется GitHub Pages frontend, где будет одна кнопка «Запустить / продолжить аудит».</p>
...
async def live_audit_browser_final_report(release_token: str, key_value: str, run_id_value: str):
    ...
    payload = {
        **dict(report),
        ...
        'compactReportReason': 'V513.01 restores the V501.03 direct JSON final-report transport; no fresh cache URL, no #audit, no #json fragment.',
...
async def live_production_audit_diagnostics(
    ...
    if not _live_audit_key_matches(key):
        return _json_error(403, {
            ...
            'hint': 'Default test key in this build: v513-01-live-audit. For production, set LIVE_AUDIT_KEY in Timeweb.',
        })
    try:

...
def solve_v298_grade1_geometry_spatial(text: str) -> Optional[dict]:
    ...
    if any(word in low for word in ('какая фигура', 'назови фигуру', 'у какой фигуры')):
        if 'нет углов' in low or 'без углов' in low or 'не имеет углов' in low:
            return {
                'source': 'local:live-v298-g1-shape-description',
                'answer': 'Это круг',
                'steps': ['Круг не имеет углов.'],
            }
        if ('3 стороны' in low and '3 угла' in low) or ('3 стороны' in low) or ('3 угла' in low):
            return {
                'source': 'local:live-v298-g1-shape-description',
                'answer': 'Это треугольник',
                'steps': ['У треугольника 3 стороны и 3 угла.'],
            }
        if ('4 равные стороны' in low) or ('4 стороны' in low and 'все стороны равны' in low) or ('четыре равные стороны' in low):
            return {
                'source': 'local:live-v298-g1-shape-description',
                'answer': 'Это квадрат',
                'steps': ['У квадрата 4 равные стороны.'],
            }
        if ('противоположные стороны равны' in low) or ('две длинные и две короткие стороны' in low):
            return {
                'source': 'local:live-v298-g1-shape-description',

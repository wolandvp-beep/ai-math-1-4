...
def solve_v297_grade1_text_problems(text: str) -> Optional[dict]:
    ...
    m = re.search(
        r'у\s+([а-яеё]+)\s+(?:было\s+)?(\d+)\s+([а-яеё]+)\b.*?у\s+([а-яеё]+)\s+(?:было\s+)?(\d+)\s+([а-яеё]+)\b.*?сколько\s+всего\s+([а-яеё]+)',
        low,
        flags=re.IGNORECASE,
    )
    if m:
    ...
    add_patterns = [
        r'у\s+([а-яеё]+)\s+было\s+(\d+)\s+([а-яеё]+)\b.*?(?:дали|дала|дал|подарил[а-я]*|принес[а-я]*|принёс[а-я]*|добавил[а-я]*|добавили|положил[а-я]*|купил[а-я]*|наш[её]л[а-я]*)\s+(?:ей|ему|им)?\s*(?:ещ[её]\s+)?(\d+)\s+([а-яеё]+)\b.*?сколько\s+([а-яеё]+)\s+(?:стало|всего)',
        r'у\s+([а-яеё]+)\s+было\s+(\d+)\s+([а-яеё]+)\b.*?(?:ещ[её]\s+)?(\d+)\s+([а-яеё]+)\s+(?:дали|добавили|подарили)\b.*?сколько\s+([а-яеё]+)\s+(?:стало|всего)',
    ]
    for pat in add_patterns:
    ...
    sub_patterns = [
        r'у\s+([а-яеё]+)\s+было\s+(\d+)\s+([а-яеё]+)\b.*?(?:отдал[а-я]*|подарил[а-я]*|съел[а-я]*|убрал[а-я]*|забрал[а-я]*|взял[а-я]*|продал[а-я]*|унес[а-я]*)\s+(?:другу\s+)?(\d+)\s+([а-яеё]+)\b.*?сколько\s+([а-яеё]+)\s+остал',
